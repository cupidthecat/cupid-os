...
int css_value_int(int off, int len) {
    /* Parses an integer with optional 'px' / 'pt' / 'em' / 'vw' / 'vh' / '%'
     * suffix. Returns px. Decimal part is consumed but truncated (e.g. '1.5em'
     * is treated as '1em'); a fractional 'em' / 'vw' is rare and not worth
     * the precision cost in v1. */
    int sign = 1;
    int i = off;
    int end = off + len;
    ...
    /* Skip a fractional part. Keep only the integer component. */
    if (i < end && css_value_pool[i] == '.') {
        i = i + 1;
        while (i < end && css_value_pool[i] >= '0' && css_value_pool[i] <= '9') i = i + 1;
    }
    if (i < end) {
        int a = css_value_pool[i];
        int b = (i + 1 < end) ? css_value_pool[i+1] : 0;
        if (a == 'p' && b == 't') { v = (v * 4) / 3; }
        else if (a == 'e' && b == 'm') { v = v * 8; }      /* 1em = 8px */
        else if (a == 'v' && b == 'w') {
            int vw_base = cur_cw - 12;                     /* viewport content w */
            if (vw_base < 0) vw_base = 0;
            v = (v * vw_base) / 100;
        }
        else if (a == 'v' && b == 'h') {
            int vh_base = cur_ch - ADDR_H - STATUS_H - 2;  /* viewport content h */
            if (vh_base < 0) vh_base = 0;
            v = (v * vh_base) / 100;
        }
        else if (a == '%') {
            /* Treated as % of viewport width. Good enough for body/div level
             * sizing in v1. Per-spec it should be % of containing block. */
