...
void cc_parse_repl_line(cc_state_t *cc, int *is_expr) {
  *is_expr = 0;
  ...
  if (cc_is_type_or_typedef(cc, tok)) {
    ...
    /* Scalar global variable */
    {
      ...
      if (gsym) {
        ...
        if (cc_match(cc, CC_TOK_EQ)) {
          /* Generate code to evaluate initializer and store to global */
          cc->entry_offset = cc->code_pos;
          cc->has_entry = 1;
          *is_expr = 0;
          ...
          /* Save scope for local parsing context */
          int saved_scope2 = cc->sym_count;
          cc->local_offset = 0;
          cc->max_local_offset = 0;

          cc_parse_expression(cc, 1);

          /* Store result to global address: mov [addr], eax */
          uint32_t addr = gsym->address;
          emit8(cc, 0xA3);
          emit32(cc, addr);

          cc->sym_count = saved_scope2;

          emit_mov_eax_imm(cc, 0);
          emit_epilogue(cc);

          /* Resolve patches for this snippet */
          for (int i = 0; i < cc->patch_count; i++) {
            cc_patch_t *p = &cc->patches[i];
            cc_symbol_t *sym = cc_sym_find(cc, p->name);
            if (sym && sym->kind == SYM_FUNC && sym->is_defined) {
              uint32_t target2 = cc->code_base + (uint32_t)sym->offset;
              uint32_t from = cc->code_base + p->code_offset + 4;
              int32_t rel = (int32_t)(target2 - from);
              patch32(cc, p->code_offset, (uint32_t)rel);
            } else if (sym && sym->kind == SYM_KERNEL) {
              uint32_t target2 = sym->address;
              uint32_t from = cc->code_base + p->code_offset + 4;
              int32_t rel = (int32_t)(target2 - from);
              patch32(cc, p->code_offset, (uint32_t)rel);
            }
  ...
  /* ── Statement / Expression — emit executable code ─────────────── */
  {
    ...
    emit_prologue(cc);
    uint32_t sub_esp_pos = cc->code_pos;
    emit_sub_esp(cc, 256); /* placeholder for locals */

    /* Save scope for local variables in this statement */
    int saved_scope = cc->sym_count;
    cc->local_offset = 0;
    cc->max_local_offset = 0;
    cc->param_count = 0;
    ...
    while (!cc->error && cc_peek(cc).type != CC_TOK_EOF) {
      ...
      /* Variable declaration inside statement block */
    ...
    *is_expr = last_was_expr && cc_last_expr_type != TYPE_VOID &&
    ...
    /* EAX already holds the last expression result */
    emit_epilogue(cc);

    /* Restore scope (drop locals, keep globals/functions) */
    cc->sym_count = saved_scope;
