...
#define CC2_PARSE_MAX_TYPEDEFS 64
#define CC2_PARSE_TYPEDEF_NAME 24
#define CC2_PARSE_TYPEDEF_BUF 1536
#define CC2_MAIN_SRC_MAX 262144
#define CC2_MAIN_PRE_MAX 393216
#define CC2_MAIN_CODE_MAX 524288
...
int cc2_struct_field_ptr_si[CC2_PARSE_MAX_STRUCT_META];
int cc2_struct_size[CC2_PARSE_MAX_STRUCTS];
int cc2_struct_align[CC2_PARSE_MAX_STRUCTS];
char cc2_typedef_names[CC2_PARSE_TYPEDEF_BUF];
int cc2_typedef_count;
char cc2_arr_names[CC2_PARSE_ARR_BUF];
int cc2_arr_base[CC2_PARSE_MAX_ARRAYS];
int cc2_arr_len[CC2_PARSE_MAX_ARRAYS];
int cc2_arr_dim2[CC2_PARSE_MAX_ARRAYS];
int cc2_arr_count;
...
int cc2_is_type_word(char *name) {
    ...
    if (cc2_strcmp(name, "u8") == 0) return 1;
    if (cc2_strcmp(name, "u16") == 0) return 1;
    if (cc2_strcmp(name, "u32") == 0) return 1;
    if (cc2_strcmp(name, "s8") == 0) return 1;
    if (cc2_strcmp(name, "s16") == 0) return 1;
    if (cc2_strcmp(name, "s32") == 0) return 1;
    if (cc2_strcmp(name, "size_t") == 0) return 1;
    while (i < cc2_typedef_count) {
        if (cc2_parse_slot_eq(cc2_typedef_names, CC2_PARSE_TYPEDEF_NAME, i, name)) return 1;
        i++;
    }
    return 0;
}

void cc2_typedef_reset() {
    int i = 0;
    cc2_typedef_count = 0;
    while (i < CC2_PARSE_TYPEDEF_BUF) {
        cc2_typedef_names[i] = 0;
        i++;
    }
}

int cc2_typedef_add(char *name) {
    int i = 0;
    if (!name || !name[0]) return 0;
    if (cc2_is_type_word(name)) return 1;
    while (i < cc2_typedef_count) {
        if (cc2_parse_slot_eq(cc2_typedef_names, CC2_PARSE_TYPEDEF_NAME, i, name)) return 1;
        i++;
    }
    if (cc2_typedef_count >= CC2_PARSE_MAX_TYPEDEFS) return 0;
    cc2_parse_slot_set(cc2_typedef_names, CC2_PARSE_TYPEDEF_NAME, cc2_typedef_count, name);
    cc2_typedef_count = cc2_typedef_count + 1;
    return 1;
}
...
int cc2_builtin_fn_addr(char *name, int *out) {
    ...
    if (cc2_strcmp(name, "print_int") == 0) { *out = (int)(uint32_t)print_int; return 1; }
    if (cc2_strcmp(name, "print_hex") == 0) { *out = (int)(uint32_t)print_hex; return 1; }
...
int cc2_split_dotted_name(char *name, char *base, int base_max,
    ...
    if (j <= 0) return 0;
    if (has_more && first_dot >= 0 && first_dot != last_dot) *has_more = 1;
    return 1;
}
...
int cc2_struct_add_field(int si, char *fname, int field_size, int elem_size, int is_array,
    ...
    cc2_struct_field_ptr_si[mi] = ptr_struct_si;
    cc2_struct_size[si] = off + field_size;
    if (align > cc2_struct_align[si]) cc2_struct_align[si] = align;
    if (cc2_struct_align[si] > 1) {
        while ((cc2_struct_size[si] % cc2_struct_align[si]) != 0) {
            cc2_struct_size[si] = cc2_struct_size[si] + 1;
        }
    }
...
void cc2_register_builtin_structs() {
    ...
    if (si >= 0) {
        cc2_struct_add_field(si, "type", 4, 4, 0, -1);
        cc2_struct_add_field(si, "int_val", 4, 4, 0, -1);
        cc2_struct_add_field(si, "text", 256, 1, 1, -1);
    }
    si = cc2_struct_add("cc2_define");
    if (si >= 0) {
        cc2_struct_add_field(si, "name", CC2_PP_MAX_NAME, 1, 1, -1);
        cc2_struct_add_field(si, "body", CC2_PP_MAX_BODY, 1, 1, -1);
        cc2_struct_add_field(si, "is_func", 4, 4, 0, -1);
        cc2_struct_add_field(si, "param_count", 4, 4, 0, -1);
    }
}
...
int cc2_arr_add(char *name, int base_slot, int len, int dim2) {
    int idx = 0;
...
int cc2_emit_call_imm_with_eax_arg(char *code, int *pos, int max, int fn_addr) {
    if (!cc2_emit8_chk(code, pos, max, 0x50)) return 0; // push eax
...
int cc2_emit_string_putchar_literal(char *code, int *pos, int max, char *s, int putc_addr) {
    int i = 0;
    while (i < 255) {
        int ch = (int)(unsigned char)s[i];
        if (ch == 0) break;
        if (!cc2_emit_putchar_imm(code, pos, max, ch, putc_addr)) return 0;
        i++;
    }
    return 1;
}
...
int cc2_codegen_expr_prec(struct cc2_token *cur, int min_prec,
    ...
    int post_is_u8 = 0;
    if (cur->type == CC2_TK_IDENT && cc2_strcmp(cur->text, "sizeof") == 0) {
        int size_val = 4;
        int si = -1;
        cc2_lex_next(cur);
        ...
        if (cur->type == CC2_TK_STRUCT) {
            ...
            if (cur->type != CC2_TK_IDENT) {
                serial_printf("[cc2_parse] FAIL expected struct name in sizeof\n");
                cc2_test_failures++;
                return 0;
            }
            si = cc2_struct_find(cur->text);
            if (si < 0) {
                serial_printf("[cc2_parse] FAIL unknown struct in sizeof '%s'\n", cur->text);
                cc2_test_failures++;
                return 0;
            }
            size_val = cc2_struct_size[si];
            if (size_val <= 0) size_val = 4;
            cc2_lex_next(cur);
        } else if (cur->type == CC2_TK_STAR) {
            cc2_lex_next(cur);
            if (cur->type == CC2_TK_IDENT) {
                idx = cc2_parse_find_local(local_names, local_count, cur->text);
                if (idx >= 0 && idx < CC2_PARSE_MAX_LOCALS) {
                    si = cc2_local_struct_ptr_si[idx];
                    if (si >= 0 && si < CC2_PARSE_MAX_STRUCTS && cc2_struct_size[si] > 0) {
                        size_val = cc2_struct_size[si];
                    } else {
                        size_val = 4;
                    }
            ...
            if (cur->type == CC2_TK_IDENT || cur->type == CC2_TK_INT_KW) {
                cc2_lex_next(cur);
                while (cur->type == CC2_TK_IDENT || cur->type == CC2_TK_INT_KW || cur->type == CC2_TK_STAR) {
                    cc2_lex_next(cur);
                }
            } else {
                size_val = 4;
                while (cur->type != CC2_TK_RPAREN && cur->type != CC2_TK_EOF) {
                    cc2_lex_next(cur);
                }
            }
        }
        ...
        if (!cc2_emit_mov_eax_imm(code, pos, max, size_val)) return 0;
        can_post_update = 0;
        post_is_global = 0;
        post_global_addr = 0;
        post_is_u8 = 0;
    ...
    if (cur->type == CC2_TK_BANG) {
        ...
        if (cur->type == CC2_TK_LPAREN) {
            ...
            if (!cc2_codegen_expr_prec(cur, 1, code, pos, max, local_names, local_count)) return 0;
            if (!cc2_parse_expect(cur, CC2_TK_RBRACKET, "']'")) return 0;
            if (arr_idx >= 0 && cc2_arr_dim2[arr_idx] > 0 && cur->type == CC2_TK_LBRACKET) {
                if (!cc2_emit8_chk(code, pos, max, 0x50)) return 0; // push eax (idx1)
                cc2_lex_next(cur);
                if (!cc2_codegen_expr_prec(cur, 1, code, pos, max, local_names, local_count)) return 0;
                if (!cc2_parse_expect(cur, CC2_TK_RBRACKET, "']'")) return 0;
                if (!cc2_emit8_chk(code, pos, max, 0x89)) return 0; // mov ecx,eax (idx2)
                if (!cc2_emit8_chk(code, pos, max, 0xC1)) return 0;
                if (!cc2_emit8_chk(code, pos, max, 0x58)) return 0; // pop eax (idx1)
                if (!cc2_emit8_chk(code, pos, max, 0x69)) return 0; // imul eax,eax,imm32
                if (!cc2_emit8_chk(code, pos, max, 0xC0)) return 0;
                if (!cc2_emit32le_chk(code, pos, max, cc2_arr_dim2[arr_idx])) return 0;
                if (!cc2_emit8_chk(code, pos, max, 0x01)) return 0; // add eax,ecx
                if (!cc2_emit8_chk(code, pos, max, 0xC8)) return 0;
            }
            if (arr_idx >= 0) {
            ...
            if (idx < 0) {
                ...
                if (gidx >= 0) {
                    ...
                    if (ptr_res > 0) {
                        ...
                        fn_idx = cc2_cg_find_fn(name);
                        if (fn_idx >= 0) {
                            cval = CC2_OUT_CODE_BASE;
                            cval = cval + cc2_cg_fn_pos[fn_idx];
                            if (!cc2_emit_mov_eax_imm(code, pos, max, cval)) return 0;
                            can_post_update = 0;
                            post_is_global = 0;
                            post_global_addr = 0;
...
int cc2_codegen_one_stmt(struct cc2_token *cur, char *code, int *pos, int max,
    ...
    if (cur->type == CC2_TK_STRUCT) {
        ...
        if (cur->type == CC2_TK_LBRACE) {
            ...
            while (cur->type != CC2_TK_RBRACE && cur->type != CC2_TK_EOF) {
                ...
                int field_size = 4;
                int field_struct_si = -1;
                int field_ptr_target_si = -1;
                if (cur->type == CC2_TK_STRUCT) {
                    cc2_lex_next(cur);
                    if (cur->type != CC2_TK_IDENT) {
                ...
                if (field_ptr_depth > 0 && field_struct_si >= 0) field_ptr_target_si = field_struct_si;
                if (cur->type != CC2_TK_IDENT) {
                ...
                if (field_size <= 0) field_size = 4;
                if (!cc2_struct_add_field(si, field_name, field_size, field_elem_size,
                                          field_is_array, field_ptr_target_si)) {
        ...
        if (!local_names || !local_count || !max_offset) {
            ...
            while (fi < cc2_struct_field_count[si]) {
                ...
                field_ptr_si_i = cc2_struct_field_ptr_si[meta];
                if (field_size_i <= 0) field_size_i = 4;
                if (field_elem_i <= 0) field_elem_i = 4;
                gidx = cc2_global_add_alias(full_name, base_off + field_off,
                                            field_size_i, field_elem_i, field_is_arr_i);
                if (gidx < 0) {
                    serial_printf("[cc2_parse] FAIL global alloc '%s'\n", full_name);
                    cc2_test_failures++;
                    return 0;
                }
                cc2_global_struct_ptr_si[gidx] = field_ptr_si_i;
                fi++;
            }
            return 1;
        }
        ...
        while (fi < cc2_struct_field_count[si]) {
            ...
            *local_count = *local_count + 1;
            ...
            meta = si * CC2_PARSE_MAX_STRUCT_FIELDS + fi;
            cc2_local_struct_ptr_si[idx] = cc2_struct_field_ptr_si[meta];
            if (((idx + 1) * 4) > *max_offset) *max_offset = (idx + 1) * 4;
            fi++;
        }
        return 1;
    ...
    if (cur->type == CC2_TK_INT_KW ||
        ...
        if (cur->type == CC2_TK_LBRACKET) {
            ...
            if (cur->type == CC2_TK_LBRACKET) {
                cc2_lex_next(cur);
                if (!cc2_parse_array_len_token_stream(cur, &arr_len2)) return 0;
            }
            if (!cc2_parse_expect(cur, CC2_TK_SEMI, "';'")) return 0;
            arr_total = arr_len;
            if (arr_len2 > 0) arr_total = arr_len * arr_len2;
            if (arr_total <= 0) arr_total = arr_len;
            arr_base = *local_count + 1;
            if ((*local_count + arr_total) > CC2_PARSE_MAX_LOCALS) {
                serial_printf("[cc2_parse] FAIL line %d too many locals count=%d need=%d max=%d\n",
                              cc2_lex_line, *local_count, arr_total, CC2_PARSE_MAX_LOCALS);
                cc2_test_failures++;
                return 0;
            }
            if (!cc2_arr_add(name, arr_base, arr_total, arr_len2)) {
                serial_printf("[cc2_parse] FAIL too many arrays\n");
                cc2_test_failures++;
                return 0;
            }
            *local_count = *local_count + arr_total;
        ...
        if (cur->type == CC2_TK_IDENT &&
            ...
            if (cur->type == CC2_TK_LPAREN) {
                ...
                if (cur->type == CC2_TK_STRING) {
                    ...
                    if (!cc2_emit_string_putchar_literal(code, pos, max, cur->text, putc_addr)) return 0;
...
int cc2_codegen_program() {
    ...
    while (cur->type != CC2_TK_EOF) {
        ...
        if (cur->type == CC2_TK_IDENT &&
            ...
            int td_brack = 0;
            int td_brace = 0;
            char td_alias[CC2_PARSE_TYPEDEF_NAME];
            td_alias[0] = 0;
            cc2_lex_next(cur);
            while (cur->type != CC2_TK_EOF) {
                if (cur->type == CC2_TK_SEMI &&
                ...
                if (cur->type == CC2_TK_IDENT &&
                    td_paren == 0 && td_brack == 0 && td_brace == 0) {
                    cc2_strncpy(td_alias, cur->text, CC2_PARSE_TYPEDEF_NAME - 1);
                    td_alias[CC2_PARSE_TYPEDEF_NAME - 1] = 0;
                }
            ...
            if (!cc2_parse_expect(cur, CC2_TK_SEMI, "';'")) return 0;
            if (td_alias[0]) {
                if (!cc2_typedef_add(td_alias)) {
                    serial_printf("[cc2_parse] FAIL too many typedef aliases\n");
                    cc2_test_failures++;
                    return 0;
                }
            }
...
void cc2_default_out_path(char *in_path, char *out_path, int out_max) {
    ...
    if (in_path[0] == '/' &&
        ...
        in_path[3] == 'n' &&
        in_path[4] == '/') {
        char *home_bin = "/home/bin/";
        while (home_bin[i] && i < out_max - 1) {
            out_path[i] = home_bin[i];
            i++;
        }
        src_pos = 5;
        while (in_path[src_pos]) src_pos = src_pos + 1;
        while (src_pos > 5 && in_path[src_pos - 1] != '/') src_pos = src_pos - 1;
        while (in_path[src_pos] && i < out_max - 1) {
            out_path[i] = in_path[src_pos];
            i++;
            src_pos = src_pos + 1;
        }
    } else {
