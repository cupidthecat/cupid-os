...
class CupidBuildCliTests(unittest.TestCase):
    ...
    def test_typed_profile_manifest_does_not_remove_foreign_rollback_contents(self):
        with tempfile.TemporaryDirectory(
            ...
            profile_key = b'"profiles":'
            self.assertEqual(payload.count(profile_key), 1)
            self._replace_seed_tool_bytes(
                manifest,
                "cupidobj",
                payload.replace(profile_key, b'"profilet":'),
            )
            output = root / "build" / "bootstrap" / "doom-cupidc-inputs.json"
            if os.name != "nt":
                output.parent.mkdir(parents=True)
    ...
    def test_typed_profile_manifest_preserves_recovery_inside_a_replaced_output_parent(
        self,
    ):
        with tempfile.TemporaryDirectory(
            ...
            root = Path(temporary)
            stable_parent_lookup = (
                self._retained_directory_lookup_survives_rename(root)
            ...
            self.assertFalse(mutation_errors, repr(mutation_errors))
            if os.name == "nt":
                self.assertTrue(blocked.is_set(), "the retained parent was not observed")
                self.assertFalse(changed.is_set())
                self.assertFalse(displaced.exists())
                self.assertEqual(result.returncode, 0, result.stderr)
                self.assertEqual(
...
''',
    ...
    def test_six_tool_v2_contract_rejects_parent_and_revision_drift(self):
        ...
        def use_retired_v2_execution_parent(document):
            if os.name == "nt":
                document["provenance"].update(
                    {
                        "parent_execution_seed_manifest_sha256": (
        ...
        def use_retired_v2_parent_generation(document):
            use_retired_v2_execution_parent(document)
            if os.name == "nt":
                document["provenance"].update(
                    {
                        "parent_plan_seed_manifest_sha256": (
        ...
        cases = (
            ...
            ("retired v2 execution parent", use_retired_v2_execution_parent),
            ("retired v2 parent generation", use_retired_v2_parent_generation),
            (
        ...
        if os.name == "nt":
            cases += (
                ...
                (
                    "retired v2 plan parent generation",
                    lambda document: document["provenance"].update(
                        {
                            "parent_plan_seed_manifest_sha256": (
                                "770f979407f930deba0c9ba887bcd14f2350a785b1c0df6b31ddc2659c46eaae"
