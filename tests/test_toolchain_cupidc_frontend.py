...
class ToolchainCupidCFrontendContractTests(unittest.TestCase):
    ...
    def test_active_return_inventory_is_drift_gated(self):
        ...
        self.assertEqual(feature["occurrences"], 17791)
    ...
    def test_active_for_statement_inventory_is_drift_gated(self):
        ...
        self.assertEqual(feature["occurrences"], 3403)
    ...
    def test_active_while_statement_inventory_is_drift_gated(self):
        ...
        self.assertEqual(feature["occurrences"], 2568)
        self.assertEqual(len(feature["files"]), 255)
    ...
    def test_active_do_statement_inventory_is_drift_gated(self):
        ...
        self.assertEqual(feature["occurrences"], 63)
        self.assertEqual(len(feature["files"]), 38)

    def test_active_switch_label_inventory_is_drift_gated(self):
        audit_path = REPO_ROOT / "docs/bootstrap/audits/active-build.json"
        audit = json.loads(audit_path.read_text(encoding="utf-8"))
        features = {item["id"]: item for item in audit["features"]}
        self.assertEqual(features["c.control.switch"]["occurrences"], 218)
        self.assertEqual(len(features["c.control.switch"]["files"]), 69)
        self.assertEqual(features["c.control.case"]["occurrences"], 1607)
        self.assertEqual(len(features["c.control.case"]["files"]), 69)
        self.assertEqual(features["c.control.default"]["occurrences"], 149)
    ...
    def test_active_if_else_inventory_is_drift_gated(self):
        ...
        audit = json.loads(audit_path.read_text(encoding="utf-8"))
        features = {item["id"]: item for item in audit["features"]}
        self.assertEqual(features["c.control.if"]["occurrences"], 29336)
        self.assertEqual(len(features["c.control.if"]["files"]), 364)
        self.assertEqual(features["c.control.else"]["occurrences"], 3928)
        self.assertEqual(len(features["c.control.else"]["files"]), 275)
    ...
    def test_active_goto_inventory_is_drift_gated(self):
        ...
        self.assertEqual(feature["occurrences"], 1578)
        self.assertEqual(len(feature["files"]), 25)
