...
class ToolchainCupidCFrontendContractTests(unittest.TestCase):
    ...
    def test_active_return_inventory_is_drift_gated(self):
        ...
        self.assertEqual(feature["occurrences"], 15760)
    ...
    def test_active_for_statement_inventory_is_drift_gated(self):
        ...
        self.assertEqual(feature["occurrences"], 3078)
    ...
    def test_active_while_statement_inventory_is_drift_gated(self):
        ...
        self.assertEqual(feature["occurrences"], 2507)
        self.assertEqual(len(feature["files"]), 252)
    ...
    def test_active_switch_label_inventory_is_drift_gated(self):
        ...
        self.assertEqual(features["c.control.switch"]["occurrences"], 203)
        self.assertEqual(len(features["c.control.switch"]["files"]), 65)
        self.assertEqual(features["c.control.case"]["occurrences"], 1520)
        self.assertEqual(len(features["c.control.case"]["files"]), 65)
        self.assertEqual(features["c.control.default"]["occurrences"], 134)
        self.assertEqual(len(features["c.control.default"]["files"]), 52)

    def test_active_if_else_inventory_is_drift_gated(self):
        audit_path = REPO_ROOT / "docs/bootstrap/audits/active-build.json"
        audit = json.loads(audit_path.read_text(encoding="utf-8"))
        features = {item["id"]: item for item in audit["features"]}
        self.assertEqual(features["c.control.if"]["occurrences"], 25725)
        self.assertEqual(len(features["c.control.if"]["files"]), 361)
        self.assertEqual(features["c.control.else"]["occurrences"], 3513)
    ...
    def test_active_goto_inventory_is_drift_gated(self):
        ...
        self.assertEqual(feature["occurrences"], 1036)
