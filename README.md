...
The hosted path can now move matching `float` and `double` values. This slice copies values; it does not compute with them. Values can be loaded from file, block-static, automatic, pointer, member, and indexed objects, then initialized, assigned, discarded, passed to fixed direct or indirect calls, returned, and captured from a call result. A `float` keeps its raw four-byte representation. A `double` uses one Linear IR handle backed by a private eight-byte snapshot. The i386 caller places them in four-byte or eight-byte cdecl stack slots. Callees return both kinds in x87 `ST0`; after call cleanup, a caller stores a `float` result in a four-byte semantic stack slot or a `double` result in a private eight-byte frame snapshot. An existing `double` can also cross an ellipsis or an unprototyped call, and `va_arg(double)` advances the cursor by eight bytes. Floating values are not truth operands in this transport slice: Linear IR and object emission reject logical, conditional, or branch metadata that tries to use one. The decoder-driven oracle checks sixteen-byte ESP alignment before every executed call and models `FLD` and `FSTP` as bit copies. Its positive zero, negative zero, infinity, and quiet NaN cases prove byte transport, not native x87 execution. Floating literals, arithmetic, comparisons, conditions, conditional expressions, value-producing floating conversions, `float` to `double` default promotion, explicit static floating initializers, `long double`, SIMD values, atomic access, and over-aligned object emission remain open. Implicit zero initialization for static floating objects already uses `.bss`, and casts to `void` already discard represented floating values.
