...
The normal image build uses the checked CupidC seed for 146 checked-in
objects and the generated kernel symbol object. All 147 sources use `.cc`.
...
frontier compiles all 146 checked-in sources twice. The complete two-pass
frontier now passes against a 434-file snapshot with SHA-256
`4bf3bca0564df68e1049e496ac990691023c4ccb5d8815e419c98bf6a11d2b53`.
Both 146-object passes are byte-identical and total 3,586,148 bytes. The
...
146-root graph passes clean normal and partitioned image builds.
...
external-inline finalization, leaving nine strict checked-in roots on the
host compiler.
...
graph. Its i386-word initializer preserves the 104,933-byte symbol blob. Two
compiles produce the same 105,348-byte object with SHA-256
`223c19ab43e228e0ef83b2b878617121a687f0eb86e5f9545f1b1754808ef36e`.
...
Poisoned-host checks cover all 147 normal CupidC recipes and fail if a
...
is 153 transforms for CupidC, 144 C transforms for the host compiler,
166 transforms for host Python, and five transforms for Make recursion. One
...
Windows user build.
The CupidC transforms are 146 checked-in normal roots, the generated kernel
symbol table, three generated installation tables, and three example
programs. The host compiler still produces 92 root objects. The renamed graph
passes both CupidLD links and CupidObj flattening. The latest clean image
includes the transferred lexer, Nuked OPL3, and the complete 146-root checked
...
audio playback, and in-OS CupidC execution at `0x01100000`. A separate smoke
...
checked-seed path already owns the 146-source checked-in production cohort
