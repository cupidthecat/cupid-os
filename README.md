...
The normal image build still uses Clang or GCC for its C objects. The shared hosted CupidC path now carries one-byte, two-byte, and four-byte integers through target-sized locals, file objects, members, indexed access, conditions, conversions, assignment, and fixed direct or indirect calls. Narrow loads extend into canonical 32-bit values, stores use the declared byte or word width, and fixed cdecl arguments keep four-byte stack slots while callers and callees normalize narrow results. Referenced fixed arrays and records also receive target-sized i386 stack storage with alignment up to four bytes. Aggregate values and initialization, narrow compound assignments and updates, atomic access, 64-bit integers and floating-point values, variadic calls, 16-byte call-site alignment, and production integration remain open under issue #25. The private in-kernel CupidC compiler continues to handle embedded runtime JIT and AOT compilation. See [the bootstrap record](docs/bootstrap/README.md), [ADR 0044](docs/adr/0044-cupidc-fixed-automatic-object-storage.md), and [ADR 0045](docs/adr/0045-cupidc-narrow-integer-values.md).
