...
Plain assignment now works for represented non-atomic bit fields in four-byte storage units. Linear IR keeps the selected member until the i386 emitter masks and stores it. Partial fields preserve neighboring bits and return the stored lane after truncation and signed extension. A full-width field uses a direct store. The contracts cover pointer-derived records and Doom's unchanged `colors[index].r = value` address shape. Compound bit-field operations, atomic access, and non-four-byte storage units remain open.
...
In GNU C mode, `__builtin_va_list` is a target `char *` cursor. Explicit frontend and IR operations cover `__builtin_va_start`, `__builtin_va_arg`, `__builtin_va_copy`, and `__builtin_va_end`. The emitter starts the cursor after the final named cdecl argument and advances it in four-byte slots. Reads support non-atomic pointers and non-atomic four-byte signed or unsigned `int`, `long`, and enum types.
