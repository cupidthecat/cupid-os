...
Scalar variadic calls now follow that same hosted path. The frontend applies lvalue, array, function, and integer default conversions to each ellipsis argument. Linear IR retains the actual argument count, and the i386 emitter uses it for cdecl argument order, indirect callee placement, alignment, and caller cleanup. Direct and indirect calls can pass represented four-byte integers and pointers. Variadic definitions can use their named parameters. Floating promotion to `double`, non-scalar ellipsis arguments, `va_list`, and access to unnamed arguments remain unfinished.
