...
The static i386 Linux compiler now reaches a complete compiler fixed point under Linux or WSL. Generation one compiles all eleven C objects in its own closure and links stage two with CupidASM startup and CupidLD. Stage two repeats the build for stage three. Every stage-two and stage-three C object matches, the independently assembled startup objects match, and the generation-one, stage-two, and stage-three compiler images are byte-identical. Stages two and three also agree on a valid object and a malformed-source diagnostic while preserving failed outputs. The 1,812,712-byte compiler image has SHA-256 `29CD222C6E33590932457D36F3728705134C8C6750947E7CFBC4ABA3B7C5500B`. A host C compiler still builds generation zero, the contract executables, and normal Cupid OS C objects. Checked seeds, a native Windows fixed point, whole-toolchain fixed points, and production handoff remain open.
