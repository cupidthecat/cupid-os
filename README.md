...
Union and Cupid class values, string leaves, explicit bit-field leaves, volatile or atomic aggregate access, over-aligned structures, Boolean mutation, direct 64-bit and floating-point values, variadic calls, block-static addresses in other block-static initializers, and production integration remain open under issue #25. A copied structure may contain wide or floating members because this path moves its complete target representation. The private in-kernel CupidC compiler continues to handle embedded runtime JIT and AOT compilation. See [the bootstrap record](docs/bootstrap/README.md), [ADR 0049](docs/adr/0049-cupidc-structure-values-and-cdecl-abi.md), [ADR 0050](docs/adr/0050-cupidc-sixteen-byte-call-alignment.md), and [ADR 0051](docs/adr/0051-cupidc-block-scope-static-object-emission.md).
