...
The hosted path also carries a limited eight-byte integer value through constants, matching conditional arms, fixed direct and indirect call results, discarded expressions, and returns. One Linear IR entry names an emitter-owned two-word snapshot. On the i386 boundary, EAX carries the low word and EDX carries the high word. Eight-byte parameters, lvalue access, conditions, arithmetic, mutation, variadic transport, and mixed-width conversions remain open.
