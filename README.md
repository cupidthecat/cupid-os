...
The hosted path can now move matching `float` and `double` values. It also performs the C default argument promotion from `float` to `double` at ellipsis and unprototyped call positions. Other floating computation is still outside this slice. Values can be loaded from file, block-static, automatic, pointer, member, and indexed objects, then initialized, assigned, discarded, passed to fixed direct or indirect calls, returned, and captured from a call result. A `float` keeps its raw four-byte representation. A `double` uses one Linear IR handle backed by a private eight-byte snapshot. For default argument promotion, the frontend records the conversion after lvalue conversion, Linear IR keeps a typed `CONVERT`, and the i386 emitter loads the four-byte source with `FLD` before spilling the converted value into an eight-byte snapshot with `FSTP`. The caller places represented values in four-byte or eight-byte cdecl stack slots. Callees return both kinds in x87 `ST0`; after call cleanup, a caller stores a `float` result in a four-byte semantic stack slot or a `double` result in a private eight-byte frame snapshot. A `double` can cross an ellipsis or an unprototyped call, and `va_arg(double)` advances the cursor by eight bytes. Floating values are not truth operands in this transport slice: Linear IR and object emission reject logical, conditional, or branch metadata that tries to use one. The decoder-driven oracle checks sixteen-byte ESP alignment before every executed call. It models same-width `FLD` and `FSTP` as bit copies and models the exact supported widening separately; it is not native x87 execution proof. Floating literals, arithmetic, comparisons, conditions, conditional expressions, fixed-parameter and explicit floating conversions, explicit static floating initializers, `long double`, SIMD values, atomic access, and over-aligned object emission remain open. Implicit zero initialization for static floating objects already uses `.bss`, and casts to `void` already discard represented floating values.
