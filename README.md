...
The normal image build still uses Clang or GCC for its C objects. The shared hosted CupidC path now carries one-byte, two-byte, and four-byte integers through target-sized locals, file objects, members, indexed access, conditions, conversions, assignment, mutation, and fixed direct or indirect calls. Narrow loads extend into canonical 32-bit values, while stores use the declared byte or word width. Compound assignments and increment or decrement evaluate byte and word destinations once, compute through the required 32-bit promotions, and store at the declared width. Signed byte and word mutation wraps through the low i386 lane and then sign-extends the stored result. Fixed cdecl arguments keep four-byte stack slots while callers and callees normalize narrow results. Referenced fixed arrays and records also receive target-sized i386 stack storage with alignment up to four bytes. An explicit cast to `void` evaluates its operand once. The hosted IR discards a represented integer, object pointer, or function pointer value, while a `void` operand leaves no abstract-stack value. Aggregate values and initialization, Boolean mutation, atomic access, 64-bit integers and floating-point values, narrow bit fields, variadic calls, 16-byte call-site alignment, and production integration remain open under issue #25. The private in-kernel CupidC compiler continues to handle embedded runtime JIT and AOT compilation. See [the bootstrap record](docs/bootstrap/README.md), [ADR 0045](docs/adr/0045-cupidc-narrow-integer-values.md), [ADR 0046](docs/adr/0046-cupidc-narrow-integer-mutation.md), and [ADR 0047](docs/adr/0047-cupidc-casts-to-void.md).
