...
The hosted path accepts zero-parameter definitions written with an empty identifier list and preserves their non-prototype function type. Direct and indirect calls without a prototype apply the default argument promotions to every argument, retain the actual count in Linear IR, and use the represented four-byte scalar cdecl path. The exact Doom profile now passes `doomgeneric_Tick()` and reaches the anonymous block-scope `struct` in `kernel/doom/src/d_main.c:689`. Nonempty identifier-list definitions, atomic variadic access, floating promotion to `double`, non-scalar arguments without declared parameter types, wider or aggregate reads, and block tag specifiers remain unfinished.
