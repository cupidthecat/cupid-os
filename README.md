...
Committed source `16a86f5b` keeps all 500 supported kernel-inspection inputs
within the Windows command limit by using retained private filenames.
Anonymous POSIX author and inspector calls run from the pinned `/proc`
directory; the generic checked runner keeps its caller's working directory.
Positive and failure tests cover both launch paths and failed-launch child
cleanup.
...
and ADRs 0387, 0388, and 0389 describe these boundaries. Both checked seeds
carry the repairs from `16a86f5b`, whose clean paired proofs passed.
...
DMA-ownership panic; gameplay acceptance remains open. The earlier
`962e476b` Windows replay stopped at its command-line limit and preserved
the old raw kernel; ADR 0387 records the repair now carried by both seeds.
The Windows image with preserved FAT contents separately passes an SMP and
`ls` smoke.

The ISO spanning-fixture recipe now selects CupidBuild's
`assemble-iso-pattern` transaction directly. It binds the assembly source,
Makefile, and complete six-tool seed; the transaction checks the 4,096-byte
...
checks all 83 Doom sources and 304 headers or include fragments, runs CupidObj
first, and requires exact agreement with an independent native JSON renderer.
Unchanged bytes keep their timestamp; failed checks preserve the previous
manifest. Make prepares `build/bootstrap` on POSIX before the transaction.

Before recipes write beneath `kernel/`, `drivers/`, or `toolchain/`, Make
finishes the profile check through an order-only dependency. This includes
the three generated installation sources, not just their objects. Other
roots still build in parallel. A profile timestamp change alone does not
rebuild the 171 order-only consumers; the 83 Doom consumers retain their
ordinary content dependencies. The audit keeps scheduling edges separate
from content inputs.
...
Source-head CupidBuild also has a typed `generate-profile-manifest`
transaction for the Doom compiler inputs. It freezes the exact 83-source
cohort and 304 `.h`/`.inc` inputs, asks CupidObj to produce the JSON first,
and requires byte parity with an independent native renderer before guarded
publication. Windows may create either missing component in the fixed
`build/bootstrap` chain. Parent-relative `NtCreateFile` returns the new
directory handle atomically, and rollback removes only empty directories with
the recorded identity. POSIX requires both parents to exist. CupidBuild refuses
a clean root because `mkdirat` followed by `openat` cannot prove which
directory a same-user process left at the public name.

Discovery retains every directory handle or descriptor. Each publication
checkpoint makes two complete passes over the retained and public bindings.
POSIX compares file identity with nanosecond mtime and ctime. Windows compares
the retained handle with an exact named `FileIdFullDirectoryInformation`
record, including file ID, LastWriteTime, and ChangeTime. These checks detect
changes observed before the last successful pass; they do not turn the source
tree into a durable filesystem snapshot.

POSIX flat transactions keep frozen inputs, maps, and captured streams in
sealed anonymous memfds and pass tools `/proc/self/fd/N` paths. The candidate,
its publication alias, the reservation, and the owner lock remain named where
publication requires a filesystem entry. On DrvFS, no-replace moves fall back
to a destination hard link followed by source unlink. Existing-output fallback
parks a verified hard link before the candidate replaces the public name.
After source-only drift, a verified rollback permits normal cleanup. Ambiguous
root, parent, output, lock, or candidate bindings instead preserve transaction
residue for recovery rather than deleting an uncertain successor.

The focused WSL checks pass ordinary create and unchanged publication plus two
restored-directory races. Both races preserve the previous bytes and mtime and
leave no transaction residue. The dedicated native Windows process module
covers inherited-handle sentinels, the exact handle allowlist, frozen-input
sharing, retained cleanup authority, and the directory-record race. Strict
native Clang and WSL syntax checks also pass.
The WSL post-install source-drift race restores the previous manifest and
leaves no private state. Windows now retains frozen inputs with read sharing
while blocking write and delete opens. It reopens the same verified identity
with delete access only during cleanup. Mutable candidates keep read, write,
and delete sharing.
Guarded assembly also asks CupidASM for caller-owned output, so CupidBuild's
...
A Windows rerun with the corrected temporary manifests completed all 19/7/24
behavior groups. A clean, commit-pinned two-host proof was still required at
that checkpoint. Its combined source-head matrices defined 31 failure,
7 help, and 37 success cases on Linux, and 19 failure, 7 help, and 24 success
cases on native Windows. The checked seeds then predated the command, and
the normal Make rule remained Python-coordinated. ADRs 0382 and 0383 record
the completed repaired-seed promotion and direct recipe handoff.
[ADR 0377](docs/adr/0377-add-typed-doom-profile-publication-to-cupidbuild.md)
records the source capability and the remaining seed and Make handoffs. ADR
0381 records the caller-owned output and Windows handle contracts.
...
that seed pair. The next pair from revision
`9d10c223fc7aa22901e6f4ae81ce800ff1b62ad6` changed only CupidBuild; ADR 0370
records it. The current pair binds revision
...
Hosted `strrchr` changes the runtime linked into every tool, so all six Linux
and Windows images moved together. The paired candidate proofs converged and
passed the complete behavior matrices. ADR 0374 records this promotion.
...
invalid-option failure. The source-head behavior totals are 28 failures, six
help cases, and 35 successes on Linux, plus 17 failures, six help cases, and
22 successes on native Windows. The added cases cover typed kernel-symbol and
kernel-flatten generation with rollback. Fresh paired fixed points carried the runner
...
command. At that checkpoint, the kernel-flatten handoff raised CupidBuild
participation to 195 transforms and reduced Python participation to 257. ADR 0360
...
cases. With the kernel-symbol and kernel-flatten pairs, the source-head
inventories are 28/6/35 on Linux and 17/6/22 on native Windows. Both active seed cohorts carry the JPEG command and its behavior
...
58-only check when the normal build failed closed with `fixed-point provenance
...
ADR 0370 records the preceding promotion.

At the ADR 0374 checkpoint, the policy-bound OS build passed all 83 Doom roots,
both CupidLD links, strict CupidDis validation, all 16 exact artifacts, and
image publication. Its flat kernel was 9,536,524 bytes; the final and pass-one ELFs were
9,765,196 and 9,634,124 bytes. The Cupid-built artifact contract accepts the
...
streams in binary mode to preserve their exact bytes. `STARTUPINFOEXA` carries
one exact `PROC_THREAD_ATTRIBUTE_HANDLE_LIST`: standard input, standard output,
and standard error. An unrelated inheritable handle remains unavailable to the
child. Borrowed output handles are made inheritable only around
`CreateProcessA`, then cleared. This launch window assumes CupidBuild remains
single threaded. Cleanup removes a mutated file if its identity still belongs
to the runner and preserves a replacement with a different identity.
...
checks under ADR 0323. ADR 0353 records the paired v2 contract, and ADR 0367
...
records native Windows author execution. At the ADR 0360 checkpoint, the source graph had 748 active inputs,
452 transforms, 255 feature requirements, and 28 accounted unreachable files.
Participation
is CupidC 250, CupidObj 192, CupidASM 9, CupidLD 9, CupidDis 10, CupidBuild 192,
...
with a Cupid participant. That historical schema-v3 `CUPMAN4`
...
That publication carried the six-tool candidate, including
`cupidc-cupidbuild.elf`, without promoting it: the checked input manifest then
...
help, and success counts of 28/6/35 for Linux and 17/6/22 for Windows.
...
| --- | ---: | --- |
| `manifest.json` | 6,602 | `470fcd1b8b1a1506f26d3dd33d51f55d6896571aacb7329b792d4612f9434781` |
| CupidASM | 496,688 | `d1a2eac04da01a7b656a4a6ffc58f91600da6b2651804f1b48f6ee92ee4407f2` |
| CupidBuild | 361,632 | `51b366d7ec6423b76b91e17720c1b77ea1387163c175c692779853dd8d2b8e71` |
| CupidC | 2,691,780 | `b17b2c5588fad1735d8dd1226bbeba7ae6f92fa2e70e5d70c1f0b4b979e34e17` |
| CupidDis | 538,580 | `ec6f381883f2f1230b7df20fb7fbc5d629ac895cc26dd72b091bdf4c40ebd2ef` |
| CupidLD | 312,952 | `ec1981e97b0a11bf3538df34e85d071aa81ea01596913cfb1364013f35a507c0` |
| CupidObj | 392,844 | `2988a0d4e7a8416dcef0a5b178df5eaa51faf80d94881eb6e46a1c2a9d9475b6` |

| Windows seed member | Bytes | SHA-256 |
| --- | ---: | --- |
| `manifest.json` | 2,852 | `e7e65908eb03eec43e44e2946b395723b164f5701d980aae8ffaaf1006c3d7e4` |
| CupidASM | 480,256 | `dc004aa0026d37dfdf623441a85b539c2c65497c9159c7c394be03a9bc16b7ad` |
| CupidBuild | 378,880 | `ff47743b7963691142dd2ef818dac40d66dc81869828fe1efcbd91ac1e872c27` |
| CupidC | 2,620,416 | `1a1bf6e016b5a61dfa61ff6f9c02672d338f2608341a26bf1a91b6376753f13b` |
| CupidDis | 517,120 | `36af89b475fdc11f674b9ca7cc1cb61d9f756ca212332a3f7fa9076d863d29e5` |
| CupidLD | 296,960 | `b89b42dd7df9fca44e59d93b97bbfd8f79da3352587da2dd50adfa72afee8f29` |
| CupidObj | 376,320 | `82e20c148756a1b1db7ed7b62b1077b88d45320fc01aea203111be34eea1e7b5` |
...
contract, ADR 0367 records the preceding six-tool promotion, ADR 0370 records
the kernel-symbol pair, and ADR 0374 records the active promotion. The fresh promotion candidates match all stage-three and
...
- Raw CupidASM output can publish a deterministic source-derived map of
  ...
  Parent-replacement tests exposed a POSIX candidate leak. Windows now keeps
  its handle-pinned private directory below the stable repository root. POSIX
  reserves one exclusive `.reserve` file there. Frozen inputs, maps, and
  captured streams live in sealed anonymous memfds, while the named candidate
  and its publication alias stay on the target filesystem for atomic install.
  Tools receive direct `/proc/self/fd/N` paths. Both caller modules pass all 10
  tests on Windows and through WSL.
...
CupidObj in 192, CupidASM in nine, CupidLD in nine, and CupidDis in ten. Four
...
same bytes. It produces no OS code. Checked CupidASM assembles `big.bin` from
`test_iso/big_pattern.asm` inside CupidBuild's typed `assemble-iso-pattern`
transaction. CupidBuild freezes the inputs, checks the exact 4,096-byte
candidate and data-only map, runs checked CupidDis, and publishes atomically.
Both repaired seeds carry this direct recipe, including timestamp-preserving
reuse and rollback. ADR 0386 records the handoff.
...
It binds revision `0232cb57aad5d6bdfd7bd77499762514b2f0ebfd`, the
59-input snapshot
`0b591a0bef928186641b3aa1fb98c1e145e6c4905c8b6cb87c34a1ace4bc87d2`,
the native plan
`f9dce66230a693de9d9d0e60127a4a6c44ea465989f381c995086bfe723cff14`,
...
objects, three assembly objects, and all six tools, then passed the 17/6/22
failure, help, and success matrix. Its 64,681-byte candidate report has
SHA-256
...
two. ADR 0374 records that historical pair and its fixed-point evidence.
CupidDis checks static ELF entry points and defined function symbols against
decoded instruction starts.

At that checkpoint, output-bearing Windows recipes ran private copies of this cohort directly.
...
earlier promotion. ADR 0353 records the paired v2 contract, and ADR 0367
records a preceding six-tool cohort.
...
The wrapper freezes each selected source and the complete set of 304 `.h` and
`.inc` inputs for both profiles. Its content-addressed manifest fixes the
three-source and 80-source memberships. The current 72,950-byte manifest has
SHA-256
...
unsigned ASCII order, and hashes all 304 captured inputs with its own SHA-256
implementation. The 951,725-byte active snapshot contains 921,527 captured
bytes, 691 memberships, and 995 encoded path records. The normal Make target
passes the checked seed manifest. The wrapper derives both the snapshot and an
independent Python JSON oracle from one stable capture, then runs CupidObj from
the exact frozen seed.
...
root, input membership or byte drift, a symbolic link, or an NTFS junction
...
`0232cb57aad5d6bdfd7bd77499762514b2f0ebfd` and source snapshot
`0b591a0bef928186641b3aa1fb98c1e145e6c4905c8b6cb87c34a1ace4bc87d2`.
...
capabilities. CupidASM is 496,688 bytes with SHA-256
...
records that checkpoint's pair. [ADR 0375](docs/adr/0375-transfer-kernel-flatten-to-cupidbuild.md)
...
manifest and binaries. It copies all 59 source inputs into a private compiler
...
At the preceding 58-input ADR 0356 checkpoint, the promotion proof compared all
22 C objects, the independently assembled startup object, and the six linked
images between stages three and four. It passed six help paths, 31 successful
operations, and 24 useful failures. The
...
contract, checked runner, and independent policy modules contain 58 tests. They
