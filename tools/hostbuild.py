...
def write_ksyms_source(nm: str, elf: Path, out: Path) -> None:
    ...
    with out.open("w", newline="\n") as f:
        ...
        f.write("/* i386 words preserve the blob bytes with fewer initializers. */\n")
        f.write("const unsigned int\n")
        f.write('__attribute__((section(".ksyms"), used, aligned(4)))\n')
        f.write("ksym_blob[] = {\n")
        for i in range(0, len(words), 8):
            chunk = words[i : i + 8]
            f.write("  " + " ".join(f"0x{word:08x}u," for word in chunk) + "\n")
        f.write("};\n\n")
        f.write(f"const unsigned int ksym_blob_size = {len(blob)}u;\n")
