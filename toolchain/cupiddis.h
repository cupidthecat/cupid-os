...
/* The source descriptor and bytes, raw labels and their names, and all ELF
 * names and payload views are borrowed and must outlive rendering.  ELF
 * metadata arrays and every derived raw/function/symbol/relocation order array
 * are owned by the inspecting job's arena, so that job must also outlive
 * rendering.  Only an unmodified report returned successfully by
 * ctool_dis_inspect may be passed to ctool_dis_render.
 *
