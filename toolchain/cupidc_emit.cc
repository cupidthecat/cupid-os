...
typedef struct {
  ...
  ctool_u32 *returns_twice_spill_offsets;
...
static ctool_bool cemit_dglibc_legacy_template_equal(
    ctool_string_t text) {
  static const char legacy_template[] =
      ".global dg_setjmp\n"
      "dg_setjmp:\n"
      "    movl  4(%esp), %eax\n"
      "    movl  %ebx,  0(%eax)\n"
      ...
      "    movl  %ecx,  %eax\n"
      "    jmp  *%edx\n";
  return cemit_strings_equal(text, ctool_string(legacy_template));
}
...
static cemit_file_assembly_kind_t cemit_file_assembly_template_kind(
  ...
  static const char *const templates[] = {
      ...
      "    jmp  *%edx\n"};
  ctool_u32 kind;
  if (cemit_dglibc_legacy_template_equal(text) == CTOOL_TRUE) {
    return CEMIT_FILE_ASSEMBLY_DGLIBC_JUMPS;
  }
...
static ctool_status_t cemit_index_file_assemblies(
  ...
  for (index = 0u; index < context->ir.file_assembly_count; index++) {
    ...
    if (kind == CEMIT_FILE_ASSEMBLY_DGLIBC_JUMPS) {
      ...
      ctool_bool legacy =
          cemit_dglibc_legacy_template_equal(
      ...
      }
      {
        ...
        if (setjmp_candidate->kind != CTOOL_C_BINDING_FUNCTION ||
            ...
            setjmp_candidate->attributes !=
                (legacy == CTOOL_TRUE
                     ? 0u
                     : CTOOL_C_DECL_ATTR_RETURNS_TWICE) ||
            longjmp_candidate->attributes !=
                (legacy == CTOOL_TRUE
                     ? 0u
...
static ctool_status_t cemit_index_initializers(cemit_context_t *context) {
  ...
  for (index = 0u; index < context->unit->initializer_count; index++) {
    ...
    if (initializer->kind == CTOOL_C_INITIALIZER_EXPRESSION) {
      ...
      if (initializer->address_kind ==
          CTOOL_C_INITIALIZER_ADDRESS_BINDING) {
        const ctool_c_binding_t *addressed = &context->unit->bindings
            [initializer->address_reference];
        if (addressed->kind == CTOOL_C_BINDING_FUNCTION &&
            (addressed->attributes &
             CTOOL_C_DECL_ATTR_RETURNS_TWICE) != 0u) {
          return cemit_emit_failure(
              context, CTOOL_ERR_UNSUPPORTED,
              CTOOL_C_EMIT_DIAG_INITIALIZER, &initializer->location,
              "CupidC requires returns_twice functions to be called "
              "directly instead of stored in a function pointer");
        }
...
static ctool_status_t cemit_ensure_binding_symbol(
  ...
  if ((binding->kind != CTOOL_C_BINDING_OBJECT &&
      ...
      ((binding->attributes & CTOOL_C_DECL_ATTR_NORETURN) != 0u &&
       binding->kind != CTOOL_C_BINDING_FUNCTION) ||
      ((binding->attributes & CTOOL_C_DECL_ATTR_RETURNS_TWICE) != 0u &&
       binding->kind != CTOOL_C_BINDING_FUNCTION) ||
      ((binding->attributes &
...
static ctool_status_t cemit_x86_x87_memory(
  ...
  if (!(((mnemonic == CTOOL_X86_MN_FLD ||
         ...
         (width_bits == 32u || width_bits == 64u ||
          width_bits == 80u)) ||
        ((mnemonic == CTOOL_X86_MN_FILD ||
          mnemonic == CTOOL_X86_MN_FISTP) &&
...
static ctool_status_t cemit_x86_enter_extended_precision(
    cemit_context_t *context) {
  ctool_status_t status = cemit_x86_reserve_locals(context, 4u);
  if (status == CTOOL_OK) {
  ...
  if (status == CTOOL_OK) {
    status = cemit_x86_word_stack_immediate(
        context, CTOOL_X86_MN_AND, 2, 0xfcffu);
  }
  if (status == CTOOL_OK) {
    status = cemit_x86_word_stack_immediate(
        context, CTOOL_X86_MN_OR, 2, 0x0300u);
  }
  if (status == CTOOL_OK) {
    status = cemit_x86_x87_control_memory(
...
static ctool_status_t cemit_x86_leave_extended_precision(
    cemit_context_t *context) {
  ctool_status_t status = cemit_x86_x87_control_memory(
      context, CTOOL_X86_MN_FLDCW, 4u, 0);
  return status == CTOOL_OK
             ? cemit_x86_add_register_constant(context, 4u, 4u)
             : status;
}
...
static ctool_bool cemit_ir_type_is_truth_scalar(
  ...
  return cemit_ir_type_is_value_integer(context, type) == CTOOL_TRUE ||
                 ...
                 (cemit_ir_type_is_floating_value(context, type) ==
                      CTOOL_TRUE &&
                  cemit_type_has_atomic_qualification(context, type) ==
                      CTOOL_FALSE)
             ? CTOOL_TRUE
             : CTOOL_FALSE;
}
...
static ctool_bool cemit_ir_floating_conversion_is_valid(
  ...
  if (source == (const ctool_c_type_node_t *)0 ||
      ...
      cemit_type_has_atomic_qualification(context, source_type) ==
          CTOOL_TRUE ||
      cemit_type_has_atomic_qualification(context, target_type) ==
          CTOOL_TRUE) {
    return CTOOL_FALSE;
  }
  source_floating =
...
static ctool_status_t cemit_x86_push_floating_result(
  ...
  status = CTOOL_OK;
  if (layout->size == 12u) {
    status = cemit_x86_move_register_constant(context, 0u, 0u);
    if (status == CTOOL_OK) {
      status = cemit_x86_store_local_register(
          context, temporary_offset, 8u, 0u);
    }
...
static ctool_status_t cemit_x86_materialize_floating_truth(
    cemit_context_t *context, ctool_u32 type, ctool_bool logical_not) {
  const ctool_c_type_layout_t *layout;
  ...
  ctool_status_t status;
  if (cemit_ir_type_is_floating_value(context, type) == CTOOL_FALSE ||
      (logical_not != CTOOL_FALSE && logical_not != CTOOL_TRUE)) {
    return CTOOL_ERR_INTERNAL;
  }
  layout = &context->unit->layout.types[type];
  predicate = logical_not == CTOOL_TRUE ? CTOOL_X86_MN_SETE
                                        : CTOOL_X86_MN_SETNE;
  ...
  if (layout->size == 12u) {
    ...
    if (status == CTOOL_OK) {
      status = cemit_x86_no_operand(context, CTOOL_X86_MN_FLDZ);
    }
    if (status == CTOOL_OK) {
      status = cemit_x86_two_registers(
    ...
    if (status == CTOOL_OK) {
      status = cemit_x86_two_registers(
          context, CTOOL_X86_MN_PXOR, CTOOL_X86_REG_XMM, 1u,
  ...
  if (status == CTOOL_OK) {
    status = cemit_x86_move_register_constant(
        context, 0u, logical_not == CTOOL_TRUE ? 0u : 1u);
  }
  done_target = ctool_buffer_view(context->active_text).size;
  if (status == CTOOL_OK) {
    status = cemit_patch_branch(
...
static ctool_status_t cemit_x86_load_x87_power_of_two(
    cemit_context_t *context, ctool_u32 doublings) {
  ctool_u32 index;
  ctool_status_t status;
  if (doublings == 0u || doublings > 2u) {
    return CTOOL_ERR_INTERNAL;
  }
  status = cemit_x86_push_integer(context, 0x40000000u);
  if (status == CTOOL_OK) {
    status = cemit_x86_push_integer(context, 0u);
  }
  if (status == CTOOL_OK) {
    status = cemit_x86_x87_memory(
        context, CTOOL_X86_MN_FILD, 4u, 0, 64u);
  }
  if (status == CTOOL_OK) {
    status = cemit_x86_discard_arguments(context, 8u);
  }
  for (index = 0u; status == CTOOL_OK && index < doublings; index++) {
    status = cemit_x86_one_register(
        context, CTOOL_X86_MN_FLD, CTOOL_X86_REG_X87, 0u, 32u);
    if (status == CTOOL_OK) {
      status = cemit_x86_one_register(
          context, CTOOL_X86_MN_FADDP,
          CTOOL_X86_REG_X87, 1u, 32u);
    }
  }
  return status;
}

static ctool_status_t cemit_x86_load_integer_x87(
    cemit_context_t *context, ctool_u32 source_type) {
  const ctool_c_type_layout_t *layout;
  ctool_bool source_wide;
  ctool_u32 done_patch = CTOOL_C_AST_NONE;
  ...
  layout = &context->unit->layout.types[source_type];
  source_wide = cemit_ir_type_is_wide_integer(context, source_type);
  if (source_wide == CTOOL_TRUE) {
    ...
    if (status == CTOOL_OK && layout->is_signed == CTOOL_TRUE) {
      status = cemit_x86_shift_register(
          context, CTOOL_X86_MN_SAR, 2u, 31u);
    } else if (status == CTOOL_OK) {
      status = cemit_x86_two_registers(
  ...
  if (status == CTOOL_OK && source_wide == CTOOL_TRUE &&
      layout->is_signed == CTOOL_FALSE) {
    status = cemit_x86_two_registers(
        context, CTOOL_X86_MN_TEST, CTOOL_X86_REG_GPR32, 2u,
        CTOOL_X86_REG_GPR32, 2u, 32u);
    if (status == CTOOL_OK) {
      status = cemit_x86_branch(
          context, CTOOL_X86_MN_JNS, &done_patch, &done_after);
    }
    if (status == CTOOL_OK) {
      status = cemit_x86_enter_extended_precision(context);
    }
    if (status == CTOOL_OK) {
      status = cemit_x86_load_x87_power_of_two(context, 2u);
    }
    if (status == CTOOL_OK) {
      status = cemit_x86_one_register(
          context, CTOOL_X86_MN_FADDP,
    ...
    if (status == CTOOL_OK) {
      status = cemit_x86_leave_extended_precision(context);
    }
    done_target = ctool_buffer_view(context->active_text).size;
    if (status == CTOOL_OK) {
      status = cemit_patch_branch(
...
static ctool_status_t cemit_x86_convert_integer_to_floating(
  ...
  if (target->kind == CTOOL_C_TYPE_LONG_DOUBLE ||
      cemit_ir_type_is_wide_integer(context, source_type) == CTOOL_TRUE) {
    status = cemit_x86_load_integer_x87(context, source_type);
    return status == CTOOL_OK
               ? cemit_x86_push_floating_result(
                     context, target_type, temporary_offset)
               : status;
  }
  status = cemit_x86_one_register(
      context, CTOOL_X86_MN_POP, CTOOL_X86_REG_GPR32, 0u, 32u);
...
static ctool_status_t cemit_x86_x87_truncate_to_signed_wide(
    cemit_context_t *context) {
  ctool_status_t status = cemit_x86_reserve_locals(context, 12u);
  if (status == CTOOL_OK) {
    status = cemit_x86_x87_control_memory(
        context, CTOOL_X86_MN_FNSTCW, 4u, 0);
  }
  if (status == CTOOL_OK) {
    status = cemit_x86_move_word_stack_ax(context, CTOOL_TRUE, 0);
  }
  if (status == CTOOL_OK) {
  ...
  if (status == CTOOL_OK) {
    status = cemit_x86_word_ax_immediate(
        context, CTOOL_X86_MN_OR, 0x0c00u);
  }
  if (status == CTOOL_OK) {
    status = cemit_x86_move_word_stack_ax(context, CTOOL_FALSE, 2);
  }
  if (status == CTOOL_OK) {
    status = cemit_x86_x87_control_memory(
        context, CTOOL_X86_MN_FLDCW, 4u, 2);
  ...
  if (status == CTOOL_OK) {
    status = cemit_x86_x87_memory(
        context, CTOOL_X86_MN_FISTP, 4u, 4, 64u);
  }
  if (status == CTOOL_OK) {
    status = cemit_x86_x87_control_memory(
        context, CTOOL_X86_MN_FLDCW, 4u, 0);
  ...
  if (status == CTOOL_OK) {
    status = cemit_x86_load_stack(context, 2u, 8u);
  }
  if (status == CTOOL_OK) {
    status = cemit_x86_add_register_constant(context, 4u, 12u);
  }
  return status;
}

static ctool_status_t cemit_x86_convert_floating_to_integer_x87(
    cemit_context_t *context, ctool_u32 source_type,
    ctool_u32 target_type, ctool_u32 temporary_offset) {
  const ctool_c_type_layout_t *target_layout;
  ctool_bool target_wide;
  ctool_u32 low_patch = CTOOL_C_AST_NONE;
  ctool_u32 low_after = CTOOL_C_AST_NONE;
  ctool_u32 ready_patch = CTOOL_C_AST_NONE;
  ctool_u32 ready_after = CTOOL_C_AST_NONE;
  ctool_u32 low_target;
  ctool_u32 ready_target;
  ctool_status_t status;
  if (target_type >= context->unit->layout.type_count ||
      cemit_ir_type_is_floating_value(
          context, source_type) == CTOOL_FALSE ||
      cemit_ir_type_is_value_integer(
          context, target_type) == CTOOL_FALSE) {
  ...
  target_wide = cemit_ir_type_is_wide_integer(context, target_type);
  status = cemit_x86_load_floating_stack_value(
      context, source_type, 0u);
  if (status == CTOOL_OK) {
    status = cemit_x86_discard_arguments(context, 4u);
  }
  if (status == CTOOL_OK && target_wide == CTOOL_TRUE &&
      target_layout->is_signed == CTOOL_FALSE) {
    status = cemit_x86_enter_extended_precision(context);
    if (status == CTOOL_OK) {
      status = cemit_x86_load_x87_power_of_two(context, 1u);
    }
    if (status == CTOOL_OK) {
      status = cemit_x86_two_registers(
          context, CTOOL_X86_MN_FUCOMIP,
    ...
    if (status == CTOOL_OK) {
      status = cemit_x86_branch(
          context, CTOOL_X86_MN_JA, &low_patch, &low_after);
    }
    if (status == CTOOL_OK) {
      status = cemit_x86_load_x87_power_of_two(context, 1u);
    }
    ...
    if (status == CTOOL_OK) {
      status = cemit_x86_branch(
          context, CTOOL_X86_MN_JMP, &ready_patch, &ready_after);
    }
    low_target = ctool_buffer_view(context->active_text).size;
    if (status == CTOOL_OK) {
      status = cemit_patch_branch(
    ...
    if (status == CTOOL_OK) {
      status = cemit_x86_two_registers(
          context, CTOOL_X86_MN_XOR, CTOOL_X86_REG_GPR32, 1u,
          CTOOL_X86_REG_GPR32, 1u, 32u);
    }
    ready_target = ctool_buffer_view(context->active_text).size;
    if (status == CTOOL_OK) {
      status = cemit_patch_branch(
          context->active_text, ready_patch, ready_after, ready_target);
    }
    if (status == CTOOL_OK) {
      status = cemit_x86_leave_extended_precision(context);
    }
  ...
  if (status == CTOOL_OK) {
    status = cemit_x86_x87_truncate_to_signed_wide(context);
  }
  if (status == CTOOL_OK && target_wide == CTOOL_TRUE &&
      target_layout->is_signed == CTOOL_FALSE) {
    status = cemit_x86_two_registers(
        context, CTOOL_X86_MN_OR, CTOOL_X86_REG_GPR32, 2u,
        CTOOL_X86_REG_GPR32, 1u, 32u);
  }
  if (status != CTOOL_OK) {
    return status;
  }
  if (target_wide == CTOOL_TRUE) {
    return cemit_x86_push_wide_register_snapshot(
        context, temporary_offset, 0u, 2u);
  }
  status = cemit_x86_canonicalize_scalar_eax(context, target_type);
  return status == CTOOL_OK
             ? cemit_x86_one_register(
      ...
      (cemit_ir_type_is_wide_integer(
       ...
       !(source->kind == CTOOL_C_TYPE_DOUBLE &&
         ...
         context->unit->layout.types[target_type].is_signed ==
             CTOOL_FALSE))) {
    return cemit_x86_convert_floating_to_integer_x87(
        context, source_type, target_type, temporary_offset);
  }
...
static ctool_status_t cemit_emit_direct_call(
    cemit_context_t *context,
    const ctool_c_ir_instruction_t *instruction,
    ctool_u32 temporary_offset, ctool_u32 returns_twice_spill_offset,
    ctool_u32 stack_base_residue, ctool_u32 frame_size,
    ctool_u32 stack_depth) {
  const ctool_c_binding_t *binding;
  const ctool_c_type_node_t *function_type;
  ...
  if (returns_twice == CTOOL_TRUE &&
    ...
    return cemit_emit_failure(
        context, CTOOL_ERR_UNSUPPORTED,
        CTOOL_C_EMIT_DIAG_UNSUPPORTED, &instruction->location,
  ...
  if (returns_twice == CTOOL_TRUE) {
    ...
    ctool_u32 argument_bytes_for_spill;
    if (stack_depth < instruction->argument_count ||
        cemit_multiply_overflows(instruction->argument_count, 4u) ==
            CTOOL_TRUE) {
      return CTOOL_ERR_INTERNAL;
    }
    live_count = stack_depth - instruction->argument_count;
    argument_bytes_for_spill = instruction->argument_count * 4u;
    if ((live_count == 0u &&
         returns_twice_spill_offset != CTOOL_C_AST_NONE) ||
        (live_count != 0u &&
         returns_twice_spill_offset == CTOOL_C_AST_NONE)) {
      return CTOOL_ERR_INTERNAL;
    }
    for (live_index = 0u; status == CTOOL_OK &&
                         live_index < live_count;
         live_index++) {
      ctool_u32 stack_offset;
      ctool_u32 local_byte_offset;
      if (cemit_multiply_overflows(live_index, 4u) == CTOOL_TRUE ||
          cemit_add_overflows(argument_bytes_for_spill,
                              live_index * 4u) == CTOOL_TRUE) {
        return CTOOL_ERR_OVERFLOW;
      }
      local_byte_offset = live_index * 4u;
      stack_offset = argument_bytes_for_spill + local_byte_offset;
      status = cemit_x86_load_stack(context, 0u, stack_offset);
      if (status == CTOOL_OK) {
        status = cemit_x86_store_local_register(
            context, returns_twice_spill_offset,
  ...
  if (status == CTOOL_OK && returns_twice == CTOOL_TRUE) {
    ctool_u32 live_count =
        stack_depth - instruction->argument_count;
    ctool_u32 live_index;
    for (live_index = 0u; status == CTOOL_OK &&
                         live_index < live_count;
         live_index++) {
      ...
      status = cemit_x86_load_local_register(
          context, 1u, returns_twice_spill_offset,
          local_byte_offset);
      if (status == CTOOL_OK) {
        status = cemit_x86_store_stack(
            context, local_byte_offset, 1u);
      }
    }
  }
  if (status == CTOOL_OK &&
...
typedef enum {
  CEMIT_STATE_MEMORY_INPUT_NONE = 0,
  CEMIT_STATE_MEMORY_INPUT_FLDCW,
  CEMIT_STATE_MEMORY_INPUT_LDMXCSR
} cemit_state_memory_input_kind_t;

static cemit_state_memory_input_kind_t
cemit_state_memory_input_template_kind(ctool_string_t template_text) {
  if (cemit_string_equals_literal(
          template_text, "fldcw %0") == CTOOL_TRUE) {
    return CEMIT_STATE_MEMORY_INPUT_FLDCW;
  }
  return cemit_string_equals_literal(
             template_text, "ldmxcsr %0") == CTOOL_TRUE
             ? CEMIT_STATE_MEMORY_INPUT_LDMXCSR
             : CEMIT_STATE_MEMORY_INPUT_NONE;
}

static ctool_bool cemit_state_memory_input_metadata_is_valid(
    const cemit_context_t *context,
    const ctool_c_assembly_t *assembly,
    cemit_state_memory_input_kind_t kind) {
  const ctool_c_assembly_operand_t *operand;
  const ctool_c_type_layout_t *layout;
  if (context == (const cemit_context_t *)0 ||
      assembly == (const ctool_c_assembly_t *)0 ||
      kind == CEMIT_STATE_MEMORY_INPUT_NONE ||
      assembly->flags != CTOOL_C_ASSEMBLY_VOLATILE ||
      assembly->output_count != 0u ||
      assembly->input_count != 1u ||
      assembly->first_operand >=
  ...
  return layout->is_integer == CTOOL_TRUE &&
                 ...
                 layout->is_complete_object == CTOOL_TRUE &&
                 layout->size ==
                     (kind == CEMIT_STATE_MEMORY_INPUT_FLDCW ? 2u : 4u)
             ? CTOOL_TRUE
             : CTOOL_FALSE;
}

static ctool_status_t cemit_emit_state_memory_input_assembly(
    cemit_context_t *context,
    const ctool_c_assembly_t *assembly,
    ctool_u32 temporary_offset,
    cemit_state_memory_input_kind_t kind) {
  ctool_status_t status;
  if (temporary_offset != 0u ||
      cemit_state_memory_input_metadata_is_valid(
          context, assembly, kind) == CTOOL_FALSE) {
    return cemit_emit_failure(
        context, CTOOL_ERR_UNSUPPORTED, CTOOL_C_EMIT_DIAG_UNSUPPORTED,
        &assembly->location,
  ...
  status = cemit_x86_one_register(
      context, CTOOL_X86_MN_POP, CTOOL_X86_REG_GPR32, 0u, 32u);
  if (status == CTOOL_OK &&
      kind == CEMIT_STATE_MEMORY_INPUT_FLDCW) {
    status = cemit_x86_x87_control_memory(
        context, CTOOL_X86_MN_FLDCW, 0u, 0);
  } else if (status == CTOOL_OK) {
...
static ctool_status_t cemit_emit_assembly(
  ...
  {
    cemit_state_memory_input_kind_t state_input_kind =
        cemit_state_memory_input_template_kind(
            assembly->template_text);
    if (state_input_kind != CEMIT_STATE_MEMORY_INPUT_NONE) {
      return cemit_emit_state_memory_input_assembly(
          context, assembly, temporary_offset, state_input_kind);
    }
  }
...
static ctool_status_t cemit_emit_ir_instruction(
    ...
    ctool_u32 returns_twice_spill_offset,
    ctool_u32 stack_base_residue, ctool_u32 frame_size,
    ctool_u32 stack_depth,
  ...
  if (ir_instruction->kind == CTOOL_C_IR_INSTRUCTION_CONVERT) {
    ...
    if (floating_conversion == CTOOL_TRUE) {
      ...
      if (source_floating == CTOOL_TRUE &&
        ...
        if (target != (const ctool_c_type_node_t *)0 &&
            target->kind == CTOOL_C_TYPE_BOOL) {
          status = cemit_x86_materialize_floating_truth(
              context, ir_instruction->input_type, CTOOL_FALSE);
          return status == CTOOL_OK
                     ? cemit_x86_one_register(
                           context, CTOOL_X86_MN_PUSH,
  ...
  if (ir_instruction->kind == CTOOL_C_IR_INSTRUCTION_BRANCH_ZERO) {
    ...
    if (cemit_ir_type_is_floating_value(
      ...
      if (status == CTOOL_OK) {
        status = cemit_x86_branch(context, CTOOL_X86_MN_JNE,
  ...
  for (index = 0u; index < function->instruction_count; index++) {
    ...
    if (context->value_temporary_offsets[absolute] == 0u) {
      ...
      if (instruction->kind == CTOOL_C_IR_INSTRUCTION_ASSEMBLY) {
        ...
        if (cemit_state_memory_input_template_kind(
                assembly->template_text) !=
            CEMIT_STATE_MEMORY_INPUT_NONE) {
          continue;
        }
...
static ctool_status_t cemit_prepare_returns_twice_spills(
    cemit_context_t *context,
    const ctool_c_ir_function_t *function,
    const ctool_u32 *stack_depths, ctool_u32 *frame_size) {
  ctool_u32 *visited = (ctool_u32 *)0;
  ctool_u32 *worklist = (ctool_u32 *)0;
  ctool_u32 worklist_count = 0u;
  ctool_u32 worklist_cursor = 0u;
  ctool_bool has_live_prefix_call = CTOOL_FALSE;
  ctool_u32 index;
  if (function == (const ctool_c_ir_function_t *)0 ||
      frame_size == (ctool_u32 *)0 ||
      (function->instruction_count != 0u &&
       (stack_depths == (const ctool_u32 *)0 ||
        context->returns_twice_spill_offsets ==
            (ctool_u32 *)0))) {
    return CTOOL_ERR_INTERNAL;
  }
  for (index = 0u; index < function->instruction_count; index++) {
    ...
    const ctool_c_ir_instruction_t *instruction =
        &context->ir.instructions[absolute];
    const ctool_c_binding_t *binding;
    context->returns_twice_spill_offsets[absolute] =
        CTOOL_C_AST_NONE;
    if (instruction->kind != CTOOL_C_IR_INSTRUCTION_CALL_DIRECT) {
      continue;
    }
    ...
    binding = &context->unit->bindings[instruction->reference];
    if ((binding->attributes & CTOOL_C_DECL_ATTR_RETURNS_TWICE) ==
        0u) {
      continue;
    }
    if (stack_depths[index] == CTOOL_C_AST_NONE ||
        stack_depths[index] < instruction->argument_count) {
      return CTOOL_ERR_INTERNAL;
    }
    if (stack_depths[index] > instruction->argument_count) {
      has_live_prefix_call = CTOOL_TRUE;
    }
  }
  if (has_live_prefix_call == CTOOL_FALSE) {
    return CTOOL_OK;
  }
  {
    ctool_status_t allocation_status = cemit_alloc_array(
        context, function->instruction_count,
        (ctool_u32)sizeof(ctool_u32), (void **)&visited);
    if (allocation_status == CTOOL_OK) {
      allocation_status = cemit_alloc_array(
          context, function->instruction_count,
          (ctool_u32)sizeof(ctool_u32), (void **)&worklist);
    }
    if (allocation_status != CTOOL_OK) {
  ...
  for (index = 0u; index < function->instruction_count; index++) {
    ...
    if (index + 1u >= function->instruction_count) {
      return CTOOL_ERR_INTERNAL;
    }
    continuation = index + 1u;
    if (visited[continuation] == 0u) {
      if (worklist_count >= function->instruction_count) {
        return CTOOL_ERR_INTERNAL;
      }
      visited[continuation] = 1u;
      worklist[worklist_count++] = continuation;
    }
  }
  while (worklist_cursor < worklist_count) {
    ...
    const ctool_c_ir_instruction_t *candidate =
        &context->ir.instructions[function->first_instruction + relative];
    ctool_u32 successor_count = 0u;
    ctool_u32 successors[2];
    ctool_u32 successor_index;
    ctool_bool falls_through = CTOOL_TRUE;
    if (candidate->kind == CTOOL_C_IR_INSTRUCTION_CALL_DIRECT) {
      const ctool_c_binding_t *binding =
          &context->unit->bindings[candidate->reference];
      if ((binding->attributes & CTOOL_C_DECL_ATTR_RETURNS_TWICE) != 0u &&
          stack_depths[relative] > candidate->argument_count) {
        return cemit_emit_failure(
            context, CTOOL_ERR_UNSUPPORTED,
            CTOOL_C_EMIT_DIAG_UNSUPPORTED, &candidate->location,
            "returns_twice calls with live expression operands must not be "
            "reachable again after a returns_twice checkpoint");
      }
    }
    if (candidate->kind == CTOOL_C_IR_INSTRUCTION_RETURN_VALUE ||
        candidate->kind == CTOOL_C_IR_INSTRUCTION_RETURN_VOID) {
      continue;
    }
    if (candidate->kind == CTOOL_C_IR_INSTRUCTION_JUMP ||
        candidate->kind == CTOOL_C_IR_INSTRUCTION_BRANCH_ZERO) {
      successors[successor_count++] = candidate->reference;
      if (candidate->kind == CTOOL_C_IR_INSTRUCTION_JUMP) {
        falls_through = CTOOL_FALSE;
      }
    }
    if (falls_through == CTOOL_TRUE) {
      ...
      successors[successor_count++] = relative + 1u;
    }
    for (successor_index = 0u; successor_index < successor_count;
         successor_index++) {
      ctool_u32 successor = successors[successor_index];
      if (successor >= function->instruction_count) {
        return CTOOL_ERR_INTERNAL;
      }
      if (visited[successor] == 0u) {
        ...
        worklist[worklist_count++] = successor;
      }
    }
  }
  for (index = 0u; index < function->instruction_count; index++) {
    ...
    live_count = stack_depths[index] - instruction->argument_count;
    if (live_count == 0u) {
      continue;
    }
    if (cemit_multiply_overflows(live_count, 4u) == CTOOL_TRUE) {
      return CTOOL_ERR_OVERFLOW;
    }
    spill_bytes = live_count * 4u;
    if (*frame_size > 0x7fffffffu - spill_bytes) {
      return CTOOL_ERR_OVERFLOW;
    }
    *frame_size += spill_bytes;
    context->returns_twice_spill_offsets[absolute] = *frame_size;
  }
  return CTOOL_OK;
}
...
static ctool_status_t cemit_emit_dglibc_setjmp_body(
    cemit_context_t *context, ctool_bool legacy) {
  static const ctool_u8 saved_registers[] = {3u, 6u, 7u, 5u};
  ctool_u32 index;
  ...
  if (status == CTOOL_OK && legacy == CTOOL_TRUE) {
    status = cemit_x86_store_register_at_register_offset(
        context, 0u, 16u, 4u);
  } else if (status == CTOOL_OK) {
    status = cemit_x86_lea_stack(context, 1u, 4u);
    if (status == CTOOL_OK) {
      status = cemit_x86_store_register_at_register_offset(
          context, 0u, 16u, 1u);
    }
...
static ctool_status_t cemit_place_dglibc_jumps(
  ...
  status = cemit_emit_dglibc_setjmp_body(
      context,
      cemit_dglibc_legacy_template_equal(assembly->template_text));
...
static ctool_status_t cemit_place_function(cemit_context_t *context,
  ...
  if (status == CTOOL_OK) {
    status = cemit_prepare_returns_twice_spills(
        context, function, stack_depths, &frame_size);
  }
...
ctool_status_t ctool_c_emit_object(
  ...
  if (status == CTOOL_OK) {
    status = cemit_alloc_array(
        &context, context.ir.instruction_count,
        (ctool_u32)sizeof(ctool_u32),
        (void **)&context.returns_twice_spill_offsets);
  }
  if (status == CTOOL_OK) {
