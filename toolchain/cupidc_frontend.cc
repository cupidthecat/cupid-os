...
typedef struct {
  ...
  union {
    ...
    struct {
      ...
      ctool_u32 first_parameter_enumerator;
      ctool_u32 parameter_enumerator_count;
    } function;
  } detail;
...
typedef struct {
  ...
  ctool_bool integer_constant_expression;
...
typedef struct {
  ctool_c_object_definition_t definition;
...
typedef struct {
  ...
  cfront_vector_t parameter_enumerators;
  cfront_vector_t type_walk_seen;
  cfront_vector_t object_definitions;
  ...
  ctool_u32 static_initializer_depth;
...
static void cfront_close_scratch(cfront_context_t *context) {
  ...
  cfront_vector_close(&context->object_definitions);
  cfront_vector_close(&context->type_walk_seen);
  cfront_vector_close(&context->parameter_enumerators);
...
#define CFRONT_OPEN_VECTOR(FIELD, TYPE)                                      \
  ...
  CFRONT_OPEN_VECTOR(parameter_enumerators, ctool_c_binding_t)
  CFRONT_OPEN_VECTOR(type_walk_seen, ctool_u8)
  CFRONT_OPEN_VECTOR(object_definitions, cfront_object_definition_t)
...
static ctool_status_t cfront_storage_failure_at(
    cfront_context_t *context, ctool_status_t status,
    const ctool_c_pp_token_t *token) {
  return cfront_emit_failure(
      context, status,
...
static ctool_status_t cfront_storage_failure(cfront_context_t *context,
                                              ctool_status_t status) {
  return cfront_storage_failure_at(context, status, cfront_peek(context));
}
...
typedef struct {
  ctool_c_type_node_t left;
  ctool_c_type_node_t right;
  ctool_u32 left_qualifiers;
  ctool_u32 right_qualifiers;
  ctool_bool compatible;
  ctool_bool atomic_match;
  ctool_bool left_object;
...
static ctool_status_t cfront_classify_pointer_pair(
  ...
  ctool_u32 left_base = CTOOL_C_TYPE_NONE;
  ctool_u32 right_base = CTOOL_C_TYPE_NONE;
  ctool_status_t status;
  cfront_zero(pair_out, (ctool_u32)sizeof(*pair_out));
  status = cfront_underlying_type(
      context, left_referent_type, &left_base, &pair_out->left_qualifiers,
      &pair_out->left);
  if (status == CTOOL_OK) {
    status = cfront_underlying_type(
  ...
  pair_out->atomic_match =
      ((pair_out->left_qualifiers ^ pair_out->right_qualifiers) &
       CTOOL_C_QUAL_ATOMIC) == 0u
          ? CTOOL_TRUE
          : CTOOL_FALSE;
  pair_out->left_object = pair_out->left.kind != CTOOL_C_TYPE_FUNCTION
                              ? CTOOL_TRUE
                              : CTOOL_FALSE;
  pair_out->right_object = pair_out->right.kind != CTOOL_C_TYPE_FUNCTION
                               ? CTOOL_TRUE
                               : CTOOL_FALSE;
  return CTOOL_OK;
}

static ctool_bool cfront_pointer_assignment_qualifiers_allowed(
    const cfront_pointer_pair_t *pair) {
  return (pair->right_qualifiers & ~pair->left_qualifiers) == 0u
             ? CTOOL_TRUE
             : CTOOL_FALSE;
}
...
static ctool_bool cfront_find_visible_function_type(
    const cfront_context_t *context, ctool_string_t name,
    ctool_u32 *type_out) {
  ctool_c_block_binding_t block_binding;
  ctool_c_binding_t file_binding;
  ctool_u32 parameter;
  ctool_u32 parameter_type;
  if (cfront_find_active_block_binding(
          context, name, &block_binding, (ctool_u32 *)0) == CTOOL_TRUE) {
    if (block_binding.kind == CTOOL_C_BINDING_FUNCTION &&
        block_binding.linkage_binding != CTOOL_C_AST_NONE) {
      *type_out = block_binding.type;
      return CTOOL_TRUE;
    }
    return CTOOL_FALSE;
  ...
  if (cfront_find_file_binding_index(
          context, name, &file_binding, (ctool_u32 *)0) == CTOOL_TRUE &&
      file_binding.kind == CTOOL_C_BINDING_FUNCTION) {
    *type_out = file_binding.type;
    return CTOOL_TRUE;
  }
  return CTOOL_FALSE;
}
...
static ctool_status_t cfront_append_binding(
  ...
  if (context->prototype_scope_depth != 0u) {
    ...
    if (cfront_find_active_block_binding(
      ...
      if (((visible_block.kind == CTOOL_C_BINDING_OBJECT &&
            visible_block.storage == CTOOL_C_STORAGE_EXTERN) ||
           (visible_block.kind == CTOOL_C_BINDING_FUNCTION &&
            (visible_block.storage == CTOOL_C_STORAGE_NONE ||
             visible_block.storage == CTOOL_C_STORAGE_EXTERN))) &&
      ...
      if (status == CTOOL_OK && compatible == CTOOL_TRUE) {
        ...
        if (status == CTOOL_OK &&
          ...
          if (kind == CTOOL_C_BINDING_FUNCTION &&
            ...
            } else if (context->in_function_body == CTOOL_FALSE &&
                       existing.file_scope_visible == CTOOL_FALSE) {
...
static ctool_status_t cfront_prepare_pointer_additive(
    cfront_context_t *context, const ctool_c_pp_token_t *operator_token,
    ctool_c_expression_operator_t operation,
    cfront_expression_value_t *left, cfront_expression_value_t *right,
    ctool_bool *handled_out, ctool_u32 *result_type_out);
static ctool_status_t cfront_apply_update(
    cfront_context_t *context, const ctool_c_pp_token_t *operator_token,
    ctool_c_expression_operator_t operation,
    cfront_expression_value_t *operand);
...
static ctool_status_t cfront_parse_constant_primary(
  ...
  if (cfront_token_is(token, "(") == CTOOL_TRUE) {
    ...
    (void)cfront_advance(context);
    status = cfront_parse_constant_conditional(context, value_out);
    if (status == CTOOL_OK) {
      status = cfront_expected(context, ")");
    }
    cfront_leave_syntax(context);
  ...
  if (token->kind == CTOOL_C_PP_TOKEN_NUMBER) {
    if (context->static_initializer_depth != 0u &&
        cfront_body_floating_constant(token->spelling) == CTOOL_TRUE) {
      return cfront_emit_failure(
    ...
    while (cfront_peek_is(context, ")") == CTOOL_FALSE) {
      ...
      if (cfront_token_is(name, "packed") == CTOOL_TRUE ||
        ...
        if (cfront_peek_is(context, "(") == CTOOL_TRUE) {
          ...
          argument = cfront_peek(context);
          if (cfront_peek_is(context, ")") == CTOOL_FALSE) {
            status = cfront_parse_constant_conditional(context, &alignment);
            if (status != CTOOL_OK) {
              return status;
            }
          }
...
static ctool_status_t cfront_append_function_declarator(
    ...
    ctool_u32 tag_mark, ctool_u32 binding_mark,
    const ctool_c_pp_token_t *open_token, ctool_u32 *root_out) {
  ctool_u32 tag_index;
  ctool_u32 binding_index;
  ctool_status_t status = CTOOL_OK;
  if (context->tags.count < tag_mark ||
      context->bindings.count < binding_mark) {
    return cfront_emit_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
        open_token, "function parameter scope is unavailable");
  }
  ...
  function->detail.function.first_parameter_enumerator =
      context->parameter_enumerators.count;
  function->detail.function.parameter_enumerator_count =
      context->bindings.count - binding_mark;
  ...
  for (binding_index = binding_mark;
    ...
    ctool_c_binding_t enumerator;
    status = cfront_vector_get(&context->bindings, binding_index,
                               &enumerator);
    if (status != CTOOL_OK ||
        enumerator.kind != CTOOL_C_BINDING_ENUMERATOR) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
          open_token, "function parameter enumerator is unavailable");
    }
    status = cfront_vector_append(&context->parameter_enumerators,
                                  &enumerator, (ctool_u32 *)0);
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
  }
...
static ctool_status_t cfront_parse_function_suffix_body(
  ...
  if (cfront_peek_is(context, ")") == CTOOL_TRUE) {
    ...
    return cfront_append_function_declarator(
        context, &function, tag_mark, binding_mark, open_token, root_out);
  }
...
static ctool_status_t cfront_parse_function_suffix(
  ...
  status = cfront_parse_function_suffix_body(
      context, child, open_token, prototype_mark, tag_mark, binding_mark,
      root_out);
...
static ctool_status_t cfront_parse_member_declaration(
  ...
  for (;;) {
    ...
    if (cfront_peek_is(context, ":") == CTOOL_TRUE) {
      ...
      (void)cfront_advance(context);
      status = cfront_parse_constant_conditional(context, &width);
      if (status != CTOOL_OK) {
        return status;
      }
...
static ctool_status_t cfront_attach_expression_block_bindings(
    cfront_context_t *context, ctool_u32 expression_index,
    ctool_u32 first_binding, ctool_u32 binding_count,
    ctool_u32 child_offset, const ctool_c_pp_token_t *token) {
  ctool_c_expression_t expression;
  ctool_u32 binding_index;
  ctool_status_t status;
  if (binding_count == 0u ||
  ...
  if (expression_index >= context->expressions.count ||
      first_binding > context->block_bindings.count ||
    ...
    return cfront_emit_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL, token,
        "expression block-binding slice is invalid");
  }
  for (binding_index = first_binding;
       binding_index < first_binding + binding_count; binding_index++) {
    ctool_c_block_binding_t binding;
  ...
  expression.first_block_binding = first_binding;
  expression.block_binding_count = binding_count;
...
static ctool_bool cfront_ordinary_narrow_string_token(
  ...
  return token != (const ctool_c_pp_token_t *)0 &&
         token->kind == CTOOL_C_PP_TOKEN_STRING &&
         token->spelling.size >= 2u && token->spelling.data[0] == '"' &&
...
static ctool_status_t cfront_decode_narrow_string_token(
  ...
  ctool_u32 index;
  if (cfront_ordinary_narrow_string_token(token) == CTOOL_FALSE) {
    return cfront_emit_failure(
...
static ctool_status_t cfront_parse_body_string(
  ...
  ctool_bytes_t owned = {0};
  ctool_c_type_node_t array;
  ctool_c_expression_t expression;
  ctool_u32 character_type;
  ctool_u32 array_type;
  ctool_status_t status = cfront_decode_body_string(context, &owned);
  if (status != CTOOL_OK) {
    return status;
  }
...
static ctool_status_t cfront_enter_block_scope(cfront_context_t *context) {
  ...
  if (context->in_function_body == CTOOL_TRUE &&
    ...
    if (context->active_function_binding_mark != CFRONT_NONE) {
      mark.binding_mark = context->active_function_binding_mark;
    }
    if (context->active_function_tag_mark != CFRONT_NONE) {
...
static void cfront_initializer_init(
    ctool_c_initializer_t *initializer, ctool_c_initializer_kind_t kind,
    ctool_u32 type, const ctool_c_pp_token_t *token) {
  cfront_zero(initializer, (ctool_u32)sizeof(*initializer));
  ...
  if (token != (const ctool_c_pp_token_t *)0) {
    initializer->location = token->location;
    initializer->physical_location = token->physical_location;
  }
}

static ctool_status_t cfront_append_initializer(
    cfront_context_t *context, const ctool_c_initializer_t *initializer,
    ctool_u32 *initializer_out) {
  ctool_status_t status = cfront_vector_append(
      &context->initializers, initializer, initializer_out);
  return status == CTOOL_OK ? CTOOL_OK
                            : cfront_storage_failure(context, status);
}

static ctool_status_t cfront_attach_initializer_block_bindings(
    cfront_context_t *context, ctool_u32 initializer_index,
    ctool_u32 first_binding, ctool_u32 binding_count,
    const ctool_c_pp_token_t *token) {
  ...
  if (initializer_index >= context->initializers.count ||
      first_binding > context->block_bindings.count ||
      binding_count > context->block_bindings.count - first_binding ||
      cfront_vector_get(&context->initializers, initializer_index,
                        &initializer) != CTOOL_OK ||
      initializer.first_block_binding != CTOOL_C_AST_NONE ||
      initializer.block_binding_count != 0u) {
    return cfront_emit_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL, token,
        "initializer block-binding slice is invalid");
  }
  for (binding_index = first_binding;
       binding_index < first_binding + binding_count; binding_index++) {
    ctool_c_block_binding_t binding;
    ...
    if (status != CTOOL_OK ||
      ...
      return cfront_emit_failure(
          context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL, token,
          "initializer enumerator binding is invalid");
    }
    binding.activation_initializer = initializer_index;
    status = cfront_vector_replace(
        context, &context->block_bindings, binding_index, &binding);
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
  ...
  initializer.first_block_binding = first_binding;
  initializer.block_binding_count = binding_count;
  status = cfront_vector_replace(
      context, &context->initializers, initializer_index, &initializer);
  return status == CTOOL_OK ? CTOOL_OK
                            : cfront_storage_failure(context, status);
}

static ctool_status_t cfront_append_pending_initializer_element(
    cfront_context_t *context,
    const ctool_c_initializer_element_t *element) {
  ctool_status_t status = cfront_vector_append(
      &context->pending_initializer_elements, element, (ctool_u32 *)0);
  return status == CTOOL_OK ? CTOOL_OK
                            : cfront_storage_failure(context, status);
}

static ctool_status_t cfront_publish_initializer_elements(
    cfront_context_t *context, ctool_u32 pending_mark,
    ctool_u32 *first_out, ctool_u32 *count_out) {
  ctool_u32 index;
  ctool_status_t status = CTOOL_OK;
  if (pending_mark > context->pending_initializer_elements.count) {
    return cfront_emit_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
        cfront_peek(context), "initializer element scratch mark is invalid");
  }
  *first_out = context->initializer_elements.count;
  *count_out = context->pending_initializer_elements.count - pending_mark;
  for (index = pending_mark;
       status == CTOOL_OK &&
       index < context->pending_initializer_elements.count;
       index++) {
    ctool_c_initializer_element_t element;
    status = cfront_vector_get(&context->pending_initializer_elements,
                               index, &element);
    if (status == CTOOL_OK) {
      status = cfront_vector_append(&context->initializer_elements,
                                    &element, (ctool_u32 *)0);
    }
  }
  if (status != CTOOL_OK) {
  ...
  status = cfront_vector_rewind(&context->pending_initializer_elements,
                                pending_mark);
  return status == CTOOL_OK ? CTOOL_OK
                            : cfront_storage_failure(context, status);
}
...
static ctool_status_t cfront_publish_parameter_enumerator(
    cfront_context_t *context, const ctool_c_binding_t *enumerator,
    const ctool_c_pp_token_t *definition_token) {
  ctool_c_block_binding_t binding;
  ctool_u32 binding_index;
  if (enumerator->kind != CTOOL_C_BINDING_ENUMERATOR ||
      enumerator->storage != CTOOL_C_STORAGE_NONE ||
      enumerator->linkage != CTOOL_C_LINKAGE_NONE ||
      enumerator->file_scope_visible != CTOOL_FALSE ||
      enumerator->attributes != 0u ||
      enumerator->function_declaration_flags != 0u ||
      enumerator->minimum_alignment != 0u || enumerator->name.size == 0u ||
      (enumerator->integer_unsigned != CTOOL_FALSE &&
       enumerator->integer_unsigned != CTOOL_TRUE)) {
    return cfront_emit_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
        definition_token, "function parameter enumerator is invalid");
  }
  cfront_block_binding_init(
      &binding, enumerator->name, enumerator->type, CFRONT_STORAGE_NONE,
      &enumerator->location, &enumerator->physical_location);
  binding.kind = CTOOL_C_BINDING_ENUMERATOR;
  binding.integer_bits = enumerator->integer_bits;
  binding.integer_unsigned = enumerator->integer_unsigned;
  return cfront_append_block_binding(
      context, &binding, definition_token, CTOOL_TRUE, &binding_index);
}

static ctool_status_t cfront_prepare_pending_block_binding(
    cfront_context_t *context,
    const ctool_c_block_binding_t *binding) {
  ctool_u32 binding_index;
  ctool_status_t status = cfront_vector_append(
      &context->block_bindings, binding, &binding_index);
  if (status != CTOOL_OK) {
    return cfront_storage_failure(context, status);
  }
  context->pending_block_binding = *binding;
  context->pending_block_binding_index = binding_index;
  context->has_pending_block_binding = CTOOL_TRUE;
  return CTOOL_OK;
}

static ctool_status_t cfront_publish_pending_block_binding(
    cfront_context_t *context,
    const ctool_c_block_binding_t *binding,
    ctool_u32 *binding_out) {
  ctool_u32 binding_index = context->pending_block_binding_index;
  ctool_status_t status;
  if (context->has_pending_block_binding == CTOOL_FALSE ||
      binding_index >= context->block_bindings.count ||
      cfront_string_equal(context->pending_block_binding.name,
                          binding->name) == CTOOL_FALSE) {
    return cfront_emit_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
        cfront_peek(context), "pending block binding is invalid");
  }
  status = cfront_vector_replace(
      context, &context->block_bindings, binding_index, binding);
  if (status == CTOOL_OK) {
    status = cfront_vector_append(&context->active_block_binding_indices,
                                  &binding_index, (ctool_u32 *)0);
  }
  if (status != CTOOL_OK) {
    return cfront_storage_failure(context, status);
  }
  *binding_out = binding_index;
  return CTOOL_OK;
}
...
static ctool_u32 cfront_u64_width(ctool_u64 value) {
  ctool_u32 width = 0u;
  while (value != 0ull) {
    value >>= 1u;
    width++;
...
static ctool_bool cfront_u64_multiply_ten(ctool_u64 *value) {
...
static ctool_bool cfront_u128_shifted_u64(ctool_u64 value,
                                         ctool_u32 shift,
                                         cfront_u128_t *result_out) {
  ctool_u32 width = cfront_u64_width(value);
  if (shift >= 128u ||
      (width != 0u && width > 128u - shift)) {
    return CTOOL_FALSE;
  }
  if (shift == 0u) {
    result_out->high = 0ull;
    result_out->low = value;
  } else if (shift < 64u) {
    result_out->high = value >> (64u - shift);
    result_out->low = value << shift;
  } else {
    result_out->high = value << (shift - 64u);
    result_out->low = 0ull;
  }
  return CTOOL_TRUE;
}

static ctool_bool cfront_u128_divide_u64(cfront_u128_t numerator,
                                         ctool_u64 denominator,
                                         ctool_u64 *quotient_out,
                                         ctool_u64 *remainder_out) {
  ctool_u64 quotient = 0ull;
  ctool_u64 remainder = 0ull;
  ctool_bool quotient_started = CTOOL_FALSE;
  ctool_u32 cursor = 128u;
  if (denominator == 0ull) {
    return CTOOL_FALSE;
  }
  while (cursor != 0u) {
    ctool_u32 bit_index = cursor - 1u;
    ctool_u32 bit =
        bit_index >= 64u
            ? (ctool_u32)((numerator.high >> (bit_index - 64u)) & 1ull)
            : (ctool_u32)((numerator.low >> bit_index) & 1ull);
    ctool_u64 gap = denominator - remainder;
    ctool_bool subtract =
        remainder >= gap ||
                (bit != 0u && remainder + 1ull == gap)
            ? CTOOL_TRUE
            : CTOOL_FALSE;
    ctool_u32 quotient_bit = 0u;
    if (subtract == CTOOL_TRUE) {
      remainder =
          remainder >= gap ? remainder - gap + (ctool_u64)bit
                           : remainder + 1ull - gap;
      quotient_bit = 1u;
    } else {
      remainder = remainder * 2ull + (ctool_u64)bit;
    }
    if (quotient_started == CTOOL_TRUE || quotient_bit != 0u) {
      if (quotient > 0x7fffffffffffffffull) {
        return CTOOL_FALSE;
      }
      quotient = (quotient << 1u) | (ctool_u64)quotient_bit;
      quotient_started = CTOOL_TRUE;
    }
    cursor--;
  }
...
static ctool_status_t cfront_decimal_floating_bits(
  ...
  ctool_u64 numerator = 0ull;
  ctool_u64 denominator = 1ull;
  ...
  while (index < end && spelling.data[index] != 'e' &&
    ...
    if (cfront_u64_multiply_ten(&numerator) == CTOOL_FALSE ||
        numerator >
            0xffffffffffffffffull -
                (ctool_u64)(character - '0')) {
      return cfront_emit_failure(
          context, CTOOL_ERR_UNSUPPORTED,
          CTOOL_C_PARSE_DIAG_EXPRESSION, token,
          "decimal floating constant exceeds the supported precision");
    }
    numerator += (ctool_u64)(character - '0');
  }
  if (index < end) {
    index++;
    if (index < end &&
    ...
    while (index < end) {
      ...
      if (exponent_magnitude > 409u ||
          exponent_magnitude * 10u + digit > 4096u) {
        return cfront_emit_failure(
            context, CTOOL_ERR_UNSUPPORTED,
            CTOOL_C_PARSE_DIAG_EXPRESSION, token,
            "decimal floating constant exceeds the supported scale");
  ...
  while (decimal_exponent < 0) {
    if (cfront_u64_multiply_ten(&denominator) == CTOOL_FALSE) {
      return cfront_emit_failure(
          context, CTOOL_ERR_UNSUPPORTED,
          CTOOL_C_PARSE_DIAG_EXPRESSION, token,
          "decimal floating constant exceeds the supported normal range");
  ...
  binary_exponent =
      (ctool_i32)cfront_u64_width(numerator) -
      (ctool_i32)cfront_u64_width(denominator);
  if (binary_exponent >= 0) {
    ctool_u32 shift = (ctool_u32)binary_exponent;
    ctool_u64 scaled_denominator =
        shift < 64u &&
                cfront_u64_width(denominator) <= 64u - shift
            ? denominator << shift
            : 0ull;
    if (scaled_denominator == 0ull ||
        numerator < scaled_denominator) {
      binary_exponent--;
    }
  } else {
    ctool_u32 shift = (ctool_u32)(0 - binary_exponent);
    ctool_u64 scaled_numerator =
        shift < 64u &&
                cfront_u64_width(numerator) <= 64u - shift
            ? numerator << shift
            : 0xffffffffffffffffull;
  ...
  if (significand_shift >= 0) {
    cfront_u128_t scaled;
    if (cfront_u128_shifted_u64(
            numerator, (ctool_u32)significand_shift,
            &scaled) == CTOOL_FALSE ||
        cfront_u128_divide_u64(
            scaled, denominator, &quotient,
            &remainder) == CTOOL_FALSE) {
      return cfront_emit_failure(
          context, CTOOL_ERR_UNSUPPORTED,
          CTOOL_C_PARSE_DIAG_EXPRESSION, token,
          "decimal floating constant exceeds the supported precision");
    ...
    ctool_u32 denominator_shift =
        (ctool_u32)(0 - significand_shift);
    ctool_u64 scaled_denominator;
    if (denominator_shift >= 64u ||
        cfront_u64_width(denominator) >
            64u - denominator_shift) {
      return cfront_emit_failure(
          context, CTOOL_ERR_UNSUPPORTED,
          CTOOL_C_PARSE_DIAG_EXPRESSION, token,
          "decimal floating constant exceeds the supported precision");
    }
    scaled_denominator = denominator << denominator_shift;
    quotient = numerator / scaled_denominator;
...
static void cfront_constant_integer_set(cfront_expression_value_t *value,
                                        cfront_integer_t constant) {
  cfront_constant_value_clear(value);
  value->integer_constant_expression = CTOOL_TRUE;
  value->integer_constant_value_valid = CTOOL_TRUE;
  value->integer_constant_value = constant;
}

static ctool_status_t cfront_constant_integer_convert(
    cfront_context_t *context, ctool_u32 target_type,
    cfront_expression_value_t *value) {
  ...
  status = cfront_integer_type(context, target_type, &integer, &is_integer);
  if (status != CTOOL_OK) {
    return status;
  }
  ...
  if (value->integer_constant_value_valid == CTOOL_TRUE) {
    value->integer_constant_value = cfront_integer_convert_to_type(
        value->integer_constant_value, &integer);
  }
  value->void_cast_null_pointer_constant = CTOOL_FALSE;
  return CTOOL_OK;
}

static ctool_bool cfront_integer_value_less_than(cfront_integer_t left,
...
static void cfront_constant_apply_unary(
    ctool_c_expression_operator_t operation,
    const cfront_expression_value_t *operand,
    cfront_expression_value_t *result) {
  cfront_integer_t value = operand->integer_constant_value;
  cfront_constant_value_clear(result);
  if (operand->integer_constant_expression == CTOOL_FALSE) {
    return;
  }
  result->integer_constant_expression = CTOOL_TRUE;
  if (operand->integer_constant_value_valid == CTOOL_FALSE) {
    return;
  }
  switch (operation) {
  case CTOOL_C_EXPRESSION_OPERATOR_UNARY_PLUS:
    break;
  case CTOOL_C_EXPRESSION_OPERATOR_UNARY_NEGATE:
    if (cfront_integer_unsigned(value.kind) == CTOOL_FALSE &&
        cfront_integer_magnitude(&value) ==
            (cfront_integer_width(value.kind) == 32u
                 ? 0x80000000ull
                 : 0x8000000000000000ull)) {
      return;
    }
    value.bits = cfront_integer_normalize_bits(0ull - value.bits, value.kind);
    break;
  case CTOOL_C_EXPRESSION_OPERATOR_BITWISE_NOT:
    value.bits = cfront_integer_normalize_bits(~value.bits, value.kind);
    break;
  case CTOOL_C_EXPRESSION_OPERATOR_LOGICAL_NOT:
    value.bits = value.bits == 0ull ? 1ull : 0ull;
    value.kind = CFRONT_INTEGER_SIGNED_32;
    break;
  default:
    result->integer_constant_expression = CTOOL_FALSE;
    return;
  }
  result->integer_constant_value_valid = CTOOL_TRUE;
  result->integer_constant_value = value;
}

static void cfront_constant_apply_binary(
    ctool_c_expression_operator_t operation,
    const cfront_expression_value_t *left,
    const cfront_expression_value_t *right,
    cfront_expression_value_t *result) {
  cfront_integer_t left_value = left->integer_constant_value;
  cfront_integer_t right_value = right->integer_constant_value;
  cfront_integer_t value = {0ull, CFRONT_INTEGER_SIGNED_32};
  ctool_bool valid = CTOOL_TRUE;
  cfront_constant_value_clear(result);
  if (left->integer_constant_expression == CTOOL_FALSE ||
      right->integer_constant_expression == CTOOL_FALSE) {
    return;
  }
  result->integer_constant_expression = CTOOL_TRUE;
  if (operation == CTOOL_C_EXPRESSION_OPERATOR_LOGICAL_AND ||
      operation == CTOOL_C_EXPRESSION_OPERATOR_LOGICAL_OR) {
    if (left->integer_constant_value_valid == CTOOL_FALSE) {
      return;
    }
    if ((operation == CTOOL_C_EXPRESSION_OPERATOR_LOGICAL_AND &&
         left_value.bits == 0ull) ||
        (operation == CTOOL_C_EXPRESSION_OPERATOR_LOGICAL_OR &&
         left_value.bits != 0ull)) {
      value.bits =
          operation == CTOOL_C_EXPRESSION_OPERATOR_LOGICAL_OR ? 1ull : 0ull;
    } else if (right->integer_constant_value_valid == CTOOL_TRUE) {
      value.bits = right_value.bits != 0ull ? 1ull : 0ull;
    } else {
      return;
    }
    result->integer_constant_value_valid = CTOOL_TRUE;
  ...
  if (left->integer_constant_value_valid == CTOOL_FALSE ||
      right->integer_constant_value_valid == CTOOL_FALSE) {
    return;
  }
  value.kind = left_value.kind;
  switch (operation) {
  case CTOOL_C_EXPRESSION_OPERATOR_MULTIPLY:
    if (cfront_integer_unsigned(value.kind) == CTOOL_TRUE) {
      value.bits = cfront_integer_normalize_bits(
          left_value.bits * right_value.bits, value.kind);
    } else {
      ctool_bool negative =
          cfront_integer_negative(&left_value) !=
                  cfront_integer_negative(&right_value)
              ? CTOOL_TRUE
              : CTOOL_FALSE;
      ctool_u64 left_magnitude = cfront_integer_magnitude(&left_value);
      ctool_u64 right_magnitude = cfront_integer_magnitude(&right_value);
      ctool_u64 limit =
          negative == CTOOL_TRUE
              ? (cfront_integer_width(value.kind) == 32u
                     ? 0x80000000ull
      ...
      if (left_magnitude != 0ull &&
        ...
        value.bits = left_magnitude * right_magnitude;
        if (negative == CTOOL_TRUE) {
          value.bits = 0ull - value.bits;
        }
        value.bits = cfront_integer_normalize_bits(value.bits, value.kind);
      }
    }
  ...
  case CTOOL_C_EXPRESSION_OPERATOR_REMAINDER: {
    ctool_u64 divisor = cfront_integer_magnitude(&right_value);
    if (divisor == 0ull) {
      valid = CTOOL_FALSE;
    } else if (cfront_integer_unsigned(value.kind) == CTOOL_TRUE) {
      value.bits =
          operation == CTOOL_C_EXPRESSION_OPERATOR_DIVIDE
              ? left_value.bits / right_value.bits
              : left_value.bits % right_value.bits;
    } else {
      ctool_u64 dividend = cfront_integer_magnitude(&left_value);
      if (cfront_integer_negative(&left_value) == CTOOL_TRUE &&
          cfront_integer_negative(&right_value) == CTOOL_TRUE &&
          dividend == (cfront_integer_width(value.kind) == 32u
                           ? 0x80000000ull
                           : 0x8000000000000000ull) &&
          divisor == 1ull) {
        valid = CTOOL_FALSE;
      } else if (operation == CTOOL_C_EXPRESSION_OPERATOR_DIVIDE) {
        ctool_bool negative =
            cfront_integer_negative(&left_value) !=
                    cfront_integer_negative(&right_value)
                ? CTOOL_TRUE
                : CTOOL_FALSE;
        ctool_u64 quotient = dividend / divisor;
        value.bits = negative == CTOOL_TRUE ? 0ull - quotient : quotient;
      } else {
        ctool_u64 remainder = dividend % divisor;
        value.bits = cfront_integer_negative(&left_value) == CTOOL_TRUE
                         ? 0ull - remainder
                         : remainder;
      }
      value.bits = cfront_integer_normalize_bits(value.bits, value.kind);
  ...
  case CTOOL_C_EXPRESSION_OPERATOR_SUBTRACT: {
    ...
    ctool_u64 bits = subtract == CTOOL_TRUE
                         ? left_value.bits - right_value.bits
                         : left_value.bits + right_value.bits;
    bits = cfront_integer_normalize_bits(bits, value.kind);
    if (cfront_integer_unsigned(value.kind) == CTOOL_FALSE) {
      cfront_integer_t sum = {bits, value.kind};
      ctool_bool left_negative = cfront_integer_negative(&left_value);
      ctool_bool right_negative = cfront_integer_negative(&right_value);
      ctool_bool result_negative = cfront_integer_negative(&sum);
      if ((subtract == CTOOL_FALSE && left_negative == right_negative &&
           result_negative != left_negative) ||
          (subtract == CTOOL_TRUE && left_negative != right_negative &&
           result_negative != left_negative)) {
        valid = CTOOL_FALSE;
      }
    }
  ...
  case CTOOL_C_EXPRESSION_OPERATOR_SHIFT_LEFT:
  case CTOOL_C_EXPRESSION_OPERATOR_SHIFT_RIGHT:
    if (cfront_integer_negative(&right_value) == CTOOL_TRUE ||
        right_value.bits >=
            (ctool_u64)cfront_integer_width(left_value.kind)) {
      valid = CTOOL_FALSE;
    } else if (operation == CTOOL_C_EXPRESSION_OPERATOR_SHIFT_LEFT) {
      ctool_u32 count = (ctool_u32)right_value.bits;
      if (cfront_integer_unsigned(left_value.kind) == CTOOL_FALSE &&
          (cfront_integer_negative(&left_value) == CTOOL_TRUE ||
           left_value.bits >
               ((cfront_integer_width(left_value.kind) == 32u
                     ? 0x7fffffffull
                     : 0x7fffffffffffffffull) >>
                count))) {
        valid = CTOOL_FALSE;
        ...
        value.bits = cfront_integer_normalize_bits(
            left_value.bits << count, left_value.kind);
      }
    } else if (cfront_integer_negative(&left_value) == CTOOL_TRUE) {
      ctool_u32 count = (ctool_u32)right_value.bits;
      value.bits = left_value.bits;
      if (count != 0u) {
        ctool_u64 mask = cfront_integer_mask(left_value.kind);
        ctool_u64 low = left_value.bits & mask;
        value.bits = cfront_integer_normalize_bits(
            (low >> count) | (mask ^ (mask >> count)), left_value.kind);
      }
    } else {
      value.bits = left_value.bits >> (ctool_u32)right_value.bits;
    }
    break;
  case CTOOL_C_EXPRESSION_OPERATOR_LESS:
    value.bits = cfront_integer_value_less_than(left_value, right_value) ==
                         CTOOL_TRUE
                     ? 1ull
                     : 0ull;
    value.kind = CFRONT_INTEGER_SIGNED_32;
  ...
  case CTOOL_C_EXPRESSION_OPERATOR_LESS_EQUAL:
    value.bits = cfront_integer_value_less_than(right_value, left_value) ==
                         CTOOL_TRUE
                     ? 0ull
                     : 1ull;
    value.kind = CFRONT_INTEGER_SIGNED_32;
  ...
  case CTOOL_C_EXPRESSION_OPERATOR_NOT_EQUAL: {
    ctool_bool equal = left_value.bits == right_value.bits ? CTOOL_TRUE
                                                           : CTOOL_FALSE;
    if (operation == CTOOL_C_EXPRESSION_OPERATOR_NOT_EQUAL) {
      equal = equal == CTOOL_TRUE ? CTOOL_FALSE : CTOOL_TRUE;
    }
    value.bits = equal == CTOOL_TRUE ? 1ull : 0ull;
    value.kind = CFRONT_INTEGER_SIGNED_32;
    break;
  }
  case CTOOL_C_EXPRESSION_OPERATOR_BITWISE_AND:
    value.bits = cfront_integer_normalize_bits(
        left_value.bits & right_value.bits, value.kind);
    break;
  case CTOOL_C_EXPRESSION_OPERATOR_BITWISE_XOR:
    value.bits = cfront_integer_normalize_bits(
        left_value.bits ^ right_value.bits, value.kind);
    break;
  case CTOOL_C_EXPRESSION_OPERATOR_BITWISE_OR:
    value.bits = cfront_integer_normalize_bits(
        left_value.bits | right_value.bits, value.kind);
    break;
  default:
...
static ctool_bool cfront_is_null_pointer_constant(
    const cfront_expression_value_t *value) {
  return (value->integer_constant_expression == CTOOL_TRUE &&
          value->integer_constant_value_valid == CTOOL_TRUE &&
          value->integer_constant_value.bits == 0ull) ||
                 value->void_cast_null_pointer_constant == CTOOL_TRUE
             ? CTOOL_TRUE
             : CTOOL_FALSE;
}
...
static ctool_status_t cfront_parse_body_integer_constant(
  ...
  if (cfront_body_floating_constant(token->spelling) == CTOOL_TRUE) {
    ...
    cfront_expression_init(
        &expression, CTOOL_C_EXPRESSION_FLOATING_CONSTANT,
  ...
  if (status == CTOOL_OK) {
    ...
    value_out->bit_width = 0u;
    value_out->address_forbidden = CTOOL_FALSE;
    cfront_constant_integer_set(value_out, value);
  }
  return status;
}
...
static ctool_status_t cfront_append_folded_u32_constant(
  ...
  if (status == CTOOL_OK) {
    ...
    value_out->address_forbidden = CTOOL_FALSE;
    {
      cfront_integer_t value = {(ctool_u64)bits,
                                CFRONT_INTEGER_UNSIGNED_32};
      cfront_constant_integer_set(value_out, value);
    }
  }
...
static ctool_status_t cfront_parse_body_character_constant(
  ...
  if (status == CTOOL_OK) {
    ...
    {
      cfront_integer_t constant = {
          cfront_integer_normalize_bits((ctool_u64)value,
                                        CFRONT_INTEGER_SIGNED_32),
          CFRONT_INTEGER_SIGNED_32};
      cfront_constant_integer_set(value_out, constant);
    }
  }
  return status;
}
...
static ctool_status_t cfront_parse_body_primary(
  ...
  if (cfront_token_is(token, "__builtin_offsetof") == CTOOL_TRUE) {
    ...
    if (status == CTOOL_OK) {
      status = cfront_append_folded_u32_constant(
          context, token, offset, value_out);
    }
    if (status == CTOOL_OK) {
      status = cfront_attach_expression_block_bindings(
          context, value_out->expression, first_binding,
          context->block_bindings.count - first_binding, 0u, token);
    }
    return status;
  }
  ...
  if (cfront_find_active_block_binding(context, token->spelling,
    ...
    expression.reference = block_binding.linkage_binding != CTOOL_C_AST_NONE
                               ? block_binding.linkage_binding
                               : reference;
    type = block_binding.type;
    value_out->is_lvalue =
        block_binding.kind == CTOOL_C_BINDING_OBJECT ? CTOOL_TRUE
                                                     : CTOOL_FALSE;
    value_out->is_bit_field = CTOOL_FALSE;
    value_out->bit_width = 0u;
    ...
    if (binding.kind == CTOOL_C_BINDING_ENUMERATOR) {
      cfront_integer_type_t integer;
      ctool_bool is_integer = CTOOL_FALSE;
      status = cfront_integer_type(context, binding.type, &integer,
                                   &is_integer);
      if (status != CTOOL_OK || is_integer == CTOOL_FALSE) {
        return cfront_emit_failure(
            context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL, token,
            "enumerator expression type is unavailable");
      }
      enumerator_value.bits = binding.integer_bits;
      enumerator_value =
          cfront_integer_convert_to_type(enumerator_value, &integer);
      enumerator_constant = CTOOL_TRUE;
    }
...
static ctool_status_t cfront_append_conversion_with_reference(
  ...
  if (status == CTOOL_OK) {
    ...
    if (conversion == CTOOL_C_CONVERSION_INTEGER_PROMOTION ||
      ...
      status = cfront_constant_integer_convert(context, target_type, value);
      if (status != CTOOL_OK) {
        return cfront_storage_failure(context, status);
      }
    } else {
...
static ctool_status_t cfront_apply_default_conversion(
  ...
  if (value->is_lvalue == CTOOL_TRUE) {
    status = cfront_unqualified_type_preserving_alignments(
        context, value->type, &target_type);
    if (status != CTOOL_OK) {
      if (status == CTOOL_ERR_LIMIT || status == CTOOL_ERR_OVERFLOW ||
          status == CTOOL_ERR_NO_MEMORY) {
        return cfront_storage_failure(context, status);
...
static ctool_status_t cfront_validate_variadic_argument_type(
    cfront_context_t *context, const ctool_c_pp_token_t *builtin_token,
    ctool_u32 type) {
  cfront_integer_type_t integer = {0};
  ctool_c_type_node_t node;
  ctool_bool is_integer = CTOOL_FALSE;
  ctool_u32 base;
  ctool_u32 qualifiers;
  ...
  if (node.kind == CTOOL_C_TYPE_ENUM) {
    status = cfront_integer_type(context, type, &integer, &is_integer);
    if (status != CTOOL_OK || is_integer == CTOOL_FALSE) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
...
static ctool_status_t cfront_parse_variadic_builtin(
  ...
  ctool_u32 first_binding = CTOOL_C_AST_NONE;
...
static ctool_status_t cfront_integer_promotion_type(
    ...
    ctool_u32 *target_out, ctool_bool *is_integer_out) {
  cfront_integer_type_t integer;
  ctool_status_t status = cfront_integer_type(
      context, source_type, &integer, is_integer_out);
  if (status != CTOOL_OK || *is_integer_out == CTOOL_FALSE) {
    return status;
  }
  if (is_bit_field == CTOOL_TRUE &&
      integer.kind == CTOOL_C_TYPE_UNSIGNED_INT &&
      bit_width < integer.width) {
    return cfront_scalar_type(context, CTOOL_C_TYPE_SIGNED_INT, token,
                              target_out);
  }
  if (integer.rank < 4u) {
    return cfront_scalar_type(context, CTOOL_C_TYPE_SIGNED_INT, token,
...
static ctool_status_t cfront_apply_integer_promotion(
  ...
  status = cfront_integer_promotion_type(
      context, token, value->type, was_bit_field, bit_width, &target,
      &is_integer);
  if (status != CTOOL_OK) {
    return cfront_storage_failure(context, status);
  }
...
static ctool_status_t cfront_prepare_floating_binary(
  ...
  *handled_out = CTOOL_TRUE;
  ...
  if (left_floating != right_floating) {
    status = cfront_integer_type(
        context, left->type, &left_integer, &left_is_integer);
    if (status == CTOOL_OK) {
      status = cfront_integer_type(
          context, right->type, &right_integer,
          &right_is_integer);
    }
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
...
static ctool_status_t cfront_apply_assignment_conversion(
  ...
  if (target_is_floating == CTOOL_TRUE ||
    ...
    status = cfront_integer_type(
        context, target_type, &target_integer,
        &target_is_integer);
    if (status == CTOOL_OK) {
      status = cfront_integer_type(
          context, value->type, &source_integer,
          &source_is_integer);
    }
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    ...
    if (((target_is_floating == CTOOL_TRUE &&
        ...
        ((target_qualifiers | target_node.qualifiers |
         ...
         CTOOL_C_QUAL_ATOMIC) == 0u) {
      return cfront_append_conversion(
          context, CTOOL_C_CONVERSION_ASSIGNMENT,
          target_type, value);
    }
    return cfront_emit_failure(
        context, CTOOL_ERR_UNSUPPORTED, CTOOL_C_PARSE_DIAG_EXPRESSION,
        source_token,
        target_is_integer == CTOOL_TRUE &&
                target_integer.is_unsigned == CTOOL_TRUE &&
                target_integer.width == 32u
  ...
  }
  {
    ...
    ctool_u32 target_base;
    ctool_u32 target_qualifiers;
    status = cfront_underlying_type(context, target_type, &target_base,
                                    &target_qualifiers, &target_node);
    (void)target_base;
    (void)target_qualifiers;
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
    if (target_node.kind == CTOOL_C_TYPE_POINTER &&
        cfront_is_null_pointer_constant(value) == CTOOL_TRUE) {
      return cfront_append_conversion(
          context, CTOOL_C_CONVERSION_NULL_POINTER, target_type, value);
    }
  }
  status = cfront_integer_type(context, target_type, &target_integer,
                               &target_is_integer);
  ...
  }
  {
    ...
    if (parameter_node.kind == CTOOL_C_TYPE_POINTER &&
      ...
      cfront_pointer_pair_t pair;
      ctool_c_conversion_kind_t pointer_conversion =
          CTOOL_C_CONVERSION_NONE;
      status = cfront_classify_pointer_pair(
          context, parameter_node.referenced_type,
          argument_node.referenced_type, &pair);
      if (status == CTOOL_OK && pair.compatible == CTOOL_TRUE &&
          pair.atomic_match == CTOOL_TRUE) {
        pointer_conversion = CTOOL_C_CONVERSION_QUALIFICATION;
      } else if (status == CTOOL_OK &&
                 pair.atomic_match == CTOOL_TRUE &&
                 pair.left.kind == CTOOL_C_TYPE_VOID) {
        if (pair.right_object == CTOOL_TRUE) {
          pointer_conversion = CTOOL_C_CONVERSION_POINTER;
        }
      } else if (status == CTOOL_OK &&
                 pair.atomic_match == CTOOL_TRUE &&
                 pair.right.kind == CTOOL_C_TYPE_VOID) {
        if (pair.left_object == CTOOL_TRUE) {
          pointer_conversion = CTOOL_C_CONVERSION_POINTER;
        }
      }
      if (status != CTOOL_OK) {
      ...
      if (pointer_conversion != CTOOL_C_CONVERSION_NONE &&
          cfront_pointer_assignment_qualifiers_allowed(&pair) == CTOOL_TRUE) {
        return cfront_append_conversion(
  ...
  for (;;) {
    ...
    if (status != CTOOL_OK) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
          cfront_peek(context),
          "static address expression metadata is unavailable");
    }
    traversed++;
    ...
    if (expression.kind == CTOOL_C_EXPRESSION_IMPLICIT_CONVERSION) {
      if (expression.conversion == CTOOL_C_CONVERSION_ARRAY_TO_POINTER ||
          expression.conversion == CTOOL_C_CONVERSION_FUNCTION_TO_POINTER) {
      ...
      if (expression.child_count != 1u ||
        ...
        return cfront_emit_failure(
            context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
            cfront_peek(context),
            "static address conversion child is unavailable");
      }
    ...
    if (expression.kind == CTOOL_C_EXPRESSION_UNARY &&
      ...
      if (expression.child_count != 1u ||
        ...
        return cfront_emit_failure(
            context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
            cfront_peek(context),
            "static address operand is unavailable");
      }
      status = cfront_vector_get(&context->expression_children,
                                 expression.first_child, &child);
      if (status != CTOOL_OK) {
    ...
    if (expression.kind == CTOOL_C_EXPRESSION_IDENTIFIER &&
      ...
      ctool_c_binding_t binding;
      status = cfront_vector_get(&context->bindings, expression.reference,
                                 &binding);
      if (status != CTOOL_OK) {
        return cfront_emit_failure(
            context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
            cfront_peek(context),
            "static address binding is unavailable");
      }
      if ((binding.kind == CTOOL_C_BINDING_OBJECT ||
           binding.kind == CTOOL_C_BINDING_FUNCTION) &&
...
static ctool_status_t cfront_parse_body_postfix(
  ...
  for (;;) {
    if (cfront_peek_is(context, "++") == CTOOL_TRUE ||
      ...
      ctool_status_t status = cfront_apply_update(
          context, operator_token, operation, value);
      if (status != CTOOL_OK) {
        return status;
      }
      continue;
    ...
    if (cfront_peek_is(context, "[") == CTOOL_TRUE) {
      const ctool_c_pp_token_t *open = cfront_advance(context);
      cfront_expression_value_t index;
      ctool_bool handled = CTOOL_FALSE;
      ctool_u32 result_type = CTOOL_C_TYPE_NONE;
      ctool_status_t status = cfront_enter_syntax(context, open);
      cfront_zero(&index, (ctool_u32)sizeof(index));
      if (status == CTOOL_OK) {
        status = cfront_parse_body_expression(context, &index);
      }
      if (status == CTOOL_OK) {
        status = cfront_expected(context, "]");
      }
      if (status == CTOOL_OK) {
        status = cfront_prepare_pointer_additive(
            context, open, CTOOL_C_EXPRESSION_OPERATOR_ADD, value, &index,
            &handled, &result_type);
      }
      if (status == CTOOL_OK && handled == CTOOL_FALSE) {
        status = cfront_emit_failure(
      ...
      if (status == CTOOL_OK) {
        status = cfront_append_binary_expression(
            context, open, CTOOL_C_EXPRESSION_BINARY,
            CTOOL_C_EXPRESSION_OPERATOR_ADD, result_type,
            CTOOL_C_TYPE_NONE, value, &index);
      }
      if (status == CTOOL_OK) {
        status = cfront_apply_dereference_operator(context, open, value);
      }
      cfront_leave_syntax(context);
      if (status != CTOOL_OK) {
        return status;
...
static ctool_status_t cfront_append_unary_expression(
    ...
    ctool_c_expression_operator_t operation, ctool_u32 type,
    cfront_expression_value_t *operand) {
  cfront_expression_value_t constant = *operand;
  ctool_status_t status = cfront_append_one_child_expression(
  ...
  if (status == CTOOL_OK) {
    cfront_constant_apply_unary(operation, &constant, operand);
  }
  return status;
}
...
static ctool_status_t cfront_apply_cast(
  ...
  cfront_integer_type_t target_integer_info;
  cfront_integer_type_t source_integer_info;
  ...
  (void)source_qualifiers;
  if (status == CTOOL_OK) {
    status = cfront_integer_type(context, target_type,
                                 &target_integer_info,
                                 &target_integer);
  }
  if (status == CTOOL_OK) {
    status = cfront_integer_type(context, operand->type,
                                 &source_integer_info,
                                 &source_integer);
  }
  if (status != CTOOL_OK) {
  ...
  if (target.kind == CTOOL_C_TYPE_POINTER) {
    ...
    ctool_u32 referent_qualifiers;
    status = cfront_underlying_type(
        context, target.referenced_type, &referent_base,
    ...
    exact_void_pointer =
        referent.kind == CTOOL_C_TYPE_VOID && referent_qualifiers == 0u &&
  ...
  if (status == CTOOL_OK && target_integer == CTOOL_TRUE &&
    ...
    operand->integer_constant_expression = CTOOL_TRUE;
    operand->integer_constant_value_valid =
        constant_operand.integer_constant_value_valid;
    operand->integer_constant_value = constant_operand.integer_constant_value;
    status = cfront_constant_integer_convert(context, target_type, operand);
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
  } else if (status == CTOOL_OK && exact_void_pointer == CTOOL_TRUE &&
             constant_operand.integer_constant_expression == CTOOL_TRUE &&
...
static ctool_status_t cfront_append_binary_expression(
  ...
  if (status == CTOOL_OK) {
    ...
    if (kind == CTOOL_C_EXPRESSION_BINARY) {
      cfront_constant_apply_binary(operation, &left_constant,
                                   &right_constant, left);
    } else {
...
static ctool_status_t cfront_append_conditional_expression(
    cfront_context_t *context, const ctool_c_pp_token_t *operator_token,
    ctool_u32 type, const cfront_expression_value_t *condition,
    const cfront_expression_value_t *when_nonzero,
    const cfront_expression_value_t *when_zero,
    cfront_expression_value_t *value_out) {
  ctool_c_expression_t expression;
  ctool_u32 first_child = context->expression_children.count;
  ctool_status_t status = cfront_vector_append(
      &context->expression_children, &condition->expression, (ctool_u32 *)0);
  if (status == CTOOL_OK) {
    status = cfront_vector_append(&context->expression_children,
                                  &when_nonzero->expression,
                                  (ctool_u32 *)0);
  }
  if (status == CTOOL_OK) {
    status = cfront_vector_append(&context->expression_children,
                                  &when_zero->expression, (ctool_u32 *)0);
  }
  if (status != CTOOL_OK) {
    return cfront_storage_failure(context, status);
  }
  cfront_expression_init(&expression, CTOOL_C_EXPRESSION_CONDITIONAL,
                         &operator_token->location,
                         &operator_token->physical_location);
  expression.type = type;
  expression.first_child = first_child;
  ...
  if (status == CTOOL_OK) {
    ...
    value_out->address_forbidden = CTOOL_FALSE;
    cfront_constant_value_clear(value_out);
    if (condition->integer_constant_expression == CTOOL_TRUE &&
        when_nonzero->integer_constant_expression == CTOOL_TRUE &&
        when_zero->integer_constant_expression == CTOOL_TRUE) {
      const cfront_expression_value_t *selected = when_zero;
      value_out->integer_constant_expression = CTOOL_TRUE;
      if (condition->integer_constant_value_valid == CTOOL_TRUE) {
        if (condition->integer_constant_value.bits != 0ull) {
          selected = when_nonzero;
        }
        if (selected->integer_constant_value_valid == CTOOL_TRUE) {
          value_out->integer_constant_value_valid = CTOOL_TRUE;
          value_out->integer_constant_value =
              selected->integer_constant_value;
        }
      }
    }
  }
...
static ctool_status_t cfront_append_conditional_conversion_if_needed(
    cfront_context_t *context, ctool_c_conversion_kind_t conversion,
    ctool_u32 target_type, cfront_expression_value_t *value) {
  ctool_bool same = CTOOL_FALSE;
  ctool_status_t status = cfront_types_same(
...
static ctool_status_t cfront_prepare_conditional_pointer_result(
    cfront_context_t *context, const ctool_c_pp_token_t *operator_token,
    cfront_expression_value_t *when_nonzero,
    cfront_expression_value_t *when_zero, ctool_u32 *type_out) {
  ctool_c_type_node_t nonzero_pointer;
  ctool_c_type_node_t zero_pointer;
  cfront_pointer_pair_t pair;
  ctool_u32 nonzero_base;
  ctool_u32 zero_base;
  ctool_u32 nonzero_qualifiers;
  ctool_u32 zero_qualifiers;
  ctool_u32 referent_base = CTOOL_C_TYPE_NONE;
  ctool_u32 referent = CTOOL_C_TYPE_NONE;
  ctool_u32 qualifiers;
  ctool_bool object_void_pair = CTOOL_FALSE;
  ctool_status_t status = cfront_underlying_type(
      context, when_nonzero->type, &nonzero_base, &nonzero_qualifiers,
      &nonzero_pointer);
  if (status == CTOOL_OK) {
    status = cfront_underlying_type(
        context, when_zero->type, &zero_base, &zero_qualifiers,
        &zero_pointer);
  }
  (void)nonzero_base;
  (void)zero_base;
  ...
  if (status != CTOOL_OK || nonzero_pointer.kind != CTOOL_C_TYPE_POINTER ||
      zero_pointer.kind != CTOOL_C_TYPE_POINTER) {
    return cfront_emit_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
        operator_token, "conditional pointer operand type is unavailable");
  }
  if (when_nonzero->void_cast_null_pointer_constant !=
      when_zero->void_cast_null_pointer_constant) {
    ctool_bool same_pointer_types = CTOOL_FALSE;
    cfront_expression_value_t *null_value =
        when_nonzero->void_cast_null_pointer_constant == CTOOL_TRUE
            ? when_nonzero
            : when_zero;
    cfront_expression_value_t *pointer_value =
        null_value == when_nonzero ? when_zero : when_nonzero;
    *type_out = pointer_value->type;
    status = cfront_types_same(context, null_value->type,
                               pointer_value->type, &same_pointer_types);
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
    if (same_pointer_types == CTOOL_TRUE) {
      return CTOOL_OK;
    }
    return cfront_append_conversion(
        context, CTOOL_C_CONVERSION_NULL_POINTER, *type_out, null_value);
  }
  status = cfront_classify_pointer_pair(
      context, nonzero_pointer.referenced_type, zero_pointer.referenced_type,
      &pair);
  if (status != CTOOL_OK) {
    return cfront_storage_failure(context, status);
  }
  qualifiers = pair.left_qualifiers | pair.right_qualifiers;
  if (pair.compatible == CTOOL_TRUE) {
    ctool_u32 left_referent_base;
    ctool_u32 right_referent_base;
    status = cfront_unqualified_type_preserving_alignments(
        context, nonzero_pointer.referenced_type, &left_referent_base);
    if (status == CTOOL_OK) {
      status = cfront_unqualified_type_preserving_alignments(
          context, zero_pointer.referenced_type, &right_referent_base);
    }
    if (status == CTOOL_OK) {
      status = cfront_composite_type(context, left_referent_base,
                                     right_referent_base, &referent_base);
    }
  } else if (pair.left.kind == CTOOL_C_TYPE_VOID &&
             pair.right_object == CTOOL_TRUE) {
    referent_base = nonzero_pointer.referenced_type;
    object_void_pair = CTOOL_TRUE;
  } else if (pair.right.kind == CTOOL_C_TYPE_VOID &&
             pair.left_object == CTOOL_TRUE) {
    referent_base = zero_pointer.referenced_type;
    object_void_pair = CTOOL_TRUE;
  } else {
    return cfront_emit_failure(
        context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_EXPRESSION,
        operator_token,
        "conditional pointer operands require compatible referents or an object and void pointer pair");
  }
  if (status != CTOOL_OK) {
    return cfront_storage_failure(context, status);
  }
  if (object_void_pair == CTOOL_TRUE &&
      (qualifiers & CTOOL_C_QUAL_ATOMIC) != 0u) {
    return cfront_emit_failure(
        context, CTOOL_ERR_UNSUPPORTED, CTOOL_C_PARSE_DIAG_EXPRESSION,
        operator_token,
        "atomic-qualified void conditional result is outside the type layout slice");
  }
  if (object_void_pair == CTOOL_TRUE) {
    ctool_u32 base;
    ctool_u32 ignored_qualifiers;
    ctool_c_type_node_t node;
    status = cfront_underlying_type(context, referent_base, &base,
                                    &ignored_qualifiers, &node);
    if (status == CTOOL_OK) {
      referent_base = base;
    }
  }
  if (status == CTOOL_OK) {
    status = cfront_qualified_type(context, referent_base, qualifiers,
                                   operator_token, &referent);
  }
  if (status == CTOOL_OK) {
    status = cfront_pointer_type_at(context, referent, operator_token,
                                    type_out);
  }
  if (status != CTOOL_OK) {
    return cfront_storage_failure(context, status);
  }
  status = cfront_append_conditional_conversion_if_needed(
      context,
      object_void_pair == CTOOL_TRUE &&
              pair.left.kind != CTOOL_C_TYPE_VOID
          ? CTOOL_C_CONVERSION_POINTER
          : CTOOL_C_CONVERSION_QUALIFICATION,
      *type_out, when_nonzero);
  if (status == CTOOL_OK) {
    status = cfront_append_conditional_conversion_if_needed(
        context,
        object_void_pair == CTOOL_TRUE &&
...
static ctool_status_t cfront_prepare_conditional_result(
    cfront_context_t *context, const ctool_c_pp_token_t *operator_token,
    cfront_expression_value_t *when_nonzero,
    cfront_expression_value_t *when_zero, ctool_u32 *type_out) {
  cfront_scalar_value_kind_t nonzero_kind = CFRONT_SCALAR_VALUE_OTHER;
  cfront_scalar_value_kind_t zero_kind = CFRONT_SCALAR_VALUE_OTHER;
  ctool_c_type_node_t nonzero_node;
  ctool_c_type_node_t zero_node;
  ctool_u32 nonzero_base;
  ctool_u32 zero_base;
  ctool_u32 nonzero_qualifiers;
  ctool_u32 zero_qualifiers;
  ...
  if (nonzero_kind == CFRONT_SCALAR_VALUE_POINTER &&
    ...
    return cfront_prepare_conditional_pointer_result(
        context, operator_token, when_nonzero, when_zero, type_out);
  }
  if ((nonzero_kind == CFRONT_SCALAR_VALUE_POINTER &&
       zero_kind == CFRONT_SCALAR_VALUE_INTEGER) ||
      (zero_kind == CFRONT_SCALAR_VALUE_POINTER &&
       nonzero_kind == CFRONT_SCALAR_VALUE_INTEGER)) {
    cfront_expression_value_t *integer_value =
        nonzero_kind == CFRONT_SCALAR_VALUE_INTEGER ? when_nonzero
                                                    : when_zero;
    cfront_expression_value_t *pointer_value =
        integer_value == when_nonzero ? when_zero : when_nonzero;
    if (cfront_is_null_pointer_constant(integer_value) == CTOOL_TRUE) {
      *type_out = pointer_value->type;
      return cfront_append_conversion(
          context, CTOOL_C_CONVERSION_NULL_POINTER, *type_out,
          integer_value);
    }
    return cfront_emit_failure(
        context, CTOOL_ERR_INPUT,
  ...
  return cfront_emit_failure(
      context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_EXPRESSION,
      operator_token,
      "conditional operands do not satisfy a common C11 result category");
}
...
static ctool_status_t cfront_prepare_pointer_additive(
  ...
  ctool_bool left_is_integer = CTOOL_FALSE;
  ctool_bool right_is_integer = CTOOL_FALSE;
  ctool_bool left_is_pointer = CTOOL_FALSE;
  ctool_bool right_is_pointer = CTOOL_FALSE;
  ctool_bool left_was_bit_field = left->is_bit_field;
  ctool_bool right_was_bit_field = right->is_bit_field;
  ctool_u32 left_bit_width = left->bit_width;
  ctool_u32 right_bit_width = right->bit_width;
  ctool_bool complete = CTOOL_FALSE;
  ctool_u32 left_base = CTOOL_C_TYPE_NONE;
  ctool_u32 right_base = CTOOL_C_TYPE_NONE;
  ctool_u32 left_qualifiers = 0u;
  ctool_u32 right_qualifiers = 0u;
  ...
  *handled_out = CTOOL_FALSE;
  ...
  (void)left_integer;
  (void)right_integer;
  if (status == CTOOL_OK && left_is_integer == CTOOL_FALSE) {
    status = cfront_underlying_type(context, left->type, &left_base,
                                    &left_qualifiers, &left_node);
    if (status == CTOOL_OK && left_node.kind == CTOOL_C_TYPE_POINTER) {
      left_is_pointer = CTOOL_TRUE;
    }
  }
  if (status == CTOOL_OK && right_is_integer == CTOOL_FALSE) {
    status = cfront_underlying_type(context, right->type, &right_base,
                                    &right_qualifiers, &right_node);
    if (status == CTOOL_OK && right_node.kind == CTOOL_C_TYPE_POINTER) {
      right_is_pointer = CTOOL_TRUE;
    }
  }
  (void)left_base;
  (void)right_base;
  ...
  if (left_is_pointer == CTOOL_FALSE && right_is_pointer == CTOOL_FALSE) {
    left->is_bit_field = left_was_bit_field;
    left->bit_width = left_bit_width;
    right->is_bit_field = right_was_bit_field;
    right->bit_width = right_bit_width;
    return CTOOL_OK;
  }
  *handled_out = CTOOL_TRUE;
  if (operation == CTOOL_C_EXPRESSION_OPERATOR_SUBTRACT &&
    ...
    if (status == CTOOL_OK && complete == CTOOL_TRUE) {
      status = cfront_type_is_complete_object_now(
          context, right_node.referenced_type, &complete);
    }
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
    if (complete == CTOOL_FALSE) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_EXPRESSION,
          operator_token,
          "pointer arithmetic requires pointers to complete object types");
    }
    status = cfront_classify_pointer_pair(
        context, left_node.referenced_type, right_node.referenced_type,
        &pair);
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
    if (pair.compatible == CTOOL_FALSE) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_EXPRESSION,
          operator_token,
          "pointer subtraction requires compatible pointed-to types");
    }
    return cfront_scalar_type(context, CTOOL_C_TYPE_SIGNED_INT,
                              operator_token, result_type_out);
  }
  if (!((left_is_pointer == CTOOL_TRUE && right_is_integer == CTOOL_TRUE) ||
        (operation == CTOOL_C_EXPRESSION_OPERATOR_ADD &&
         right_is_pointer == CTOOL_TRUE && left_is_integer == CTOOL_TRUE))) {
    return cfront_emit_failure(
        context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_EXPRESSION,
        operator_token,
        operation == CTOOL_C_EXPRESSION_OPERATOR_ADD
  ...
  if (integer_value == left) {
    integer_value->is_bit_field = left_was_bit_field;
    integer_value->bit_width = left_bit_width;
  } else {
    integer_value->is_bit_field = right_was_bit_field;
    integer_value->bit_width = right_bit_width;
  }
  status = cfront_type_is_complete_object_now(context, referent_type,
                                               &complete);
  if (status != CTOOL_OK) {
    return cfront_storage_failure(context, status);
  }
  ...
  status = cfront_apply_integer_promotion(context, operator_token,
                                          integer_value);
  if (status == CTOOL_OK) {
...
static ctool_status_t cfront_prepare_pointer_comparison(
    cfront_context_t *context, const ctool_c_pp_token_t *operator_token,
    ctool_c_expression_operator_t operation,
    cfront_expression_value_t *left, cfront_expression_value_t *right,
    ctool_bool *handled_out) {
  cfront_integer_type_t left_integer;
  cfront_integer_type_t right_integer;
  ctool_c_type_node_t left_node;
  ctool_c_type_node_t right_node;
  ...
  *handled_out = CTOOL_TRUE;
  if (left_is_pointer == CTOOL_FALSE || right_is_pointer == CTOOL_FALSE) {
    if ((left_is_pointer == CTOOL_TRUE && right_is_integer == CTOOL_TRUE) ||
        (right_is_pointer == CTOOL_TRUE && left_is_integer == CTOOL_TRUE)) {
      cfront_expression_value_t *integer_value =
          left_is_pointer == CTOOL_TRUE ? right : left;
      cfront_expression_value_t *pointer_value =
          left_is_pointer == CTOOL_TRUE ? left : right;
      if (equality == CTOOL_FALSE) {
        return cfront_emit_failure(
            context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_EXPRESSION,
            operator_token,
      ...
      if (cfront_is_null_pointer_constant(integer_value) == CTOOL_TRUE) {
        return cfront_append_conversion(
            context, CTOOL_C_CONVERSION_NULL_POINTER,
            pointer_value->type, integer_value);
      }
      return cfront_emit_failure(
          context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_EXPRESSION,
          operator_token,
    ...
    return cfront_emit_failure(
        context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_EXPRESSION,
        operator_token, "pointer comparison requires two pointer operands");
  }
  if (equality == CTOOL_TRUE &&
      left->void_cast_null_pointer_constant !=
          right->void_cast_null_pointer_constant) {
    ctool_bool same_pointer_types = CTOOL_FALSE;
    cfront_expression_value_t *null_value =
        left->void_cast_null_pointer_constant == CTOOL_TRUE ? left : right;
    cfront_expression_value_t *pointer_value =
        null_value == left ? right : left;
    status = cfront_types_same(context, null_value->type,
                               pointer_value->type, &same_pointer_types);
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    ...
    return cfront_append_conversion(
        context, CTOOL_C_CONVERSION_NULL_POINTER, pointer_value->type,
        null_value);
  }
  status = cfront_classify_pointer_pair(
      context, left_node.referenced_type, right_node.referenced_type, &pair);
  if (status != CTOOL_OK) {
    return cfront_storage_failure(context, status);
  }
  if (equality == CTOOL_TRUE) {
    ...
    if (pair.left.kind == CTOOL_C_TYPE_VOID &&
        (pair.left_qualifiers & CTOOL_C_QUAL_ATOMIC) == 0u &&
        pair.right_object == CTOOL_TRUE) {
      return cfront_append_conversion(
          context, CTOOL_C_CONVERSION_POINTER, left->type, right);
    }
    if (pair.right.kind == CTOOL_C_TYPE_VOID &&
        (pair.right_qualifiers & CTOOL_C_QUAL_ATOMIC) == 0u &&
        pair.left_object == CTOOL_TRUE) {
      return cfront_append_conversion(
          context, CTOOL_C_CONVERSION_POINTER, right->type, left);
    }
    return cfront_emit_failure(
        context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_EXPRESSION,
        operator_token,
  ...
  if (pair.compatible == CTOOL_FALSE || pair.left_object == CTOOL_FALSE ||
      pair.right_object == CTOOL_FALSE) {
    return cfront_emit_failure(
        context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_EXPRESSION,
        operator_token,
...
static ctool_status_t cfront_parse_body_unary(
  ...
  if (cfront_token_is(token, "++") == CTOOL_TRUE ||
      cfront_token_is(token, "--") == CTOOL_TRUE) {
    ctool_c_expression_operator_t update =
        cfront_token_is(token, "++") == CTOOL_TRUE
    ...
    if (status == CTOOL_OK) {
      status = cfront_apply_update(context, token, update, value_out);
    }
    cfront_leave_syntax(context);
    return status;
  }
  ...
  if (cfront_token_is(token, "(") == CTOOL_TRUE &&
    ...
    ctool_u32 first_binding = context->block_bindings.count;
    ctool_u32 binding_end = first_binding;
    ctool_u32 target_type = CTOOL_C_TYPE_NONE;
    status = cfront_enter_syntax(context, token);
    if (status != CTOOL_OK) {
      return status;
    ...
    status = cfront_parse_type_name(context, &target_type);
    if (status == CTOOL_OK) {
      binding_end = context->block_bindings.count;
      status = cfront_expected(context, ")");
    }
    if (status == CTOOL_OK &&
        cfront_peek_is(context, "{") == CTOOL_TRUE) {
      ...
      if (status == CTOOL_OK) {
        status = cfront_attach_expression_block_bindings(
            context, value_out->expression, first_binding,
            binding_end - first_binding, 0u, token);
      }
      cfront_leave_syntax(context);
      return status;
    }
...
static ctool_status_t cfront_apply_update(
  ...
  ctool_bool complete = CTOOL_FALSE;
  ctool_u32 result_type = CTOOL_C_TYPE_NONE;
  ctool_u32 computation_type = CTOOL_C_TYPE_NONE;
  ctool_u32 base;
  ctool_u32 qualifiers;
  ctool_u32 first_child;
  ctool_c_expression_t expression;
  ...
  if (status == CTOOL_OK) {
    status = cfront_integer_promotion_type(
        ...
        operand->bit_width, &computation_type, &is_integer);
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
  }
  if (status == CTOOL_OK && is_integer == CTOOL_FALSE) {
    status = cfront_underlying_type(context, result_type, &base,
                                    &qualifiers, &node);
    (void)base;
    (void)qualifiers;
  ...
  first_child = context->expression_children.count;
  status = cfront_vector_append(&context->expression_children,
                                &operand->expression, (ctool_u32 *)0);
  if (status != CTOOL_OK) {
    return cfront_storage_failure(context, status);
  }
  cfront_expression_init(&expression, CTOOL_C_EXPRESSION_UPDATE,
                         &operator_token->location,
                         &operator_token->physical_location);
  expression.type = result_type;
  expression.first_child = first_child;
  ...
  if (status == CTOOL_OK) {
    ...
    operand->is_bit_field = CTOOL_FALSE;
    operand->bit_width = 0u;
    operand->address_forbidden = CTOOL_FALSE;
    cfront_constant_value_clear(operand);
  }
  return status;
}
...
static ctool_status_t cfront_prepare_compound_assignment(
    ...
    ctool_c_expression_operator_t operation, ctool_bool shift,
    cfront_expression_value_t *right,
    ctool_u32 *computation_type_out) {
  cfront_integer_type_t left_integer;
  cfront_integer_type_t right_integer;
  ctool_c_type_node_t left_node;
  ctool_c_type_node_t right_node;
  ...
  if (left_is_integer == CTOOL_FALSE) {
    ...
    if (operation != CTOOL_C_EXPRESSION_OPERATOR_ADD_ASSIGN &&
      ...
      return cfront_emit_failure(
          context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_EXPRESSION,
          operator_token,
          "pointer compound assignment supports only += and -=");
    }
    status = cfront_type_is_complete_object_now(
        context, left_node.referenced_type, &complete);
    if (status != CTOOL_OK) {
  ...
  if (status == CTOOL_OK) {
    status = cfront_integer_type(context, right_promoted, &right_integer,
                                 &right_is_integer);
  }
  if (status != CTOOL_OK) {
    return cfront_storage_failure(context, status);
  ...
  if (shift == CTOOL_TRUE) {
    *computation_type_out = left_promoted;
    return CTOOL_OK;
  }
  status = cfront_common_integer_type(
      context, &left_integer, &right_integer, operator_token,
...
typedef struct {
  ...
  ctool_c_expression_operator_t operation;
  ctool_bool shift;
} cfront_compound_operator_t;

static ctool_bool cfront_body_compound_operator(
    const ctool_c_pp_token_t *token,
    cfront_compound_operator_t *operator_out) {
  static const cfront_compound_operator_t operators[] = {
      {"*=", CTOOL_C_EXPRESSION_OPERATOR_MULTIPLY_ASSIGN, CTOOL_FALSE},
      {"/=", CTOOL_C_EXPRESSION_OPERATOR_DIVIDE_ASSIGN, CTOOL_FALSE},
      {"%=", CTOOL_C_EXPRESSION_OPERATOR_REMAINDER_ASSIGN, CTOOL_FALSE},
      {"+=", CTOOL_C_EXPRESSION_OPERATOR_ADD_ASSIGN, CTOOL_FALSE},
      {"-=", CTOOL_C_EXPRESSION_OPERATOR_SUBTRACT_ASSIGN, CTOOL_FALSE},
      {"<<=", CTOOL_C_EXPRESSION_OPERATOR_SHIFT_LEFT_ASSIGN, CTOOL_TRUE},
      {">>=", CTOOL_C_EXPRESSION_OPERATOR_SHIFT_RIGHT_ASSIGN, CTOOL_TRUE},
      {"&=", CTOOL_C_EXPRESSION_OPERATOR_BITWISE_AND_ASSIGN, CTOOL_FALSE},
      {"^=", CTOOL_C_EXPRESSION_OPERATOR_BITWISE_XOR_ASSIGN, CTOOL_FALSE},
      {"|=", CTOOL_C_EXPRESSION_OPERATOR_BITWISE_OR_ASSIGN, CTOOL_FALSE}};
  ctool_u32 index;
  for (index = 0u;
       index < (ctool_u32)(sizeof(operators) / sizeof(operators[0]));
       index++) {
...
static ctool_status_t cfront_parse_body_conditional(
    cfront_context_t *context, cfront_expression_value_t *value_out) {
  ctool_status_t status = cfront_parse_body_logical_or(context, value_out);
  if (status == CTOOL_OK && cfront_peek_is(context, "?") == CTOOL_TRUE) {
    const ctool_c_pp_token_t *operator_token = cfront_advance(context);
    cfront_expression_value_t when_nonzero;
    cfront_expression_value_t when_zero;
    ctool_u32 result_type = CTOOL_C_TYPE_NONE;
    status = cfront_enter_syntax(context, operator_token);
    cfront_zero(&when_nonzero, (ctool_u32)sizeof(when_nonzero));
    cfront_zero(&when_zero, (ctool_u32)sizeof(when_zero));
    if (status == CTOOL_OK) {
      status = cfront_require_controlling_value(context, operator_token,
                                                value_out);
    }
    if (status == CTOOL_OK) {
    ...
    if (status == CTOOL_OK) {
      status = cfront_parse_body_conditional(context, &when_zero);
    }
    if (status == CTOOL_OK) {
      status = cfront_prepare_conditional_result(
          context, operator_token, &when_nonzero, &when_zero, &result_type);
    }
    if (status == CTOOL_OK) {
      cfront_expression_value_t condition = *value_out;
      status = cfront_append_conditional_expression(
          context, operator_token, result_type, &condition, &when_nonzero,
          &when_zero, value_out);
    }
    cfront_leave_syntax(context);
  }
...
static ctool_status_t cfront_parse_body_assignment(
    cfront_context_t *context, cfront_expression_value_t *value_out) {
  ctool_status_t status = cfront_parse_body_conditional(context, value_out);
  cfront_compound_operator_t compound;
  if (status == CTOOL_OK && cfront_peek_is(context, "=") == CTOOL_TRUE) {
    const ctool_c_pp_token_t *operator_token = cfront_advance(context);
    ...
    cfront_leave_syntax(context);
             ...
             cfront_body_compound_operator(cfront_peek(context),
                                           &compound) == CTOOL_TRUE) {
    const ctool_c_pp_token_t *operator_token = cfront_advance(context);
    cfront_expression_value_t right;
    ctool_u32 result_type = CTOOL_C_TYPE_NONE;
    ...
    if (status == CTOOL_OK) {
      status = cfront_prepare_compound_assignment(
          context, operator_token, value_out, result_type,
          compound.operation, compound.shift, &right,
          &computation_type);
    }
    if (status == CTOOL_OK) {
      status = cfront_append_binary_expression(
          context, operator_token, CTOOL_C_EXPRESSION_ASSIGNMENT,
          compound.operation, result_type,
          computation_type, value_out, &right);
    }
    cfront_leave_syntax(context);
  }
  return status;
...
static ctool_bool cfront_x87_atan2_memory_template(
    ctool_string_t template_text) {
  return cfront_string_literal(
             template_text,
             "fldl  %1\n\t"
...
static ctool_bool cfront_x87_sine_memory_template(
    ctool_string_t template_text) {
  return cfront_string_literal(
      template_text, "fldl %1\n\tfsin\n\tfstpl %0\n\t");
}

static ctool_bool cfront_flags_restore_assembly_template(
    ctool_string_t template_text) {
  return cfront_string_literal(
      template_text, "pushl %0\n\tpopfl\n\t");
}

static ctool_bool cfront_x87_round_down_memory_template(
    ctool_string_t template_text) {
  return cfront_string_literal(
      template_text,
      "fldl %1\n\t"
...
static ctool_bool cfront_x87_double_memory_template(
  ...
  return cfront_x87_sine_memory_template(template_text) == CTOOL_TRUE ||
                 ...
                 cfront_x87_pow_memory_template(
                     template_text) == CTOOL_TRUE ||
                 cfront_x87_atan2_memory_template(
                     template_text) == CTOOL_TRUE ||
                 cfront_x87_exp_memory_template(
                     template_text) == CTOOL_TRUE
             ? CTOOL_TRUE
             : CTOOL_FALSE;
}
...
static ctool_u32 cfront_fixed_assembly_output_register(
    ctool_string_t constraint) {
  if (cfront_string_literal(constraint, "=a") == CTOOL_TRUE) {
    return CTOOL_C_ASSEMBLY_FIXED_A;
  }
  if (cfront_string_literal(constraint, "=b") == CTOOL_TRUE) {
    return CTOOL_C_ASSEMBLY_FIXED_B;
  }
  if (cfront_string_literal(constraint, "=c") == CTOOL_TRUE ||
      cfront_string_literal(constraint, "+c") == CTOOL_TRUE) {
    return CTOOL_C_ASSEMBLY_FIXED_C;
  }
  if (cfront_string_literal(constraint, "=d") == CTOOL_TRUE) {
    return CTOOL_C_ASSEMBLY_FIXED_D;
  }
  if (cfront_string_literal(constraint, "+S") == CTOOL_TRUE) {
    return CTOOL_C_ASSEMBLY_FIXED_SI;
  }
  return cfront_string_literal(constraint, "+D") == CTOOL_TRUE
             ? CTOOL_C_ASSEMBLY_FIXED_DI
             : 0u;
}
...
static ctool_status_t cfront_validate_assembly_output(
  ...
  ctool_bool x87_atan2_memory =
      cfront_x87_atan2_memory_template(template_text);
  ctool_bool x87_exp_memory =
      cfront_x87_exp_memory_template(template_text);
  cfront_movss_memory_kind_t movss_kind =
      cfront_movss_memory_template_kind(template_text);
  ctool_bool x87_sine_memory =
      cfront_x87_sine_memory_template(template_text);
  ...
  if (x87_atan2_memory == CTOOL_TRUE &&
      cfront_string_literal(constraint, "=m") == CTOOL_FALSE) {
    return cfront_emit_failure(
        context, CTOOL_ERR_UNSUPPORTED, CTOOL_C_PARSE_DIAG_STATEMENT,
        constraint_token,
        "GNU x87 atan2 assembly output requires the =m constraint");
  }
  if (x87_exp_memory == CTOOL_TRUE &&
      cfront_string_literal(constraint, "=m") == CTOOL_FALSE) {
    return cfront_emit_failure(
        context, CTOOL_ERR_UNSUPPORTED, CTOOL_C_PARSE_DIAG_STATEMENT,
        constraint_token,
  ...
  fixed_register =
      cfront_fixed_assembly_output_register(constraint);
  if (fixed_register != 0u) {
    if (cfront_string_literal(constraint, "=a") == CTOOL_TRUE) {
      any_integer_width = CTOOL_TRUE;
    } else if (cfront_string_literal(constraint, "+S") == CTOOL_TRUE ||
               cfront_string_literal(constraint, "+D") == CTOOL_TRUE) {
      pointer_only = CTOOL_TRUE;
    }
  } else if (sqrtsd_register == CTOOL_TRUE) {
        ...
        sqrtsd_register == CTOOL_TRUE
            ...
            ? x87_sine_memory == CTOOL_TRUE
              ...
              : x87_powf_memory == CTOOL_TRUE
               ...
               : x87_atan2_memory == CTOOL_TRUE
                   ? "GNU x87 atan2 assembly =m output requires a modifiable "
                     "non-atomic double lvalue"
               : x87_exp_memory == CTOOL_TRUE
                   ? "GNU x87 exp assembly =m output requires a modifiable "
                     "non-atomic double lvalue"
  ...
  if (is_pointer == CTOOL_TRUE) {
    ...
    if (cfront_string_literal(constraint, "=r") == CTOOL_FALSE &&
        cfront_string_literal(constraint, "+S") == CTOOL_FALSE &&
        cfront_string_literal(constraint, "+D") == CTOOL_FALSE) {
      return cfront_emit_failure(
          context, CTOOL_ERR_UNSUPPORTED,
          CTOOL_C_PARSE_DIAG_STATEMENT, constraint_token,
  ...
  if (memory_output == CTOOL_TRUE &&
    ...
    if (x87_double_memory == CTOOL_TRUE &&
        node.kind != CTOOL_C_TYPE_DOUBLE) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_STATEMENT,
          constraint_token,
...
static ctool_status_t cfront_parse_assembly_operand_name(
    cfront_context_t *context, ctool_u32 *position_io,
    ctool_string_t *name_out,
    const ctool_c_pp_token_t **label_token_out) {
  const ctool_c_pp_token_t *token =
      cfront_token(context, *position_io);
  const ctool_c_pp_token_t *name;
  name_out->data = (const char *)0;
  name_out->size = 0u;
  if (label_token_out != (const ctool_c_pp_token_t **)0) {
    *label_token_out = token;
  }
  if (token == (const ctool_c_pp_token_t *)0 ||
      cfront_token_is(token, "[") == CTOOL_FALSE) {
    return CTOOL_OK;
  }
  (*position_io)++;
  name = cfront_token(context, *position_io);
  if (label_token_out != (const ctool_c_pp_token_t **)0) {
    *label_token_out = name;
  }
  if (name == (const ctool_c_pp_token_t *)0 ||
      name->kind != CTOOL_C_PP_TOKEN_IDENTIFIER) {
    return cfront_emit_failure(
        context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_STATEMENT, name,
        "GNU inline assembly operand label requires a C identifier");
  }
  *name_out = name->spelling;
  (*position_io)++;
  token = cfront_token(context, *position_io);
  if (token == (const ctool_c_pp_token_t *)0 ||
      cfront_token_is(token, "]") == CTOOL_FALSE) {
    return cfront_emit_failure(
        context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_STATEMENT,
...
static ctool_status_t cfront_append_assembly_operand_name(
    cfront_context_t *context, cfront_vector_t *names,
    ctool_string_t name, const ctool_c_pp_token_t *token) {
  ctool_u32 index;
  ctool_status_t status;
  for (index = 0u; index < names->count; index++) {
    ctool_string_t prior;
    status = cfront_vector_get(names, index, &prior);
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
    if (name.size != 0u && cfront_string_equal(name, prior) == CTOOL_TRUE) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_STATEMENT, token,
          "GNU inline assembly operand label is listed twice");
    }
  }
  status = cfront_vector_append(names, &name, (ctool_u32 *)0);
  return status == CTOOL_OK ? CTOOL_OK : cfront_storage_failure(context, status);
}

static ctool_status_t cfront_collect_assembly_operand_names(
    cfront_context_t *context, ctool_u32 first_position,
    cfront_vector_t *names) {
  ctool_u32 position = first_position;
  ctool_u32 clause = 0u;
  ctool_status_t status = CTOOL_OK;
  while (status == CTOOL_OK && clause < 2u) {
    const ctool_c_pp_token_t *token = cfront_token(context, position);
    ctool_string_t name = {0};
    ctool_u32 parenthesis_depth = 0u;
    if (token == (const ctool_c_pp_token_t *)0 ||
        cfront_token_is(token, ")") == CTOOL_TRUE) {
      break;
    }
    ...
    status = cfront_parse_assembly_operand_name(
        context, &position, &name, &token);
    if (status != CTOOL_OK) {
      return status;
    }
    status = cfront_append_assembly_operand_name(
        context, names, name, token);
    while (status == CTOOL_OK) {
      token = cfront_token(context, position);
      if (token == (const ctool_c_pp_token_t *)0) {
        break;
      }
      if (cfront_token_is(token, "(") == CTOOL_TRUE) {
    ...
    if (status != CTOOL_OK || token == (const ctool_c_pp_token_t *)0 ||
        cfront_token_is(token, ")") == CTOOL_TRUE) {
      break;
    }
    if (cfront_token_is(token, ",") == CTOOL_TRUE) {
...
static ctool_bool cfront_assembly_identifier(ctool_string_t name) {
  ctool_u32 index;
  if (name.data == (const char *)0 || name.size == 0u ||
      cfront_identifier_first_character(name.data[0]) == CTOOL_FALSE) {
    return CTOOL_FALSE;
  }
  for (index = 1u; index < name.size; index++) {
    if (cfront_identifier_character(name.data[index]) == CTOOL_FALSE) {
      return CTOOL_FALSE;
    }
  }
  return CTOOL_TRUE;
...
static ctool_bool cfront_assembly_template_has_named_operand(
    ctool_string_t template_text) {
  ctool_u32 index;
  for (index = 0u; index + 1u < template_text.size; index++) {
    if (template_text.data[index] == '%') {
      if (template_text.data[index + 1u] == '%') {
        index++;
      } else if (template_text.data[index + 1u] == '[') {
        return CTOOL_TRUE;
      }
    }
  }
...
static ctool_status_t cfront_normalize_assembly_operand_names(
    cfront_context_t *context, const ctool_c_pp_token_t *keyword,
    cfront_vector_t *names, ctool_string_t *template_io) {
  const ctool_limits_t *limits = ctool_job_limits(context->job);
  ctool_buffer_t *normalized = (ctool_buffer_t *)0;
  ctool_u32 cursor = 0u;
  ctool_status_t status;
  if (cfront_assembly_template_has_named_operand(*template_io) == CTOOL_FALSE) {
    return CTOOL_OK;
  }
  status = ctool_job_open_buffer(
      context->job, limits->output_bytes < 64u ? limits->output_bytes : 64u,
      limits->output_bytes, &normalized);
  while (status == CTOOL_OK && cursor < template_io->size) {
    if (template_io->data[cursor] == '%' &&
        cursor + 1u < template_io->size &&
        template_io->data[cursor + 1u] == '%') {
      status = ctool_buffer_put_u8(normalized, (ctool_u8)'%');
      if (status == CTOOL_OK) {
        status = ctool_buffer_put_u8(normalized, (ctool_u8)'%');
      }
      cursor += 2u;
    } else if (template_io->data[cursor] == '%' &&
               cursor + 1u < template_io->size &&
               template_io->data[cursor + 1u] == '[') {
      ctool_u32 name_start = cursor + 2u;
      ctool_u32 name_end = name_start;
      ctool_u32 operand = 0u;
      ctool_bool found = CTOOL_FALSE;
      ctool_string_t placeholder;
      while (name_end < template_io->size &&
             template_io->data[name_end] != ']') {
        name_end++;
      }
      if (name_end == template_io->size) {
        status = cfront_emit_failure(
      ...
      placeholder.data = template_io->data + name_start;
      placeholder.size = name_end - name_start;
      if (cfront_assembly_identifier(placeholder) == CTOOL_FALSE) {
        status = cfront_emit_failure(
            context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_STATEMENT, keyword,
            "GNU inline assembly named operand reference requires a C "
            "identifier");
        break;
      }
      for (operand = 0u; operand < names->count; operand++) {
        ctool_string_t candidate;
        status = cfront_vector_get(names, operand, &candidate);
        if (status != CTOOL_OK) {
          status = cfront_storage_failure(context, status);
          break;
        }
        if (cfront_string_equal(candidate, placeholder) == CTOOL_TRUE) {
          found = CTOOL_TRUE;
          break;
        }
      }
      ...
      if (found == CTOOL_FALSE) {
        status = cfront_emit_failure(
            context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_STATEMENT, keyword,
            "GNU inline assembly template names an unknown operand");
        break;
      }
      status = ctool_buffer_put_u8(normalized, (ctool_u8)'%');
      if (status == CTOOL_OK) {
        ...
        ctool_u32 digit_count = 0u;
        do {
          digits[digit_count++] = (char)('0' + operand % 10u);
          operand /= 10u;
        } while (operand != 0u);
        while (status == CTOOL_OK && digit_count != 0u) {
          digit_count--;
          status = ctool_buffer_put_u8(
              normalized, (ctool_u8)digits[digit_count]);
        }
      }
      cursor = name_end + 1u;
    } else {
      status = ctool_buffer_put_u8(
          normalized, (ctool_u8)template_io->data[cursor]);
      cursor++;
    }
  }
  if (status == CTOOL_OK) {
    ctool_bytes_t bytes = ctool_buffer_view(normalized);
    ctool_string_t text;
    text.data = (const char *)bytes.data;
    text.size = bytes.size;
    status = ctool_arena_copy_string(
        ctool_job_arena(context->job), text, template_io);
  }
  if (normalized != (ctool_buffer_t *)0) {
    ctool_buffer_close(normalized);
  }
  return status == CTOOL_OK ? CTOOL_OK
...
static ctool_status_t cfront_parse_assembly_output(
    ...
    ctool_u32 *fixed_registers_io) {
  const ctool_c_pp_token_t *constraint_token;
...
static ctool_bool cfront_named_matching_assembly_constraint(
    ctool_string_t constraint) {
  ctool_string_t name;
  if (constraint.data == (const char *)0 || constraint.size < 3u ||
      constraint.data[0] != '[' ||
      constraint.data[constraint.size - 1u] != ']') {
    return CTOOL_FALSE;
  }
  name.data = constraint.data + 1u;
  name.size = constraint.size - 2u;
  return cfront_assembly_identifier(name);
}
...
static ctool_status_t cfront_find_fixed_assembly_output(
    ...
    ctool_u32 *matching_output_out, ctool_bool *found_out) {
  ...
  *found_out = CTOOL_FALSE;
  for (output_index = 0u; output_index < output_count; output_index++) {
    ...
    ctool_status_t status = cfront_vector_get(
        &context->assembly_operands, first_operand + output_index,
        &output);
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
    if (output.constraint.data != (const char *)0 &&
        output.constraint.size == 2u &&
        output.constraint.data[0] == '=' &&
        cfront_fixed_assembly_output_register(output.constraint) ==
            fixed_register) {
      *matching_output_out = output_index;
...
static ctool_status_t cfront_parse_assembly_input(
  ...
  ctool_c_type_node_t source_input_node;
  ...
  ctool_bool fixed_output_found = CTOOL_FALSE;
  ctool_bool memory_input = CTOOL_FALSE;
  ctool_bool sqrtsd_register =
      cfront_sqrtsd_register_template(template_text);
  ctool_bool sqrtsd_input = CTOOL_FALSE;
  ctool_bool sqrtsd_input_atomic = CTOOL_FALSE;
  ctool_bool flags_restore =
      cfront_flags_restore_assembly_template(template_text);
  ctool_bool x87_atan2_memory =
      cfront_x87_atan2_memory_template(template_text);
  ctool_bool x87_exp_memory =
      cfront_x87_exp_memory_template(template_text);
  ...
  ctool_u32 source_input_qualifiers;
  ...
  if (status == CTOOL_OK &&
      cfront_named_matching_assembly_constraint(
          operand.constraint) == CTOOL_TRUE) {
    status = cfront_emit_failure(
        context, CTOOL_ERR_UNSUPPORTED, CTOOL_C_PARSE_DIAG_STATEMENT,
        constraint_token,
  ...
  if (status == CTOOL_OK &&
      x87_atan2_memory == CTOOL_TRUE &&
      cfront_string_literal(operand.constraint, "m") == CTOOL_FALSE) {
    status = cfront_emit_failure(
        context, CTOOL_ERR_UNSUPPORTED, CTOOL_C_PARSE_DIAG_STATEMENT,
        constraint_token,
  ...
  if (status == CTOOL_OK &&
    ...
    if (fixed_register != 0u &&
        (*fixed_registers_io & fixed_register) != 0u) {
      status = cfront_find_fixed_assembly_output(
          context, first_operand, output_count, fixed_register,
          &operand.matching_output, &fixed_output_found);
      if (status == CTOOL_OK && fixed_output_found == CTOOL_TRUE) {
        independent = CTOOL_FALSE;
        matching = CTOOL_TRUE;
      } else if (status == CTOOL_OK) {
        status = cfront_emit_failure(
            context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_STATEMENT,
    ...
    matching = status == CTOOL_OK ? CTOOL_TRUE : CTOOL_FALSE;
  }
  ...
  if (status == CTOOL_OK && flags_restore == CTOOL_TRUE) {
    status = cfront_underlying_type(
        context, value.type, &source_input_base,
        &source_input_qualifiers, &source_input_node);
  }
  if (status == CTOOL_OK && flags_restore == CTOOL_TRUE &&
      ((source_input_qualifiers | source_input_node.qualifiers) &
       CTOOL_C_QUAL_ATOMIC) != 0u) {
    (void)source_input_base;
    status = cfront_emit_failure(
        context, CTOOL_ERR_UNSUPPORTED,
        CTOOL_C_PARSE_DIAG_STATEMENT, constraint_token,
        "GNU flags-restore assembly requires one volatile non-atomic "
        "32-bit r input and one cc clobber");
  }
  ...
  if (memory_input == CTOOL_TRUE &&
    ...
    return cfront_emit_failure(
        ...
        : x87_double_memory == CTOOL_TRUE
            ? x87_pow_memory == CTOOL_TRUE
                  ? "GNU x87 pow assembly m input requires an addressable "
                    "non-atomic double lvalue"
              : x87_round_down_memory == CTOOL_TRUE
                  ? "GNU x87 round-down assembly m input requires an "
                    "addressable non-atomic double lvalue"
               : x87_atan2_memory == CTOOL_TRUE
                   ? "GNU x87 atan2 assembly y and x m inputs require "
                     "addressable non-atomic double lvalues"
               : x87_exp_memory == CTOOL_TRUE
                   ? "GNU x87 exp assembly x and log2e m inputs require "
                     "addressable non-atomic double lvalues"
...
static ctool_bool cfront_x87_double_memory_operand_is_valid(
    cfront_context_t *context,
    const ctool_c_assembly_operand_t *operand, ctool_bool output) {
  ctool_c_type_node_t node;
  ctool_u32 base;
...
static ctool_status_t cfront_validate_x87_sine_memory_assembly(
    cfront_context_t *context, const ctool_c_pp_token_t *keyword,
    const ctool_c_assembly_t *assembly) {
  ctool_c_assembly_operand_t output;
  ctool_c_assembly_operand_t input;
  ctool_status_t status = CTOOL_OK;
  if (cfront_x87_sine_memory_template(
          assembly->template_text) == CTOOL_FALSE) {
    return CTOOL_OK;
  }
  if (assembly->flags != CTOOL_C_ASSEMBLY_VOLATILE ||
...
static ctool_status_t cfront_validate_x87_round_down_memory_assembly(
    cfront_context_t *context, const ctool_c_pp_token_t *keyword,
    const ctool_c_assembly_t *assembly) {
  ctool_c_assembly_operand_t output;
  ctool_c_assembly_operand_t input;
...
static ctool_status_t cfront_validate_x87_pow_memory_assembly(
    cfront_context_t *context, const ctool_c_pp_token_t *keyword,
    const ctool_c_assembly_t *assembly) {
  ctool_c_assembly_operand_t operand;
  ctool_u32 index;
  ctool_status_t status = CTOOL_OK;
  if (cfront_x87_pow_memory_template(
          assembly->template_text) == CTOOL_FALSE) {
    return CTOOL_OK;
  }
  if (assembly->flags !=
...
static ctool_status_t cfront_validate_x87_atan2_memory_assembly(
    cfront_context_t *context, const ctool_c_pp_token_t *keyword,
    const ctool_c_assembly_t *assembly) {
  ctool_c_assembly_operand_t operand;
  ctool_u32 index;
  ctool_status_t status = CTOOL_OK;
  if (cfront_x87_atan2_memory_template(
          assembly->template_text) == CTOOL_FALSE) {
    return CTOOL_OK;
  }
  if (assembly->flags !=
      ...
      assembly->first_operand > context->assembly_operands.count ||
      3u > context->assembly_operands.count - assembly->first_operand) {
    status = CTOOL_ERR_UNSUPPORTED;
  }
  for (index = 0u; status == CTOOL_OK && index < 3u; index++) {
    status = cfront_vector_get(
        &context->assembly_operands,
        assembly->first_operand + index, &operand);
    if (status == CTOOL_OK &&
...
static ctool_status_t cfront_validate_x87_exp_memory_assembly(
    cfront_context_t *context, const ctool_c_pp_token_t *keyword,
    const ctool_c_assembly_t *assembly) {
  ctool_c_assembly_operand_t operand;
  ctool_u32 index;
  ctool_status_t status = CTOOL_OK;
  if (cfront_x87_exp_memory_template(
          assembly->template_text) == CTOOL_FALSE) {
    return CTOOL_OK;
  }
  if (assembly->flags !=
...
static ctool_status_t cfront_validate_flags_restore_assembly(
    cfront_context_t *context, const ctool_c_pp_token_t *keyword,
    const ctool_c_assembly_t *assembly) {
  ctool_c_assembly_operand_t operand;
  ctool_c_type_node_t input_node;
  ...
  ctool_bool exact_template =
      cfront_flags_restore_assembly_template(assembly->template_text);
  ctool_u32 input_base = CTOOL_C_TYPE_NONE;
  ctool_u32 input_qualifiers = 0u;
  ctool_status_t status;
  if (exact_template == CTOOL_FALSE) {
  ...
  if (assembly->flags !=
      ...
      assembly->output_count != 0u || assembly->input_count != 1u ||
      assembly->first_operand >= context->assembly_operands.count) {
    return cfront_emit_failure(
        context, CTOOL_ERR_UNSUPPORTED, CTOOL_C_PARSE_DIAG_STATEMENT,
        keyword,
  ...
  if (status == CTOOL_OK) {
    status = cfront_underlying_type(
        context, operand.type, &input_base, &input_qualifiers,
        &input_node);
  }
  if (status != CTOOL_OK) {
    return ctool_job_diagnostic_count(context->job) == 0u
  ...
  (void)input_base;
  if (cfront_string_literal(operand.constraint, "r") == CTOOL_FALSE ||
      operand.matching_output != CTOOL_C_AST_NONE ||
      is_integer == CTOOL_FALSE || integer.width != 32u ||
      ((input_qualifiers | input_node.qualifiers) &
       CTOOL_C_QUAL_ATOMIC) != 0u) {
...
static ctool_status_t cfront_parse_gnu_assembly_statement(
  ...
  cfront_vector_t assembly_operand_names;
  ...
  cfront_zero(&assembly_operand_names,
              (ctool_u32)sizeof(assembly_operand_names));
  assembly.first_operand = context->assembly_operands.count;
  ...
  if (status == CTOOL_OK && basic_assembly == CTOOL_FALSE) {
    status = cfront_vector_open(
        context, &assembly_operand_names, (ctool_u32)sizeof(ctool_string_t));
  }
  if (status == CTOOL_OK && basic_assembly == CTOOL_FALSE) {
    status = cfront_collect_assembly_operand_names(
        context, context->position, &assembly_operand_names);
  }
  if (status == CTOOL_OK && basic_assembly == CTOOL_FALSE) {
    status = cfront_normalize_assembly_operand_names(
        context, keyword, &assembly_operand_names, &assembly.template_text);
  }
    ...
    while (status == CTOOL_OK &&
      ...
      if (status == CTOOL_OK &&
        ...
        status = cfront_emit_failure(
            context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_STATEMENT,
            clobber_token,
            "GNU inline assembly ax clobber is listed twice");
      }
      if (status == CTOOL_OK &&
  ...
  if (status == CTOOL_OK) {
    status = cfront_validate_flags_restore_assembly(
  ...
  if (status == CTOOL_OK) {
    status = cfront_validate_x87_sine_memory_assembly(
        context, keyword, &assembly);
  }
  if (status == CTOOL_OK) {
  ...
  if (status == CTOOL_OK) {
    status = cfront_validate_x87_atan2_memory_assembly(
        context, keyword, &assembly);
  }
  if (status == CTOOL_OK) {
    status = cfront_validate_x87_exp_memory_assembly(
        context, keyword, &assembly);
  }
  if (status == CTOOL_OK) {
  ...
  cfront_vector_close(&assembly_operand_names);
  return status == CTOOL_OK ? CTOOL_OK
                            : ctool_job_diagnostic_count(context->job) == 0u
                                  ? cfront_storage_failure(context, status)
                                  : status;
...
static ctool_bool cfront_character_type_kind(ctool_c_type_kind_t kind) {
  return kind == CTOOL_C_TYPE_CHAR || kind == CTOOL_C_TYPE_SIGNED_CHAR ||
                 kind == CTOOL_C_TYPE_UNSIGNED_CHAR
             ? CTOOL_TRUE
             : CTOOL_FALSE;
}
...
static ctool_status_t cfront_inspect_initializer_target(
  ...
  ctool_u32 qualifiers;
  ctool_status_t status = cfront_underlying_type(
      context, type, base_out, &qualifiers, target_out);
  (void)qualifiers;
  cfront_zero(element_out, (ctool_u32)sizeof(*element_out));
  *element_qualifiers_out = 0u;
  if (status == CTOOL_OK && target_out->kind == CTOOL_C_TYPE_ARRAY) {
    ctool_u32 element_base;
    status = cfront_underlying_type(
        context, target_out->referenced_type, &element_base,
        element_qualifiers_out, element_out);
    (void)element_base;
  }
  return status;
}

static ctool_status_t cfront_static_string_pointer_compatible(
    cfront_context_t *context, const ctool_c_type_node_t *pointer,
    ctool_bool *compatible_out) {
  ctool_c_type_node_t referent;
  ctool_u32 base;
  ctool_u32 qualifiers;
  ...
  *compatible_out = CTOOL_FALSE;
  ...
  status = cfront_underlying_type(
      context, pointer->referenced_type, &base, &qualifiers, &referent);
  (void)base;
  if (status == CTOOL_OK &&
      (qualifiers & CTOOL_C_QUAL_ATOMIC) == 0u &&
      (referent.kind == CTOOL_C_TYPE_CHAR ||
       referent.kind == CTOOL_C_TYPE_VOID)) {
    *compatible_out = CTOOL_TRUE;
  }
  return status;
}
...
static ctool_status_t cfront_parse_string_initializer(
  ...
  if (array.kind == CTOOL_C_TYPE_BOOL) {
    ctool_arena_t *arena = ctool_job_arena(context->job);
    ctool_arena_mark_t decoded_mark = ctool_arena_mark(arena);
    status = cfront_decode_body_string(context, &decoded);
    if (status != CTOOL_OK) {
      return status;
    }
    status = ctool_arena_rewind(arena, decoded_mark);
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
    {
      ctool_c_initializer_t initializer;
      cfront_initializer_init(&initializer, CTOOL_C_INITIALIZER_INTEGER,
                              *object_type_io, value_token);
      initializer.integer_bits = 1ull;
      status = cfront_append_initializer(context, &initializer,
                                         initializer_out);
    }
    return status;
  }
  if (array.kind == CTOOL_C_TYPE_POINTER) {
    status = cfront_static_string_pointer_compatible(
        context, &array, &compatible_pointer);
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
    if (compatible_pointer == CTOOL_FALSE) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INPUT,
    ...
    if (status == CTOOL_OK) {
      ...
      cfront_initializer_init(&initializer, CTOOL_C_INITIALIZER_ADDRESS,
                              *object_type_io, value_token);
      initializer.string_bytes = decoded;
      initializer.address_kind = CTOOL_C_INITIALIZER_ADDRESS_STRING;
  ...
  if (array.array_bound_kind == CTOOL_C_ARRAY_UNSPECIFIED) {
    ...
    status = cfront_type_append(context, &array, object_type_io);
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
  ...
  {
    ctool_c_initializer_t initializer;
    cfront_initializer_init(&initializer, CTOOL_C_INITIALIZER_STRING,
                            *object_type_io, value_token);
    initializer.string_bytes = ctool_bytes(decoded.data, copy_size);
    status = cfront_append_initializer(context, &initializer,
                                       initializer_out);
  }
  return status;
...
static ctool_status_t cfront_next_record_initializer_member(
  ...
  *found_out = CTOOL_FALSE;
  if (record->first_member > context->members.count ||
      record->member_count > context->members.count - record->first_member ||
      *member_cursor_io > record->member_count) {
    return CTOOL_ERR_INTERNAL;
  }
...
static ctool_status_t cfront_parse_aggregate_initializer(
  ...
  if (aggregate.kind == CTOOL_C_TYPE_ARRAY) {
    ...
    return cfront_emit_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL, origin,
        "aggregate initializer target is not an aggregate");
  }
  status = cfront_enter_syntax(context, origin);
  if (status != CTOOL_OK) {
    return status;
  }
  if (explicit_braces == CTOOL_TRUE &&
      cfront_peek_is(context, "}") == CTOOL_TRUE) {
    cfront_leave_syntax(context);
    return cfront_emit_failure(
  ...
  while (status == CTOOL_OK &&
    ...
    ctool_u32 first_designator_binding = context->block_bindings.count;
    ctool_u32 designator_binding_end;
    ...
    designator_binding_end = context->block_bindings.count;
    if (aggregate.kind == CTOOL_C_TYPE_ARRAY) {
      ...
      if (status != CTOOL_OK) {
        status = cfront_emit_failure(
            context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
            cfront_peek(context), "record initializer member is unavailable");
        break;
      }
      if (found == CTOOL_FALSE) {
    ...
    {
      ctool_c_type_node_t child;
      ctool_c_type_node_t element;
      ctool_u32 child_base;
      ctool_u32 element_qualifiers;
      ctool_bool character_string = CTOOL_FALSE;
    ...
    if (status == CTOOL_OK) {
      status = cfront_attach_initializer_block_bindings(
          context, child_initializer, first_designator_binding,
          designator_binding_end - first_designator_binding, clause_start);
    }
    if (status == CTOOL_OK) {
      edge.initializer = child_initializer;
      status = cfront_append_pending_initializer_element(context, &edge);
    }
    if (status != CTOOL_OK) {
      break;
    }
  ...
  if (status == CTOOL_OK) {
    ctool_c_initializer_t initializer;
    cfront_initializer_init(&initializer, CTOOL_C_INITIALIZER_LIST,
                            *object_type_io, origin);
    status = cfront_publish_initializer_elements(
        context, pending_mark, &initializer.first_element,
  ...
  if (status != CTOOL_OK &&
      context->pending_initializer_elements.count >= pending_mark) {
    ctool_status_t rewind_status = cfront_vector_rewind(
        &context->pending_initializer_elements, pending_mark);
    if (rewind_status != CTOOL_OK) {
...
typedef enum {
  CFRONT_STATIC_SCALAR_NONE = 0,
  CFRONT_STATIC_SCALAR_INTEGER,
  CFRONT_STATIC_SCALAR_FLOATING
} cfront_static_scalar_kind_t;

typedef struct {
  cfront_static_scalar_kind_t kind;
  cfront_integer_t integer;
  ctool_c_type_kind_t floating_kind;
  ctool_u64 floating_bits;
} cfront_static_scalar_t;
...
static ctool_bool cfront_static_floating_format(
    ctool_c_type_kind_t kind, ctool_u32 *precision_out,
    ctool_u32 *fraction_width_out, ctool_i32 *minimum_exponent_out,
    ctool_i32 *maximum_exponent_out, ctool_u32 *bias_out,
    ctool_u64 *sign_mask_out) {
  if (kind == CTOOL_C_TYPE_FLOAT) {
    *precision_out = 24u;
    *fraction_width_out = 23u;
    *minimum_exponent_out = -126;
    *maximum_exponent_out = 127;
    *bias_out = 127u;
    *sign_mask_out = 0x0000000080000000ull;
    return CTOOL_TRUE;
  }
  if (kind == CTOOL_C_TYPE_DOUBLE) {
    *precision_out = 53u;
    *fraction_width_out = 52u;
    *minimum_exponent_out = -1022;
    *maximum_exponent_out = 1023;
    *bias_out = 1023u;
    *sign_mask_out = 0x8000000000000000ull;
    return CTOOL_TRUE;
  }
  return CTOOL_FALSE;
}

static ctool_u64 cfront_static_floating_special_bits(
    ctool_c_type_kind_t kind, ctool_bool negative, ctool_bool nan) {
  ctool_u64 sign =
      negative == CTOOL_TRUE
  ...
  return sign |
         (kind == CTOOL_C_TYPE_FLOAT
              ? (nan == CTOOL_TRUE ? 0x000000007fc00000ull
                                   : 0x000000007f800000ull)
              : (nan == CTOOL_TRUE ? 0x7ff8000000000000ull
                                   : 0x7ff0000000000000ull));
}

static ctool_u64 cfront_static_shift_right_jam_u64(
    ctool_u64 value, ctool_u32 distance) {
  if (distance == 0u) {
    return value;
  }
  if (distance < 64u) {
    ctool_u64 discarded = value & ((1ull << distance) - 1ull);
    return (value >> distance) |
           (discarded != 0ull ? 1ull : 0ull);
  }
  return value != 0ull ? 1ull : 0ull;
}

static ctool_u32 cfront_static_u128_width(cfront_u128_t value) {
  return value.high != 0ull ? 64u + cfront_u64_width(value.high)
                            : cfront_u64_width(value.low);
}

static cfront_u128_t cfront_static_multiply_u64(
    ctool_u64 left, ctool_u64 right) {
  ctool_u64 left_low = left & 0xffffffffull;
  ctool_u64 left_high = left >> 32u;
  ctool_u64 right_low = right & 0xffffffffull;
  ctool_u64 right_high = right >> 32u;
  ctool_u64 low_product = left_low * right_low;
  ctool_u64 middle =
      left_high * right_low + (low_product >> 32u);
  ctool_u64 middle_low = middle & 0xffffffffull;
  ctool_u64 middle_high = middle >> 32u;
  cfront_u128_t result;
  middle_low += left_low * right_high;
  result.high =
      left_high * right_high + middle_high + (middle_low >> 32u);
  result.low =
      (middle_low << 32u) | (low_product & 0xffffffffull);
  return result;
}

static ctool_u64 cfront_static_shift_right_jam_u128(
    cfront_u128_t value, ctool_u32 distance) {
  ctool_u64 result;
  ctool_bool discarded = CTOOL_FALSE;
  if (distance == 0u) {
    return value.low;
  }
  if (distance < 64u) {
    result = (value.high << (64u - distance)) |
             (value.low >> distance);
    discarded =
        (value.low & ((1ull << distance) - 1ull)) != 0ull
            ? CTOOL_TRUE
            : CTOOL_FALSE;
  } else if (distance == 64u) {
    result = value.high;
    discarded = value.low != 0ull ? CTOOL_TRUE : CTOOL_FALSE;
  } else if (distance < 128u) {
    ctool_u32 high_distance = distance - 64u;
    result = value.high >> high_distance;
    discarded =
        value.low != 0ull ||
                (value.high &
                 ((1ull << high_distance) - 1ull)) != 0ull
            ? CTOOL_TRUE
            : CTOOL_FALSE;
  } else {
    result = 0ull;
    discarded =
        value.high != 0ull || value.low != 0ull
            ? CTOOL_TRUE
            : CTOOL_FALSE;
  }
...
static ctool_status_t cfront_static_pack_rounded(
  ...
  ctool_u32 precision;
  ctool_u32 fraction_width;
  ctool_i32 minimum_exponent;
  ctool_i32 maximum_exponent;
  ctool_u32 bias;
  ctool_u64 sign_mask;
  ctool_u32 width;
  ctool_u32 target_width;
  ctool_u64 rounded;
  ctool_u64 round_bits;
  ctool_u64 hidden;
  ctool_u64 fraction;
  ctool_u64 exponent_bits;
  if (cfront_static_floating_format(
          kind, &precision, &fraction_width, &minimum_exponent,
          &maximum_exponent, &bias, &sign_mask) == CTOOL_FALSE) {
    return cfront_emit_location_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
        location, "static floating result type is invalid");
  }
  if (extended == 0ull) {
    *bits_out = negative == CTOOL_TRUE ? sign_mask : 0ull;
    return CTOOL_OK;
  }
  width = cfront_u64_width(extended);
  target_width = precision + 3u;
  if (width > target_width) {
    ctool_u32 shift = width - target_width;
    extended = cfront_static_shift_right_jam_u64(extended, shift);
    exponent += (ctool_i32)shift;
  } else if (width < target_width) {
    ctool_u32 shift = target_width - width;
    extended <<= shift;
    exponent -= (ctool_i32)shift;
  }
  if (exponent < minimum_exponent) {
    extended = cfront_static_shift_right_jam_u64(
        extended, (ctool_u32)(minimum_exponent - exponent));
    exponent = minimum_exponent;
  }
  round_bits = extended & 7ull;
  rounded = extended >> 3u;
  if (round_bits > 4ull ||
      (round_bits == 4ull && (rounded & 1ull) != 0ull)) {
    rounded++;
  }
  if (rounded == 0ull) {
    *bits_out = negative == CTOOL_TRUE ? sign_mask : 0ull;
  ...
  hidden = 1ull << (precision - 1u);
  if (rounded >= (hidden << 1u)) {
    rounded >>= 1u;
    exponent++;
  }
  if (exponent > maximum_exponent) {
    *bits_out = cfront_static_floating_special_bits(
        kind, negative, CTOOL_FALSE);
    return CTOOL_OK;
  }
  fraction = rounded & (hidden - 1ull);
  exponent_bits =
      exponent == minimum_exponent && rounded < hidden
          ? 0ull
          : (ctool_u64)((ctool_u32)(exponent + (ctool_i32)bias));
  *bits_out =
      (negative == CTOOL_TRUE ? sign_mask : 0ull) |
      (exponent_bits << fraction_width) | fraction;
  return CTOOL_OK;
}

static ctool_status_t cfront_static_pack_exact_u128(
    cfront_context_t *context, ctool_c_type_kind_t kind,
    ctool_bool negative, cfront_u128_t magnitude, ctool_i32 scale,
    const ctool_c_pp_location_t *location, ctool_u64 *bits_out) {
  ctool_u32 precision;
  ctool_u32 fraction_width;
  ctool_i32 minimum_exponent;
  ...
  (void)bias;
  (void)sign_mask;
  width = cfront_static_u128_width(magnitude);
  if (width == 0u) {
    return cfront_static_pack_rounded(
        context, kind, negative, 0, 0ull, location, bits_out);
  }
  if (width > precision + 3u) {
    shift = width - (precision + 3u);
  }
  extended = cfront_static_shift_right_jam_u128(magnitude, shift);
  return cfront_static_pack_rounded(
      context, kind, negative,
      scale + (ctool_i32)shift + (ctool_i32)precision + 2,
...
static ctool_status_t cfront_static_decode_floating(
  ...
  ctool_u32 bias;
  ctool_u64 sign_mask;
  ctool_u64 exponent_mask;
  ctool_u64 fraction_mask;
  ctool_u64 raw_exponent;
  ctool_u64 fraction;
  cfront_zero(value_out, (ctool_u32)sizeof(*value_out));
  if (cfront_static_floating_format(
          kind, &precision, &fraction_width, &minimum_exponent,
          &maximum_exponent, &bias, &sign_mask) == CTOOL_FALSE ||
      (kind == CTOOL_C_TYPE_FLOAT &&
       (bits & 0xffffffff00000000ull) != 0ull)) {
    return cfront_emit_location_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
        location, "static floating constant metadata is invalid");
  }
  (void)maximum_exponent;
  exponent_mask =
      kind == CTOOL_C_TYPE_FLOAT ? 0xffull : 0x7ffull;
  fraction_mask = (1ull << fraction_width) - 1ull;
  raw_exponent = (bits >> fraction_width) & exponent_mask;
  fraction = bits & fraction_mask;
  value_out->kind = kind;
  value_out->negative =
      (bits & sign_mask) != 0ull ? CTOOL_TRUE : CTOOL_FALSE;
  if (raw_exponent == exponent_mask) {
    value_out->infinity =
        fraction == 0ull ? CTOOL_TRUE : CTOOL_FALSE;
    value_out->nan =
        fraction != 0ull ? CTOOL_TRUE : CTOOL_FALSE;
    return CTOOL_OK;
  }
  if (raw_exponent == 0ull) {
    ...
    width = cfront_u64_width(fraction);
    shift = precision - width;
    value_out->significand = fraction << shift;
    value_out->exponent =
        minimum_exponent - (ctool_i32)shift;
    return CTOOL_OK;
  }
  value_out->exponent =
      (ctool_i32)raw_exponent - (ctool_i32)bias;
  value_out->significand =
      (1ull << (precision - 1u)) | fraction;
  return CTOOL_OK;
}
...
static ctool_status_t cfront_static_add_floating(
  ...
  ctool_u32 bias;
  ctool_u64 sign_mask;
  ctool_u64 left_extended;
  ctool_u64 right_extended;
  ctool_u64 result;
  ctool_i32 exponent;
  ...
  if (left.nan == CTOOL_TRUE || right.nan == CTOOL_TRUE ||
    ...
    *bits_out = cfront_static_floating_special_bits(
        kind, CTOOL_FALSE, CTOOL_TRUE);
    return CTOOL_OK;
  }
  if (left.infinity == CTOOL_TRUE) {
    *bits_out = cfront_static_floating_special_bits(
        kind, left.negative, CTOOL_FALSE);
    return CTOOL_OK;
  }
  if (right.infinity == CTOOL_TRUE) {
    *bits_out = cfront_static_floating_special_bits(
        kind, right.negative, CTOOL_FALSE);
    return CTOOL_OK;
  }
  ...
  if (left.exponent < right.exponent) {
    cfront_static_floating_t temporary_value = left;
    ctool_u64 temporary_bits = left_bits;
    left = right;
    right = temporary_value;
    left_bits = right_bits;
    right_bits = temporary_bits;
  }
  exponent = left.exponent;
  left_extended = left.significand << 3u;
  right_extended = cfront_static_shift_right_jam_u64(
      right.significand << 3u,
      (ctool_u32)(left.exponent - right.exponent));
  if (left.negative == right.negative) {
    result = left_extended + right_extended;
  } else if (left_extended >= right_extended) {
    result = left_extended - right_extended;
  } else {
    result = right_extended - left_extended;
    left.negative = right.negative;
  }
  if (result == 0ull) {
    left.negative = CTOOL_FALSE;
  }
  return cfront_static_pack_rounded(
      context, kind, left.negative, exponent, result, location, bits_out);
}

static ctool_status_t cfront_static_multiply_floating(
    cfront_context_t *context, ctool_c_type_kind_t kind,
  ...
  return cfront_static_pack_exact_u128(
      context, kind, negative, product,
      left.exponent + right.exponent -
          (ctool_i32)(2u * (precision - 1u)),
      location, bits_out);
}

static ctool_status_t cfront_static_divide_floating(
    cfront_context_t *context, ctool_c_type_kind_t kind,
    ctool_u64 left_bits, ctool_u64 right_bits,
    const ctool_c_pp_location_t *location, ctool_u64 *bits_out) {
  cfront_static_floating_t left;
  ...
  if (cfront_u128_shifted_u64(
          left.significand, shift, &numerator) == CTOOL_FALSE ||
      cfront_u128_divide_u64(
          numerator, right.significand, &quotient,
          &remainder) == CTOOL_FALSE) {
    return cfront_emit_location_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
        location, "static floating division exceeded its integer workspace");
  }
  if (remainder != 0ull) {
    quotient |= 1ull;
  }
...
static ctool_status_t cfront_static_compare_floating(
  ...
  equal =
      unordered == CTOOL_FALSE &&
          ...
          : CTOOL_FALSE;
  less = CTOOL_FALSE;
  if (unordered == CTOOL_FALSE && equal == CTOOL_FALSE) {
    if (left.negative != right.negative) {
      less = left.negative;
  ...
  case CTOOL_C_EXPRESSION_OPERATOR_LESS_EQUAL:
    *result_out =
        unordered == CTOOL_FALSE &&
                (less == CTOOL_TRUE || equal == CTOOL_TRUE)
            ? CTOOL_TRUE
            : CTOOL_FALSE;
    break;
  case CTOOL_C_EXPRESSION_OPERATOR_GREATER:
    *result_out =
        unordered == CTOOL_FALSE && less == CTOOL_FALSE &&
                equal == CTOOL_FALSE
            ? CTOOL_TRUE
            : CTOOL_FALSE;
    break;
  case CTOOL_C_EXPRESSION_OPERATOR_GREATER_EQUAL:
    *result_out =
        unordered == CTOOL_FALSE &&
                (less == CTOOL_FALSE || equal == CTOOL_TRUE)
            ? CTOOL_TRUE
            : CTOOL_FALSE;
    break;
  case CTOOL_C_EXPRESSION_OPERATOR_EQUAL:
  ...
  case CTOOL_C_EXPRESSION_OPERATOR_NOT_EQUAL:
    *result_out =
        equal == CTOOL_TRUE ? CTOOL_FALSE : CTOOL_TRUE;
    break;
  default:
    return cfront_emit_location_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
        location, "static floating comparison metadata is invalid");
  }
  return CTOOL_OK;
}

static ctool_status_t cfront_static_scalar_truth(
    cfront_context_t *context, const cfront_static_scalar_t *value,
    const ctool_c_pp_location_t *location, ctool_bool *truth_out) {
  if (value->kind == CFRONT_STATIC_SCALAR_INTEGER) {
    *truth_out = value->integer.bits != 0ull
                     ? CTOOL_TRUE
                     : CTOOL_FALSE;
    return CTOOL_OK;
  }
  if (value->kind == CFRONT_STATIC_SCALAR_FLOATING) {
    ...
    *truth_out =
        floating.zero == CTOOL_TRUE ? CTOOL_FALSE : CTOOL_TRUE;
    return CTOOL_OK;
  }
  return cfront_emit_location_failure(
      context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
      location, "static scalar truth conversion metadata is invalid");
}

static cfront_integer_kind_t cfront_static_integer_kind(
    const cfront_integer_type_t *integer) {
  if (integer->width == 64u) {
    return integer->is_unsigned == CTOOL_TRUE
               ? CFRONT_INTEGER_UNSIGNED_64
               : CFRONT_INTEGER_SIGNED_64;
  }
  return integer->is_unsigned == CTOOL_TRUE &&
...
static ctool_status_t cfront_static_integer_to_floating(
    cfront_context_t *context, const cfront_integer_t *integer,
    ctool_c_type_kind_t target_kind,
    const ctool_c_pp_location_t *location, ctool_u64 *bits_out) {
  cfront_u128_t magnitude;
  magnitude.high = 0ull;
  magnitude.low = cfront_integer_magnitude(integer);
  return cfront_static_pack_exact_u128(
      context, target_kind, cfront_integer_negative(integer),
      magnitude, 0, location, bits_out);
}

static ctool_status_t cfront_static_floating_to_integer(
    cfront_context_t *context, ctool_c_type_kind_t source_kind,
    ctool_u64 source_bits, const cfront_integer_type_t *target,
    const ctool_c_pp_location_t *location,
    cfront_integer_t *integer_out) {
  ...
  ctool_u64 limit;
  ctool_status_t status = cfront_static_decode_floating(
      context, source_kind, source_bits, location, &source);
  if (status != CTOOL_OK) {
    return status;
  }
  if (source.nan == CTOOL_TRUE || source.infinity == CTOOL_TRUE) {
    return cfront_emit_location_failure(
        ...
        "static floating-to-integer conversion exceeds its destination range");
  }
  if (cfront_static_floating_format(
          source_kind, &precision, &fraction_width, &minimum_exponent,
          &maximum_exponent, &bias, &sign_mask) == CTOOL_FALSE) {
    return cfront_emit_location_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
        location, "static floating source type is invalid");
  }
  (void)fraction_width;
  (void)minimum_exponent;
  (void)maximum_exponent;
  (void)bias;
  (void)sign_mask;
  if (source.zero == CTOOL_TRUE || source.exponent < 0) {
    magnitude = 0ull;
  } else {
    scale = source.exponent - (ctool_i32)(precision - 1u);
    if (scale >= 0) {
      if ((ctool_u32)scale >= 64u ||
          cfront_u64_width(source.significand) >
              64u - (ctool_u32)scale) {
        return cfront_emit_location_failure(
            context, CTOOL_ERR_INPUT,
            CTOOL_C_PARSE_DIAG_CONSTANT_EXPRESSION, location,
      ...
      magnitude = source.significand << (ctool_u32)scale;
    } else {
      ctool_u32 shift = (ctool_u32)(0 - scale);
      magnitude = shift >= 64u ? 0ull
                               : source.significand >> shift;
    }
  }
  if (target->is_unsigned == CTOOL_TRUE) {
    limit = target->width == 64u
                ? 0xffffffffffffffffull
                : (1ull << target->width) - 1ull;
    if ((source.negative == CTOOL_TRUE && magnitude != 0ull) ||
        magnitude > limit) {
      return cfront_emit_location_failure(
          context, CTOOL_ERR_INPUT,
          CTOOL_C_PARSE_DIAG_CONSTANT_EXPRESSION, location,
    ...
    limit = 1ull << (target->width - 1u);
    if (magnitude >
        (source.negative == CTOOL_TRUE ? limit : limit - 1ull)) {
      return cfront_emit_location_failure(
          context, CTOOL_ERR_INPUT,
          CTOOL_C_PARSE_DIAG_CONSTANT_EXPRESSION, location,
          "static floating-to-integer conversion exceeds its destination range");
  ...
  integer_out->kind = cfront_static_integer_kind(target);
  integer_out->bits =
      source.negative == CTOOL_TRUE ? 0ull - magnitude : magnitude;
  *integer_out = cfront_integer_convert_to_type(*integer_out, target);
  return CTOOL_OK;
}
...
static ctool_status_t cfront_static_expression_child(
  ...
  if (offset >= expression->child_count ||
      expression->first_child >=
          context->expression_children.count ||
      offset >
          context->expression_children.count -
              expression->first_child - 1u ||
      cfront_vector_get(
          &context->expression_children,
          expression->first_child + offset, child_out) != CTOOL_OK) {
    return cfront_emit_location_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
        &expression->location,
        "static scalar expression child metadata is unavailable");
  }
  return CTOOL_OK;
}

static ctool_status_t cfront_convert_static_scalar(
    cfront_context_t *context, ctool_u32 target_type,
    const ctool_c_pp_location_t *location,
    cfront_static_scalar_t *value, ctool_bool *matched_out) {
  cfront_integer_type_t target_integer;
  ctool_bool target_is_integer = CTOOL_FALSE;
  ctool_c_type_node_t target;
  ctool_u32 target_base;
  ctool_u32 target_qualifiers;
  ctool_status_t status = cfront_integer_type(
      context, target_type, &target_integer, &target_is_integer);
  if (status != CTOOL_OK) {
    return cfront_storage_failure(context, status);
  }
  status = cfront_underlying_type(
  ...
  if (target_is_integer == CTOOL_TRUE) {
    if (target_integer.kind == CTOOL_C_TYPE_BOOL) {
      ...
      value->integer.kind = CFRONT_INTEGER_SIGNED_32;
      value->integer.bits =
          truth == CTOOL_TRUE ? 1ull : 0ull;
      value->integer = cfront_integer_convert_to_type(
          value->integer, &target_integer);
    } else if (value->kind == CFRONT_STATIC_SCALAR_INTEGER) {
      value->integer =
          cfront_integer_convert_to_type(value->integer, &target_integer);
    } else if (value->kind == CFRONT_STATIC_SCALAR_FLOATING) {
      status = cfront_static_floating_to_integer(
          context, value->floating_kind, value->floating_bits,
          &target_integer, location, &value->integer);
      if (status != CTOOL_OK) {
        return status;
      }
    } else {
    ...
    value->kind = CFRONT_STATIC_SCALAR_INTEGER;
    *matched_out = CTOOL_TRUE;
    return CTOOL_OK;
  }
  if (target.kind == CTOOL_C_TYPE_FLOAT ||
...
static ctool_status_t cfront_evaluate_static_scalar(
    cfront_context_t *context, ctool_u32 expression_index,
    ctool_u32 depth,
    const ctool_c_pp_location_t *fallback_location,
    cfront_static_scalar_t *value_out,
    ctool_bool *matched_out) {
  ctool_c_expression_t expression;
  const ctool_c_pp_location_t *current_location;
  ctool_status_t status;
  cfront_zero(value_out, (ctool_u32)sizeof(*value_out));
  *matched_out = CTOOL_FALSE;
  if (expression_index >= context->expressions.count) {
    return cfront_emit_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
        cfront_peek(context),
        "static scalar expression metadata is unavailable");
  }
  status = cfront_vector_get(
      &context->expressions, expression_index, &expression);
  if (status != CTOOL_OK) {
  ...
  if (expression.location.line != 0u ||
      expression.location.column != 0u) {
    current_location = &expression.location;
  }
  if (depth >= CTOOL_C_PARSE_NESTING_LIMIT) {
    if (current_location !=
        (const ctool_c_pp_location_t *)0) {
      return cfront_emit_location_failure(
          context, CTOOL_ERR_LIMIT, CTOOL_C_PARSE_DIAG_LIMIT,
          current_location,
  ...
  if (expression.kind == CTOOL_C_EXPRESSION_INTEGER_CONSTANT) {
    ...
    ctool_bool is_integer = CTOOL_FALSE;
    status = cfront_integer_type(
        context, expression.type, &integer, &is_integer);
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
    if (is_integer == CTOOL_FALSE) {
      return cfront_emit_location_failure(
          context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
          &expression.location,
          "static integer constant metadata is invalid");
    }
    value_out->kind = CFRONT_STATIC_SCALAR_INTEGER;
    value_out->integer.kind = cfront_static_integer_kind(&integer);
    value_out->integer.bits = cfront_integer_normalize_bits(
        expression.integer_bits, value_out->integer.kind);
    value_out->integer =
        cfront_integer_convert_to_type(value_out->integer, &integer);
    *matched_out = CTOOL_TRUE;
    return CTOOL_OK;
  }
  if (expression.kind == CTOOL_C_EXPRESSION_IDENTIFIER ||
      expression.kind == CTOOL_C_EXPRESSION_BLOCK_BINDING) {
    ctool_u32 type = CTOOL_C_TYPE_NONE;
    ctool_u64 bits = 0ull;
    cfront_integer_type_t integer;
    ctool_bool is_enumerator = CTOOL_FALSE;
    ctool_bool is_integer = CTOOL_FALSE;
    if (expression.kind == CTOOL_C_EXPRESSION_IDENTIFIER) {
      ctool_c_binding_t binding;
      status = cfront_vector_get(
          &context->bindings, expression.reference, &binding);
      if (status != CTOOL_OK) {
        return cfront_emit_location_failure(
            context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
            &expression.location,
            "static enumerator binding metadata is unavailable");
      }
      if (binding.kind == CTOOL_C_BINDING_ENUMERATOR) {
        type = binding.type;
        bits = binding.integer_bits;
        is_enumerator = CTOOL_TRUE;
      }
    } else {
      ctool_c_block_binding_t binding;
      status = cfront_vector_get(
          &context->block_bindings, expression.reference, &binding);
      if (status != CTOOL_OK) {
        return cfront_emit_location_failure(
            context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
            &expression.location,
            "static block enumerator metadata is unavailable");
      }
      if (binding.kind == CTOOL_C_BINDING_ENUMERATOR) {
        type = binding.type;
        bits = binding.integer_bits;
    ...
    if (is_integer == CTOOL_FALSE) {
      return cfront_emit_location_failure(
          context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
          &expression.location,
          "static enumerator type is not a represented integer");
    }
    value_out->kind = CFRONT_STATIC_SCALAR_INTEGER;
    value_out->integer.kind = cfront_static_integer_kind(&integer);
    value_out->integer.bits = cfront_integer_normalize_bits(
        bits, value_out->integer.kind);
    value_out->integer =
        cfront_integer_convert_to_type(value_out->integer, &integer);
    *matched_out = CTOOL_TRUE;
    return CTOOL_OK;
  ...
  if (expression.kind == CTOOL_C_EXPRESSION_FLOATING_CONSTANT) {
    ...
    ctool_u32 source_base;
    ctool_u32 source_qualifiers;
    cfront_static_floating_t decoded;
    status = cfront_underlying_type(
    ...
    status = cfront_static_decode_floating(
        ...
        &expression.location, &decoded);
    if (status != CTOOL_OK) {
      return status;
    }
    value_out->kind = CFRONT_STATIC_SCALAR_FLOATING;
    value_out->floating_kind = source.kind;
    value_out->floating_bits = expression.integer_bits;
    *matched_out = CTOOL_TRUE;
    return CTOOL_OK;
  }
  if (expression.kind == CTOOL_C_EXPRESSION_IMPLICIT_CONVERSION ||
    ...
    ctool_u32 child;
    ctool_bool child_matched = CTOOL_FALSE;
    if (expression.child_count != 1u) {
      return cfront_emit_location_failure(
          context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
          &expression.location,
          "static scalar conversion metadata is invalid");
    }
    status = cfront_static_expression_child(
        context, &expression, 0u, &child);
    if (status == CTOOL_OK) {
      status = cfront_evaluate_static_scalar(
          context, child, depth + 1u, current_location,
          value_out, &child_matched);
    }
    if (status != CTOOL_OK || child_matched == CTOOL_FALSE) {
      return status;
    }
    return cfront_convert_static_scalar(
  ...
  if (expression.kind == CTOOL_C_EXPRESSION_UNARY) {
    ...
    if (expression.operation ==
      ...
      ctool_bool truth;
      status = cfront_static_scalar_truth(
          context, value_out, &expression.location, &truth);
      if (status != CTOOL_OK) {
        return status;
      }
      value_out->kind = CFRONT_STATIC_SCALAR_INTEGER;
      ...
      value_out->integer.bits =
          truth == CTOOL_TRUE ? 0ull : 1ull;
    } else if (expression.operation ==
               CTOOL_C_EXPRESSION_OPERATOR_BITWISE_NOT) {
      if (value_out->kind != CFRONT_STATIC_SCALAR_INTEGER) {
        return CTOOL_OK;
      }
      value_out->integer.bits = cfront_integer_normalize_bits(
          ~value_out->integer.bits, value_out->integer.kind);
    } else if (expression.operation ==
        CTOOL_C_EXPRESSION_OPERATOR_UNARY_NEGATE) {
      if (value_out->kind == CFRONT_STATIC_SCALAR_FLOATING) {
        value_out->floating_bits ^=
            value_out->floating_kind == CTOOL_C_TYPE_FLOAT
                ? 0x0000000080000000ull
                : 0x8000000000000000ull;
      } else if (value_out->kind == CFRONT_STATIC_SCALAR_INTEGER) {
        if (cfront_integer_unsigned(value_out->integer.kind) ==
                CTOOL_FALSE &&
            cfront_integer_magnitude(&value_out->integer) ==
                (cfront_integer_width(value_out->integer.kind) == 32u
                     ? 0x80000000ull
                     : 0x8000000000000000ull)) {
          return cfront_emit_location_failure(
              context, CTOOL_ERR_OVERFLOW,
              CTOOL_C_PARSE_DIAG_OVERFLOW, &expression.location,
              "static integer unary negation overflows its target type");
        }
        value_out->integer.bits = cfront_integer_normalize_bits(
            0ull - value_out->integer.bits,
            value_out->integer.kind);
      } else {
        return CTOOL_OK;
      }
    }
  ...
  if (expression.kind == CTOOL_C_EXPRESSION_BINARY) {
    cfront_static_scalar_t left;
    cfront_static_scalar_t right;
    ctool_bool left_matched = CTOOL_FALSE;
    ctool_bool right_matched = CTOOL_FALSE;
    ...
    if (expression.child_count != 2u) {
      return cfront_emit_location_failure(
          context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
          &expression.location,
          "static scalar binary metadata is invalid");
    }
    status = cfront_static_expression_child(
        context, &expression, 0u, &left_child);
    if (status == CTOOL_OK) {
      status = cfront_static_expression_child(
          context, &expression, 1u, &right_child);
    }
    if (status == CTOOL_OK) {
      status = cfront_evaluate_static_scalar(
          context, left_child, depth + 1u, current_location,
          &left, &left_matched);
    }
    if (status == CTOOL_OK && left_matched == CTOOL_TRUE &&
        (expression.operation ==
             CTOOL_C_EXPRESSION_OPERATOR_LOGICAL_AND ||
         expression.operation ==
             CTOOL_C_EXPRESSION_OPERATOR_LOGICAL_OR)) {
      ctool_bool left_truth;
      ctool_bool right_truth = CTOOL_FALSE;
      status = cfront_static_scalar_truth(
          context, &left, &expression.location, &left_truth);
      if (status != CTOOL_OK) {
        return status;
      }
      if ((expression.operation ==
               CTOOL_C_EXPRESSION_OPERATOR_LOGICAL_AND &&
           left_truth == CTOOL_FALSE) ||
          (expression.operation ==
               CTOOL_C_EXPRESSION_OPERATOR_LOGICAL_OR &&
           left_truth == CTOOL_TRUE)) {
        right_truth =
            expression.operation ==
                    CTOOL_C_EXPRESSION_OPERATOR_LOGICAL_OR
                ? CTOOL_TRUE
        ...
        status = cfront_evaluate_static_scalar(
            context, right_child, depth + 1u, current_location,
            &right, &right_matched);
        if (status != CTOOL_OK || right_matched == CTOOL_FALSE) {
          return status;
        }
        status = cfront_static_scalar_truth(
            context, &right, &expression.location, &right_truth);
        if (status != CTOOL_OK) {
          return status;
        }
      }
      ...
      value_out->integer.kind = CFRONT_INTEGER_SIGNED_32;
      value_out->integer.bits =
          right_truth == CTOOL_TRUE ? 1ull : 0ull;
      *matched_out = CTOOL_TRUE;
      return CTOOL_OK;
    }
    if (status == CTOOL_OK && left_matched == CTOOL_TRUE) {
    ...
    if (status != CTOOL_OK || left_matched == CTOOL_FALSE ||
        right_matched == CTOOL_FALSE) {
      return status;
    }
    if (left.kind == CFRONT_STATIC_SCALAR_FLOATING &&
      ...
      if (expression.operation ==
          ...
          expression.operation ==
              CTOOL_C_EXPRESSION_OPERATOR_SUBTRACT) {
        status = cfront_static_add_floating(
            context, left.floating_kind, left.floating_bits,
            right.floating_bits,
            expression.operation ==
                    CTOOL_C_EXPRESSION_OPERATOR_SUBTRACT
                ? CTOOL_TRUE
                : CTOOL_FALSE,
            &expression.location, &bits);
      } else if (expression.operation ==
                 CTOOL_C_EXPRESSION_OPERATOR_MULTIPLY) {
        status = cfront_static_multiply_floating(
            context, left.floating_kind, left.floating_bits,
            right.floating_bits, &expression.location, &bits);
      } else if (expression.operation ==
                 CTOOL_C_EXPRESSION_OPERATOR_DIVIDE) {
        status = cfront_static_divide_floating(
            context, left.floating_kind, left.floating_bits,
            right.floating_bits, &expression.location, &bits);
      } else {
        return CTOOL_OK;
      ...
      value_out->kind = CFRONT_STATIC_SCALAR_FLOATING;
      value_out->floating_kind = left.floating_kind;
      value_out->floating_bits = bits;
      *matched_out = CTOOL_TRUE;
      return CTOOL_OK;
    }
    if (left.kind == CFRONT_STATIC_SCALAR_INTEGER &&
        right.kind == CFRONT_STATIC_SCALAR_INTEGER) {
      cfront_expression_value_t left_value;
      cfront_expression_value_t right_value;
      cfront_expression_value_t result_value;
      cfront_zero(&left_value, (ctool_u32)sizeof(left_value));
      cfront_zero(&right_value, (ctool_u32)sizeof(right_value));
      cfront_zero(&result_value, (ctool_u32)sizeof(result_value));
      cfront_constant_integer_set(&left_value, left.integer);
      cfront_constant_integer_set(&right_value, right.integer);
      cfront_constant_apply_binary(
          expression.operation, &left_value, &right_value,
          &result_value);
      if (result_value.integer_constant_expression == CTOOL_FALSE ||
          result_value.integer_constant_value_valid == CTOOL_FALSE) {
        return cfront_emit_location_failure(
            context, CTOOL_ERR_INPUT,
            CTOOL_C_PARSE_DIAG_CONSTANT_EXPRESSION,
            &expression.location,
            "static integer arithmetic is not a valid constant expression");
      }
      value_out->kind = CFRONT_STATIC_SCALAR_INTEGER;
      value_out->integer = result_value.integer_constant_value;
      *matched_out = CTOOL_TRUE;
      return CTOOL_OK;
    }
    return CTOOL_OK;
  ...
  if (expression.kind == CTOOL_C_EXPRESSION_CONDITIONAL) {
    cfront_static_scalar_t condition;
    ctool_bool condition_matched = CTOOL_FALSE;
    ctool_bool condition_truth;
    ctool_u32 child;
    ctool_u32 selected;
    if (expression.child_count != 3u) {
      return cfront_emit_location_failure(
          context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
          &expression.location,
          "static conditional expression metadata is invalid");
    }
    status = cfront_static_expression_child(
        context, &expression, 0u, &child);
    if (status == CTOOL_OK) {
      status = cfront_evaluate_static_scalar(
          context, child, depth + 1u, current_location, &condition,
          &condition_matched);
    }
    if (status != CTOOL_OK || condition_matched == CTOOL_FALSE) {
      return status;
    }
    status = cfront_static_scalar_truth(
        context, &condition, &expression.location, &condition_truth);
    if (status != CTOOL_OK) {
      return status;
    }
    status = cfront_static_expression_child(
        context, &expression,
        condition_truth == CTOOL_TRUE ? 1u : 2u, &selected);
    if (status != CTOOL_OK) {
      return status;
    }
    return cfront_evaluate_static_scalar(
...
static ctool_bool cfront_static_initializer_has_floating_syntax(
    const cfront_context_t *context) {
  ctool_u32 position = context->position;
  ctool_u32 depth = 0u;
  while (position < context->tape->token_count) {
    const ctool_c_pp_token_t *token =
        &context->tape->tokens[position++];
    if (token->kind == CTOOL_C_PP_TOKEN_NUMBER &&
        cfront_body_floating_constant(token->spelling) == CTOOL_TRUE) {
    ...
    if (cfront_token_is(token, "float") == CTOOL_TRUE ||
        cfront_token_is(token, "double") == CTOOL_TRUE) {
      return CTOOL_TRUE;
    }
    if (token->kind == CTOOL_C_PP_TOKEN_IDENTIFIER) {
      ...
      ctool_u32 qualifiers;
      ctool_c_type_node_t node;
      if (cfront_find_typedef(context, token->spelling, &type) ==
              CTOOL_TRUE &&
          cfront_underlying_type(
              context, type, &base, &qualifiers, &node) == CTOOL_OK &&
          (node.kind == CTOOL_C_TYPE_FLOAT ||
           node.kind == CTOOL_C_TYPE_DOUBLE ||
           node.kind == CTOOL_C_TYPE_LONG_DOUBLE)) {
        (void)base;
...
static ctool_status_t cfront_parse_static_scalar_expression(
    cfront_context_t *context, ctool_u32 target_type,
    const ctool_c_pp_token_t *value_token, ctool_u32 *initializer_out) {
  cfront_expression_value_t expression_value;
  cfront_static_scalar_t scalar;
  cfront_integer_type_t target_integer;
  ctool_bool target_is_integer = CTOOL_FALSE;
  ...
  ctool_status_t status;
  cfront_zero(&expression_value, (ctool_u32)sizeof(expression_value));
  cfront_zero(&scalar, (ctool_u32)sizeof(scalar));
  context->static_initializer_depth++;
  status = cfront_parse_body_conditional(context, &expression_value);
  context->static_initializer_depth--;
  if (status == CTOOL_OK) {
    status = cfront_evaluate_static_scalar(
        context, expression_value.expression, 0u,
        value_token != (const ctool_c_pp_token_t *)0
  ...
  if (status == CTOOL_OK && matched == CTOOL_TRUE) {
    status = cfront_convert_static_scalar(
        context, target_type,
        value_token != (const ctool_c_pp_token_t *)0
            ? &value_token->location
            : (const ctool_c_pp_location_t *)0,
  ...
  if (status == CTOOL_OK && matched == CTOOL_FALSE) {
    status = cfront_emit_failure(
        context, CTOOL_ERR_UNSUPPORTED,
        CTOOL_C_PARSE_DIAG_CONSTANT_EXPRESSION, value_token,
  ...
  if (status == CTOOL_OK && target_is_integer == CTOOL_TRUE &&
      scalar.kind == CFRONT_STATIC_SCALAR_INTEGER) {
    ctool_c_initializer_t initializer;
    cfront_initializer_init(
        &initializer, CTOOL_C_INITIALIZER_INTEGER, target_type,
        value_token);
    initializer.integer_bits =
        target_integer.width < 64u
            ? scalar.integer.bits &
                  ((1ull << target_integer.width) - 1ull)
            : scalar.integer.bits;
    status = cfront_append_initializer(
        context, &initializer, initializer_out);
             ...
             (target.kind == CTOOL_C_TYPE_FLOAT ||
              target.kind == CTOOL_C_TYPE_DOUBLE) &&
             scalar.kind == CFRONT_STATIC_SCALAR_FLOATING &&
             scalar.floating_kind == target.kind) {
    ctool_c_initializer_t initializer;
    cfront_initializer_init(
        &initializer, CTOOL_C_INITIALIZER_FLOATING, target_type,
        value_token);
    initializer.integer_bits = scalar.floating_bits;
    status = cfront_append_initializer(
        context, &initializer, initializer_out);
  } else if (status == CTOOL_OK) {
    status = cfront_emit_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
        value_token,
        "static scalar evaluation produced an incompatible result");
  }
  {
    ctool_status_t rewind_status = cfront_vector_rewind(
        &context->expression_children, child_mark);
...
static ctool_status_t cfront_parse_static_initializer(
    ...
    if (status == CTOOL_OK && cfront_peek_is(context, ",") == CTOOL_TRUE) {
      ...
      if (cfront_peek_is(context, "}") == CTOOL_FALSE) {
        status = cfront_emit_failure(
            context, CTOOL_ERR_INPUT,
            CTOOL_C_PARSE_DIAG_CONSTANT_EXPRESSION, cfront_peek(context),
            "static initializer list has excess elements");
      }
    }
    if (status == CTOOL_OK) {
      status = cfront_expected(context, "}");
  ...
  if (is_integer == CTOOL_FALSE) {
    ...
    ctool_u32 target_qualifiers;
    status = cfront_underlying_type(
        context, *object_type_io, &target_base, &target_qualifiers, &target);
    (void)target_base;
    (void)target_qualifiers;
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
    if (target.kind == CTOOL_C_TYPE_ARRAY ||
        target.kind == CTOOL_C_TYPE_RECORD) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INPUT,
          CTOOL_C_PARSE_DIAG_CONSTANT_EXPRESSION, value_token,
    ...
    if (target.kind == CTOOL_C_TYPE_POINTER) {
      ...
      context->static_initializer_depth++;
      status = cfront_parse_body_conditional(context, &pointer_value);
      context->static_initializer_depth--;
      if (status == CTOOL_OK &&
          cfront_is_null_pointer_constant(&pointer_value) == CTOOL_TRUE) {
        ctool_c_initializer_t initializer;
        ...
        if (status == CTOOL_OK && has_string_address == CTOOL_FALSE &&
          ...
          status = cfront_emit_failure(
              context, CTOOL_ERR_UNSUPPORTED,
              CTOOL_C_PARSE_DIAG_CONSTANT_EXPRESSION, value_token,
              "static pointer initialization requires a supported address "
              "constant");
        }
      }
      {
        ctool_status_t rewind_status = cfront_vector_rewind(
            &context->expression_children, child_mark);
        if (rewind_status == CTOOL_OK) {
          rewind_status = cfront_vector_rewind(&context->expressions,
                                               expression_mark);
        }
        if (rewind_status != CTOOL_OK) {
          return cfront_storage_failure(context, rewind_status);
    ...
    if (target.kind == CTOOL_C_TYPE_FLOAT ||
      ...
      return cfront_parse_static_scalar_expression(
          context, *object_type_io, value_token, initializer_out);
    }
    return cfront_emit_failure(
        context, CTOOL_ERR_UNSUPPORTED,
        CTOOL_C_PARSE_DIAG_CONSTANT_EXPRESSION, value_token,
        "static initializer type is outside this constant-data slice");
  }
  if (value_token == (const ctool_c_pp_token_t *)0 ||
      cfront_peek_is(context, ";") == CTOOL_TRUE ||
      cfront_peek_is(context, ",") == CTOOL_TRUE ||
  ...
  if (cfront_static_initializer_has_floating_syntax(context) ==
      CTOOL_TRUE) {
    return cfront_parse_static_scalar_expression(
        context, *object_type_io, value_token, initializer_out);
  }
  ...
  if (status == CTOOL_OK) {
    ctool_c_initializer_t initializer;
    value = cfront_integer_convert_to_type(value, &target_integer);
    cfront_initializer_init(&initializer, CTOOL_C_INITIALIZER_INTEGER,
                            *object_type_io, value_token);
    initializer.integer_bits =
        target_integer.width < 64u
            ? value.bits & ((1ull << target_integer.width) - 1ull)
            : value.bits;
    status = cfront_append_initializer(context, &initializer,
                                       initializer_out);
  }
...
static ctool_status_t cfront_append_zero_initializer(
    cfront_context_t *context, ctool_u32 object_type,
  ...
  return status == CTOOL_OK
             ? CTOOL_OK
             : cfront_storage_failure_at(context, status, token);
}

static ctool_bool cfront_find_object_definition(
    const cfront_context_t *context, ctool_u32 binding,
    cfront_object_definition_t *definition_out, ctool_u32 *index_out) {
  ctool_u32 index;
  for (index = 0u; index < context->object_definitions.count; index++) {
    cfront_object_definition_t definition;
    if (cfront_vector_get(&context->object_definitions, index,
                          &definition) != CTOOL_OK) {
      return CTOOL_FALSE;
    }
    if (definition.definition.binding == binding) {
      if (definition_out != (cfront_object_definition_t *)0) {
...
static ctool_status_t cfront_record_object_definition(
    cfront_context_t *context, ctool_u32 binding, ctool_u32 declared_type,
    cfront_storage_t storage, ctool_c_object_definition_kind_t kind,
    ctool_u32 initializer, const ctool_c_pp_token_t *token,
    const ctool_c_pp_location_t *location,
    const ctool_c_pp_location_t *physical_location) {
  cfront_object_definition_t existing;
  cfront_object_definition_t definition;
  ctool_u32 existing_index = CTOOL_C_AST_NONE;
  ctool_bool found = cfront_find_object_definition(
      context, binding, &existing, &existing_index);
  if (found == CTOOL_TRUE &&
      existing.definition.kind == CTOOL_C_OBJECT_DEFINITION_EXPLICIT) {
    if (kind == CTOOL_C_OBJECT_DEFINITION_EXPLICIT) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_REDEFINITION, token,
          "file-scope object already has a definition");
    }
    return CTOOL_OK;
  }
  cfront_zero(&definition, (ctool_u32)sizeof(definition));
  definition.definition.binding = binding;
  definition.definition.declared_type = declared_type;
  definition.definition.storage = cfront_public_storage(storage);
  definition.definition.kind = kind;
  definition.definition.initializer = initializer;
  definition.definition.location = *location;
  definition.definition.physical_location = *physical_location;
  definition.token = token;
  if (found == CTOOL_TRUE) {
    ctool_status_t status = cfront_vector_replace(
        context, &context->object_definitions, existing_index, &definition);
    return status == CTOOL_OK ? CTOOL_OK
                              : cfront_storage_failure(context, status);
  }
  {
    ctool_status_t status = cfront_vector_append(
        &context->object_definitions, &definition, (ctool_u32 *)0);
    return status == CTOOL_OK ? CTOOL_OK
                              : cfront_storage_failure(context, status);
  }
}

static ctool_status_t cfront_update_definition_binding_type(
    cfront_context_t *context, ctool_u32 binding_index,
    ctool_u32 definition_type, const ctool_c_pp_token_t *token) {
  ctool_c_binding_t binding;
  ctool_bool compatible = CTOOL_FALSE;
  ctool_u32 composite = CTOOL_C_TYPE_NONE;
  ctool_status_t status = cfront_binding_get(
      context, binding_index, &binding);
  if (status == CTOOL_OK) {
    status = cfront_types_compatible(
        context, binding.type, definition_type, &compatible);
  }
  if (status == CTOOL_OK && compatible == CTOOL_TRUE) {
    status = cfront_composite_type(
        context, binding.type, definition_type, &composite);
  }
  if (status != CTOOL_OK) {
    if (status == CTOOL_ERR_LIMIT || status == CTOOL_ERR_OVERFLOW ||
        status == CTOOL_ERR_NO_MEMORY) {
    ...
    return cfront_emit_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL, token,
        "file-scope definition type merge failed");
  }
  ...
  if (binding.type != composite) {
    binding.type = composite;
    status = cfront_vector_replace(context, &context->bindings,
...
static ctool_status_t cfront_finalize_object_definitions(
    cfront_context_t *context) {
  ctool_u32 index;
  for (index = 0u; index < context->object_definitions.count; index++) {
    cfront_object_definition_t definition;
    ctool_c_binding_t binding;
    ctool_c_type_node_t type;
    ctool_u32 base;
    ctool_u32 qualifiers;
    cfront_zero(&definition, (ctool_u32)sizeof(definition));
    cfront_zero(&binding, (ctool_u32)sizeof(binding));
    ctool_status_t status = cfront_vector_get(
        &context->object_definitions, index, &definition);
    if (status == CTOOL_OK) {
      status = cfront_binding_get(
          context, definition.definition.binding, &binding);
    }
    if (status != CTOOL_OK || binding.kind != CTOOL_C_BINDING_OBJECT) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
          definition.token, "file-scope object definition is unavailable");
    }
    if (definition.definition.kind ==
        CTOOL_C_OBJECT_DEFINITION_EXPLICIT) {
      continue;
    }
    definition.definition.declared_type = binding.type;
    ...
    if (status != CTOOL_OK) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
          definition.token, "tentative definition type is unavailable");
    }
    if (type.kind == CTOOL_C_TYPE_ARRAY &&
        type.array_bound_kind == CTOOL_C_ARRAY_UNSPECIFIED) {
      ctool_u32 completed_type = CTOOL_C_TYPE_NONE;
      ...
      type.element_count = 1u;
      status = cfront_type_append(context, &type, &completed_type);
      if (status == CTOOL_OK) {
        status = cfront_update_definition_binding_type(
            context, definition.definition.binding, completed_type,
            definition.token);
      }
      if (status != CTOOL_OK) {
        return status;
      ...
      if (status != CTOOL_OK) {
        return cfront_emit_failure(
            context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
            definition.token,
            "completed tentative definition binding is unavailable");
      }
      definition.definition.declared_type = binding.type;
    } else if ((type.kind == CTOOL_C_TYPE_RECORD &&
                type.record_complete == CTOOL_FALSE) ||
               type.kind == CTOOL_C_TYPE_VOID ||
               type.kind == CTOOL_C_TYPE_FUNCTION) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_TYPE_NAME,
          definition.token,
          "tentative definition requires a complete object type");
    }
    status = cfront_append_zero_initializer(
        context, definition.definition.declared_type, definition.token,
        &definition.definition.initializer);
    if (status != CTOOL_OK) {
      return status;
    }
    status = cfront_vector_replace(context, &context->object_definitions,
                                   index, &definition);
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
...
static ctool_status_t cfront_parse_block_declaration(
  ...
  for (;;) {
    ...
    ctool_bool binding_reserved = CTOOL_FALSE;
    ...
    if (specifiers.storage != CFRONT_STORAGE_TYPEDEF) {
      ...
      if (node.kind == CTOOL_C_TYPE_FUNCTION) {
        if (is_for_initializer == CTOOL_TRUE) {
          return cfront_emit_failure(
              context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_STATEMENT,
              name_token, "for initializer cannot declare a function");
        }
        if (specifiers.storage != CFRONT_STORAGE_NONE &&
            specifiers.storage != CFRONT_STORAGE_EXTERN) {
          return cfront_emit_failure(
              ...
              "block function declaration has an invalid storage class");
        }
        if (has_initializer == CTOOL_TRUE) {
          return cfront_emit_failure(
              context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_DECLARATOR,
              cfront_peek(context),
              "block function declaration cannot have an initializer");
        }
        if (cfront_peek_is(context, "{") == CTOOL_TRUE) {
          return cfront_emit_failure(
              context, CTOOL_ERR_UNSUPPORTED,
              ...
              cfront_peek(context),
              "nested function definitions are outside this body slice");
        }
        block_binding.kind = CTOOL_C_BINDING_FUNCTION;
      } else {
        status = cfront_validate_function_specifier_context(
    ...
    if (block_binding.kind == CTOOL_C_BINDING_FUNCTION ||
      ...
      ctool_u32 prior_index;
      ctool_u32 visible_function_type = CTOOL_C_TYPE_NONE;
      ctool_bool has_visible_function_type = CTOOL_FALSE;
      if (block_binding.kind == CTOOL_C_BINDING_FUNCTION) {
        has_visible_function_type = cfront_find_visible_function_type(
            context, name, &visible_function_type);
      }
      if (cfront_current_block_name_exists(context, name) == CTOOL_TRUE &&
          cfront_find_active_block_binding(context, name, &prior_binding,
                                           &prior_index) == CTOOL_TRUE &&
          prior_binding.kind == block_binding.kind &&
          prior_binding.linkage_binding != CTOOL_C_AST_NONE) {
        (void)prior_index;
        compatible_redeclaration = CTOOL_TRUE;
      }
      ...
      status = cfront_append_binding(
          context, block_binding.kind,
          cfront_public_storage(specifiers.storage), name, type,
          binding_semantics, name_token, &location, &physical_location,
          0ull, CTOOL_FALSE, &linkage_binding);
      if (status != CTOOL_OK) {
        return status;
      }
      ...
      if (status != CTOOL_OK) {
        return cfront_emit_failure(
            context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
            name_token, "block linked binding is unavailable");
      }
      if (compatible_redeclaration == CTOOL_TRUE &&
          prior_binding.linkage_binding != linkage_binding) {
        return cfront_emit_failure(
            context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
            name_token, "block redeclaration changed linked identity");
      }
      if (block_binding.kind == CTOOL_C_BINDING_FUNCTION) {
        block_binding.type = type;
        if (has_visible_function_type == CTOOL_TRUE) {
          status = cfront_composite_type(
              context, visible_function_type, type, &block_binding.type);
          if (status != CTOOL_OK) {
            return status;
          }
        }
      ...
      status = cfront_prepare_pending_block_binding(
          context, &block_binding);
      if (status == CTOOL_OK) {
        binding_reserved = CTOOL_TRUE;
        initializer_token = cfront_advance(context);
        status = specifiers.storage == CFRONT_STORAGE_STATIC
      ...
      block_binding.type = type;
    } else if (specifiers.storage == CFRONT_STORAGE_STATIC) {
      status = cfront_append_zero_initializer(
          context, type, name_token, &block_binding.initializer);
      if (status != CTOOL_OK) {
        return status;
      }
    }
    ...
    status = binding_reserved == CTOOL_TRUE
                 ? cfront_publish_pending_block_binding(
                       context, &block_binding, &binding_index)
                 : cfront_append_block_binding(
                       context, &block_binding, name_token,
                       compatible_redeclaration, &binding_index);
    if (binding_reserved == CTOOL_TRUE) {
      cfront_clear_pending_block_binding(context);
    }
    (void)binding_index;
    if (status != CTOOL_OK) {
...
static ctool_status_t cfront_parse_case_statement(
  ...
  if (status == CTOOL_OK) {
    status = cfront_attach_expression_block_bindings(
        context, expression, first_binding,
        context->block_bindings.count - first_binding, 0u, case_token);
  }
  if (status == CTOOL_OK) {
    (void)cfront_advance(context);
    if (cfront_peek(context) == (const ctool_c_pp_token_t *)0 ||
...
static ctool_status_t cfront_parse_do_statement(
    cfront_context_t *context, ctool_u32 *statement_out) {
  const ctool_c_pp_token_t *do_token = cfront_advance(context);
  ctool_u32 diagnostic_count = ctool_job_diagnostic_count(context->job);
  ctool_u32 condition = CTOOL_C_AST_NONE;
  ctool_u32 body = CTOOL_C_AST_NONE;
  ctool_bool loop_entered = CTOOL_FALSE;
  ctool_status_t status = cfront_enter_syntax(context, do_token);
  if (status != CTOOL_OK) {
    return status;
  }
  if (cfront_peek(context) == (const ctool_c_pp_token_t *)0 ||
    ...
    status = cfront_emit_failure(
        context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_STATEMENT,
        cfront_peek(context), "do statement requires a body");
  }
  if (status == CTOOL_OK) {
    context->breakable_statement_depth++;
    context->iteration_statement_depth++;
  ...
  if (status == CTOOL_OK &&
      cfront_peek_is(context, "while") == CTOOL_FALSE) {
    status = cfront_emit_failure(
        context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_EXPECTED_TOKEN,
        cfront_peek(context),
        "do statement requires while after its body");
  }
  if (status == CTOOL_OK) {
    status = cfront_expected(context, "while");
  }
  if (status == CTOOL_OK && cfront_peek_is(context, "(") == CTOOL_FALSE) {
    status = cfront_emit_failure(
        context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_EXPECTED_TOKEN,
  ...
  if (status == CTOOL_OK) {
    ...
    if (cfront_peek_is(context, ")") == CTOOL_TRUE) {
      status = cfront_emit_failure(
          context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_EXPRESSION,
          condition_token, "do controlling expression is required");
    } else {
      status = cfront_parse_body_expression(context, &value);
    }
    if (status == CTOOL_OK) {
  ...
  if (status == CTOOL_OK && cfront_peek_is(context, ";") == CTOOL_FALSE) {
    status = cfront_emit_failure(
        context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_EXPECTED_TOKEN,
        cfront_peek(context), "do statement requires a semicolon");
  }
  if (status == CTOOL_OK) {
    status = cfront_expected(context, ";");
  }
  if (status == CTOOL_OK) {
    ctool_c_statement_t statement;
    cfront_statement_init(&statement, CTOOL_C_STATEMENT_DO, do_token);
    statement.condition = condition;
    statement.body = body;
    status = cfront_append_statement(context, &statement, statement_out);
  }
...
static ctool_status_t cfront_parse_statement(
  ...
  if (cfront_peek_is(context, "do") == CTOOL_TRUE) {
    return cfront_parse_do_statement(context, statement_out);
  }
...
static ctool_status_t cfront_parse_function_definition(
  ...
  ctool_u32 first_block_binding = context->block_bindings.count;
  ctool_u32 function_binding_mark =
  ...
  if (definition_declarator.detail.function.first_parameter_enumerator >
          context->parameter_enumerators.count ||
      definition_declarator.detail.function.parameter_enumerator_count >
          context->parameter_enumerators.count -
              definition_declarator.detail.function
                  .first_parameter_enumerator) {
    return cfront_emit_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
        name_token,
        "function definition parameter enumerator slice is invalid");
  }
  ...
  for (index = 0u;
    ...
    ctool_c_binding_t enumerator;
    status = cfront_vector_get(
        &context->parameter_enumerators,
        definition_declarator.detail.function.first_parameter_enumerator +
            index,
        &enumerator);
    if (status == CTOOL_OK) {
      status = cfront_publish_parameter_enumerator(
          context, &enumerator, name_token);
    }
    if (status != CTOOL_OK) {
      (void)cfront_vector_rewind(&context->tags, function_tag_mark);
      (void)cfront_vector_rewind(&context->active_block_binding_indices,
                                 function_binding_mark);
      if (status == CTOOL_ERR_LIMIT || status == CTOOL_ERR_OVERFLOW ||
          status == CTOOL_ERR_NO_MEMORY) {
        return cfront_storage_failure(context, status);
      }
  ...
  if (status == CTOOL_OK) {
    ...
    definition.first_block_binding = first_block_binding;
...
static ctool_status_t cfront_parse_external_declaration(
  ...
  for (;;) {
    ...
    cfront_attributes_t declarator_attributes;
    ctool_bool has_initializer;
    ctool_bool complete_object = CTOOL_FALSE;
    const ctool_c_pp_token_t *name_token = cfront_peek(context);
    cfront_zero(&binding_semantics,
                (ctool_u32)sizeof(binding_semantics));
    declarator_attributes = specifiers.attributes;
    ...
    if (has_initializer == CTOOL_TRUE &&
      ...
      return cfront_emit_failure(
          context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_DECLARATOR,
          cfront_peek(context),
          "file-scope initializer requires an object declarator");
    }
    if (declarator_attributes.noreturn == CTOOL_TRUE) {
      if (kind != CTOOL_C_BINDING_FUNCTION) {
        return cfront_emit_failure(
    ...
    if (kind == CTOOL_C_BINDING_OBJECT &&
      ...
      if (status != CTOOL_OK) {
        return cfront_emit_failure(
            context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
            name_token, "internal tentative definition binding is unavailable");
      }
      status = cfront_type_is_complete_object_now(
          context, canonical_binding.type, &complete_object);
      if (status != CTOOL_OK) {
        return cfront_storage_failure(context, status);
      }
      if (complete_object == CTOOL_FALSE) {
        return cfront_emit_failure(
            context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_TYPE_NAME,
            name_token,
            "internal tentative definition requires a complete object type");
    ...
    if (kind == CTOOL_C_BINDING_OBJECT) {
      if (has_initializer == CTOOL_TRUE) {
        cfront_object_definition_t existing_definition;
        ctool_u32 initializer = CTOOL_C_AST_NONE;
        ctool_u32 definition_type = type;
        if (cfront_find_object_definition(
                context, binding_index, &existing_definition,
                (ctool_u32 *)0) == CTOOL_TRUE &&
            existing_definition.definition.kind ==
                CTOOL_C_OBJECT_DEFINITION_EXPLICIT) {
          return cfront_emit_failure(
              context, CTOOL_ERR_INPUT, CTOOL_C_PARSE_DIAG_REDEFINITION,
              name_token, "file-scope object already has a definition");
        }
        (void)cfront_advance(context);
        status = cfront_parse_static_initializer(
            context, &definition_type, &initializer);
        if (status == CTOOL_OK) {
          status = cfront_update_definition_binding_type(
              context, binding_index, definition_type, name_token);
        }
        if (status == CTOOL_OK) {
          status = cfront_record_object_definition(
              context, binding_index, definition_type, specifiers.storage,
              CTOOL_C_OBJECT_DEFINITION_EXPLICIT, initializer, name_token,
              &location, &physical_location);
        }
        if (status != CTOOL_OK) {
          return status;
        ...
        status = cfront_record_object_definition(
            ...
            CTOOL_C_OBJECT_DEFINITION_TENTATIVE, CTOOL_C_AST_NONE,
            name_token, &location, &physical_location);
        if (status != CTOOL_OK) {
          return status;
        }
      }
...
static ctool_status_t cfront_freeze(cfront_context_t *context,
  ...
  ctool_c_initializer_element_t *initializer_elements =
      (ctool_c_initializer_element_t *)0;
  ...
  if (context->active_block_binding_indices.count != 0u ||
      context->block_scope_marks.count != 0u ||
      context->has_pending_block_binding == CTOOL_TRUE ||
      context->pending_initializer_elements.count != 0u ||
      context->breakable_statement_depth != 0u ||
      context->iteration_statement_depth != 0u ||
      context->static_initializer_depth != 0u ||
  ...
  if (status == CTOOL_OK) {
    status = cfront_alloc_array(
        context, context->object_definitions.count,
        (ctool_u32)sizeof(*object_definitions),
  ...
  if (status == CTOOL_OK) {
    status = cfront_alloc_array(context, context->initializers.count,
                                (ctool_u32)sizeof(*initializers),
                                (void **)&initializers);
  }
  if (status == CTOOL_OK) {
    status = cfront_alloc_array(
        context, context->initializer_elements.count,
        (ctool_u32)sizeof(*initializer_elements),
        (void **)&initializer_elements);
  }
  if (status == CTOOL_OK) {
    status = cfront_alloc_array(
  ...
  for (index = 0u; index < context->object_definitions.count; index++) {
    cfront_object_definition_t definition;
    status = cfront_vector_get(&context->object_definitions, index,
                               &definition);
    if (status == CTOOL_OK) {
      object_definitions[index] = definition.definition;
      status = cfront_copy_location_owned(
          context, &object_definitions[index].location,
          &object_definitions[index].location);
    }
    if (status == CTOOL_OK) {
      status = cfront_copy_location_owned(
          context, &object_definitions[index].physical_location,
          &object_definitions[index].physical_location);
    }
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
  ...
  for (index = 0u; index < context->initializers.count; index++) {
    ...
    if (status == CTOOL_OK) {
      status = cfront_copy_location_owned(
          context, &initializers[index].location,
          &initializers[index].location);
    }
    if (status == CTOOL_OK) {
      status = cfront_copy_location_owned(
          context, &initializers[index].physical_location,
          &initializers[index].physical_location);
    }
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
  }
  for (index = 0u; index < context->initializer_elements.count; index++) {
    status = cfront_vector_get(&context->initializer_elements, index,
                               &initializer_elements[index]);
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
  }
  ...
  }
  {
    ...
    for (index = 0u; index < context->function_definitions.count; index++) {
      ...
      for (block_binding_index = definition->first_block_binding;
           block_binding_index <
               definition->first_block_binding +
                   definition->block_binding_count;
           block_binding_index++) {
        if (block_bindings[block_binding_index].kind !=
                CTOOL_C_BINDING_ENUMERATOR ||
            block_bindings[block_binding_index].activation_expression !=
                CTOOL_C_AST_NONE ||
            block_bindings[block_binding_index].activation_initializer !=
                CTOOL_C_AST_NONE) {
          return cfront_emit_failure(
              context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
              cfront_peek(context),
              "function definition block-binding prefix is invalid");
        }
      }
  ...
  for (index = 0u; index < context->initializers.count; index++) {
    const ctool_c_initializer_t *initializer = &initializers[index];
    cfront_integer_type_t integer;
    ctool_bool is_integer = CTOOL_FALSE;
    ctool_bool no_address =
        initializer->address_kind == CTOOL_C_INITIALIZER_ADDRESS_NONE &&
                initializer->address_reference == CTOOL_C_AST_NONE &&
                initializer->address_addend == 0
            ? CTOOL_TRUE
            : CTOOL_FALSE;
    ctool_bool no_elements =
        initializer->first_element == CTOOL_C_AST_NONE &&
                initializer->element_count == 0u
            ? CTOOL_TRUE
            : CTOOL_FALSE;
    ctool_bool no_string =
        initializer->string_bytes.data == (const ctool_u8 *)0 &&
                initializer->string_bytes.size == 0u
            ? CTOOL_TRUE
            : CTOOL_FALSE;
    ctool_bool valid = initializer->type < context->types.count
                           ? CTOOL_TRUE
                           : CTOOL_FALSE;
    ...
    if (valid == CTOOL_TRUE &&
      ...
      for (binding_offset = 0u;
           ...
           binding_offset++) {
        const ctool_c_block_binding_t *binding =
            &block_bindings[initializer->first_block_binding +
                            binding_offset];
        if (binding->kind != CTOOL_C_BINDING_ENUMERATOR ||
            binding->activation_expression != CTOOL_C_AST_NONE ||
            binding->activation_initializer != index) {
          valid = CTOOL_FALSE;
        }
      }
    }
    if (valid == CTOOL_TRUE &&
      ...
      valid = initializer->expression < context->expressions.count &&
      ...
      valid = initializer->expression == CTOOL_C_AST_NONE &&
                      initializer->integer_bits == 0ull &&
                      no_string == CTOOL_TRUE &&
                      no_address == CTOOL_TRUE &&
                      no_elements == CTOOL_TRUE
      ...
      ctool_u64 mask;
      status = cfront_integer_type(context, initializer->type, &integer,
                                   &is_integer);
      mask = status == CTOOL_OK && is_integer == CTOOL_TRUE &&
                     integer.width < 64u
                 ? (1ull << integer.width) - 1ull
                 : 0xffffffffffffffffull;
      valid = status == CTOOL_OK && is_integer == CTOOL_TRUE &&
                      initializer->expression == CTOOL_C_AST_NONE &&
                      no_string == CTOOL_TRUE &&
                      no_address == CTOOL_TRUE &&
                      no_elements == CTOOL_TRUE &&
      ...
      status = cfront_inspect_initializer_target(
          ...
          &element_qualifiers);
      (void)array_base;
      valid = status == CTOOL_OK && array.kind == CTOOL_C_TYPE_ARRAY &&
                      array.array_bound_kind == CTOOL_C_ARRAY_FIXED &&
                      cfront_character_type_kind(element.kind) == CTOOL_TRUE &&
                      (element_qualifiers & CTOOL_C_QUAL_ATOMIC) == 0u &&
                      initializer->expression == CTOOL_C_AST_NONE &&
                      ...
                      initializer->string_bytes.data != (const ctool_u8 *)0 &&
                      initializer->string_bytes.size != 0u &&
                      initializer->string_bytes.size <= array.element_count &&
                      no_address == CTOOL_TRUE &&
                      no_elements == CTOOL_TRUE
                  ? CTOOL_TRUE
                  : CTOOL_FALSE;
      ...
      ctool_c_type_node_t pointer = {0};
      ctool_u32 pointer_base;
      ctool_u32 pointer_qualifiers;
      status = cfront_underlying_type(
          context, initializer->type, &pointer_base, &pointer_qualifiers,
          &pointer);
      (void)pointer_base;
      (void)pointer_qualifiers;
      valid = status == CTOOL_OK && pointer.kind == CTOOL_C_TYPE_POINTER &&
                      initializer->expression == CTOOL_C_AST_NONE &&
                      initializer->integer_bits == 0ull &&
                      no_elements == CTOOL_TRUE
                  ? CTOOL_TRUE
      ...
      if (valid == CTOOL_TRUE &&
        ...
        ctool_bool compatible = CTOOL_FALSE;
        status = cfront_static_string_pointer_compatible(
            context, &pointer, &compatible);
        valid = status == CTOOL_OK && compatible == CTOOL_TRUE &&
                        initializer->string_bytes.data !=
                            (const ctool_u8 *)0 &&
                        initializer->string_bytes.size != 0u &&
                        initializer->string_bytes.data[
                            initializer->string_bytes.size - 1u] == 0u &&
                        initializer->address_reference == CTOOL_C_AST_NONE &&
                        initializer->address_addend == 0
                    ? CTOOL_TRUE
                    : CTOOL_FALSE;
        ...
        const ctool_c_binding_t *binding =
            initializer->address_reference < context->bindings.count
                ? &bindings[initializer->address_reference]
                : (const ctool_c_binding_t *)0;
        valid = binding != (const ctool_c_binding_t *)0 &&
                        (binding->kind == CTOOL_C_BINDING_OBJECT ||
                         binding->kind == CTOOL_C_BINDING_FUNCTION) &&
                        binding->linkage != CTOOL_C_LINKAGE_NONE &&
                        no_string == CTOOL_TRUE
      ...
      ctool_c_type_node_t aggregate = {0};
      ctool_u32 aggregate_base;
      ctool_u32 aggregate_qualifiers;
      ...
      valid = status == CTOOL_OK &&
                      ...
                      initializer->first_element ==
                          initializer_element_cursor &&
                      initializer->first_element <=
                          context->initializer_elements.count &&
                      initializer->element_count <=
                          context->initializer_elements.count -
                              initializer->first_element &&
                      ((aggregate.kind == CTOOL_C_TYPE_ARRAY &&
      ...
      if (valid == CTOOL_TRUE) {
        initializer_element_cursor += initializer->element_count;
      }
    } else {
      valid = CTOOL_FALSE;
    }
    if (valid == CTOOL_FALSE) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
          cfront_peek(context), "frozen object initializer is invalid");
    }
  }
  if (initializer_element_cursor != context->initializer_elements.count) {
    return cfront_emit_failure(
        context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
        cfront_peek(context), "frozen initializer elements are unowned");
  }
  for (index = 0u; index < context->object_definitions.count; index++) {
    const ctool_c_object_definition_t *definition =
        &object_definitions[index];
    const ctool_c_binding_t *binding =
        definition->binding < context->bindings.count
            ? &bindings[definition->binding]
            : (const ctool_c_binding_t *)0;
    const ctool_c_initializer_t *initializer =
        definition->initializer < context->initializers.count
            ? &initializers[definition->initializer]
            : (const ctool_c_initializer_t *)0;
    ctool_u32 previous;
    ctool_bool duplicate = CTOOL_FALSE;
    for (previous = 0u; previous < index; previous++) {
      if (object_definitions[previous].binding == definition->binding) {
        duplicate = CTOOL_TRUE;
      }
    }
    if (binding == (const ctool_c_binding_t *)0 ||
        binding->kind != CTOOL_C_BINDING_OBJECT ||
        definition->declared_type >= context->types.count ||
        definition->initializer >= context->initializers.count ||
        ...
        (definition->storage != CTOOL_C_STORAGE_NONE &&
         definition->storage != CTOOL_C_STORAGE_EXTERN &&
        ...
        (definition->kind == CTOOL_C_OBJECT_DEFINITION_TENTATIVE &&
         (definition->storage == CTOOL_C_STORAGE_EXTERN ||
          definition->declared_type != binding->type ||
          initializer->kind != CTOOL_C_INITIALIZER_ZERO)) ||
        duplicate == CTOOL_TRUE) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
          cfront_peek(context),
          "frozen file-scope object definition is invalid");
    }
  }
  for (index = 0u; index < context->block_bindings.count; index++) {
          ...
          (binding->activation_expression != CTOOL_C_AST_NONE &&
           (binding->activation_expression >= context->expressions.count ||
            expressions[binding->activation_expression]
                    .first_block_binding > index ||
            index - expressions[binding->activation_expression]
                        .first_block_binding >=
                expressions[binding->activation_expression]
                    .block_binding_count)) ||
          (binding->activation_initializer != CTOOL_C_AST_NONE &&
           (binding->activation_initializer >= context->initializers.count ||
            initializers[binding->activation_initializer]
                    .first_block_binding > index ||
            index - initializers[binding->activation_initializer]
                        .first_block_binding >=
                initializers[binding->activation_initializer]
                    .block_binding_count)) ||
    ...
    if (binding->kind == CTOOL_C_BINDING_FUNCTION) {
      ctool_c_type_node_t block_function;
      ctool_c_type_node_t linked_function;
      ctool_u32 block_base;
      ctool_u32 block_qualifiers;
      ctool_u32 linked_base;
      ctool_u32 linked_qualifiers;
      ctool_bool function_types_valid = CTOOL_FALSE;
      if (binding->type < context->types.count &&
          linked != (const ctool_c_binding_t *)0 &&
          linked->type < context->types.count) {
        status = cfront_underlying_type(
            context, binding->type, &block_base, &block_qualifiers,
            &block_function);
        if (status == CTOOL_OK) {
          status = cfront_underlying_type(
              context, linked->type, &linked_base, &linked_qualifiers,
              &linked_function);
        }
        (void)block_base;
        (void)block_qualifiers;
        (void)linked_base;
        (void)linked_qualifiers;
        if (status != CTOOL_OK) {
          return status;
        }
        function_types_valid =
      ...
      if ((binding->storage != CTOOL_C_STORAGE_NONE &&
          ...
          function_types_valid == CTOOL_FALSE ||
  ...
  }
  {
    ...
    for (index = 0u;
      ...
      for (child_offset = 0u; child_offset < initializer->element_count;
        ...
        ctool_u32 child = initializer_elements[
                              ...
                              .initializer;
        if (child >= context->initializers.count || owners[child] != 0u) {
    ...
    for (index = 0u;
         status == CTOOL_OK && ownership_valid == CTOOL_TRUE &&
         index < context->object_definitions.count;
         index++) {
      ctool_u32 root = object_definitions[index].initializer;
      if (root >= context->initializers.count || owners[root] != 0u) {
        ownership_valid = CTOOL_FALSE;
        break;
      }
      owners[root] = 1u;
      storage_domains[root] = CFRONT_INITIALIZER_DOMAIN_STATIC;
    }
    for (index = 0u;
         status == CTOOL_OK && ownership_valid == CTOOL_TRUE &&
         index < context->block_bindings.count;
         index++) {
      ctool_u32 root = block_bindings[index].initializer;
    ...
    rewind_status = ctool_arena_rewind(arena, ownership_mark);
    if (rewind_status != CTOOL_OK) {
      return cfront_emit_failure(
          context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
          cfront_peek(context), "initializer ownership scratch rewind failed");
    }
    if (status != CTOOL_OK) {
      return cfront_storage_failure(context, status);
    }
      ...
      if (statement->first_child != CTOOL_C_AST_NONE ||
        ...
        return cfront_emit_failure(
            ...
            statement->kind == CTOOL_C_STATEMENT_WHILE
                ? "frozen while statement is invalid"
                : "frozen do statement is invalid");
      }
  ...
  for (index = 0u; index < context->expressions.count; index++) {
    ...
    if ((expression->first_block_binding == CTOOL_C_AST_NONE &&
      ...
      return cfront_emit_failure(
          context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
          cfront_peek(context),
          "frozen expression block-binding slice is invalid");
    }
    if (expression->first_block_binding != CTOOL_C_AST_NONE) {
      ctool_u32 binding_offset;
      for (binding_offset = 0u;
    ...
    case CTOOL_C_EXPRESSION_FLOATING_CONSTANT:
      if (expression->child_count != 0u ||
          expression->first_child != CTOOL_C_AST_NONE ||
          expression->reference != CTOOL_C_AST_NONE ||
          expression->operation != CTOOL_C_EXPRESSION_OPERATOR_NONE ||
      ...
      if (expression->kind ==
        ...
        if (status != CTOOL_OK || floating == CTOOL_FALSE) {
          return cfront_emit_failure(
              ...
              "frozen floating literal type is invalid");
        }
      }
      break;
    ...
    case CTOOL_C_EXPRESSION_BINARY:
      ...
      break;
    case CTOOL_C_EXPRESSION_CONDITIONAL:
      if (expression->child_count != 3u ||
          expression->reference != CTOOL_C_AST_NONE ||
          expression->operation != CTOOL_C_EXPRESSION_OPERATOR_NONE ||
          expression->conversion != CTOOL_C_CONVERSION_NONE ||
        ...
        return cfront_emit_failure(
            context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
            cfront_peek(context), "frozen conditional expression is invalid");
      }
      break;
    case CTOOL_C_EXPRESSION_ASSIGNMENT:
      ...
      break;
    case CTOOL_C_EXPRESSION_UPDATE:
      if (expression->child_count != 1u ||
          expression->reference != CTOOL_C_AST_NONE ||
          expression->operation <
              CTOOL_C_EXPRESSION_OPERATOR_PREFIX_INCREMENT ||
        ...
        return cfront_emit_failure(
            context, CTOOL_ERR_INTERNAL, CTOOL_C_PARSE_DIAG_INTERNAL,
            cfront_peek(context), "frozen update expression is invalid");
      }
      break;
    default:
      return cfront_emit_failure(
  ...
  unit->object_definitions = object_definitions;
  unit->object_definition_count = context->object_definitions.count;
  unit->block_bindings = block_bindings;
  unit->block_binding_count = context->block_bindings.count;
  unit->initializers = initializers;
...
static ctool_status_t cfront_parse_constant_conditional(
    cfront_context_t *context, cfront_integer_t *value_out) {
  ctool_status_t status =
      cfront_parse_constant_logical_or(context, value_out);
  if (status == CTOOL_OK && cfront_peek_is(context, "?") == CTOOL_TRUE) {
    const ctool_c_pp_token_t *operator_token = cfront_advance(context);
    cfront_integer_t condition = *value_out;
    cfront_integer_t when_nonzero = {0ull, CFRONT_INTEGER_SIGNED_32};
    cfront_integer_t when_zero = {0ull, CFRONT_INTEGER_SIGNED_32};
    cfront_integer_kind_t result_kind;
    ctool_bool suppress_nonzero =
        context->constant_evaluation_suppression_depth != 0u ||
                condition.bits == 0ull
            ? CTOOL_TRUE
            : CTOOL_FALSE;
    ctool_bool suppress_zero =
        context->constant_evaluation_suppression_depth != 0u ||
                condition.bits != 0ull
            ? CTOOL_TRUE
            : CTOOL_FALSE;
    ctool_bool nonzero_suppression_entered = CTOOL_FALSE;
    ctool_bool zero_suppression_entered = CTOOL_FALSE;
    status = cfront_enter_syntax(context, operator_token);
    if (status != CTOOL_OK) {
      return status;
    }
    if (suppress_nonzero == CTOOL_TRUE) {
      context->constant_evaluation_suppression_depth++;
      nonzero_suppression_entered = CTOOL_TRUE;
    }
    status = cfront_parse_constant_conditional(context, &when_nonzero);
    if (nonzero_suppression_entered == CTOOL_TRUE) {
      context->constant_evaluation_suppression_depth--;
    }
    if (status == CTOOL_OK) {
      status = cfront_expected(context, ":");
    }
    if (status == CTOOL_OK && suppress_zero == CTOOL_TRUE) {
      context->constant_evaluation_suppression_depth++;
      zero_suppression_entered = CTOOL_TRUE;
    }
    if (status == CTOOL_OK) {
      status = cfront_parse_constant_conditional(context, &when_zero);
    }
    if (zero_suppression_entered == CTOOL_TRUE) {
      context->constant_evaluation_suppression_depth--;
    }
    if (status == CTOOL_OK) {
      result_kind = cfront_integer_usual_kind(when_nonzero.kind,
                                              when_zero.kind);
      *value_out = cfront_integer_convert(
          condition.bits != 0ull ? when_nonzero : when_zero, result_kind);
    }
    cfront_leave_syntax(context);
  }
  return status;
