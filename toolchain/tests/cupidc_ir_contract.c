...
static const char wide_function_pointer_source[] =
    ...
    "long long call_wide(wide_callback_t callback) { return callback(1LL); }\n";
...
static const char wide_operation_source[] =
    "typedef unsigned char ctool_u8;\n"
    "typedef unsigned int ctool_u32;\n"
    "typedef unsigned long long ctool_u64;\n"
    "typedef long long ctool_i64;\n"
...
static const char wide_count_operation_source[] =
    "typedef unsigned long long ctool_u64;\n"
...
static int active_source_is_unchanged(ctool_job_t *job) {
  ...
  if (!check_status(status, CTOOL_OK, "load active core source") ||
      ...
      strstr((const char *)source.contents.data,
             active_wide_put_body) == NULL ||
      strstr((const char *)source.contents.data,
             active_wide_patch_body) == NULL) {
...
static int run_active_leaf(const char *host_root) {
  ...
  fingerprint = unit_fingerprint(&wide_call_unit);
  diagnostic_count = ctool_job_diagnostic_count(job);
  (void)memset(&ir, 0xa5, sizeof(ir));
  status = ctool_c_lower_ir(job, &wide_call_unit, &ir);
  if (!check_status(status, CTOOL_OK, "wide direct call lowering") ||
      ctool_job_diagnostic_count(job) != diagnostic_count ||
      unit_fingerprint(&wide_call_unit) != fingerprint ||
      wide_call_unit.function_definition_count != 1u ||
      ir.function_count != 1u || ir.instruction_count != 3u ||
      ir.functions == NULL || ir.instructions == NULL ||
      ir.functions[0].instruction_count != 3u ||
      ir.functions[0].maximum_stack_depth != 1u ||
      ir.instructions[0].kind != CTOOL_C_IR_INSTRUCTION_INTEGER ||
      ir.instructions[0].integer_bits != 1u ||
      ir.instructions[1].kind != CTOOL_C_IR_INSTRUCTION_CALL_DIRECT ||
      ir.instructions[1].argument_count != 1u ||
      ir.instructions[2].kind != CTOOL_C_IR_INSTRUCTION_RETURN_VALUE) {
    (void)fprintf(stderr, "wide direct call IR differs\n");
    (void)ctool_job_render_diagnostics(job);
    goto cleanup;
  }
  ...
  if (!parse_source(job, "/wide-parameter.c", abi_source, &abi_unit)) {
    goto cleanup;
  }
  fingerprint = unit_fingerprint(&abi_unit);
  diagnostic_count = ctool_job_diagnostic_count(job);
  (void)memset(&ir, 0xa5, sizeof(ir));
  status = ctool_c_lower_ir(job, &abi_unit, &ir);
  if (!check_status(status, CTOOL_OK, "wide parameter lowering") ||
      ctool_job_diagnostic_count(job) != diagnostic_count ||
      unit_fingerprint(&abi_unit) != fingerprint ||
      abi_unit.function_definition_count != 1u ||
      abi_unit.parameter_count != 1u || ir.function_count != 1u ||
      ir.instruction_count != 3u || ir.functions == NULL ||
      ir.instructions == NULL || ir.functions[0].instruction_count != 3u ||
      ir.functions[0].maximum_stack_depth != 1u ||
      ir.instructions[0].kind !=
          CTOOL_C_IR_INSTRUCTION_PARAMETER_ADDRESS ||
      ir.instructions[0].type != abi_unit.parameters[0].type ||
      ir.instructions[0].reference != 0u ||
      ir.instructions[1].kind != CTOOL_C_IR_INSTRUCTION_LOAD ||
      ir.instructions[1].type != abi_unit.parameters[0].type ||
      ir.instructions[1].input_type != abi_unit.parameters[0].type ||
      ir.instructions[2].kind != CTOOL_C_IR_INSTRUCTION_RETURN_VALUE ||
      ir.instructions[2].type != abi_unit.parameters[0].type) {
    (void)fprintf(stderr, "wide parameter IR differs\n");
    (void)ctool_job_render_diagnostics(job);
    goto cleanup;
  }
...
static int run_function_pointers(const char *host_root) {
  ...
  if (!check_status(status, CTOOL_OK, "wide indirect call lowering") ||
      ctool_job_diagnostic_count(job) != diagnostic_count ||
      unit_fingerprint(&wide_unit) != fingerprint ||
      wide_unit.function_definition_count != 1u || ir.function_count != 1u ||
      ir.instruction_count != 5u || ir.functions == NULL ||
      ir.instructions == NULL || ir.functions[0].instruction_count != 5u ||
      ir.functions[0].maximum_stack_depth != 2u ||
      ir.instructions[0].kind !=
          CTOOL_C_IR_INSTRUCTION_PARAMETER_ADDRESS ||
      ir.instructions[0].reference >= wide_unit.parameter_count ||
      ir.instructions[0].type !=
          wide_unit.parameters[ir.instructions[0].reference].type ||
      ir.instructions[1].kind != CTOOL_C_IR_INSTRUCTION_LOAD ||
      ir.instructions[1].type != ir.instructions[0].type ||
      ir.instructions[1].input_type != ir.instructions[0].type ||
      ir.instructions[2].kind != CTOOL_C_IR_INSTRUCTION_INTEGER ||
      ir.instructions[2].integer_bits != 1u ||
      ir.instructions[3].kind != CTOOL_C_IR_INSTRUCTION_CALL_INDIRECT ||
      ir.instructions[3].type != ir.instructions[2].type ||
      ir.instructions[3].input_type != ir.instructions[1].type ||
      ir.instructions[3].argument_count != 1u ||
      ir.instructions[4].kind != CTOOL_C_IR_INSTRUCTION_RETURN_VALUE ||
      ir.instructions[4].type != ir.instructions[3].type) {
    (void)fprintf(stderr, "wide indirect call IR differs\n");
    (void)ctool_job_render_diagnostics(job);
    goto cleanup;
  }
...
static int validate_wide_conversion_ir(
    const ctool_c_translation_unit_t *unit,
    const ctool_c_ir_unit_t *ir) {
  ctool_u32 signed_widenings = 0u;
  ctool_u32 unsigned_widenings = 0u;
  ctool_u32 explicit_signed_widenings = 0u;
  ctool_u32 explicit_unsigned_widenings = 0u;
  ctool_u32 wide_assignment_retypes = 0u;
  ctool_u32 dword_narrowings = 0u;
  ctool_u32 unsigned_word_narrowings = 0u;
  ctool_u32 assignment_narrowings = 0u;
  ctool_u32 unsigned_byte_narrowings = 0u;
  ctool_u32 signed_word_narrowings = 0u;
  ctool_u32 signed_byte_narrowings = 0u;
  ctool_u32 boolean_narrowings = 0u;
  ctool_u32 index;
  if (unit == NULL || ir == NULL || unit->function_definition_count != 14u ||
      ir->function_count != 14u || ir->functions == NULL ||
      ir->instructions == NULL) {
    return 0;
  }
  for (index = 0u; index < ir->instruction_count; index++) {
    const ctool_c_ir_instruction_t *instruction = &ir->instructions[index];
    const ctool_c_type_layout_t *source;
    const ctool_c_type_layout_t *target;
    const ctool_c_type_node_t *target_node;
    if (instruction->kind != CTOOL_C_IR_INSTRUCTION_CONVERT) {
      continue;
    }
    ...
    source = &unit->layout.types[instruction->input_type];
    target = &unit->layout.types[instruction->type];
    target_node = &unit->graph.types[instruction->type];
    if (source->is_integer != CTOOL_TRUE ||
        target->is_integer != CTOOL_TRUE) {
      return 0;
    }
    if (source->size <= 4u && target->size == 8u &&
        instruction->conversion == CTOOL_C_CONVERSION_ASSIGNMENT) {
      if (source->is_signed == CTOOL_TRUE) {
        signed_widenings++;
      } else {
        unsigned_widenings++;
      }
    } else if (source->size <= 4u && target->size == 8u &&
               instruction->conversion == CTOOL_C_CONVERSION_NONE) {
      if (source->is_signed == CTOOL_TRUE) {
        explicit_signed_widenings++;
      } else {
        explicit_unsigned_widenings++;
      }
    } else if (source->size == 8u && target->size == 8u &&
               source->is_signed == CTOOL_FALSE &&
               target->is_signed == CTOOL_TRUE &&
               instruction->conversion == CTOOL_C_CONVERSION_ASSIGNMENT) {
      wide_assignment_retypes++;
    } else if (source->size == 8u && target->size == 4u &&
               target->is_signed == CTOOL_FALSE &&
               instruction->conversion == CTOOL_C_CONVERSION_NONE) {
      dword_narrowings++;
    } else if (source->size == 8u && target->size == 2u &&
               target->is_signed == CTOOL_FALSE &&
               instruction->conversion == CTOOL_C_CONVERSION_NONE) {
      unsigned_word_narrowings++;
    } else if (source->size == 8u && target->size == 2u &&
               target->is_signed == CTOOL_FALSE &&
               instruction->conversion == CTOOL_C_CONVERSION_ASSIGNMENT) {
      assignment_narrowings++;
    } else if (source->size == 8u && target->size == 1u &&
               target->is_signed == CTOOL_FALSE &&
               target_node->kind != CTOOL_C_TYPE_BOOL &&
               instruction->conversion == CTOOL_C_CONVERSION_NONE) {
      unsigned_byte_narrowings++;
    } else if (source->size == 8u && target->size == 2u &&
               target->is_signed == CTOOL_TRUE &&
               instruction->conversion == CTOOL_C_CONVERSION_NONE) {
      signed_word_narrowings++;
    } else if (source->size == 8u && target->size == 1u &&
               target->is_signed == CTOOL_TRUE &&
               instruction->conversion == CTOOL_C_CONVERSION_NONE) {
      signed_byte_narrowings++;
    } else if (source->size == 8u && target->size == 1u &&
               target_node->kind == CTOOL_C_TYPE_BOOL &&
               instruction->conversion == CTOOL_C_CONVERSION_NONE) {
      boolean_narrowings++;
  ...
  return signed_widenings == 2u && unsigned_widenings == 2u &&
                 explicit_signed_widenings == 1u &&
                 explicit_unsigned_widenings == 1u &&
                 wide_assignment_retypes == 1u &&
                 dword_narrowings == 1u &&
                 unsigned_word_narrowings == 1u &&
                 assignment_narrowings == 1u &&
                 unsigned_byte_narrowings == 1u &&
                 signed_word_narrowings == 1u &&
                 signed_byte_narrowings == 1u &&
                 boolean_narrowings == 1u
             ? 1
             : 0;
}

static int validate_wide_operation_ir(
    const ctool_c_translation_unit_t *unit,
    const ctool_c_ir_unit_t *ir) {
  ctool_u32 left_shifts = 0u;
  ctool_u32 right_shifts = 0u;
  ctool_u32 bitwise_ands = 0u;
  ctool_u32 bitwise_ors = 0u;
  ctool_u32 bitwise_xors = 0u;
  ctool_u32 widenings = 0u;
  ctool_u32 wide_usual_conversions = 0u;
  ctool_u32 wide_promotions = 0u;
  ctool_u32 narrowings = 0u;
  ctool_u32 index;
  if (unit == NULL || ir == NULL || unit->function_definition_count != 9u ||
    ...
    if (instruction->kind == CTOOL_C_IR_INSTRUCTION_BINARY &&
      ...
      if (source->size == 4u && target->size == 8u &&
        ...
        widenings++;
                 ...
                 source->is_signed == CTOOL_TRUE &&
                 target->is_signed == CTOOL_FALSE &&
                 instruction->conversion ==
                     CTOOL_C_CONVERSION_USUAL_ARITHMETIC) {
        wide_usual_conversions++;
      } else if (source->size == 8u && target->size == 8u &&
                 instruction->conversion ==
                     CTOOL_C_CONVERSION_INTEGER_PROMOTION) {
        wide_promotions++;
  ...
  if (left_shifts != 1u || right_shifts != 3u || bitwise_ands != 4u ||
      bitwise_ors != 1u || bitwise_xors != 1u || widenings != 1u ||
      wide_usual_conversions != 2u || wide_promotions != 1u ||
      narrowings != 1u) {
    (void)fprintf(stderr,
                  "wide operation IR inventory differs: shl=%u shr=%u "
                  "and=%u or=%u xor=%u widen=%u wide-uac=%u "
                  "wide-promote=%u narrow=%u\n",
                  (unsigned int)left_shifts, (unsigned int)right_shifts,
                  (unsigned int)bitwise_ands, (unsigned int)bitwise_ors,
                  (unsigned int)bitwise_xors, (unsigned int)widenings,
                  (unsigned int)wide_usual_conversions,
                  (unsigned int)wide_promotions,
                  (unsigned int)narrowings);
    return 0;
  }
  return 1;
}
...
static int validate_active_wide_helper_ir(
  ...
  for (index = 0u; index < ir->instruction_count; index++) {
    ...
    if (instruction->kind == CTOOL_C_IR_INSTRUCTION_CALL_DIRECT) {
      ...
      if (instruction->operation == CTOOL_C_EXPRESSION_OPERATOR_SHIFT_RIGHT) {
        shifts++;
  ...
  return shifts == 2u && bitwise_ands == 2u && widenings == 2u &&
                 narrowings == 2u && direct_calls == 3u
             ? 1
             : 0;
}

static int wide_call_named_argument_count(
    const ctool_c_translation_unit_t *unit,
    const ctool_c_ir_instruction_t *instruction,
    ctool_u32 *wide_count_out) {
  const ctool_c_type_node_t *function;
  ctool_u32 parameter;
  if (unit == NULL || instruction == NULL || wide_count_out == NULL ||
      instruction->input_type >= unit->graph.type_count ||
      (instruction->kind != CTOOL_C_IR_INSTRUCTION_CALL_DIRECT &&
       instruction->kind != CTOOL_C_IR_INSTRUCTION_CALL_INDIRECT)) {
    return 0;
  ...
  *wide_count_out = 0u;
  function = &unit->graph.types[instruction->input_type];
  if (instruction->kind == CTOOL_C_IR_INSTRUCTION_CALL_INDIRECT) {
    if (function->kind != CTOOL_C_TYPE_POINTER ||
        function->referenced_type >= unit->graph.type_count) {
      return 0;
    }
    function = &unit->graph.types[function->referenced_type];
  }
  ...
  for (parameter = 0u; parameter < function->parameter_count; parameter++) {
    if (ir_type_is_wide_integer(
            unit, unit->graph.parameter_types
                      [function->first_parameter + parameter]) != 0) {
      (*wide_count_out)++;
    }
  }
  return 1;
...
static int validate_wide_parameter_ir(
    const ctool_c_translation_unit_t *unit,
    const ctool_c_ir_unit_t *ir) {
  ctool_u32 parameter_addresses = 0u;
  ctool_u32 wide_parameter_addresses = 0u;
  ctool_u32 wide_loads = 0u;
  ctool_u32 direct_calls = 0u;
  ctool_u32 indirect_calls = 0u;
  ctool_u32 wide_call_arguments = 0u;
  ctool_u32 narrow_direct_calls = 0u;
  ctool_u32 narrow_call_arguments = 0u;
  ctool_u32 wide_returns = 0u;
  ctool_u32 narrow_returns = 0u;
  ctool_u32 variadic_starts = 0u;
  ctool_u32 variadic_arguments = 0u;
  ctool_u32 variadic_ends = 0u;
  ctool_u32 expected_first = 0u;
  ctool_u32 index;
  if (unit == NULL || ir == NULL ||
      unit->function_definition_count != 9u ||
      unit->parameter_count != 19u || ir->function_count != 9u ||
      ir->functions == NULL || ir->instructions == NULL) {
    (void)fprintf(stderr,
                  "wide parameter IR shape differs: definitions=%u "
                  "parameters=%u functions=%u instructions=%u\n",
                  unit == NULL ? 0u
                               : (unsigned int)unit->function_definition_count,
                  unit == NULL ? 0u : (unsigned int)unit->parameter_count,
                  ir == NULL ? 0u : (unsigned int)ir->function_count,
                  ir == NULL ? 0u : (unsigned int)ir->instruction_count);
    return 0;
  }
  for (index = 0u; index < ir->function_count; index++) {
    const ctool_c_function_definition_t *definition =
    ...
    if (function->binding != definition->binding ||
        ...
        function->maximum_stack_depth == 0u ||
        function->instruction_count >
            ir->instruction_count - expected_first) {
      (void)fprintf(stderr,
                    "wide parameter function %u differs: first=%u "
                    "count=%u depth=%u expected-first=%u\n",
                    (unsigned int)index,
  ...
  for (index = 0u; index < ir->instruction_count; index++) {
    ...
    if (instruction->kind ==
        CTOOL_C_IR_INSTRUCTION_PARAMETER_ADDRESS) {
      if (instruction->reference >= unit->parameter_count ||
          unit->parameters[instruction->reference].type !=
              instruction->type ||
          instruction->input_type != CTOOL_C_TYPE_NONE) {
        return 0;
      }
      parameter_addresses++;
      if (ir_type_is_wide_integer(unit, instruction->type) != 0) {
        wide_parameter_addresses++;
      }
    } else if (instruction->kind == CTOOL_C_IR_INSTRUCTION_LOAD &&
               ir_type_is_wide_integer(unit, instruction->type) != 0) {
      if (ir_type_is_wide_integer(unit, instruction->input_type) == 0 ||
      ...
      wide_loads++;
               ...
               instruction->kind ==
                   CTOOL_C_IR_INSTRUCTION_CALL_INDIRECT) {
      ctool_u32 call_wide_arguments;
      if (!wide_call_named_argument_count(
              unit, instruction, &call_wide_arguments) ||
          wide_call_arguments > 0xffffffffu - call_wide_arguments) {
        return 0;
      }
      wide_call_arguments += call_wide_arguments;
      if (ir_type_is_wide_integer(unit, instruction->type) != 0) {
        ...
        narrow_direct_calls++;
        narrow_call_arguments += instruction->argument_count;
      }
    } else if (instruction->kind ==
               CTOOL_C_IR_INSTRUCTION_RETURN_VALUE) {
      if (ir_type_is_wide_integer(unit, instruction->type) != 0) {
  ...
  if (parameter_addresses != 14u || wide_parameter_addresses != 11u ||
      wide_loads != 11u || direct_calls != 3u || indirect_calls != 1u ||
      wide_call_arguments != 7u || narrow_direct_calls != 1u ||
      narrow_call_arguments != 2u || wide_returns != 7u ||
      narrow_returns != 2u || variadic_starts != 1u ||
      variadic_arguments != 1u || variadic_ends != 1u) {
    (void)fprintf(
        stderr,
        "wide parameter IR inventory differs: parameters=%u wide=%u "
        "loads=%u direct=%u indirect=%u wide-arguments=%u narrow-calls=%u "
        "narrow-arguments=%u returns=%u/%u variadic=%u/%u/%u\n",
        (unsigned int)parameter_addresses,
        (unsigned int)wide_parameter_addresses, (unsigned int)wide_loads,
        (unsigned int)direct_calls, (unsigned int)indirect_calls,
        (unsigned int)wide_call_arguments, (unsigned int)narrow_direct_calls,
        (unsigned int)narrow_call_arguments, (unsigned int)wide_returns,
        (unsigned int)narrow_returns, (unsigned int)variadic_starts,
        (unsigned int)variadic_arguments, (unsigned int)variadic_ends);
    return 0;
  }
  return 1;
}
...
static int run_wide_returns(const char *host_root) {
  ...
  ctool_c_ir_unit_t operation_repeat_ir;
  ctool_c_ir_unit_t active_helper_ir;
  ctool_c_ir_unit_t active_helper_repeat_ir;
  ...
  (void)memset(&operation_ir, 0xa5, sizeof(operation_ir));
  (void)memset(&operation_repeat_ir, 0xa5,
               sizeof(operation_repeat_ir));
  (void)memset(&active_helper_ir, 0xa5, sizeof(active_helper_ir));
  (void)memset(&active_helper_repeat_ir, 0xa5,
               sizeof(active_helper_repeat_ir));
  if (!open_job(host_root, &adapter, &config, &job) ||
      !active_source_is_unchanged(job) ||
  ...
  if (!parse_source_mode(job, "/wide-parameter.c", wide_parameter_source,
                         CTOOL_TRUE, &parameter_unit)) {
    goto cleanup;
  }
  fingerprint = unit_fingerprint(&parameter_unit);
  diagnostic_count = ctool_job_diagnostic_count(job);
  status = ctool_c_lower_ir(job, &parameter_unit, &parameter_ir);
  if (!check_status(status, CTOOL_OK, "wide parameter lowering") ||
      ctool_job_diagnostic_count(job) != diagnostic_count ||
      unit_fingerprint(&parameter_unit) != fingerprint ||
      !validate_wide_parameter_ir(&parameter_unit, &parameter_ir)) {
    (void)ctool_job_render_diagnostics(job);
    goto cleanup;
  }
  status = ctool_c_lower_ir(job, &parameter_unit,
                            &parameter_repeat_ir);
  if (!check_status(status, CTOOL_OK,
                    "repeat wide parameter lowering") ||
      ctool_job_diagnostic_count(job) != diagnostic_count ||
      unit_fingerprint(&parameter_unit) != fingerprint ||
      !validate_wide_parameter_ir(&parameter_unit,
                                  &parameter_repeat_ir) ||
      ir_instruction_fingerprint(&parameter_ir) !=
          ir_instruction_fingerprint(&parameter_repeat_ir)) {
    (void)fprintf(stderr,
  ...
  fingerprint = unit_fingerprint(&conversion_unit);
  diagnostic_count = ctool_job_diagnostic_count(job);
  status = ctool_c_lower_ir(job, &conversion_unit, &conversion_ir);
  if (!check_status(status, CTOOL_OK, "wide integer conversion") ||
      ctool_job_diagnostic_count(job) != diagnostic_count ||
      unit_fingerprint(&conversion_unit) != fingerprint ||
      !validate_wide_conversion_ir(&conversion_unit, &conversion_ir)) {
    (void)ctool_job_render_diagnostics(job);
    goto cleanup;
  }
  status = ctool_c_lower_ir(job, &conversion_unit,
                            &conversion_repeat_ir);
  if (!check_status(status, CTOOL_OK,
                    "repeat wide integer conversion") ||
      ctool_job_diagnostic_count(job) != diagnostic_count ||
      unit_fingerprint(&conversion_unit) != fingerprint ||
      !validate_wide_conversion_ir(&conversion_unit,
                                   &conversion_repeat_ir) ||
      ir_instruction_fingerprint(&conversion_ir) !=
          ir_instruction_fingerprint(&conversion_repeat_ir)) {
    (void)fprintf(stderr,
                  "wide integer conversion lowering is not deterministic\n");
    goto cleanup;
  }
  for (index = 0u; index < conversion_unit.expression_count; index++) {
    const ctool_c_expression_t *expression =
    ...
    if (expression->kind != CTOOL_C_EXPRESSION_IMPLICIT_CONVERSION ||
        ...
        expression->first_child >=
            conversion_unit.expression_child_count) {
      continue;
    }
    ...
    if (expression->kind == CTOOL_C_EXPRESSION_IMPLICIT_CONVERSION &&
        conversion_unit.layout.types[expression->type].size == 2u &&
  ...
  if (narrowing_conversion == CTOOL_C_AST_NONE ||
      ...
      sizeof(*invalid_conversion_expressions) >
          SIZE_MAX / (size_t)conversion_unit.expression_count) {
    (void)fprintf(stderr, "wide conversion fixture metadata differs\n");
    goto cleanup;
  }
  ...
  if (!expect_ir_failure_preserves_unit(
          ...
          CTOOL_C_IR_DIAG_INVALID_UNIT,
          "CupidC IR lowering received an invalid translation unit",
          "wide narrowing mislabeled as integer promotion")) {
    goto cleanup;
  }
  ...
  invalid_reverse_conversion_unit.expressions =
      invalid_reverse_conversion_expressions;
  if (!expect_ir_failure_preserves_unit(
          job, &invalid_reverse_conversion_unit, CTOOL_ERR_INPUT,
          CTOOL_C_IR_DIAG_INVALID_UNIT,
          "CupidC IR lowering received an invalid translation unit",
          "unsigned wide conversion mislabeled as usual arithmetic")) {
    goto cleanup;
  }
  if (!parse_source_mode(job, "/wide-operations.c", wide_operation_source,
                         CTOOL_TRUE, &operation_unit)) {
    goto cleanup;
  }
  fingerprint = unit_fingerprint(&operation_unit);
  diagnostic_count = ctool_job_diagnostic_count(job);
  status = ctool_c_lower_ir(job, &operation_unit, &operation_ir);
  if (!check_status(status, CTOOL_OK, "wide integer operations") ||
      ctool_job_diagnostic_count(job) != diagnostic_count ||
      unit_fingerprint(&operation_unit) != fingerprint ||
      !validate_wide_operation_ir(&operation_unit, &operation_ir)) {
    (void)ctool_job_render_diagnostics(job);
    goto cleanup;
  }
  status = ctool_c_lower_ir(job, &operation_unit, &operation_repeat_ir);
  if (!check_status(status, CTOOL_OK,
                    "repeat wide integer operations") ||
      ctool_job_diagnostic_count(job) != diagnostic_count ||
      unit_fingerprint(&operation_unit) != fingerprint ||
      !validate_wide_operation_ir(&operation_unit,
                                  &operation_repeat_ir) ||
      ir_instruction_fingerprint(&operation_ir) !=
          ir_instruction_fingerprint(&operation_repeat_ir)) {
    (void)fprintf(stderr,
                  "wide integer operation lowering is not deterministic\n");
    goto cleanup;
  }
  ...
  if (wide_enum_promotion == CTOOL_C_AST_NONE ||
    ...
    (void)fprintf(stderr, "wide operation promotion metadata differs\n");
    goto cleanup;
  }
  ...
  if (!expect_ir_failure_preserves_unit(
          ...
          CTOOL_C_IR_DIAG_INVALID_UNIT,
          "CupidC IR lowering received an invalid translation unit",
          "wide enum promotion to the wrong compatible type")) {
    goto cleanup;
  }
  if (!parse_source(job, "/wide-count-operations.c",
                    wide_count_operation_source,
                    &wide_count_operation_unit) ||
      !expect_ir_failure_preserves_unit(
          job, &wide_count_operation_unit, CTOOL_ERR_UNSUPPORTED,
          CTOOL_C_IR_DIAG_UNSUPPORTED_TYPE,
          "CupidC IR lowering does not yet support this value type",
          "wide shift count")) {
    goto cleanup;
  }
  if (!parse_source(job, "/active-wide-helpers.c",
                    active_wide_helper_source, &active_helper_unit)) {
    goto cleanup;
  }
  fingerprint = unit_fingerprint(&active_helper_unit);
  diagnostic_count = ctool_job_diagnostic_count(job);
  status = ctool_c_lower_ir(job, &active_helper_unit, &active_helper_ir);
  if (!check_status(status, CTOOL_OK, "active wide helper lowering") ||
      ctool_job_diagnostic_count(job) != diagnostic_count ||
      unit_fingerprint(&active_helper_unit) != fingerprint ||
      !validate_active_wide_helper_ir(&active_helper_unit,
                                      &active_helper_ir)) {
    (void)ctool_job_render_diagnostics(job);
    goto cleanup;
  }
  status = ctool_c_lower_ir(job, &active_helper_unit,
                            &active_helper_repeat_ir);
  if (!check_status(status, CTOOL_OK,
                    "repeat active wide helper lowering") ||
      ctool_job_diagnostic_count(job) != diagnostic_count ||
      unit_fingerprint(&active_helper_unit) != fingerprint ||
      !validate_active_wide_helper_ir(&active_helper_unit,
                                      &active_helper_repeat_ir) ||
      ir_instruction_fingerprint(&active_helper_ir) !=
          ir_instruction_fingerprint(&active_helper_repeat_ir)) {
    (void)fprintf(stderr,
                  "active wide helper lowering is not deterministic\n");
    goto cleanup;
  }
  passed = 1;
