...
rechecks the captured bytes afterward. The 26-input closure includes its helper
...
boot/disassembly/shell smokes pass on both hosts, with matching images unchanged
through runtime checks. ADR 0402 records the candidate-authoring and authority
rules. Native command ownership and installed seeds are unchanged.
...
The preceding ADR 0399 extraction passed paired staged proofs, normal OS builds,
all sixteen
...
calls away from the repository. It is not a process sandbox. ADR 0389 records
the launch boundary.
...
Recovery inspection must preserve the displaced directory, its
`.cupidbuild-old-*` backup, and transaction files. Stop concurrent writers and
verify the regular, non-symlink backup against the known old output before
manual restoration. The replacement public directory may belong to the
concurrent writer; a matching backup name does not establish safe recovery.

The committed Windows repair passes short private filenames to the
disassembler during whole-kernel inspection. This admits all 500 supported
inputs without exceeding the command line. Linux keeps sealed descriptor
paths. The transaction still verifies every input and compares the complete
flat output with an independent renderer. ADR 0387 records the repair. Both
checked seeds retain these changes; the earlier `16a86f5b` clean paired proofs
and the current `9d2529a7` proofs independently compare final-stage artifacts.

The final-cohort 155-test CupidBuild CLI suite passed on each host. Both
500-input cases passed; only platform-specific skips remained. Linux ran from
the supported `/mnt/c` location. These results are separate from the earlier
source preflight with the `962e476b` tools.

The corrected 167-case production suite and the isolated Linux archive's
48-case handoff suite also passed. Linux executes the profile-parent symlink
and replaced-output-directory cases skipped on Windows, as well as parallel
ordering and ISO publication checks.

The pre-correction Linux normal build passed all 16 exact artifact rows and
published an image byte-identical to the successful private four-CPU
validation frontier. The first main Windows replay timed out, but an
isolated guarded replay later published the Linux-matching raw kernel with
...
checks. The earlier `962e476b` Windows replay exceeded the command-line
limit with 431 absolute kernel-flatten input paths and preserved the old raw
kernel. That failed replay is separate from the new paired proofs. The
...
fixture, and equal bytes retain its timestamp. The normal recipe selects this
command directly with the source, Makefile, and full seed closure. Its
production transaction is carried by both repaired seeds. ADR 0386 records
the handoff. The enclosing ISO image remains a separate Hostbuild publication.

Make finishes `test_iso/hello.iso` before `verify-artifact-sizes` through an
order-only prerequisite. The pattern transaction's temporary root entries
change directory mtime even when it reuses equal output. Finishing ISO work
first keeps those writes outside the artifact verifier's pinned-directory
check. The audit enforces this exact edge without changing size-policy inputs.
That scheduling repair changed no production owner; both repaired builds passed.

The promoted CupidBuild seeds provide `generate-profile-manifest` for the closed
...
returned identity still matches. POSIX requires both parents to exist, so Make
runs `mkdir -p build/bootstrap` after rejecting existing links at either
...
directory, suffix, and image closure with `override` assignments. The graph
records 354 CupidBuild and 98 Python participations across 452 transforms.
Disk and ISO publication remain the two Python-coordinated CupidObj paths.
ADR 0383 records this handoff.

Make finishes the profile before any of the 254 normal writers beneath
`kernel/`, `drivers/`, or `toolchain/` can run. The order-only barrier covers
the complete matching artifact cohort and the three generated installation
sources. Other roots remain parallel. The audit records scheduling edges as
`order_only_inputs`, keeps them reachable, and does not count them as file
content consumed by the writer. Its checked user ABI gate retains the same
distinction.
...
not passed to the child. The promoted pair binds revision
...
Its candidate fixed points matched every final object and tool image, then
passed 36/7/42 failure/help/success groups on Linux and 24/7/29 on Windows.
Those matrices cover the checked CupidC runner, typed profile publication,
the ISO pattern transaction, and closed kernel compilation.
ADR 0377 records the transaction, and ADR 0393 records seed promotion.
Windows separates read-shared frozen inputs from mutable candidates,
...
CupidBuild instead of nesting a second publisher. The common Windows startup
provides the file-information wrapper used by the shared host adapter, and
the ordinary and linker profiles include its import.
...
ADR 0385 records the repair now carried by the checked seed.

The repaired Windows plan is
`98e09aab876a9fa37ec07c38a0a57a014549a14c0ab10c740b3f80ede9d65669`.

Temporary Windows behavior manifests name the plan executed by their copied
tool cohort. This keeps the behavior fixture's import checks consistent with
...
accepts the kernel-symbol and kernel-flattening parent pairs, while
...
records the removal of the temporary compiler bridge. That checkpoint's
production build and four-CPU GUI-terminal smoke passed with the Windows
checked seed.
...
cases. Together with typed profile publication, the promoted candidate
matrices include closed kernel compilation and passed 36/7/42 on Linux and
24/7/29 on Windows. ADR 0360 records the original runner
boundary, ADR 0361 records the preceding paired promotion, and ADR 0376 records
the source-head compiler admission. ADRs 0367 and 0370 record earlier pairs;
ADR 0393 records the current promotion. The
...
kernel-symbol, kernel-flatten, profile, ISO-pattern, and 157 kernel compiler
handoffs, the audit records 354 CupidBuild and 98 Python participations. ADR 0363 records the link handoff, ADR 0371
...
native Windows. The normal Make edge invokes the platform CupidBuild seed
...
records the source capability, ADRs 0367 and 0370 record earlier seed carriage,
and ADR 0368 records the handoff.
...
The ADR 0374 source closure had 59 inputs and snapshot SHA-256
`0b591a0bef928186641b3aa1fb98c1e145e6c4905c8b6cb87c34a1ace4bc87d2`.
The Linux and Windows build-plan hashes did not change at that checkpoint.
CupidBuild accepted a promoted v2 manifest with `source_input_count` 58 or 59
and rejected 57 or 60. Paired artifact verification also required the Linux
and Windows counts to match, so a mixed 58/59 pair failed. This kept the
preceding 58-input pair usable and made that 59-input promotion consumable.
...
Windows. Those ADR 0374 manifests have SHA-256
...
The ADR 0374 policy-bound OS build passed all 83 Doom roots, both CupidLD links, strict
CupidDis inspection, all 16 exact artifact rows, and image publication. The
flat kernel was 9,536,524 bytes; the final and pass-one ELFs were 9,765,196 and
...
The child calls `fchdir` before remapping captured streams. A tool descriptor
in slot 0, 1, or 2 is duplicated above the standard descriptors before
`fexecve` or `execveat`. The `dup2`, pipe read and write, and wait loops retry
`EINTR`; `dup2` also retries `EBUSY`. Captured streams are fully sealed
anonymous memfds. The close-on-exec launch-status pipe preserves a genuine
exit of 125, and the static i386 startup provides `cupid_linux_syscall5`.

Windows pins and rechecks the working-directory identity, then uses a
handle-pinned private root and files. It retains a tool handle without write or
delete sharing through `CreateProcessA` and forwards the captured streams in
binary mode to preserve exact bytes. Cleanup removes an identity-owned file
even if it changed in place, but preserves a replacement identity. Both paths
...
All six CupidASM source tests passed in 3.771 seconds. Strict Windows and
freestanding i386 adapter compilation passed, along with the
...
bootloader, and the SMP trampoline. Its first hosted command performs a
...
tools. The object Make recipes invoke the host seed with the full seed closure and
...
CupidBuild also has typed bootloader and SMP-trampoline operations.
They assemble a private raw image and v2 map, enforce the artifact's size and
layout policy, and run strict source-edge inspection before publishing only
the image. The fixed-point behavior gate exercises both operations across its
two compared generations. The refreshed checked seeds carry both commands,
and both normal raw-image recipes invoke the promoted CupidBuild seed directly.
ADR 0355 records the source boundary, ADR 0356 records carriage, and ADR 0357
records the recipe handoff.
The ADR 0353 refresh candidates found CupidBuild alone different from the
preceding seed at stage two, then matched all six final tools.

That paired v2 trust unit froze 58 source inputs and built 22 Linux C objects,
one startup object, and all six tools. Native Windows added its
publication runtime and CupidBuild startup bridge, with the exact
...
Both ADR 0353 candidate proofs passed. Linux matched 22 C objects, startup,
and six tools with 24/6/31 failure, help, and success cases. Native Windows
matched 23 C objects, three assembly objects, and six tools with 13/6/18
behavior. Both froze revision `43c747f0e683d0527984bae05bf944879e64a07b`
and the same
58-file source snapshot,
...
reporting, relocation ownership, anchors, and local-target setup. The fresh
Linux and Windows candidates converged and now supply the active v2 cohorts.
Promoted-seed self-consumption also passes on both platforms, with all six
initial images equal to stage two. ADR 0347 records object certification and
the startup closure, and ADR 0350 records decode-map reuse.
...
The hosted link builds CupidBuild beside CupidC, CupidASM,
...
The active checked production seeds now carry that six-tool closure.

Cupid OS carries a checked static i386 Linux seed for six hosted tools:
CupidC, CupidASM, CupidDis, CupidLD, CupidObj, and CupidBuild. The seed starts
a complete toolchain rebuild without GCC, Clang, NASM, a host linker, `nm`, or
`objcopy`. CupidBuild is checked as an artifact but is not a code producer.

Cupid OS also carries a checked native i386 Windows execution seed for those
six tools. Windows selects the PE32 cohort for output-bearing production
...
Checked-seed CupidASM and CupidDis share raw-map v2 control-edge metadata.
...
The bootstrap target publishes only to
`build/bootstrap/checked-windows-seed`. Its dry run and two Make contract tests
pass. For debugging, the underlying command is:
...
python tools/bootstrap_toolchain.py bootstrap-windows \
  ...
  --root . --output build/bootstrap/checked-windows-seed
```
...
the new trio, and builds stage four from stage three. Stages two and three are
transition generations; stages three and four are the convergence pair. The
driver compares every object and all six tools, then runs useful native
success and failure cases on the final pair. It publishes one
`cupid.windows-bootstrap-report.v1` bundle only after rehashing the frozen and
live source closures. A source-stable Windows run stopped safely at
`cupidobj_main` after 821.9 seconds under the old comparison. Linux stopped at
the same transition after 883.3 seconds. Neither published. Later uncapped
...
1,294.3 seconds, promoted the stage-four seed, and passed a 1,473.9-second
reproof with every initial seed comparison true. Native Windows then passed a
clean 1,253.4-second proof, promoted its stage-four PE32 seed, and passed a
1,061.3-second reproof with every initial seed comparison true. The reproofs
pass 5/18/17 and 5/5/6 behavior, including executable relocation ownership.
ADRs 0278 and 0279 record the driver and added generation. ADRs 0280 and 0281
preserve the preceding promotions, and ADR 0292 records the strict-relocation promotion.
...
profile for all six tool mains.

At that earlier v1 checkpoint, the seed lived under
`bootstrap/seeds/i386-linux/`. Its manifest recorded each
...
facts from separate reads. The publisher accepts only a dedicated
`cupidc-contracts` directory inside the
...
host-selected checked seed then builds the
...
inventories contain 76 publication inputs and 59 candidate files. The
75-input, 58-file publication above is historical. The earlier 16a cohort
published all 22 artifacts with the 76/59 inventories and passed native
`CUPMAN2` verification in a stable copy. Its manifest has SHA-256
`ba5e531172f4861237238d403f8119f229c3e6d1267a483277efc3f19e615c92`.
That earlier publication after the preprocessor-count and ABI-consumer
repairs in `b25a3374`, plus refreshed object expectations, agreed on all 62
stage pairs. All six published tools matched the preceding checked Linux
seed, and all 22 artifacts passed native `CUPMAN2` verification on Linux and
Windows. Its 29,619-byte manifest has SHA-256
...
anchors. It rejects loaded spans above CupidLD's 2 GiB RVA limit. At ADR 0338,
all five checked Windows seed reports matched an independent Python
reconstruction, but the promoted CupidDis executables then predated the
reader. The current six-tool seeds carry it.
...
The preceding full publication gate passed in 4,589.9 seconds. It compared seventeen
...
The warmed verification path for that publication passed in 12.2 seconds.
...
The full command reads all 50 current source inputs once. The closure includes
...
Linux runs private copies of the static bootstrap tools directly. Windows
stages those ELF images in a mode-0700 WSL directory created by `mktemp` for
fixed-point and Linux-contract work. Output-bearing production recipes freeze
and run private copies of the checked native PE32 cohort directly.
...
`make verify-artifact-sizes` uses one wrapper command to pin the raw policy,
both v2 manifests, the complete six-image Windows seed, and all sixteen file
observations. Checked CupidC compiles a strict C11 policy contract, CupidASM
supplies startup, and CupidLD links a static ELF on Linux or a native PE on
Windows. The contract validates the policy, Linux manifest, exact manifest
pairing, producer roles, and every observation, rereads its request, and emits
canonical JSON. Python
also validates the captured Windows cohort and must produce
...
The preceding paired-stage proof produced matching PE images for all five tools.
...
That complete proof passed in 801.9 seconds. Its 50-input source
closure has SHA-256
...
The preceding strict-relocation Windows execution seed was:
...
The clean proof passed in 1,253.4 seconds. Its old match vector was false for
CupidASM, CupidC, and CupidDis and true for CupidLD and CupidObj. The
1,061.3-second reproof matched all five initial seed images and repeated the
20/2/5 artifact and 5/5/6 behavior gates.

The earlier v1 Windows execution seed was:
...
It binds revision `a17c9465911da41d59b7ada71733d36c39faa5ea`, the exact
50-input snapshot
...
The candidate proof passed cleanly. It matched 20 C objects, two
assembly objects, and five tools with every initial seed comparison true, then
passed the 5/8/9 behavior matrix. ADR 0336 records this
promotion and reproof.
[ADR
...
native driver. [ADR 0292](../docs/adr/0292-promote-strict-relocation-production-seeds.md)
...
records the preceding pair, and ADR 0370 records the preceding pair; ADR 0393 records the active pair.
...
and atomic publication. Each caller keeps its image-size and raw-map policy.
...
The ISR and context-switch rules call the promoted CupidBuild seed directly.
Each operation freezes the source and six-tool seed, validates a private i386
relocatable with executable bytes, and runs strict CupidDis over every
executable section. Drift or a failed tool leaves the prior object in place.
This closes the earlier gap in which the final kernel check ran only after both
public objects had already fed the link. ADR 0286 records the guarded boundary,
and ADR 0354 records direct publication ownership.
...
The preceding strict-relocation stage-four seed came from clean revision
...
`e76d36ed4edc7679e91ac237135fe476dff6e69946bbffca56077afbf19a47f9`,
and the stage-three producers. The clean candidate proof passed in 1,294.3
seconds. Stages three and four matched 19 C objects, startup, and five tools,
and both stages passed the 5/18/16 behavior matrix. A 1,473.9-second reproof
from the promoted manifest reproduced all five seed images and passed the
stronger 5/18/17 matrix, including executable relocation ownership. ADR 0265
and ADR 0280 preserve the earlier promotion records. ADR 0292 records the
current strict-relocation promotion.

The earlier v1 Linux seed was:
...
`b6e34a2e18dd18aba91c6358116eafde39953566efeadb224575ac8c13ab2c1b`.
It binds revision `a17c9465911da41d59b7ada71733d36c39faa5ea` and the exact
50-input snapshot
`46c5335c80d822dd5085ee22077486ea647e5396482d42454847c87e4222aa67`.
The clean candidate proof passed. It matched 19 C objects, startup, and five
tools between stages three and four; CupidASM and CupidDis differed from the
preceding seed. Both compared stages passed five help, 22 success, and 21
useful failure cases. The promoted-seed reproof reports all five initial
comparisons false because the new startup function types remain in static ELF
symbol tables and flow into every linked tool; its final stages still match
exactly. ADR 0336 records this promotion and reproof.

The first full run of the intermediate 86-test seed suite completed
in 2,394.660 seconds but failed on stale test data. The tiny source roots lacked
the Windows `publication_start.asm` and `publication_runtime.cc` inputs now
required by the frozen closure. The v1 Windows CupidASM report also
expected the older four-byte result. The fixtures now include both publication
inputs, and the report expects the current six-byte result with SHA-256
...
module later passed all 92 tests in 2,820.626 seconds. Those v1 seeds carried
that snapshot. The normal kernel publisher applies strict linked-image
...
The strict non-Doom portion of the normal image has 157 checked CupidC
transforms: 156 checked-in sources and the generated
`kernel/cpu/ksyms_data.cc` source. All 157 sources use `.cc`. The 83 Doom
roots bring the normal checked-in total to 239. The five shared Toolchain
roots also belong to the 22-source i386 Linux fixed point. Native GCC and
...
and emits canonical JSON for both profiles. At the preceding seed checkpoint,
the matrices had 28 failure, six help, and 35 success groups on Linux, plus
17 failure, six help, and 22 success groups on Windows. The typed CupidBuild
transaction and checked CupidC runner extended the source definitions to
31/7/37 and 19/7/24 before their seed promotion. These checks include SHA
padding boundaries, unsafe paths, case collisions, and preserved failure
output. The earlier Python wrapper derived the snapshot and independent
oracle from one stable capture, ran CupidObj from the frozen seed, and
required byte parity. Its no-follow lock guarded input, seed, candidate,
parent, and output rechecks before unchanged or atomic publication. ADRs 0382
and 0383 record the repaired seed and current native transaction. CupidObj authors
...
The strict cohort now contains 156 approved sources. The latest complete
two-pass proof predates the last addition and covers 155 sources.
...
concurrent build do not enter the repository snapshot. That complete frontier
...
six-tool capture to the shared runner. It verifies the complete live cohort
...
ADR 0367 records the preceding pair, and ADR 0370 records the preceding pair; ADR 0393 records the active pair.
...
`unsigned int` words and records the logical 130,506-byte blob length
separately. Promoted CupidBuild freezes the pass-one kernel and checked seed,
captures canonical text through checked CupidDis, and gives the exact text to
checked CupidObj. Its independent native renderer catches malformed text,
missing output, byte mismatch, lock contention, or live input drift before
atomic publication. The
checked compiler wrapper freezes that source and its header closure before it
publishes the object. The word array ends with two zero pad bytes. The final
kernel retains all 5,161 selected pass-one text-symbol entries. ADR 0224
...
hostbuild transaction also freezes the selected seed manifest and all six
...
publication. Every failure preserves the prior raw kernel. At the first
reviewed transaction checkpoint, the operation passed with exit 0 in 187.054
seconds and published an 8,946,332-byte
...
Source-head and checked-seed CupidDis also check the relocation fields in executable ELF32
...
in data sections do not enter this check. The checked Linux and Windows seeds
...
Promoted-seed CupidDis applies explicit raw control-flow checks in both
...
The 2,560-byte boot image contributes nine local relative, two local far, and
one resolved external far edge. The 4,096-byte SMP trampoline contributes four
local relative, one local far, and one indirect unprovable edge. The source
edge policy rejects a changed destination even when it remains a valid decoded
start. ADR 0300 records the decoder rule, ADR 0305 records earlier raw-image
carriage, ADR 0340 records source-resolved edges, and ADR 0336 records their
v1 promotion and adoption. ADR 0353 records the paired v2 contract, and ADR
0367 records active carriage.
...
Checked CupidDis also applies the option to linked i386 ELF32 images. It
...
domain. Typed and CLI negative cases cover both forms. The normal kernel
transaction applies the linked rule to its frozen pass-one and final ELFs
before CupidObj flattening. ADR 0314 records the decoder boundary, and ADR 0318
...
Checked CupidDis provides `--require-code-anchors` for static ELF objects and
linked images. An object function must name a decoded start in executable
`PROGBITS`. A linked image adds the ELF entry and file-backed executable
regions. Function aliases count separately. The local-target and code-anchor
policies share one instruction-start map when selected together. Production
...
A fixed callback parameter may itself have a retained callback signature. Raw
nested declarators use child handles from the raw-signature pool, while
typedef-backed children keep their typedef handles. Compatibility checking
walks those handles recursively across initialization, assignment,
higher-order arguments, indirect calls, conditionals, and matching function
declarations or definitions. Each level checks its result, fixed parameters,
record-pointer identities, and variadic boundary. Compatible uses keep the
existing unprototyped callback rule, while declaration and definition matching
also requires the same prototype state. Raw and typedef children may be mixed
...
artifact-size policy covers sixteen paths, including all six Windows seed
...
parent link, exact six-tool inventory, and observed sizes and digests. The
historical schema-v3 CUPMAN4 publication and final CUPMAN2 verification
for this checkpoint are recorded above. Its build and private guest evidence are
recorded below.
...
Its private four-vCPU E1000 frontier smoke left the source image unchanged.
The framebuffer changed 101,335 pixels. AC97 produced 36,533,414 stereo 44.1
kHz frames at peak 25,600, and the PC speaker produced 79,215 frames at peak
30,937. The guest reported
...
Cupid participant. Its first poisoned-host build reached the new gate in 695.8
...
oracle, and all nine exact artifacts passed at that checkpoint.
...
build passed in 692.768 seconds and published that checkpoint's image.

At that checkpoint, Toolchain manifest verification ran through a checked
CupidC contract.
The final 21-artifact publication passed in 3,933.424 seconds. Its 22,931-byte
manifest has SHA-256
`8909105d516ef53d3c5081e5752fbef8596458fdfa673ec08275e7e435cd059a`.
An earlier final-source poisoned-host build reached only the exact-size gate in
633.542 seconds and measured `kernel.bin` 1,188 bytes above the prior policy.
After that row moved, all 38 policy and semantic-contract tests passed in
2.784 seconds, with two Windows replacement cases skipped. The repeated
poisoned build passed in 651.193 seconds and published that checkpoint's image.

| Earlier checkpoint output | Bytes | SHA-256 |
| --- | ---: | --- |
| `boot/boot.bin` | 2,560 | `46cc9778da2b5cc5e8f04d7cc4b07243c3e07d466626ad84fb813dc6fef3a0d3` |
...
CupidC and passed the SMP runtime contract in 61.926 seconds. The guest printed
...
All fourteen exact artifacts passed the policy. The 2,960-byte policy had
SHA-256
...
seconds. It printed the callback markers in this order:
`[feature14-call] PASS float4=4 double2=2 nested=2 calls=6`,
`[feature14-callback] PASS float4=4 double2=2 calls=2`, and
`[feature14-callback-typedef] PASS float4=4 calls=1`. The overall
`PASS feature14_simd` and `[cupidc] JIT execution complete` markers followed.
No reject marker appeared, and the source image was unchanged. The 33,219-byte
log has SHA-256
...
The earlier dual-NIC four-vCPU frontiers remain pre-freeze runtime evidence.
E1000 exited 0
...
literal `db` rows. The active SMP trampoline gate assembles the unchanged
4,096-byte source and marks `[0x000, 0x01f)` as code16,
`[0x01f, 0x210)` as data, `[0x210, 0x254)` as code32, and
`[0x254, 0x1000)` as data. CupidDis requires every code instruction to be
known before CupidBuild atomically publishes the CupidASM candidate. A failure
preserves the prior trampoline. ADR 0271 records the production gate, and ADR
0357 records direct publication ownership.
...
Compiler head and checked-seed CupidC apply that same conversion during runtime arithmetic, all six
...
converts only its selected arm. ADR 0288 records the source boundary, and ADR
0291 records its fixed-point promotion.
Compiler head and checked-seed CupidC also convert signed and unsigned integers through 64 bits to
...
CupidC images in the preceding linked-image cohort carried it under ADR 0318.
...
first carried the capability. The preceding linked-image cohort carried it
under ADR 0318. ADR 0353 records the paired v2 contract, and the active cohort
...
Python participates in 98 transforms, and CupidBuild in 354. The ordinary CupidC
...
root CupidDis participations from the manifest-checked six-tool seed. Three
...
contracts, and no transform is Python-only. Python participates in 98 as
orchestrator, while CupidBuild participates in 354, including two guarded assembly objects,
three guarded raw images, the typed JPEG object, the generated kernel-symbol
source, final kernel flattening, 157 kernel compilations, Doom profile publication, 186 ordinary CupidObj calls, and two normal
kernel links.
...
An earlier attempt at this audit stopped after 65.183 seconds because the
...
At that checkpoint, `make bootstrap-audit` and `make check-bootstrap-audit`
both passed. The Linux audit recorded 23 failure groups, six help groups, and
29 success groups. Windows recorded 12 failure groups, six help groups, and
16 success groups. The active seed section records the current matrices.
...
resolved path. No supplemental output is Python-only. The
`user/test-syscall-abi`, root artifact-size, Toolchain manifest author, and
Toolchain manifest verifier transforms have Cupid-built contract and Python
participants. The contracts own their semantic decisions, while Python checks
independent results and controls the native boundary. ADR 0245 records the
graph boundary, ADR 0246 records the shared invocation and post-run five-image
check, ADR 0264 records the ABI transfer, and ADR 0297 records artifact-size
verification. ADR 0302 records Toolchain manifest verification, and ADR 0304
records authorship.
...
subscripts, direct pointer updates, and assignment. Scalar floating updates
also work through dereference, array index, and direct, pointer, or
indexed-record members while evaluating the destination once. Structure and class
...
prevents stride leakage. Deeper floating pointers, pointer-to-array types, and assignment through a
...
during prescan. Compatible conditionals retain each named candidate, and a
failed source discards all new forward fixups. It also restores typedef entries,
touched prototypes, definitions, kernel bindings, and a reused implicit `__start`. The
thunk has a complete `void(void)` signature. Null initializers use represented
integer constant expressions that evaluate to zero; `sizeof(int) - 4` is
covered, while runtime zero is not. Explicit erasure survives a conditional
only when every non-null object-pointer arm is cast.
...
The repository stores the accepted sequential baseline bytes. At that
checkpoint, Hostbuild froze the exact source and gave the private snapshot to checked CupidObj
...
checked the accepted snapshot independently, required unchanged bytes, rechecked
live inputs, and published atomically. A validator disagreement was distinct
from a failed private oracle copy; either one preserved the prior object.
The current Make edge runs promoted CupidBuild `embed-jpeg` with its independent
native check and guarded publication.
...
changes, and publishes the complete candidate atomically. CupidObj's 192
...
missing kernel bindings bring the source-driven private AOT census to all 107
...
This dated checkpoint records the earlier schema v3 publication, audit,
poisoned build, artifact identities, and private runtime smoke. The current
seed identities below are separate from those earlier build and guest results.
...
`9d2529a718672edcd970f24960535db6ddbde5e4` and the 59-input snapshot
`bb4f598b2f72140d82d01fbfdedc14e4c774965d08c155ca848596daf3aa054f`.
They list CupidC, CupidASM, CupidDis, CupidLD, CupidObj, and CupidBuild.
CupidC, CupidASM, and CupidLD remain the fixed-point producers; CupidBuild is
a checked non-producer with a separate production coordination role.
...
`98e09aab876a9fa37ec07c38a0a57a014549a14c0ab10c740b3f80ede9d65669`.
Its pairing field names those exact Linux manifest bytes.

The clean candidate fixed points matched every final object and tool image
and passed 36/7/42 failure/help/success groups on Linux and 24/7/29 on Windows.
...
Doom profile publication, and all 157 kernel-profile compilations.
It also runs 186 ordinary CupidObj calls and both normal kernel links.
Participation is 354 for CupidBuild and 98 for Python across 452 transforms.
