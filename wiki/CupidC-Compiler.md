...
The exact hosted gate covers the hermetic Toolchain sources, `kernel/lang/as_elf.c`, and the complete hosted command closures for CupidASM, CupidDis, CupidLD, and CupidObj. CupidC emits every unchanged C source in those four closures under a checked i386 Linux profile with a four-byte pointer fact. Repeated emission produces identical ELF32 objects, and Cupid's ELF32 reader checks each object before linking.

The repository runtime supplies the checked file, heap, string, `errno`, `getcwd`, and formatted-output interfaces required by the four commands. CupidC emits the runtime, CupidASM assembles the repository `_start` and system-call boundary, and CupidLD links four deterministic static Linux i386 executables without unresolved symbols. Each closure object is emitted twice before the repeated links. A fifth generated executable checks allocation, tail release, files, seeks, errors, arguments, and strings. The implementation is deliberately narrow. It has unbuffered streams and single-threaded heap, stream, and `errno` state.

WSL behavior checks match the native hosted tools for raw and ELF32 assembly, include resolution, raw disassembly with mode changes, fixed-address linking, object wrapping, and missing files. Successful commands produce the same bytes or report text. Invalid assembly and malformed linker input also match the hosted status and diagnostics. This is a Linux and WSL execution proof. The native oracle and contract runners, along with normal Cupid OS C objects, still come from a host C compiler. There is no host-runnable CupidC driver, compiler fixed point, checked seed, or production ownership transfer.
