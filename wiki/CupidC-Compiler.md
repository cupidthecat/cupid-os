...
The checked CupidC seed owns 16 normal-build kernel crypto objects. All 16 now run in the 48-check TLS boot self-test, including SHA-512, SHA-384, bigint, and valid and corrupted RSA and Ed25519 signature cases. Compiler head also emits `asn1.c`, `x509.c`, and `x509_chain.c`, but those files remain host-built until the checked seed is refreshed. `csprng.c` still needs GNU extended inline assembly. Most normal C objects and the native contracts continue to use a host compiler, while the private in-kernel CupidC compiler handles embedded runtime compilation.
