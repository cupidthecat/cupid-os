...
The hosted cast path accepts a direct four-byte integer literal zero as a represented function-pointer null. Object pointers can convert to signed or unsigned eight-byte integers with a zero high word, and conversion back keeps the low word. Computed, nonzero, or wide-literal function-pointer casts and conversions between function pointers and wide integers remain unsupported. Static compatible character and void pointers accept an ordinary string literal through parentheses or a macro, but arithmetic and explicit casts on that static address remain outside the boundary.

The exact object gate covers sixteen unchanged sources. The fourteen-file CupidC, CupidASM, and CupidDis self-host cohort is ten hermetic Toolchain files, `kernel/lang/as_elf.c`, and the three hosted adapters. The hermetic gate also includes the adjacent CupidLD and CupidObj cores. `ctool_host.c`, `cupidasm_main.c`, and `cupiddis_main.c` use a checked i386 Linux profile with Cupid-owned declarations for the hosted runtime interfaces they need. That profile requires an explicit 32-bit pointer fact. Every object is emitted twice, checked for identical bytes, and read back through Cupid's ELF32 reader. The adapter checks also lock exact undefined imports and relocation shapes.

A static i386 tracer now links the CupidC-emitted `ctool_host.c` object with a CupidASM `_start` and CupidLD. Test-only providers satisfy the adapter's ten imports. Startup calls `ctool_host_adapter_init`, checks the returned root pointer and one-byte size, and exits through Linux `int 0x80`. The 21,592-byte executable is deterministic and runs under WSL with status zero. Omitting `__errno_location` proves transactional link failure and recovery. The providers are not a usable C runtime, and the tracer does not run a complete Cupid tool or compare compiler generations.
