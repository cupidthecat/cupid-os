...
The `cupidc` driver compiles one C11 input to an ELF32 object. It accepts definitions, undefinitions, forced inputs, GNU or freestanding mode, and ordered include roots. `-I` enables quoted and angle lookup; `--include-angle` enables angle lookup only. Repeatable `-include` options run before the primary source in caller order. These path options accept native paths or absolute logical paths under `--root`. A compile failure preserves the previous output. Empty volatile extended assembly with one `memory` clobber remains an IR ordering point and emits no instruction bytes. The exact audited Doom-tree command now emits 74 of 80 objects, including unchanged `kernel/doom/src/am_map.c` and `kernel/doom/src/info.c`. Six pinned sources still need IR, implicit-call, or pointer-conversion work before ownership can move.
