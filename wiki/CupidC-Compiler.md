...
The shared path carries `float` and `double` values through objects, assignment, calls, variadic reads, and returns. Explicit casts and assignment conversion work in both directions. Unary plus and minus and binary addition, subtraction, multiplication, and division accept matching or mixed floating widths. Matching floating conditional arms keep their width, while mixed arithmetic and conditional arms use `double`. A represented integer or pointer may control the selection. The four arithmetic compound assignments compute at the common width, convert back to the left width, and evaluate the lvalue once. Every changed x87 result is immediately stored at its C width. A `float` rounds into a fresh four-byte semantic slot, while a `double` receives a fresh private eight-byte snapshot. The unchanged `libm_tanh_impl` expression `(e1 - e2) / (e1 + e2)` pins nested `double` arithmetic, and the following `float` helper slice pins the width changes. At an ellipsis or unprototyped call, the frontend promotes a source `float` to `double` after lvalue conversion. Arguments occupy four-byte or eight-byte cdecl slots, results return in x87 `ST0`, and `va_arg(double)` advances by eight bytes. Floating values remain invalid truth operands. The decoder-driven model checks width-specific x87 loads and stores, IEEE rounding cases, mixed expressions, call alignment, and frame state. It does not execute native x87 code. Floating literals, integer and floating conversions, comparisons, truth, a floating controlling expression, increment and decrement, explicit static floating initializers, `long double`, SIMD values, floating atomic access, and over-aligned object emission remain unfinished.
...
The private compiler implements a broader floating and SIMD language. The hosted self-hosting path converts between `float` and `double`, evaluates matching or mixed floating arithmetic, selects matching or mixed floating conditional arms, and stores `+=`, `-=`, `*=`, and `/=` results at the left width. It also carries existing `double` values and source `float` values promoted to `double` through ellipsis and unprototyped calls and supports `va_arg(double)`. Integer and floating conversions, comparisons, truth, a floating controlling expression, increment and decrement, literals, `long double`, and SIMD remain open in the hosted path.
