...
An empty identifier-list definition has zero parameters and keeps its non-prototype function type. Calls through a function type without a prototype apply default argument promotions to every argument, retain their actual count in Linear IR, and use the represented four-byte scalar cdecl path. The exact Doom profile passes `doomgeneric_Tick()` and reaches the anonymous block-scope `struct` in `kernel/doom/src/d_main.c:689`. Nonempty identifier lists, block tag specifiers, atomic variadic access, floating default promotions, non-scalar arguments without declared parameter types, and wider or aggregate variadic reads remain unfinished.
