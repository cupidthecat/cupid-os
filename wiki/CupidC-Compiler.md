...
The checked CupidC seed owns 16 normal-build kernel crypto objects. All 16 run in the 48-check TLS boot self-test, including SHA-512, SHA-384, bigint, and valid and corrupted RSA and Ed25519 signature cases. The refreshed seed also emits all four host-owned neighbors without source changes. Its GNU assembly subset represents the outputs and matching input in `csprng.c`, emits RDTSC, CPUID, RDRAND, and SETC through Cupid's x86 model, and preserves EBX. A disposable two-pass kernel image has booted with that CupidC-produced CSPRNG object, seeded through RDRAND, passed all 48 checks, and completed an embedded CupidC JIT command. Those four files remain host-built only until the production frontier and Make rules move. Most normal C objects and the native contracts continue to use a host compiler, while the private in-kernel CupidC compiler handles embedded runtime compilation.
