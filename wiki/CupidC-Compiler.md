...
The shared path copies matching `float` and `double` values without general floating-point computation. File, block-static, automatic, pointer, member, and indexed objects can feed initialization, plain assignment, discard, fixed direct or indirect calls, call results, and returns. A `float` keeps its raw four-byte representation. A `double` uses one Linear IR handle backed by a private eight-byte snapshot. At an ellipsis or unprototyped call, the frontend promotes a source `float` to `double` after lvalue conversion. Linear IR records a typed `CONVERT`, and the i386 emitter uses `FLD` on the four-byte source followed by `FSTP` into a fresh eight-byte snapshot. Arguments occupy four-byte or eight-byte cdecl slots. Floating results return in x87 `ST0`; after call cleanup, a caller stores a `float` result in a four-byte semantic stack slot or a `double` result in a private eight-byte frame snapshot. `va_arg(double)` advances by eight bytes. Floating values remain invalid truth operands. The decoder-driven oracle checks ESP alignment before each call it executes and models the exact supported transport and widening. It is not native x87 execution proof. Floating literals, arithmetic, comparisons, conditions, conditional expressions, fixed-parameter and explicit floating conversions, explicit static floating initializers, `long double`, SIMD values, atomic access, and over-aligned object emission remain unfinished. Implicit static zero initialization and casts to `void` are represented.
