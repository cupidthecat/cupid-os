...
The `cupidc` driver compiles one C11 input to an ELF32 object. It accepts logical include roots, definitions, undefinitions, GNU or freestanding mode, and preserves the previous output when compilation fails. Under WSL, the static Cupid-built driver compiles unchanged `toolchain/cupidc_ir.c` twice. Both generation-one objects match the native generation-zero object byte for byte. Sibling command checks still match native assembly, disassembly, linking, wrapping, output, and failure behavior. The native generation, contract runners, and normal Cupid OS C objects still come from a host C compiler. A complete compiler generation, fixed point, checked seed, and production transfer remain open.
