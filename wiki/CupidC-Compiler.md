...
The exact source gate parses all twelve hermetic hosted Toolchain implementation files. The object gate emits those files plus `kernel/lang/as_elf.c` twice and requires byte-identical i386 ELF32 output after reading each object through Cupid's ELF32 reader. Eleven files belong to issue #27's fourteen-file CupidC, CupidASM, and CupidDis cohort. The remaining hosted adapters need external headers, runtime support, and a host-runnable ABI. No tool is linked or run by this gate.
