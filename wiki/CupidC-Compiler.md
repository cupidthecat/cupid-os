...
The static i386 Linux compiler reaches a complete fixed point under Linux or WSL. Generation one compiles all eleven C objects in its own closure and links stage two. Stage two repeats the work for stage three. Every C object matches across those stages, both startup objects match, and all three compiler images are byte-identical. Stages two and three also agree on a valid object and a malformed-source diagnostic while preserving both failed outputs. The current 1,812,712-byte image has SHA-256 `29CD222C6E33590932457D36F3728705134C8C6750947E7CFBC4ABA3B7C5500B`. Sibling command checks still match native assembly, disassembly, linking, wrapping, output, and failure behavior. The native generation, contract runners, and normal Cupid OS C objects still come from a host C compiler. Checked seeds, a native Windows fixed point, sibling-tool fixed points, and production transfer remain open.
