 ...
 *
 * Local modifications:
 *  - Dropped <stdio.h> and <stdlib.h> (no stdio/malloc in kernel).
 *  - Dropped #include "doomtype.h": boolean, byte, PACKED_STRUCT, SHORT
 *    are now defined in mus2midi.h using kernel/types.h primitives.
                     ...
                     uint8_t **out_midi, uint32_t *out_len)
{
    ...
    src = mem_fopen_read((void *)mus, (size_t)mus_len);
    dst = mem_fopen_write();
    if (!src || !dst) {
        if (src) mem_fclose(src);
        if (dst) mem_fclose(dst);
        return -1;
    }
    ...
    copy = (uint8_t *)kmalloc((uint32_t)len);
    if (!copy) {
        mem_fclose(src);
        mem_fclose(dst);
        return -3;
