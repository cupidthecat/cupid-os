...
int as_write_elf(as_state_t *as, const char *path) {
    if (!path || as->code_pos == 0) return -1;
    ...
    uint32_t code_size = as->code_pos;
    uint32_t data_offset = code_offset + code_size;
    /* Align data offset to 4 bytes */
    data_offset = (data_offset + 3) & ~3u;
    uint32_t data_size = as->data_pos;
    uint32_t total_file_size = data_offset + data_size;

    /* Virtual addresses */
    uint32_t code_vaddr = ASM_ELF_LOAD_ADDR;
    uint32_t data_vaddr = ASM_ELF_DATA_ADDR;

    /* Entry point virtual address */
    uint32_t entry_vaddr = code_vaddr + as->entry_offset;
    ...
    if (fd < 0) {
        serial_printf("[asm] Cannot create output file: %s (err=%d)\n",
                      path, fd);
        return fd;
    }
    ...
    /* Write code section */
    vfs_write(fd, as->code, code_size);
    current += code_size;
    ...
    /* Write data section */
    if (data_size > 0) {
        vfs_write(fd, as->data, data_size);
    }

    vfs_close(fd);

    serial_printf("[asm] Wrote ELF: %s (%u bytes code, %u bytes data, "
