...
static void notepad_get_viewport(int *vis_cols, int *vis_lines,
                                 int *edit_x, int *edit_y,
                                 int *edit_w, int *edit_h,
                                 window_t *win);
...
static void notepad_ensure_line(int idx) {
    if (idx < 0 || idx >= NOTEPAD_MAX_LINES) return;
...
static void notepad_get_viewport(int *vis_cols, int *vis_lines,
    ...
    int ch = (int)win->height - TITLEBAR_H - 2 - MENUBAR_H - STATUSBAR_H - HSCROLL_H;
...
static bool notepad_has_selection(void) {
    if (!app.selection.active) return false;
...
static void notepad_normalize_selection(int *sl, int *sc, int *el, int *ec) {
    ...
    if (s_l > e_l || (s_l == e_l && s_c > e_c)) {
        int tmp;
        tmp = s_l; s_l = e_l; e_l = tmp;
        tmp = s_c; s_c = e_c; e_c = tmp;
    }
    *sl = s_l; *sc = s_c; *el = e_l; *ec = e_c;
}
...
static void notepad_copy_selection(void) {
    ...
    for (int line = sl; line <= el && pos < CLIPBOARD_MAX_SIZE - 2; line++) {
        ...
        if (col_start > len) col_start = len;
...
static void notepad_delete_selection(void) {
    ...
    if (sl == el) {
        ...
        if (sc > first_len) sc = first_len;
        if (ec > last_len) ec = last_len;

        /* Build merged line: first[0..sc) + last[ec..] */
        int new_len = sc + (last_len - ec);
        if (new_len >= NOTEPAD_MAX_LINE_LEN) new_len = NOTEPAD_MAX_LINE_LEN - 1;

        char *merged = kmalloc((size_t)(new_len + 1));
        if (merged) {
            memcpy(merged, first, (size_t)sc);
            int tail = last_len - ec;
            if (sc + tail > NOTEPAD_MAX_LINE_LEN - 1) tail = NOTEPAD_MAX_LINE_LEN - 1 - sc;
        ...
        /* Free lines sl..el */
        for (int i = sl; i <= el; i++) {
            if (app.buffer.lines[i]) kfree(app.buffer.lines[i]);
        }
        ...
        for (int i = app.buffer.line_count - removed; i < app.buffer.line_count; i++) {
            app.buffer.lines[i] = NULL;
        }
...
static void notepad_select_all(void) {
    ...
    app.selection.end_col  = (int)np_strlen(
...
static void notepad_free_undo(void) {
    for (int i = 0; i < app.undo_line_count; i++) {
        if (app.undo_lines[i]) { kfree(app.undo_lines[i]); app.undo_lines[i] = NULL; }
    }
...
static void notepad_free_redo(void) {
    for (int i = 0; i < app.redo_line_count; i++) {
        if (app.redo_lines[i]) { kfree(app.redo_lines[i]); app.redo_lines[i] = NULL; }
    }
...
static void notepad_save_undo(void) {
    ...
    for (int i = 0; i < app.buffer.line_count && i < NOTEPAD_MAX_LINES; i++) {
        app.undo_lines[i] = notepad_strdup(
            app.buffer.lines[i] ? app.buffer.lines[i] : "");
    }
...
static void notepad_do_undo(void) {
    ...
    notepad_free_redo();
    for (int i = 0; i < app.buffer.line_count && i < NOTEPAD_MAX_LINES; i++) {
        app.redo_lines[i] = notepad_strdup(
            app.buffer.lines[i] ? app.buffer.lines[i] : "");
    }
...
static void notepad_insert_char(char c) {
    ...
    if (col > len) col = len;
    if (len >= NOTEPAD_MAX_LINE_LEN - 1) return;  /* Line full */
...
static void notepad_insert_newline(void) {
    ...
    if (app.buffer.line_count >= NOTEPAD_MAX_LINES) return;
    ...
    if (!truncated) { if (new_line) kfree(new_line); return; }
...
static void notepad_move_cursor(int dl, int dc) {
    ...
    /* Clamp line */
    if (line < 0) line = 0;
    if (line >= app.buffer.line_count) line = app.buffer.line_count - 1;
...
static void notepad_open_file(const char *name) {
    ...
    /* Copy filename */
    int i = 0;
    while (name[i] && i < 63) { app.buffer.filename[i] = name[i]; i++; }
...
static void notepad_menu_action(int menu, int item) {
    ...
    if (menu == MENU_FILE) {
        switch (item) {
            case FMENU_NEW:     notepad_do_new(); break;
            case FMENU_OPEN:    notepad_do_open(); break;
            case FMENU_SAVE:    notepad_do_save(); break;
            case FMENU_SAVE_AS: notepad_do_save_as(); break;
            case FMENU_EXIT:
        ...
        switch (item) {
            case EMENU_UNDO:       notepad_do_undo(); break;
            case EMENU_REDO:       notepad_do_redo(); break;
            case EMENU_CUT:
                notepad_copy_selection();
                notepad_delete_selection();
                break;
            case EMENU_COPY:       notepad_copy_selection(); break;
            case EMENU_PASTE: {
                const char *data = clipboard_get_data();
                if (data) {
                    if (notepad_has_selection()) notepad_delete_selection();
                    else notepad_save_undo();
            ...
            case EMENU_SELECT_ALL: notepad_select_all(); break;
            default: break;
        }
    }
}
...
static void notepad_draw_dropdown(window_t *win) {
    if (app.active_menu == MENU_NONE) return;
    ...
    if (app.active_menu == MENU_FILE) {
        ...
        items[0] = "New";       shortcuts[0] = "Ctrl+N"; separators[0] = false;
        items[1] = "Open...";   shortcuts[1] = "Ctrl+O"; separators[1] = false;
        items[2] = "Save";      shortcuts[2] = "Ctrl+S"; separators[2] = false;
        items[3] = "Save As..."; shortcuts[3] = "";       separators[3] = false;
        items[4] = "";          shortcuts[4] = "";        separators[4] = true;
        items[5] = "Exit";      shortcuts[5] = "Ctrl+Q"; separators[5] = false;
    } else {
        ddx = (int16_t)(win->x + 41);
        ddy = (int16_t)(win->y + TITLEBAR_H + MENUBAR_H);
        item_count = EMENU_COUNT;
        items[0] = "Undo";      shortcuts[0] = "Ctrl+Z"; separators[0] = false;
        items[1] = "Redo";      shortcuts[1] = "Ctrl+Y"; separators[1] = false;
        items[2] = "";          shortcuts[2] = "";        separators[2] = true;
        items[3] = "Cut";       shortcuts[3] = "Ctrl+X"; separators[3] = false;
        items[4] = "Copy";      shortcuts[4] = "Ctrl+C"; separators[4] = false;
        items[5] = "Paste";     shortcuts[5] = "Ctrl+V"; separators[5] = false;
        items[6] = "";          shortcuts[6] = "";        separators[6] = true;
        items[7] = "Select All"; shortcuts[7] = "Ctrl+A"; separators[7] = false;
    }
    ...
    for (int i = 0; i < item_count; i++) {
        ...
        /* Hover highlight */
        if (i == app.hover_item) {
            gfx_fill_rect((int16_t)(ddx + 2), iy,
                          (uint16_t)(dd_w - 4), 14, COLOR_HIGHLIGHT);
        }

        gfx_draw_text((int16_t)(ddx + 8), (int16_t)(iy + 3),
                       items[i], COLOR_BLACK);
...
static void notepad_draw_text_area(window_t *win) {
    ...
    for (int row = 0; row < vis_lines; row++) {
        ...
        if (src_line >= app.buffer.line_count) break;
...
static void notepad_draw_scrollbars(window_t *win) {
    ...
    /* Background */
    gfx_fill_rect((int16_t)vscroll_x, (int16_t)vscroll_y,
                  VSCROLL_W, (uint16_t)vscroll_h, COLOR_BORDER);

    /* Up arrow */
    gfx_fill_rect((int16_t)vscroll_x, (int16_t)vscroll_y,
                  VSCROLL_W, SCROLL_ARROW_SIZE, COLOR_TEXT_LIGHT);
    gfx_draw_rect((int16_t)vscroll_x, (int16_t)vscroll_y,
                  VSCROLL_W, SCROLL_ARROW_SIZE, COLOR_BORDER);
    ...
    gfx_fill_rect((int16_t)vscroll_x, (int16_t)down_y,
                  VSCROLL_W, SCROLL_ARROW_SIZE, COLOR_TEXT_LIGHT);
    gfx_draw_rect((int16_t)vscroll_x, (int16_t)down_y,
                  VSCROLL_W, SCROLL_ARROW_SIZE, COLOR_BORDER);
    gfx_draw_char((int16_t)(vscroll_x + 2), (int16_t)(down_y + 2),
                  'v', COLOR_BLACK);
    ...
    if (track_h > 0 && app.buffer.line_count > vis_lines) {
        ...
        if (thumb_h < SCROLL_THUMB_MIN) thumb_h = SCROLL_THUMB_MIN;
    ...
    /* Background */
    gfx_fill_rect((int16_t)hscroll_x, (int16_t)hscroll_y,
                  (uint16_t)hscroll_w, HSCROLL_H, COLOR_BORDER);

    /* Left arrow */
    gfx_fill_rect((int16_t)hscroll_x, (int16_t)hscroll_y,
                  SCROLL_ARROW_SIZE, HSCROLL_H, COLOR_TEXT_LIGHT);
    gfx_draw_rect((int16_t)hscroll_x, (int16_t)hscroll_y,
                  SCROLL_ARROW_SIZE, HSCROLL_H, COLOR_BORDER);
    gfx_draw_char((int16_t)(hscroll_x + 3), (int16_t)(hscroll_y + 2),
                  '<', COLOR_BLACK);

    /* Right arrow */
    int right_x = hscroll_x + hscroll_w - SCROLL_ARROW_SIZE;
    gfx_fill_rect((int16_t)right_x, (int16_t)hscroll_y,
                  SCROLL_ARROW_SIZE, HSCROLL_H, COLOR_TEXT_LIGHT);
    gfx_draw_rect((int16_t)right_x, (int16_t)hscroll_y,
                  SCROLL_ARROW_SIZE, HSCROLL_H, COLOR_BORDER);
    ...
    if (track_w > 0 && max_w > vis_cols) {
        ...
        if (thumb_w < SCROLL_THUMB_MIN) thumb_w = SCROLL_THUMB_MIN;
...
static void notepad_draw_statusbar(window_t *win) {
    ...
    /* Line number (1-based) */
    {
        ...
        int ti = 0;
        if (num == 0) { tmp[ti++] = '0'; }
        else { while (num > 0) { tmp[ti++] = (char)('0' + (char)(num % 10)); num /= 10; } }
        for (int i = ti - 1; i >= 0; i--) status[pos++] = tmp[i];
    }

    status[pos++] = ','; status[pos++] = ' ';
    status[pos++] = 'C'; status[pos++] = 'o'; status[pos++] = 'l';
    status[pos++] = ' ';
...
void notepad_launch(void) {
    ...
    if (notepad_wid >= 0 && gui_get_window(notepad_wid)) return;
    ...
    app.pid = process_create(notepad_process_entry, "notepad",
                             DEFAULT_STACK_SIZE);
    if (app.pid == 0) {
...
void notepad_handle_key(uint8_t scancode, char character) {
    ...
    if (ctrl_held) {
        ...
        switch (scancode) {
            case SC_KEY_N: notepad_do_new(); goto done;
            case SC_KEY_O: notepad_do_open(); goto done;
            case SC_KEY_S: notepad_do_save(); goto done;
            case SC_KEY_Q:
                gui_destroy_window(notepad_wid);
                notepad_wid = -1;
                return;
            case SC_KEY_Z: notepad_do_undo(); goto done;
            case SC_KEY_Y: notepad_do_redo(); goto done;
            case SC_KEY_X:
                notepad_copy_selection();
                notepad_delete_selection();
                goto done;
            case SC_KEY_C: notepad_copy_selection(); goto done;
            case SC_KEY_V: {
                const char *data = clipboard_get_data();
                if (data) {
                    if (notepad_has_selection()) notepad_delete_selection();
    ...
    if (ctrl_held && character >= 1 && character <= 26) {
        switch (character) {
            case 14: notepad_do_new(); goto done;       /* Ctrl+N */
            case 15: notepad_do_open(); goto done;      /* Ctrl+O */
            case 19: notepad_do_save(); goto done;      /* Ctrl+S */
            case 17:                                    /* Ctrl+Q */
                gui_destroy_window(notepad_wid);
                notepad_wid = -1;
                return;
            case 26: notepad_do_undo(); goto done;      /* Ctrl+Z */
            case 25: notepad_do_redo(); goto done;      /* Ctrl+Y */
            case 24:                                    /* Ctrl+X */
                notepad_copy_selection();
                notepad_delete_selection();
                goto done;
            case  3: notepad_copy_selection(); goto done; /* Ctrl+C */
            case 22: {                                    /* Ctrl+V */
                const char *data = clipboard_get_data();
                if (data) {
                    if (notepad_has_selection()) notepad_delete_selection();
    ...
    if (scancode == SC_ARROW_UP || scancode == SC_ARROW_DOWN ||
        ...
        switch (scancode) {
            case SC_ARROW_UP:    notepad_move_cursor(-1, 0); break;
            case SC_ARROW_DOWN:  notepad_move_cursor(1, 0); break;
            case SC_ARROW_LEFT:  notepad_move_cursor(0, -1); break;
            case SC_ARROW_RIGHT: notepad_move_cursor(0, 1); break;
        }
    ...
    if (scancode == SC_END) {
        ...
        app.buffer.cursor_col = (int)np_strlen(app.buffer.lines[app.buffer.cursor_line]);
        if (shift_held) {
            app.selection.end_line = app.buffer.cursor_line;
            app.selection.end_col  = app.buffer.cursor_col;
        }
    ...
    /* Tab - insert 4 spaces */
    if (scancode == SC_TAB) {
        for (int i = 0; i < 4; i++) notepad_insert_char(' ');
        goto done;
    }
...
void notepad_handle_mouse(int16_t mx, int16_t my, uint8_t buttons,
    ...
    if (mx < content_x || mx >= content_x + content_w ||
    ...
    if (mx >= vscroll_x && mx < vscroll_x + VSCROLL_W &&
        my >= vscroll_y && my < vscroll_y + vscroll_h) {
        if (pressed) {
            if (my < vscroll_y + SCROLL_ARROW_SIZE) {
                /* Up arrow */
                if (app.buffer.scroll_y > 0) app.buffer.scroll_y--;
            } else if (my >= vscroll_y + vscroll_h - SCROLL_ARROW_SIZE) {
                /* Down arrow */
                int max_scroll = app.buffer.line_count - vis_lines;
                if (max_scroll < 0) max_scroll = 0;
                if (app.buffer.scroll_y < max_scroll) app.buffer.scroll_y++;
            } else {
                /* Track click - page up/down */
                int track_mid = vscroll_y + vscroll_h / 2;
                if (my < track_mid) {
                    app.buffer.scroll_y -= vis_lines;
                    if (app.buffer.scroll_y < 0) app.buffer.scroll_y = 0;
                } else {
                    app.buffer.scroll_y += vis_lines;
                    int max_scroll = app.buffer.line_count - vis_lines;
                    if (max_scroll < 0) max_scroll = 0;
    ...
    if (my >= hscroll_y && my < hscroll_y + HSCROLL_H &&
        mx >= hscroll_x && mx < hscroll_x + edit_w) {
        if (pressed) {
            if (mx < hscroll_x + SCROLL_ARROW_SIZE) {
                /* Left arrow */
                if (app.buffer.scroll_x > 0) app.buffer.scroll_x--;
                ...
                if (mx < track_mid) {
                    ...
                    if (app.buffer.scroll_x < 0) app.buffer.scroll_x = 0;
                } else {
                    app.buffer.scroll_x += vis_cols;
                    int max_w = notepad_max_line_width();
                    if (app.buffer.scroll_x > max_w - vis_cols)
    ...
    if (mx >= edit_x && mx < edit_x + edit_w &&
        ...
        if (click_col > len) click_col = len;
