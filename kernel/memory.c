...
void pmm_init(uint32_t kernel_end) {
    ...
    pmm_mark_region(0x110000, 0x190000, 1);                  /* kernel stack (512KB) */
...
static int check_canaries(heap_block_t *block) {
    if (block->canary_front != CANARY_FRONT) return 0;
    uint32_t *back_canary = (uint32_t*)get_back_canary_addr(block);
    return (*back_canary == CANARY_BACK);
}

void heap_init(uint32_t initial_pages) {
    if (initial_pages == 0) return;
    ...
    serial_printf("[heap] Initialized: %u KB at 0x%x\n",
                  (total_size / 1024), (uint32_t)base);
}

static void track_allocation(void *ptr, uint32_t size, const char *file, uint32_t line) {
...
static void track_free(void *ptr) {
    for (uint32_t i = 0; i < MAX_ALLOCATIONS; i++) {
        if (tracker.records[i].active && tracker.records[i].address == ptr) {
            ...
            if (tracker.active_count > 0) tracker.active_count--;
    ...
    while (current) {
        ...
        if (current->free && current->size >= needed) {
            if (current->size >= needed + HEAP_MIN_SPLIT) {
                ...
                heap_block_t *new_block = (heap_block_t*)((uint32_t)current +
                    sizeof(heap_block_t) + current->size + sizeof(uint32_t));
                new_block->size = old_size - needed - sizeof(heap_block_t) - sizeof(uint32_t);
...
void kfree(void *ptr) {
    ...
    if (block->free) {
        serial_printf("[heap] Double-free detected at 0x%x (previously freed at %u ms)\n",
    ...
    for (uint32_t i = 0; i < MAX_ALLOCATIONS; i++) {
        if (tracker.records[i].active) {
            ...
            if (age >= threshold_ms) {
                ...
                serial_printf("[heap] LEAK: %u bytes at 0x%x (age: %u ms, %s:%u)\n",
                              tracker.records[i].size,
                              (uint32_t)tracker.records[i].address,
                              age,
