...
ADR 0066 extends the same handle through file, block-static, automatic, pointer-derived, member, and indexed object access. `LOAD` captures eight bytes in a private snapshot. `STORE` and `STORE_VALUE` copy the snapshot to an object, with `STORE_VALUE` preserving it for assignment results. Parameters, arithmetic, mutation, and mixed-width conversion were outside that extension. ADRs 0067 through 0074 later add those wide integer paths while retaining the private snapshot.

ADR 0067 carries the handle through declared parameters and named call arguments. ADR 0068 extends `BINARY` with wide left shift, signed or unsigned right shift, AND, OR, and XOR. It also extends `CONVERT` across the represented and eight-byte integer boundary. Each operation result keeps one public handle and receives a distinct emitter-owned snapshot. ADR 0069 adds wide comparisons and scalar conditions while keeping comparison results in the represented signed `int` type. ADR 0070 adds wide addition, subtraction, unary plus, unary minus, and bitwise complement. Results that can change bits receive fresh snapshots.

### Floating scalar transport extension

ADR 0076 carries `float` as one raw four-byte value and `double` as one handle to an emitter-owned eight-byte snapshot. Existing `LOAD`, `STORE`, `STORE_VALUE`, `DISCARD`, `CALL`, `VARIADIC_ARGUMENT`, and `RETURN_VALUE` instructions retain their meanings. No floating arithmetic or conversion instruction is added.
