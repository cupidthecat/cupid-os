...
The IR contract extracts the exact unchanged helper body and pins its 12 instructions, three-entry maximum stack depth, parameter identities, types, conversions, branch targets, and source locations. It also pins the unchanged `add2` function in `bin/cupidc_test3.cc` as two parameter loads followed by `ADD` and a scalar return. Separate cases cover a constant return, an empty `void` return, a signed-to-unsigned assignment conversion, a static inline definition, and a non-inline definition after an inline declaration. Negative cases cover a malformed body, an unsupported `if` statement, unsupported division, an explicit cast, plain and `extern` external-inline definitions, a 64-bit ABI, a constrained arena, rollback, and same-job recovery.
