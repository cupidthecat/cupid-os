...
The original negative contract rejected a represented 64-bit switch condition. ADR 0071 replaces that boundary with signed and unsigned full-width dispatch plus malformed wide-type contracts. The remaining negatives reject an out-of-range case expression index, extra case or default payloads, and a noncanonical 32-bit enumerator value. Object failure for a malformed case leaves output empty and rewinds temporary allocations. Every IR failure preserves the supplied translation unit.
