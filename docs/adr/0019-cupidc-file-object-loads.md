...
ADR 0020 closes the plain four-byte integer file-object assignment frontier named above. ADR 0021 extends `FILE_ADDRESS` to complete record address roots and lowers direct ordinary members through `MEMBER_ADDRESS`. ADR 0022 extracts represented four-byte bit fields through `BIT_FIELD_LOAD`, while ADRs 0063 and 0064 add assignment and mutation at that storage boundary. Issue #25 remains open for non-four-byte bit-field storage, partial volatile mutation, atomic ordering, broader values, production integration, and staged self-hosting.

ADR 0066 extends `FILE_ADDRESS`, `LOAD`, `STORE`, and `STORE_VALUE` to signed and unsigned eight-byte integer objects. The active `get_cpu_freq` function now reads its unchanged `tsc_freq` object through that path. Atomic wide access remains unsupported.

ADR 0076 extends the same object-address path to exact `float` and `double` loads and stores. File objects, block statics, automatics, pointer dereferences, ordinary members, and indexed elements use their normal addresses. A `double` load captures a private snapshot, while a `float` load keeps one raw word. Atomic floating access remains unsupported.
