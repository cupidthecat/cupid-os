...
Atomic, pointer, wide integer, floating, and aggregate assignment lowering remain unsupported. Compound assignment and update expressions also remain unsupported because they require their own read, computation, conversion, store, and prefix or postfix result rules. Member and subscript destinations remain behind their missing address forms. Malformed child references, result metadata, and stack shapes are invalid frozen units rather than deferred language features.
...
Issue #25 remains open. General statements, member and subscript addresses, compound and update lowering, atomic ordering, pointer and other value widths, floating and aggregate values, indirect and variadic calls, call-site alignment, production integration, and staged self-hosting still remain.
