...
ADR 0058 adds block-scope external objects. ADR 0059 adds block typedefs while preserving the same lexical ordinary-name scope used by local objects and parameters. ADR 0060 adds block-scope function declarations. ADR 0061 narrows this record's enum boundary by adding declaration-position block enums with lexical enumerator values. Enum definitions nested in record members or type names and enum definitions in function-definition parameter lists remain open.
