...
A block type name or record member may refer to an enum tag that is already visible. These uses retain the same enum type and do not publish another tag or enumerator. Defining a new enum in either context remains outside this slice because those contexts do not own lexical enumerator records.

Linear IR validates each enumerator record and advances the declaration cursor without allocating storage or emitting declaration work. A represented 32-bit use becomes `CTOOL_C_IR_INSTRUCTION_INTEGER`. The ELF emitter therefore sees the same instruction sequence as an equivalent direct integer constant. It creates no frame slot, symbol, or relocation for the enum name. A GNU-width enumerator may survive an unused declaration, but using a 64-bit value still stops at the existing unsupported runtime-value boundary.
