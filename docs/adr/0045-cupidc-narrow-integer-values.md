...
Narrow mutation remains outside this decision. Compound assignments and prefix or postfix updates still require the established four-byte integer path. Narrow bit-field storage, atomic access, 64-bit integers and floating-point values, aggregate values and initialization, variadic calls, casts involving function pointers, object-pointer casts to or from narrow integers, pointer-to-`_Bool` casts, and 16-byte call-site alignment also remain unsupported.
