...
stage-two producer trio builds stage three. The driver compares every C and
assembly object plus all five PE tools byte for byte. It runs help, useful
success, and useful failure behavior on both stages, rehashes the frozen and
live source closures at every boundary, and publishes one complete directory
with a `cupid.windows-bootstrap-report.v1` report. An occupied output, source
drift, seed drift, or failed behavior gate publishes nothing.
...
for Linux contract programs. A later promotion record must supply the complete
stage comparison, native behavior evidence, new seed hashes, and production
build results.
