...
Explicit casts that involve a function pointer remain outside this slice. This includes casts to or from integers, object pointers, compatible function pointers, and incompatible function signatures. The frontend may represent those scalar casts, but the IR rejects them until CupidC has a documented target conversion policy. Existing explicit casts among represented integers and object pointers are unchanged.
