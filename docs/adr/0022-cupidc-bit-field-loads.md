...
ADR 0063 later adds value-preserving plain assignment for represented four-byte bit fields, including pointer-derived and indexed record addresses. ADR 0064 adds compound assignment and prefix or postfix update for the same four-byte storage boundary. Issue #25 remains open. Narrow and wide storage units, partial volatile mutation, atomic ordering, other value widths, production integration, and staged self-hosting still remain.
