...
Useful negative contracts retain precise failures for Boolean, atomic, bit-field, and wider mutation. The malformed narrow computation-type case returns the invalid-unit diagnostic transactionally. The first positive update and compound tests stopped at the old unsupported-value check. The malformed computation test then returned unsupported until the validation path distinguished bad represented metadata from a genuinely unsupported computation type.
