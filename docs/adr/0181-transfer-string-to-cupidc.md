...
The repository-wide `make test` rerun passed 875 tests in 4,820.861 seconds,
with one platform-conditioned skip. That run also completed the GUI terminal
smoke and the checked build-graph audit.

The first repository sweep exposed two stale test records. The USB poll test
still opened `kernel/core/kernel.c`, and the x86 source manifest still
expected 212 assembly sites across 39 files. The active `.cc` source contains
218 sites across 40 files. A temporary Windows executable lock also affected
one object-contract process. After correcting the two records, the focused
18-test retry passed, and the clean full rerun completed without the lock.
