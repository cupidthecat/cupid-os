...
Issue #27 starts the compiler-on-compiler bootstrap with a fourteen-file CupidC, CupidASM, and CupidDis cohort. Ten of those files have no external hosted headers or runtime calls once they enter the existing `HOSTED_TOOLCHAIN_64` preprocessing profile. The same profile also contains the CupidLD and CupidObj cores. The separate `kernel/lang/as_elf.c` bridge brings the object gate to eleven cohort files. CupidC could parse nine implementation files, but several ordinary C forms still stopped IR lowering or object emission.
...
The active object gate now emits sixteen unchanged implementation files. Issue #27's fourteen-file CupidC, CupidASM, and CupidDis cohort consists of ten hermetic Toolchain files, `kernel/lang/as_elf.c`, and the three hosted adapters. CupidLD and CupidObj are the two adjacent cores. The adapters use the checked i386 Linux declaration profile, but a hosted runtime and a host-runnable ABI still have to be supplied at link time. The gate does not link tools, execute them on the host, compare staged compiler generations, or transfer a production build object. GCC or Clang still builds the compiler and every normal C object. Issues #25 and #27 remain open.
