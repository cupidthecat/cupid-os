...
Signed operations follow C's existing rule that overflow is undefined. Unsigned addition, subtraction, negation, and complement wrap modulo 2^64. Multiplication, division, remainder, wide compound assignment, increment and decrement, and wide `switch` dispatch remain separate work.
...
Issue #25 remains open. Wide multiplication, division, remainder, mutation, switch dispatch, values without declared parameter types, floating values, production integration, staged self-hosting, and the fixed-point bootstrap remain unfinished.
