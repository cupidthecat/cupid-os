...
Signed operations follow C's existing rule that overflow is undefined. Unsigned addition, subtraction, negation, and complement wrap modulo 2^64. Multiplication, division, remainder, wide compound assignment, and increment and decrement remain separate work. ADR 0071 later adds wide switch dispatch.
...
Issue #25 remains open. Wide multiplication, division, remainder, mutation, values without declared parameter types, floating values, production integration, staged self-hosting, and the fixed-point bootstrap remain unfinished.
