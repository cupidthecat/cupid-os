...
The hosted `ctool_c_layout_types` contract fixes scalar, pointer, array, enum, vector, function-marker, aligned-wrapper, struct, union, class, bit-field, flexible-array, packed, and explicit-alignment representation to the Cupid i386 target. It resolves immutable index graphs with an iterative strong-edge walk, preserves presumed/physical semantic locations, reclaims traversal scratch, and returns stable job-owned layouts transactionally. Manual typed graphs pin all 54 FAT16 member offsets plus active Doom, process, syscall-table, descriptor, and per-CPU ABI shapes. Those are active-source ABI oracles, not evidence that CupidC parses the unchanged headers: the declaration frontend is the next slice, the private kernel compiler still owns production parsing/code generation, and this hosted capability adds no OS build ownership or boot claim.
