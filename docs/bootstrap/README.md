...
ADR 0182 completes the separate three-root `DOOM_COMPAT_I386` frontier at
compiler head. Explicit non-atomic pointer-to-pointer casts retain a static
...
host-built current compiler and a Cupid-built current compiler produce the
same 27,992-byte dglibc, 14,352-byte libc-stub, and 10,232-byte platform
objects. The checked seed and normal recipes have not moved yet. Seed
promotion, host-object comparison, `.cc` renames, and runtime proof remain
part of the Doom ownership handoff.
