...
zeros the complete object. Explicit leaves accept a zero-valued integer
...
following pointers. All six comparisons accept matching long-double
...
finite nonzero values, subnormals, infinities, and NaNs are true. Runtime
casts, assignments, arguments, and returns convert between `long double`
and signed or unsigned integers at 8, 16, 32, and 64 bits. Integer input uses
...
and restores that copy. The unsigned 64-bit path
splits at `2^63`. Hexadecimal
...
static long-double arithmetic, comparisons, truth, conditionals, and width
or integer conversions, nonzero integer static initializers, other
floating-to-wide conversions, increment and decrement,
SIMD, floating atomics,
...
and deterministic repeated emission. The hosted i386 runtime reads the three
object words for every payload and checks aggregate markers.
`sizeof(float) - 4` remains the integer constant-expression zero case; a nonzero integer
initializer still fails with a focused diagnostic.
...
direct-call boundary, and Cupid's native type spellings in the shared
...
Source-head CupidLD accepts this fixed-layout command:
...
The source-head proof assembles the Windows entry with both CupidASM stages,
...
The matrix is now 5/17/15. The checked seed and its historical 5/15/13 proof
do not change. PE32 is not a normal-build output or a checked Windows seed, so
the 447-transform ownership census also stays unchanged. ADR 0247 records the
format boundary. ADR 0248 records the import and Windows loader boundary.

The source-head CLI uses an adjacent-candidate publisher for both ELF and PE
...
The checked-seed bootstrap copies the exact bytes of its 43 source inputs into
...
379,312-byte source has SHA-256
`45a112be18fc9edab1680b1c1622eeb2e2f6b3333e12652be3e0593a8f612f2a`.
Checked CupidDis reports the same 4,704 text-symbol address/name pairs from the
...
user drivers remain host-built until CupidC gains a Windows runtime and the
later seed carries a runnable Cupid-built Windows tool.
...
symbol blob contains 114,421 meaningful bytes followed by three zero padding
bytes. Its 114,836-byte ELF32 object has SHA-256
...
The checked-seed C11 standalone-header sweep passes 157 of 159 non-Doom
...
passed a 5/15/13 behavior matrix. The normal target passes the checked seed
...
`#exe` markers. Checked manifests classify all 2,410 include operands as
2,173 direct quoted plus 237 direct angle forms with zero macro operands
across 688 active C-family inputs. The generated manifest drives 382 tracked
...
159 general non-Doom headers in the C11 profile; `scheduler.h` and
...
treat those spellings as ordinary identifiers. The graph contains 721 active
language inputs: 29 assembly files, 290 headers, and 402 Cupid C files. No
...
inventory covers 688 files and 2,410 include occurrences, split into 2,173
quoted and 237 angle forms.

The active-source digest is
`6643bdb848320e56a0c37091a463efeedd326a8ce9ee82e0cc400172d5c180e7`.
The 2,574,680-byte audit JSON has SHA-256
`cf4e12785dbb41a3c59bbbe5a93b57261ee4418de2eb895771be3b857bd63eff`,
and the 12,218-byte summary has SHA-256
`35db6363db6bf51950670a7cb7f48af62a47abbbbf630b3f8c3d02063785ce5a`.
...
Compiler head now accepts exact `fldcw %0` through the same state-memory
...
same-job recovery checks for FLDCW. The checked seed does not carry this form.
...
The checked seed carries 596 rows, 245 canonical mnemonics, and fingerprint
`DA15E97F`. Source head carries 602 rows, 247 canonical mnemonics, and
fingerprint `64429699`. Its six added rows cover signed x87 `FILD` and
`FISTP` memory operands at 16, 32, and 64 bits. The four SHRD rows cover
immediate or CL counts at both operand widths. The forward x87 row is canonical
...
seed, and ADR 0252 records the source-head x87 integer forms.
