...
The current OS baseline is still compiler-hosted: the Makefile defaults to Clang on Windows and GCC on Linux. A platform-neutral Cupid Toolchain foundation, a typed transactional CupidC preprocessing tape, deterministic ELF32 relocatable-object/read-only executable module, and shared typed 16/32-bit x86 instruction model now build through supported hosted contracts and real kernel adapters. The preprocessing module owns translation-phase tokenization, ordered object/function/variadic macros and operators, C11 conditionals and reproducible predefined macros, direct and macro-expanded includes, forced inputs, guarded/cache-aware traversal, canonical once identity, translation-unit pack metadata, and policy-neutral typed Cupid `#exe` markers. Checked manifests now classify all 2,297 tracked include operands as 2,101 direct quoted plus 196 direct angle forms with zero macro operands, all 101 active `#if`/`#elif` occurrences, the exact five active pragma sites, and the unconditional block-form `#exe` at `bin/feature6_exe.cc:7`. A Make-derived per-translation-unit manifest drives the same operation across all 345 tracked OS-delivery roots: 152 strict kernel, six Doom compatibility, 80 Doom-tree, three user, and 104 Cupid programs. A separate non-skipping gate covers the four generated kernel roots. The audit also keeps 22 browser fragments under `bin/browser.cc`, two delivered headers without an invented standalone context, and 24 hosted units explicitly deferred (15 on external headers/runtime and nine pending a hosted Cupid profile). `#line`, named GNU variadics, hosted-tool preprocessing ownership, parser/layout consumption, and the kernel cutover remain.
