...
Both hosts pass normal OS builds, all sixteen artifact-size checks, the
generated-install replay, user syscall ABI checks, and all three user builds.
Private four-CPU boots pass SMP runtime checks, `dis /bin/ls.cc`, and `ls`
without changing the source images. Independent post-run checks verify 448
artifacts and 478 control files per host. Kernel files, generated objects, and
user executables match across hosts. The bootstrap log records these production
and runtime results separately from the fixed-point proofs.
...
[The user compilation plan](NEXT-USER-COMPILATION.md) records configurable
output directories, retained-parent checks, and the Windows pathname work
needed before the three user compiler transactions can move.

[The artifact verification plan](NEXT-ARTIFACT-VERIFICATION.md) records the next
native verification boundary and its remaining host dependencies.

The generated-install handoff check prepares the normal image, compares all
three objects with the checked-seed Python wrapper, then forces exactly three
native compiler calls with Python coordinators and host code-producing tools
poisoned:

```sh
python tools/validate_kernel_compile_handoff.py --cohort generated-install --wrapper-oracle --prepare-target all --jobs 2 --report build/bootstrap/generated-handoff-validation
```

Use a fresh report directory. The check requires unchanged artifact and control
bytes and timestamps, and rejects compiler calls from other cohorts.

The earlier kernel-only handoff check remains available after the normal build prerequisites
are available:

```sh
python tools/validate_kernel_compile_handoff.py --prepare-target all --jobs 4 --report build/bootstrap/kernel-handoff-validation
```

Use `python3` on Linux and a new report directory for each run. The utility
prepares the ordinary image, checks the original profile transaction, then
forces all 157 native compiler calls with Python coordinators and host
C/ASM/link commands poisoned. Both validation phases require unchanged
artifact and control bytes and timestamps. Dry-run wrapper predictions remain
advisory because Make cannot predict unchanged output timestamps; actual
execution must contain no forbidden command. The bootstrap log records the
actual OS-build and runtime results separately from fixed-point convergence.

The preceding `9d2529a7` pair built the normal image on Windows and native Linux and
passes all 16 exact artifact checks. Both hosts produce identical pass-one
ELF, final ELF, and raw-kernel bytes. The raw kernel is 9,552,000 bytes with
SHA-256 `5bcbc558ffb438227c96b89522a875daffdeaf1c92be93fd67f0eb62205fb418`.
Each image passes a private four-CPU SMP and CupidC `ls` smoke without changing
the published image. Full Doom gameplay acceptance remains open.

The preceding kernel-only production replays passed on both hosts: each executed all 157 native compiles
without a forbidden command and preserves 448 artifacts and 471 controls,
including their timestamps. Linux completes the full validator invocation.
Windows completes normal preparation, then a separately verified corrected
replay after repairing the stale-profile dry-run check. The log preserves the
original failure and binds the reused preparation to the successful replay.
...
[ADR 0389](../adr/0389-use-the-pinned-proc-directory-for-anonymous-tool-launches.md)
...
After a namespace-replacement failure, keep the displaced directory and
transaction files until recovery has been inspected. The old output may be
parked there under `.cupidbuild-old-*`, beside the retained output lock;
the current public directory may belong to the concurrent writer. Stop those
writers, require a regular non-symlink backup, and verify it against the known
previous output before any manual restoration. A matching filename alone is
not sufficient. Do not remove this evidence with a broad cleanup or restore
through the replacement
directory without establishing who owns it.

The committed Windows launch repair keeps the full 500-input kernel inspection
within the command-line limit by using filenames relative to the retained
private working directory. Linux keeps sealed descriptor paths. Tests cover
successful full-cohort inspection and rejection of the final input with the
previous output preserved. [ADR 0387](../adr/0387-keep-windows-kernel-inspection-within-the-command-line-limit.md)
records the repair. Clean proofs from `16a86f5b` passed 33/7/38 Linux and
21/7/25 Windows failure/help/success groups. Both checked seeds carry these
repairs; ADR 0382 records the independently verified manifests and artifacts.

The preceding `16a86f5b` Windows post-promotion proof also passed: every checked seed tool
matched stages two, three, and four, all 32 final artifact pairs matched, and
the 21/7/25 behavior inventory passed. This verifies self-consumption of the
preceding pair, separately from its original-parent clean proof. That
Linux Toolchain publication includes the corrected preprocessor inventory,
ABI consumers, and self-host object expectations. All 62 stage pairs match,
and all six published tools match the checked Linux seed exactly. Its 22
artifacts pass native manifest verification on Linux and Windows. The
29,619-byte manifest has SHA-256
`419d1e6f3b4a90b1cbfae04f5aa397c900449efe7d4197b7116f0cadb5070afd`.
It binds 76 publication inputs and 59 bootstrap inputs. Windows verification
uses a stable copy containing the exact published artifacts, both complete
seeds, and all 21 verifier build inputs. The full executable suite and user build pass.

The final-cohort 155-test CupidBuild CLI suite passed on each host, including
both 500-input cases. Windows had 12 platform skips; Linux on `/mnt/c` had
16 Windows-only skips. No old-capability skip applied. The bootstrap log
separates these results from the earlier source preflight using `962e476b` tools.

The corrected 167-case production suite passed with two platform skips. The
isolated Linux archive passed 48 handoff tests with one Windows-junction skip;
it ran both cases omitted on Windows, plus parallel ordering and ISO checks.

The pre-correction Linux normal build passed all 16 measured artifact rows
...
separate pinned-IWAD Doom probe previously panicked during HomeFS rewriting
with an EHCI DMA-ownership error. Three later 300-second probes and one
1,200-second diagnostic preserved the image and did not reproduce that panic,
but all missed timedemo completion. The longer run completed renderer
initialization and reached `ST_Init`; gameplay acceptance remains open. The first
Windows serial replay timed out in broad CupidDis inspection and preserved
the previous raw kernel. An isolated guarded replay later published the
Linux-matching raw kernel with unchanged timeouts. This is not main Windows
Make acceptance, and the original timeout's cause remains unknown. The
bootstrap log records the measured hashes. The earlier `962e476b` Windows
replay expanded 431 absolute input paths into a 45,255-byte command line and
stopped before publishing a normal raw kernel or disk image. [ADR
0382](../adr/0382-promote-profile-publication-seeds.md) separates that
failed replay from the new clean proofs. The Windows image with preserved
FAT contents separately passes an SMP and `ls` smoke.

At the earlier kernel-compiler handoff (ADR 0393), the recorded state was:

Checked CupidBuild provides `assemble-iso-pattern` for the active
...
and preserves the timestamp when the output is unchanged. The clean
`16a86f5b` proofs include successful publication and wrong-pattern rollback in
their 33/7/38 Linux and 21/7/25 Windows failure/help/success groups. The normal
recipe now selects CupidBuild directly with the source, Makefile, and complete
seed closure. Both repaired seeds carry the command from revision `16a86f5b`.
[ADR 0384](../adr/0384-add-typed-iso-pattern-publication-to-cupidbuild.md)
records the source boundary; [ADR 0386](../adr/0386-transfer-iso-pattern-publication-to-cupidbuild.md)
records the recipe handoff. The graph now has 354 CupidBuild and 98 Python
participations across 452 transforms. The enclosing ISO image still uses
Hostbuild's independent renderer and guarded publication. Make now finishes
`test_iso/hello.iso` before `verify-artifact-sizes` through an order-only
prerequisite. CupidBuild's temporary root entries change directory mtime even
when the pattern output is reused, so they must finish before the artifact
verifier pins that directory. The audit requires this exact scheduling edge;
the size-policy content inputs and ownership counts do not change.

[Compiler coordinator migration](NEXT-COMPILER-COORDINATOR.md) records the
completed handoff of all 157 kernel-profile compilations to checked CupidBuild.
Every root has an explicit frozen source/header closure. The remaining 83
Doom compilations need the discovery policy described in
[the Doom coordinator audit](NEXT-DOOM-COORDINATOR.md). ADR 0393 records the
kernel recipe handoff; its OS build and runtime checks are separate from
the completed paired bootstrap proofs.

Checked Windows publication also restores a rejected candidate through
...
protection. The repair first shipped in the `16a86f5b` pair and remains in
the checked `9d2529a7` pair. [ADR 0385](../adr/0385-restore-windows-candidates-through-a-retained-directory-bridge.md)
records the source repair; [ADR 0382](../adr/0382-promote-profile-publication-seeds.md)
records its checked-seed carriage.
...
## 2026-09-19 production profile handoff

The paired Linux and native Windows seeds now carry the profile publisher,
checked CupidC runner, and publication fixes from revision
`16a86f5b1693e017c36c6d902df9946c5d674b17`. Both bind the same 59-input
snapshot, `54b411b6ed05725101f5859106facb43639f2d02cb2b0ea2f6334e21375bda55`.
The candidate proofs match every stage-three/stage-four object and tool and
pass 33/7/38 Linux and 21/7/25 native Windows failure/help/success groups.
ADR 0382 records the exact manifests and parent lineage.

The normal Doom profile recipe invokes checked
`cupidbuild generate-profile-manifest`. CupidBuild discovers and freezes the
83-source cohort and 304 headers and include fragments, checks CupidObj's
canonical JSON against an independent native renderer, and owns drift checks,
locking, rollback, and publication. Equal bytes retain their timestamp. Make
prepares the POSIX `build/bootstrap` parent; the Windows transaction retains
its own directory-creation contract. The native Windows production fixture
passes byte parity, unchanged publication, and renamed-source rollback with
the final `16a86f5b` pair. The bootstrap log records both production suites.

The profile handoff transferred one production participation, reaching 196
for CupidBuild and 256 for Python. The ISO pattern handoff in ADR 0386 moves
the then-current counts to 197 and 255 across the same 452-transform graph.
Disk and ISO image publication, guarded C
compilation, artifact and manifest verification, and staged bootstrap
coordination still use Python. ADR 0383 records the recipe and audit boundary.
The dated checkpoints below retain their original evidence.
...
CupidLD, CupidObj, and CupidBuild calls no longer cross WSL. Linux fixed-point
...
the final report comparison authority. ADR 0318 records the preceding seed
...
signatures. ADR 0335 records explicit assembly function symbols and
relocatable code anchors, ADR 0336 records their combined promotion with
raw-map v2 and production adoption, ADR 0337 records the in-OS CupidASM
...
ADR 0377 records typed source-head Doom profile publication, ADR 0379 records
linked kernel CupidASM publication recovery, and ADR 0380 advances the exact
promoted-v2 parent window for the next paired refresh.
...
and `.inc` inputs, freezes that inventory with the complete six-tool seed, and
builds `CUPROF1` from the frozen input bytes. Frozen CupidObj authors the
candidate first. CupidBuild renders the canonical JSON independently and
requires byte parity before it rechecks membership, inputs, seed, output,
parent, and lock state. Equal bytes keep the existing timestamp. Malformed
input, links, aliases, parity failures, and drift preserve the previous
manifest.

The fixed production path has a deliberate platform split. Windows may create
missing `build` or `build/bootstrap` components with parent-relative
`NtCreateFile`. It records the returned handle identity and removes only its
own empty directories on rollback. POSIX requires both directories to exist
before the transaction opens. CupidBuild refuses a clean root because
`mkdirat` and a later `openat` cannot prove that the opened public entry is the
directory this process created.

Discovery keeps every traversed directory pinned. POSIX records identity plus
nanosecond mtime and ctime. Windows queries the exact named entry twice with
`FileIdFullDirectoryInformation`, compares its file ID, LastWriteTime, and
ChangeTime with the retained handle, and rejects devices and reparse points.
Each closure checkpoint validates all retained and public bindings in two
complete passes. The last successful pass is the checkpoint; a writer can
still change the tree afterward because neither host supplies a durable
directory snapshot here.

POSIX flat transactions put frozen inputs, maps, and captured streams in
sealed anonymous memfds and give tools `/proc/self/fd/N` paths. The reservation,
owner lock, candidate, publication alias, and a parked old output remain named
when the publication protocol needs them. DrvFS no-replace fallback links the
destination before unlinking the source. If exchange rename is unavailable,
the old output is first parked as a verified hard link and can be restored
over a failed candidate install. These compare-then-unlink cleanup steps still
have a formal same-user namespace race.

After source-only drift, a verified rollback and a second public-binding check
allow ordinary cleanup. Ambiguous root, output-parent, output, lock, or
candidate state sets the namespace-interference result. CupidBuild then closes
retained handles without deleting uncertain names, preserves the readable old
output, and leaves transaction residue for recovery. The focused WSL checks
pass ordinary create and unchanged publication plus two restored-directory
races; both races preserve the previous bytes and mtime with no residue.
The post-install source-drift race also passes, restoring the previous
manifest and removing all transaction state.
The dedicated native Windows process module covers the exact handle allowlist,
the directory-record race, frozen-input sharing, retained cleanup authority,
and discovery-handle closure. Strict native Clang and WSL syntax checks pass
as well.

Windows now retains frozen inputs with read sharing while blocking write and
delete opens. Mutable candidates allow read, write, and delete sharing.
Cleanup closes a verified frozen handle, reopens the same named identity with
delete access, and verifies the full snapshot again before disposition.
Guarded assembly uses CupidASM's
`--caller-owned-output` mode so the enclosing CupidBuild transaction keeps one
...
A Windows rerun with plan-matched temporary manifests completed all 19/7/24
behavior groups. A clean same-commit proof on both hosts was still required
before paired publication at that checkpoint.

Together with the checked CupidC runner, those source-head definitions were
31/7/37 on Linux and 19/7/24 on native Windows. The checked seeds then predated
the profile command, so Make used the Python publisher and the graph had 195
CupidBuild and 257 Python participations. ADR 0377 records that source
boundary, and ADR 0381 records the publication and handle contract. ADRs 0382
and 0383 record the completed repaired-seed promotion and direct profile
handoff. The current ownership summary includes the later ISO pattern handoff.
...
include successful flattening and malformed-manifest rollback. Both checked
seeds now carry the command from revision
...
six-image prerequisite closure immutable. The audit evaluates the command
...
prove progressive-input rollback. The later kernel-symbol and kernel-flatten
cases bring the complete inventories to 28/6/35 on Linux and 17/6/22 on native
Windows. Both promoted seed cohorts now carry all three commands. The
normal `%.jpg.o` and `%.jpeg.o` rules invoke it directly with the complete
production seed closure. ADR 0364 records the capability, ADR 0367 records its
paired carriage, and ADR 0368 records the production handoff.
...
The ADR 0374 closure had 59 inputs. Its snapshot, which also
included typed kernel-symbol and kernel-flatten publication, had
SHA-256
`0b591a0bef928186641b3aa1fb98c1e145e6c4905c8b6cb87c34a1ace4bc87d2`.
Source CupidBuild accepts a promoted v2 manifest when `source_input_count` is
58 or 59 and rejects 57 and 60. This keeps the preceding 58-input seeds usable
while admitting the active 59-input source generation. The Linux and native
...
checked directory. The ADR 0374 candidates from revision
`0232cb57aad5d6bdfd7bd77499762514b2f0ebfd` converged on the 59-input
snapshot, and promoted-seed self-consumption then matched all six initial
images on both hosts. ADR 0365 records the source repair, ADR 0366 records the
compatibility rule, ADR 0367 and ADR 0370 record preceding pairs, and ADR 0374
records that earlier pair. ADR 0393 records the current checked pair.

At that earlier checkpoint, CupidBuild and the Cupid-built artifact verifier
accepted the kernel-symbol pair and then-active kernel-flattening pair as the
two exact v2 parent generations. The historical v1 parser keeps its original parent, but a
...
At the ADR 0359 checkpoint, the 64,500-byte report had SHA-256
...
That checkpoint's execution seed matched stage two for CupidC, CupidASM, CupidDis,
CupidLD, and CupidObj. CupidBuild differs because its source-head checked
CupidObj runner had not entered the paired seeds yet.
...
At the ADR 0360 checkpoint, the full Linux fixed-point test passed in 2,158.113
...
CupidBuild, whose checked CupidObj runner still awaited paired seed promotion.
...
That raised the source-head matrices to 25/6/32 failure, help, and success
cases on Linux and 14/6/19 on Windows. The paired checked seeds now carry the
runner, and Make uses it for 175 text wrappers, eight binary wrappers, and
three installation-source generators. ADR 0360 records the runner boundary,
ADR 0361 records seed promotion, and ADR 0362 records the recipe handoff.

The ADR 0374 policy-bound build compiled all 83 Doom roots, linked both kernel
...
At that checkpoint, `kernel/kernel.bin` was 9,536,524 bytes; the final and pass-one ELFs were
9,765,196 and 9,634,124 bytes. A preceding 9,501,220-byte checkpoint, which
...
runtime verification, and `/bin/ls.cc` JIT completion.
...
forwarding uses binary mode to preserve exact bytes. The extended startup
record contains an exact three-handle allowlist for standard input, standard
output, and standard error. An unrelated inheritable handle stays in the
parent. Borrowed stream handles are marked inheritable only during the launch
window and are cleared immediately afterward, so this path assumes a
single-threaded CupidBuild process. Cleanup removes a mutated file when its
identity still belongs to the runner and preserves a replacement with a
different identity. Both platforms recheck the live seed before releasing
output.
...
trampoline exactly. At that checkpoint the graph contained 452 transforms,
including 443 under root `all`; CupidBuild participated in four and Python in
448. ADR 0357 records the transfer.
...
The active checked seeds now contain six tools on both platforms. Normal build
ownership is otherwise unchanged. Each fixed-point C object passes the preceding
...
Focused assembler, disassembler, compiler, and fixed-point boundary tests were
green before promotion. The generated audit, normal image build, and complete
private guest frontier also passed at that checkpoint. Fresh Linux and native
Windows candidate fixed points passed from the same 59-input source snapshot,
and their six stage-four images now form the paired v2 checked seeds. The
candidate Toolchain gate passed with 22 published artifacts and all 62 stage
pairs. Reproof from both promoted v2 seeds passed with all six initial images
equal to stage two. The refreshed audit and 16-artifact check also pass. The
normal image build and private QEMU smokes are recorded below.
The last source-suffix audit found no safe `.c` rename because all active
CupidC-owned translation units already used `.cc`.
...
CupidBuild parses its frozen seed manifest instead of searching its text for
tool records. The current host schema contains the exact six-tool
...
freezes all six listed tools and validates their static i386 ELF32 or strict
...
object. CupidBuild is a checked non-producer. Make ownership and the Python
dependency are unchanged.

The fixed-point coordinator consumes the six-tool v2 plan directly. Linux
stages freeze 59 source inputs and build 22
C objects, one startup object, and six linked tools. The native Windows plan
adds its publication runtime and CupidBuild startup bridge. The source-current
profile has 33 exact `KERNEL32.dll` imports and three exact `NTDLL.dll`
imports: `NtCreateFile`, `NtQueryDirectoryFile`, and
`NtSetInformationFile`. The current promoted Windows seed keeps the earlier
29-plus-one profile as an accepted bootstrap parent until the next paired
promotion. Initial
seed-to-stage-two comparisons cover all six images named by the checked
manifest.
...
publication verification recreates the checked plan before checking the
59-file source inventory. The active seed promotion does not transfer a normal
recipe.

Both complete candidate proofs pass. Linux matches 22 C objects, startup, and
six tools with 24/6/31 failure, help, and success cases. Native Windows matches
23 C objects, three assembly objects, and six tools with 13/6/18 behavior. Both
bind source SHA-256
...
and zero-length behavior. The source-head link proof built all six tools and
the runtime contract together. At that checkpoint, this was not yet a seed
...
Those were reconstruction results; the v1 five-tool seeds remained unchanged
at that checkpoint.
...
The promoted CupidDis pair also validates `e_entry` and every defined
`STT_FUNC`. Each address must begin a decoded instruction in file-backed
executable code. The normal linked-kernel pass selects this rule beside local
target validation and shares the same instruction-start map. The Linux fixed
point passes a 5/22/21 help, success, and failure matrix; native Windows passes
5/8/9. ADR 0323 records the paired promotion and production gate.

Source head also exposes typed raw requests through the public in-kernel
CupidDis adapter. Callers can select fixed 16-bit code, fixed 32-bit code, or
borrowed code16, code32, and data ranges. A strict request checks the shared
decode summary and returns before rendering when selected code is unknown,
invalid, or truncated. Existing JIT listings remain permissive fixed-32. ADR
0334 records this boundary.

The preceding fully poisoned OS build and integrated guest frontier remain
historical evidence. At source head, the artifact contract passes against all
sixteen exact artifacts. The current outputs are:
...
bootstrap tests in 2,820.626 seconds. The live closure now contains 55 inputs
and has digest
`8bf242a67510fb5ee572d91d2a7f32a969cc0752c7bf2ae55ce8a8df745eb417`.
Both checked seeds bind the preceding capability snapshot
`46c5335c80d822dd5085ee22077486ea647e5396482d42454847c87e4222aa67`
at revision `a17c9465911da41d59b7ada71733d36c39faa5ea`. ADR 0336 records their
function-anchor and source-edge carriage and production adoption.
...
call, and be cleared. Runtime initialization and assignment accept
`&(function)` and nested grouping. A defined function
...
in block-static storage, and across persistent REPL units. An omitted bound is
inferred from a nonempty initializer, while an explicit bound must be positive.
Compatible function targets and null entries fill the array in order. Missing
entries in a fixed array are zero-filled, and a function defined later uses the
existing absolute data patch. Indexed stores and calls retain the callback
...
source graph at that checkpoint retained 748
...
none was Python-only. That historical schema-v3 `CUPMAN4` publication passed. The
...
CupidBuild and the PE32 reader formed that published 75-input, 58-source
closure. The 22-artifact cohort included `cupidc-cupidbuild.elf` and bound all
62 stage pairs without changing the then-five-tool checked seed.

The current schema binds 76 publication inputs, 59 bootstrap inputs, and 22
artifacts. The earlier 16a bundle passed stable-copy `CUPMAN2` verification;
fresh publication after the contract-consumer repairs in `b25a3374` and
refreshed object expectations agrees on all 62 stage pairs and passes native
verification on Linux and Windows. The full executable suite and user build pass. The bootstrap log records the separate checkpoints.
...
27/6/34 for Linux and 16/6/21 for Windows.
...
The preceding v1 Linux seed bound revision
`a17c9465911da41d59b7ada71733d36c39faa5ea`, the 50-input snapshot
`46c5335c80d822dd5085ee22077486ea647e5396482d42454847c87e4222aa67`,
and build plan
`59c1231e6fc7caafde8781dd6a566fa0ece2909be606914f24a19a7bececadcc`.
The Linux manifest is 5,573 bytes with SHA-256
`b6e34a2e18dd18aba91c6358116eafde39953566efeadb224575ac8c13ab2c1b`.
Its candidate proof passed cleanly with CupidASM and CupidDis differing from the prior
seed, 19 C objects, startup, five tools, and 5/22/21 behavior. The Windows manifest
...
with the same two-tool transition, 20 C objects, two assembly
objects, five tools, and 5/8/9 behavior. Both production raw-image transactions
select `--require-local-targets --require-source-edges`. The linked kernel
transaction also selects `--require-code-anchors`. Production CupidASM object
publication and fixed-point startup links select the object form after
structural validation. At that v1 checkpoint, the artifact-size policy covered
fourteen outputs: four OS outputs, five Linux seed images, and five Windows
seed images.
...
The v1 Linux adoption reproof recorded the expected startup-symbol transition
in all five initial static ELF images, then matched stages three and four. It
repeated the 19-object, startup, five-tool, and 5/22/21 checks. Windows began
with all five PE images equal and repeated its 20-object, two-assembly-object,
five-tool, and 5/8/9 checks. Their report SHA-256 values are
...
language decision, and ADR 0312 records its first carriage. ADRs 0318, 0323,
and 0336 record preceding seed checkpoints. ADR 0353 records the paired v2
contract, and ADR 0367 records the preceding six-tool promotion, and ADR 0370 records the active promotion.
...
The preceding v1 promoted Windows cohort was:
...
`751e1d7787a4be08e4e86814bbb7473979fe2eb8a3292baed0241967f772eaef`.
It binds revision `a17c9465911da41d59b7ada71733d36c39faa5ea`, the
...
definition to all six tool mains. A compile-and-link parity test covers the
...
bootstrap manifest and all six seed artifacts. Native Windows checks both
...
local-target carriage, ADR 0318 records the preceding linked-image promotion,
...
POSIX reserves one exclusive file there. It keeps maps, captured streams, and
frozen inputs in sealed anonymous memfds, while the candidate and publication
alias remain named on the target filesystem. Tools receive direct
`/proc/self/fd/N` paths. Both caller modules pass all 10 tests on Windows and
all 10 through WSL, including parent replacement with no leaked candidate. The promoted Windows execution seed now
...
skip, and the native raw-source contract passed separately. These changes do
not alter either binary. The guarded boot and SMP transactions require the v2
rows before publication. ADR 0340 records the source contract, and ADR 0336
...
The generated audit reports failure, help, and success counts of 27/6/34 for
Linux and 16/6/21 for Windows.
...
every defined `STT_FUNC` and requires it to begin at a decoded instruction in
executable `PROGBITS`. For `ET_EXEC`, it counts `e_entry` plus every defined
function in file-backed executable code. Function aliases remain separate
report entries. Failures distinguish an address outside executable code from
one in the middle of an instruction.

The typed contract and CLI cover valid aliases, both invalid classes,
relocatable and linked inputs, missing disassembly state, dynamic or
...
records 27/6/34 failure/help/success cases for Linux and 16/6/21 for Windows.
...
startup, and linked kernel boundaries. The 54 active assembly functions keep
their previous section bytes and relocations. Minimal DWARF source information
remains a separate missing format slice.
[ADR 0320](../adr/0320-validate-static-elf32-code-anchors.md) records the
source decision, and [ADR 0323](../adr/0323-promote-and-adopt-static-elf-code-anchor-checks.md)
records linked carriage and production adoption. ADR 0335 records the object
form, and ADR 0336 records its carriage and active adoption.
...
failure contract. A focused four-vCPU QEMU boot also observes that marker,
`PASS feature14_simd`, and clean JIT completion. Its 33,347-byte log has
SHA-256
`14511351d544fe8c4d4293b64fbaa7de9a3fdcaeb69f2ac0553e9d0a71d29696`.
The broader integrated private frontier observes the earlier markers once and
in order before the clean JIT completion. It passes in about 889 seconds from
its recorded image.
...
target ABI. It has 50 strict C11 roots and three GNU runtime roots.
`HOSTED_I386_LINUX` owns 38 strict Linux roots, which search only the
Toolchain tree and the angle-only hosted declarations. `HOSTED_I386_WINDOWS`
owns nine roots with `_WIN32=1`. `FREESTANDING_I386`
...
contains the 22-source static Linux tool union, the Windows runtime wrapper
...
and Windows startup, the Toolchain Makefile, both strict C11 contract
...
That five-tool fixed point contained 19 C sources because test programs did not
contribute to a tool image. ADRs 0082 through 0186 record the earlier
...
still uses WSL for Linux fixed-point and Linux-contract work. The Linux and
Windows six-tool candidate proofs passed, and both stage-four cohorts were
promoted. Reproof from the promoted seeds also passed with all six initial
images equal to stage two. Python-free coordination remains open.
...
all sixteen observations, and the complete Windows manifest plus six PE
...
Windows manifest, and six regular-file size and digest observations. The C
...
now contain 58 tests. All pass, with four platform-specific skips, and the
16-artifact closure verifies successfully.
...
artifacts fail with direct diagnostics. An earlier artifact group ran 46 tests
in 4.160 seconds with four expected Windows skips. Its POSIX runner passed all
15 tests in 0.146 seconds. That checkpoint reached the exact-size gate with
changed pass-one ELF, final ELF, and raw-kernel outputs. After those three
policy rows were updated, its repeat passed in 874.531 seconds. It checked all
...
preserve earlier promotions, ADR 0323 records the preceding code-anchor seed,
and ADR 0336 records the preceding v1 seed pair.
...
340,548 bytes, CupidC at 2,691,756 bytes, CupidDis at 538,556 bytes, CupidLD at
...
The preceding v1 candidate proof passed from revision
`a17c9465911da41d59b7ada71733d36c39faa5ea`. Stages three and four matched all
19 C objects, startup, and five tool images. Both stages agreed on five help
paths, 22 successful operations, and 21 useful failures. The
50-input source snapshot had SHA-256
`46c5335c80d822dd5085ee22077486ea647e5396482d42454847c87e4222aa67`.
CupidASM and CupidDis differed from the preceding seed. That v1 promoted-seed
reproof reported all five initial comparisons false because the new startup
function types remained in static ELF symbol tables and flowed into every
linked tool; its stages three and four still matched exactly. ADRs 0265, 0280, 0292,
0312, 0318, and 0323 preserve preceding promotions; ADR 0336 records the
later v1 promotion.
...
At that pre-promotion checkpoint, the source graph recorded 452 transforms
across the three supported roots and 443 under root `all`. Tool participation
was Python 452, CupidC 250,
CupidObj 192, CupidASM nine, CupidLD nine, and CupidDis nine. The four
Cupid-built contracts covered the user ABI, artifact-size policy, Toolchain
manifest verification, and Toolchain manifest authoring. It retained the
...
mode passed in 122.30 seconds. Those counts must be refreshed for the active
six-image seed closure.

At the earlier five-tool checkpoint, the path performed strict inspection and
flattening in one hostbuild transaction. Hostbuild froze the selected seed
manifest and all five
...
published artifacts, exact 75-file publication inventory, 55-file bootstrap
closure, and Linux publication seed. The publisher gives author mode an
independent `CUPMAN4` fact snapshot for the same inventories, seventeen object
comparisons, and the remaining raw fixed-point pairs. Python checks independent
oracles and repeats live
...
rejections match the retained Python validator. The production JPEG recipe
enters the promoted `cupidbuild embed-jpeg` transaction. It freezes the asset
and complete six-tool seed, runs private CupidObj `wrap-jpeg`, compares the
candidate with its independent native check, and rejects live drift before
guarded publication. The former Python coordinator remains an oracle and
historical checkpoint, not the owner of this Make edge.
ADR 0231 records the source capability, ADR 0234 records seed carriage, and
ADR 0235 records the first production transfer; ADR 0368 records direct
CupidBuild ownership.
...
success, and failure. The active six-tool matrices are 27/6/34 on Linux and
16/6/21 on Windows, ordered as failure, help, and success.
...
and the publication transaction. It verifies the manifest and all six tool
...
The checked Linux seed now carries the complete 5/22/21 matrix. This first
...
Source head now extends that loader boundary to all six hosted tools.
...
That cohort predated its v1 seed promotion; the later v1 bootstrap proved the
promoted five-tool trust unit independently.
...
to the 22-source i386 Linux fixed point, and native GCC or Clang rules select
...
ELF32 object. The runner verifies the live manifest and all six seed images
...
The earlier six-tool matrices were 27/6/34 on Linux and 16/6/21 on Windows,
ordered as failure, help, and success. The original two-pass link used this
command with Python orchestration and independent parity checks. Promoted
CupidBuild now owns the transaction described above. ADR 0222 records the
capability boundary, ADR 0223 records seed carriage, and ADR 0224 records the
first production handoff; ADR 0371 records direct CupidBuild ownership.
...
six-tool capture to the same checked runner used by root commands. The runner
...
ADR 0312 records the preceding local-target seed, ADR 0318 records the
preceding linked-image seed, ADR 0323 records the preceding code-anchor seed,
and ADR 0336 records the later v1 seed.
...
It freezes the selected source and all 304 `.h` and `.inc` inputs visible
through the profiles' 20 include roots. The 72,950-byte input manifest has
SHA-256
...
from a bounded `CUPROF1` envelope. The 951,725-byte live snapshot holds
921,527 captured input bytes, 691 profile memberships, and 995 encoded path
records. CupidObj sorts
...
objects, startup, and five tools between stage two and stage three. That v1
promoted seed passed a 5/22/21 behavior matrix. The normal target passes
...
unlisted `.cc` file, a missing root, input membership or byte changes,
...
files belong to the i386 Linux profile and feed both the six-tool fixed point
...
observations do not. The historical ADR 0360 schema-v3 `CUPMAN4` publication
...
That publication recorded the preceding 75/58 inventories and carried the
six-tool candidate without promoting a new checked seed. The later 16a
publication used 76/59 inventories and passed stable-copy `CUPMAN2`
verification. Its manifest has SHA-256
`ba5e531172f4861237238d403f8119f229c3e6d1267a483277efc3f19e615c92`.
That earlier publication after the contract-consumer repairs in `b25a3374`
and the refreshed object expectations agreed on all 62 stage pairs and passed
native `CUPMAN2` verification on Linux and Windows. Its 29,619-byte manifest has
SHA-256 `419d1e6f3b4a90b1cbfae04f5aa397c900449efe7d4197b7116f0cadb5070afd`.
The full executable suite and user build passed at that checkpoint.
...
At that earlier audit checkpoint, generation and deterministic check mode both passed. The
generated fixed-point inventory records failure, help, and success counts of
27/6/34 for Linux and 16/6/21 for Windows.

Across the three supported roots, CupidC participates in 250 transforms,
CupidASM in nine, CupidLD in nine, CupidObj in 192, and CupidBuild in 192.
Python participates in 260 as the host-side safety, parity, and publication
layer. The direct CupidObj group accounts for 186 CupidBuild calls: 175 text
wrappers, eight binary wrappers, and three installation-source generators.
...
At that checkpoint, the kernel-symbol source was classified
as `generate_ksyms_source` with CupidDis, CupidObj, and Python participants,
and the Doom profile delivery as `generate_profile_manifest` with CupidObj
and Python. Both now run directly inside promoted CupidBuild transactions.
Root `all` has 443 transforms, all with a Cupid participant. The
...
runner rechecks the live six-tool trust unit after every command and rejects
...
equivalent sequential baseline bytes. At that checkpoint, Hostbuild froze the source and ran
...
malformed marker streams. Python then checked accepted bytes independently and
controlled atomic publication. Promoted CupidBuild `embed-jpeg` now owns that
Make edge and its independent native check. The root build
...
wrapper supplies its frozen six-tool capture. The runner verifies the complete
...
CupidASM and CupidLD in nine each, CupidObj in 192, CupidDis in ten, and
CupidBuild in 354. Four semantic contracts participate, and no transform is
Python-only. The normal JPEG wrapper, generated kernel-symbol source, raw
kernel, Doom profile manifest, ISO spanning fixture, and 157 kernel-profile
compilations run inside typed CupidBuild transactions. Python remains present
in 98 transforms
...
0305 records the raw local-target seed, ADR 0312 records the preceding
local-target seed, ADR 0318 records the preceding linked-image seed, ADR 0323
records the preceding code-anchor seed, and ADR 0336 records the later v1 seed.
...
limits, and preserves prior output on failure. The preceding v1 five-tool seed
carried the command and repeated it in the 5/22/21 fixed-point behavior matrix.
...
fully poisoned OS build, and strong full private guest frontier. The audit has 748
...
## Historical ADR 0374 six-tool manifests

This section retains the earlier manifest and artifact identities. The current
`9d2529a7` manifests and clean proof results are recorded in ADR 0393 and the
current checkpoint at the top of this guide.

The bootstrap coordinators and CupidBuild accept both the legacy v1 five-tool
contracts and the active v2 six-tool contracts. The Linux v2 manifest has the
exact 22-source plan and its six links. The Windows v2 manifest binds that
...
it does not limit normal-build ownership. At the preceding kernel-symbol
handoff, CupidBuild owned six guarded publications: two assembly objects, two
...
The v2 provenance is intentionally one-way. It names the preceding v2 manifests
...
embedding its own identity. The ADR 0374 pair binds revision
`0232cb57aad5d6bdfd7bd77499762514b2f0ebfd` and snapshot
`0b591a0bef928186641b3aa1fb98c1e145e6c4905c8b6cb87c34a1ace4bc87d2`.
...
ADR 0361 records the checked-runner promotion, ADR 0367 records the preceding
59-input pair, ADR 0370 records the kernel-symbol pair, and ADR 0374 records
that historical pair.

The 6,602-byte Linux manifest uses schema `cupid.bootstrap-seed.v2`. Its plan
has SHA-256
`52dd857bcb74e079e7e2eec45eaa90a0a0838ad2f4e817bebc35c9904efbecbd`.
It names v2 manifest
`770f979407f930deba0c9ba887bcd14f2350a785b1c0df6b31ddc2659c46eaae`
at revision `9d10c223fc7aa22901e6f4ae81ce800ff1b62ad6` as its parent. The
...
names v2 execution manifest
`bf6147cf2e8249372869a24e5b8477ffb785d9a48eef80209366cfbaff19c7db`
at the same parent revision.

| ADR 0374 Linux seed image | Bytes | SHA-256 |
| --- | ---: | --- |
| `cupidasm.elf` | 496,688 | `d1a2eac04da01a7b656a4a6ffc58f91600da6b2651804f1b48f6ee92ee4407f2` |
| `cupidbuild.elf` | 361,632 | `51b366d7ec6423b76b91e17720c1b77ea1387163c175c692779853dd8d2b8e71` |
| `cupidc.elf` | 2,691,780 | `b17b2c5588fad1735d8dd1226bbeba7ae6f92fa2e70e5d70c1f0b4b979e34e17` |
| `cupiddis.elf` | 538,580 | `ec6f381883f2f1230b7df20fb7fbc5d629ac895cc26dd72b091bdf4c40ebd2ef` |
| `cupidld.elf` | 312,952 | `ec1981e97b0a11bf3538df34e85d071aa81ea01596913cfb1364013f35a507c0` |
| `cupidobj.elf` | 392,844 | `2988a0d4e7a8416dcef0a5b178df5eaa51faf80d94881eb6e46a1c2a9d9475b6` |

| ADR 0374 Windows seed image | Bytes | SHA-256 |
| --- | ---: | --- |
| `cupidasm.exe` | 480,256 | `dc004aa0026d37dfdf623441a85b539c2c65497c9159c7c394be03a9bc16b7ad` |
| `cupidbuild.exe` | 378,880 | `ff47743b7963691142dd2ef818dac40d66dc81869828fe1efcbd91ac1e872c27` |
| `cupidc.exe` | 2,620,416 | `1a1bf6e016b5a61dfa61ff6f9c02672d338f2608341a26bf1a91b6376753f13b` |
| `cupiddis.exe` | 517,120 | `36af89b475fdc11f674b9ca7cc1cb61d9f756ca212332a3f7fa9076d863d29e5` |
| `cupidld.exe` | 296,960 | `b89b42dd7df9fca44e59d93b97bbfd8f79da3352587da2dd50adfa72afee8f29` |
| `cupidobj.exe` | 376,320 | `82e20c148756a1b1db7ed7b62b1077b88d45320fc01aea203111be34eea1e7b5` |

At the ADR 0361 seed-promotion checkpoint, the artifact-size policy was
...
The promoted-seed Linux and Windows reproofs passed with all six initial images
equal to stage two. ADR 0361 records the candidate reports, promoted seed
identities, and self-consumption evidence. Those sixteen policy rows
...
and disk-image publication. That checkpoint produced:
...
added 896 bytes. A second gate measured 9,505,656 bytes against the
...
The promoted runner now carries the 186 ordinary CupidObj recipes and both
normal kernel links. The four
...
## Historical direct CupidObj recipe handoff

At the earlier kernel-compiler handoff (ADR 0393), the recorded state was:

The handoff evidence below predates kernel flattening, profile publication,
the ISO pattern transfer, and kernel compiler adoption. The current split is
354 CupidBuild and 98 Python participations; ADR 0393 records the latest
recipe transfer.

The normal graph now binds `CUPIDOBJ` and its seed prerequisites with Make
`override` assignments. A command-line variable cannot redirect the runner or
remove part of its six-image trust unit. The evaluated negative test injects
both overrides and confirms that Make retains the promoted platform
CupidBuild, its manifest, the repository root, the Makefile, and every seed
image.
...
no invented coordinator. The checkpoint's 194/258 CupidBuild/Python split therefore
...
kernel-symbol handoff moved the checkpoint split to 194 and 258. The generated
audit still contains 452 transforms and 443 under root `all`; CupidObj remains
at 192, and no transform is Python-only.

One command from each migrated family was compared through the old runner and
CupidBuild with the same production seed and arguments. The text object was
2,004 bytes with SHA-256
`9474d113853416f8e87207d317b5514d680a0b50ca4ff91747e4d853074a2f13`;
the binary object was 12,848 bytes with SHA-256
`139e697eaa1bec2cb94473397437d1421a7f01dd087e2437dc741a3c2bb8ea7c`;
and the installation-source object was 12,845 bytes with SHA-256
`0d1f7ee032b13abbbe1767d75fe32c6f1ffa8b7014db44ae35c9d4c47ebb8305`.
Each pair matched byte for byte.

The forced `make -j4 all` pass exercised every migrated recipe. It passed all
83 Doom roots, both CupidLD links, and strict CupidDis inspection, then stopped
at the exact-size gate because the edited embedded manuals made the flat
kernel 9,501,584 bytes. After the policy was updated, `make -o FORCE all`
rebuilt the changed documentation and profile edges, repeated both links and
the strict scan, accepted all 16 exact artifacts, and published the
209,715,200-byte image. Its SHA-256 is
`64ffda977acfb06327cce3c3620faf48986fb7ee3feb403a401e306638b183d8`.
...
its bounded implementation. The normal Make recipe still uses Hostbuild until
the separate production handoff. Graph ownership therefore remains 193
CupidBuild and 259 Python participations, with five composite
Python-coordinated CupidObj paths. ADR 0369 records the source boundary, and
ADR 0370 records paired seed carriage.
...
Those exact stage-four CupidBuild images became the ADR 0370 checked pair. The
6,602-byte Linux manifest has SHA-256
`770f979407f930deba0c9ba887bcd14f2350a785b1c0df6b31ddc2659c46eaae`;
the 2,852-byte Windows manifest has SHA-256
`bf6147cf2e8249372869a24e5b8477ffb785d9a48eef80209366cfbaff19c7db`.
Both bind revision `9d10c223fc7aa22901e6f4ae81ce800ff1b62ad6` and the
59-input snapshot
`bac22f6a59871326ec40a58ab143eea1675b689251c76950d43d860cb2539fcd`.
Promoted-seed self-consumption reproduced all six initial images on both
platforms and retained the 27/6/34 and 16/6/21 behavior matrices.
...
checkpoint's flat kernel was 9,511,584 bytes. The final smoke was observed directly;
...
At that kernel-symbol checkpoint, the graph had 748 inputs, 452 transforms, 255 feature
requirements, and 28 accounted unreachable inputs. CupidBuild participates in
194 transforms, Python in 258, and CupidObj in 192. Four composite paths remained
Python-coordinated: kernel flattening, disk-image publication, ISO publication,
and the Doom profile-input manifest.

The promotion build first stopped at the exact-size gate because
`kernel/kernel.bin` measured 9,513,992 bytes instead of the previous
9,513,536-byte row. Only that exact row changed. The next complete build
compiled all 83 Doom roots, completed both CupidLD links, reproduced the direct
kernel-symbol transaction, and passed strict whole-image CupidDis inspection.
Its final contract invocation then exposed the artifact verifier's stale
parent lineage. The contract now accepts either complete adjacent parent pair
while rejecting mixed digest/revision combinations. A second full replay
...
Its three policy and runner modules passed 58 tests in 4.974 seconds, with four
expected Windows skips. The full CupidBuild CLI module passed 96 tests with
three expected platform skips, and the Hostbuild symbol
oracle passed all 20 tests in 0.277 seconds.

Audit regeneration and independent check mode both passed. The 2,779,034-byte
JSON has SHA-256
...
`/bin/ls.cc`, and completed without a panic or exception marker. The
21,888-byte log has SHA-256
