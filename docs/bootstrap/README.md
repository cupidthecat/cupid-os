...
The normal root build gives checked-seed CupidC ownership of 152 checked-in
sources and the generated kernel symbol source. All 153 sources use `.cc`.
...
transfer, and ADR 0176 records the libm transfer.
...
validator. The production frontier covers 152 approved sources, and every
...
all 152 roots twice against a 440-file snapshot with SHA-256
`2143222ba61544b44655f882bc06e55ef0ff195033c907f5ae512801251e9cc1`.
Both object passes are byte-identical and total 3,659,840 bytes. The combined
...
repository snapshot.
The 152-root graph passes the two-link symbol and memory checks plus clean
...
symbol blob contains 105,574 meaningful bytes followed by two zero padding
...
compiler emit unchanged `kernel/core/kernel.c` as the same 25,920-byte
object with SHA-256
`d44d06949d48ead865d0d8c1bdd3b76a67b429e0b7a369318ec4fbe8d9f44ed7`.
A private hybrid image built with that object reaches the desktop and
completes `/bin/ls.cc`. The checked seed does not carry this statement yet,
so production ownership and the `.c` suffix remain unchanged.
