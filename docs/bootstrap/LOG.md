...
The positive contract covers zero-parameter and empty replacements, an unused function name, the 22-parameter spliced definition, nested and parenthesized-comma arguments, an object alias becoming a call, ordinary prescan/rescan, empty and repeated arguments, an unused malformed argument, equivalent same-spelling redefinition, self/mutual recursion, exact locations, and result ownership. The negative contract pins malformed/duplicate parameters, strict C11 same-spelling redefinition, too few/many arguments, unterminated invocation, and explicit unsupported diagnostics for the pending variadic/stringify/paste slice. Every failure leaves a zero result, rewinds the arena, retains its diagnostic, and permits same-job recovery.

All nine focused modes pass under strict Windows Clang and WSL GCC. A direct WSL GCC build passes the same nine modes with address, leak, and undefined-behavior sanitizers. The complete hosted Toolchain suite, repository suite, regenerated audit, and independent reviews remain the commit gates for this slice. Variadics, stringification, token paste/GNU comma elision, full conditional expressions, predefined macros, pragmas, Cupid `#exe`, active-corpus proof, and kernel integration remain in issue #24.
