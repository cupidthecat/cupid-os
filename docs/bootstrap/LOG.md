...
The audit scanner now recognizes digraph directive markers and `%:`/`%:%:` operator spellings as well as literal `#`/`##`; active production totals remain unchanged because no current definition uses digraphs. Active raw or pasted `__VA_ARGS__` outside a valid variadic replacement fails at the token, while inactive groups remain ignored. Invalid operator placement and invalid paste re-lexes produce precise transactional diagnostics. Fifteen focused preprocessor tests and all 17 audit tests pass under Windows at this point; strict cross-host, sanitizer/analyzer, complete repository, regenerated-audit, and independent final reviews remain commit gates. No OS source or TempleOS reference file was changed.
