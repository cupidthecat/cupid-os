...
- The exercised script language includes `ENTRY`, `SECTIONS`, location-counter assignment, exact and wildcard input selectors, `ALIGN`, output and boundary symbols, `COMMON`, comments, and repeated `ASSERT`. `link.ld` now rejects a file-backed image past `0x008FF600` and a complete memory image past the fixed stack at `0x00B00000`.
...
- Exit, remote kill, and stack-canary termination only mark a PCB terminated. Cleanup runs under the BKL after scheduler detachment proves `on_cpu == 0xFF`; then the reaper frees the stack and releases exactly the consumed external generation. `context_switch.asm` loads target ESP and FPU state before releasing the scheduler's sole BKL acquisition, then restores the target interrupt policy without a second unlock. Nested switches become CPU-local pending requests for the outer unlock; remote kill publishes that bit before IPI delivery and the real handler consumes it without re-arming. NASM and CupidASM produce the same 73-byte `.text`, SHA-256 `25b78f4c2cbf3dfadc6dc87a9731a097bfd9df0675534d8449c24d890114fbfa`, with one `R_386_PC32` handoff relocation at `0x21`.

### Guest scheduler failure and interrupt-context diagnosis

The first post-handoff CupidC AOT smoke appeared to stop while allocating the HOMEFS FAT chain. Repeating the run for 180 seconds and sampling QEMU initially suggested that the desktop task was starving the terminal: the sampled PC was the instruction after `desktop_run`'s `STI; HLT`. Live PCB evidence disproved that theory. PID 3 remained `RUNNING` on CPU 0, idle and desktop were `READY`, `need_reschedule` was set, the CPU-local pending bit was clear, and the BKL ticket head/tail were equal.

Stopping at the first exception exposed the real failure. `irq_common_stub` had replaced the per-CPU `%gs` selector with flat selector `0x10` before calling C. A timer-side dirty block-cache flush reached `serial_printf`, whose BKL acquisition called `this_cpu()` and read a bogus pointer from linear address zero; the first faulting instruction was `bkl_lock+0x24` with `CR2=0xF000FF57`. Exception entry repeated the same `%gs` replacement, so diagnostics produced 14,979 identical recursive page-fault frames and eventually made the failure look like a stack underflow near the kernel page tables.

A red production-source contract now requires both common stubs to save/restore `%gs`, load the ordinary flat selector into only `%ds`/`%es`/`%fs`, and keep the per-CPU selector live across the C call. Removing the two `mov gs, ax` instructions preserves normal C data addressing and lets interrupt-side BKL users resolve the current CPU. NASM and CupidASM remain byte/semantic oracles for the unchanged source form, and the fresh-image AOT smoke completes the full 397-cluster write after the fix.

The investigation also exposed a separate pre-existing timer limitation: `timer_irq_handler` invokes every active logical channel callback on every channel-0 interrupt, regardless of the configured callback frequency. Consequently the nominal five-second block-cache callback currently runs in IRQ context every PIT tick. That behavior was not papered over in the scheduler/linker step; a correct follow-up needs deferred timer work and an explicit callback-period model rather than another special-case counter.

### Full-corpus and artifact evidence

- The final pass-one corpus has 423 objects, entry `0x00100000`, loaded end `0x006C106C`, and memory end `0x00ADD950`. Pass two has 424 objects, loaded end `0x006D7383`, and memory end `0x00AF3950`. Section addresses, sizes, alignments, load permissions, all six program headers, and the 3,780-entry `t/T/w/W` symbol projection match LLD 22.1 exactly. Final allocated sections deliberately clear input-only `M`/`S` metadata instead of copying LLD's combined `.rodata` flags with a zero entry size.
- Both passes apply 39,112 relocations: 27,599 `R_386_32` and 11,513 `R_386_PC32`. The exhaustive pre-review oracle classified every then-current relocated word, including 7,582 deliberate first-occurrence merge-address differences, with zero unexplained values. The final refresh repeats deterministic full-corpus links, exact LLD layout and symbol comparison, focused section/named-symbol merge-addend contracts, and byte-identical disassembly of the new `bkl_context_switch_release` call target.
- Pass-one ELF/raw SHA-256 values are `99222efdea1959bd363fa48f913a04e8ed98a23f10c483c109ea209876138c3a` and `5dd5a8b5e378eb076f5e5ee013912e104ec158053aeae72619cb4978d2bde1d7`. Final ELF/raw values are `0ee3011ee608cc133b7146675c1eb933d26f1ffaaa3dc4313ef151b167f1c545` and `ffb6a8b59015efcf3c8ae343e81b1c7463235a17b2df1a4042f6c3c3deb460dc`. CupidLD, LLD, and the production pass generate byte-identical 556,911-byte ksyms C source, SHA-256 `de29abf780129b046b4aaf934e9daa85e062ffb0faff9aea0a32ea0dbd2500be`; the serialized blob is 90,887 bytes.
...
- The checked graph now contains 672 active inputs, 247 feature IDs, 480 transforms, 39 accounted unreachable source-like files, and exact coverage of 431 declared artifacts/424 linked objects. Ownership is CupidLD 5, CupidObj 181, host C compiler 280, NASM 4, host symbol reader 1, Python 8, and Make recursion 3; host ELF-link and objcopy transform ownership is zero.

### Verification

| Command/check | Result | Evidence |
| --- | --- | --- |
| `make -C toolchain test` | PASS | Strict Windows Clang passed 49 hosted modes, including the new merge-addend regression and all 22 CupidASM demos. |
| WSL GCC and sanitizer toolchain gates | PASS | Strict GCC 13.3 passed the same 49 modes; a separate full build passed under ASan/UBSan with leak detection and no finding. Counting the aggregate demo corpus individually gives 70 exercised cases per gate. |
| `python -m unittest discover -s tests -p "test_*.py"` | PASS | 129 tests passed in 60.806 seconds; the Windows-only `/dev/full` case was skipped and remains covered under Linux. |
| Kernel loader/process contracts | PASS | 38 positive/negative and production-boundary cases passed under Windows Clang, including two real interrupt-stub `GS` preservation contracts; the C contracts also pass under WSL GCC. They cover malformed rollback, all three arenas, lease generations, create failure, immediate/remote kill, local/remote CPU quiescence, BKL handoff and nested deferral, real IPI request ordering/consumption, stack-canary cleanup, permanent-page non-release, and interrupt-side per-CPU identity. |
...
| Full linker oracle | PASS | Two deterministic Cupid links per pass match LLD's six program headers, allocated-section layout, and all 3,780 projected text/weak symbols; ksyms and the new scheduler handoff relocation are byte-identical. Merge-order differences remain covered by the exhaustive normalized oracle plus focused final contracts. |
...
| CupidC/CupidASM JIT | PASS | `/bin/ls.cc` and `as /demos/hello.asm` each reached `JIT execution complete` on the corrected image. |
| CupidC/CupidASM AOT | PASS | On a fresh post-fix image, `ccc` wrote `/home/cctest` after all 397 HOMEFS clusters, `exec` loaded it as PID 4 at `0x01000000`, and it exited with `GS=0x28`, `CR2=0`, and no panic. `cupidasm` separately wrote `/home/asmtest` at `0x01A00000`; it loaded, ran, exited, and reaped without panic. |
