...
Final spec review then found a missing semantic node rather than a byte-layout bug. Storing qualifiers only on an existing nominal record could not keep both `T` and `const T` in one request, while copying the record would duplicate or overlap the uniquely owned member slice. It also made the pointer-level distinction easy to erase even though both pointers have the same four-byte layout. `CTOOL_C_TYPE_QUALIFIED` was added as a strong, layout-preserving wrapper with a required nonzero qualifier mask. It copies representation/category facts, composes `_Atomic` through later qualified/aligned wrappers, and leaves qualifier-placement legality with the declaration parser. A new empty-qualification error plus direct and aligned atomic-aggregate wrapper errors bring the exact negative/recovery count to 27.
