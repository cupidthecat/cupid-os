...
- The final pass-one corpus has 423 objects, entry `0x00100000`, loaded end `0x006C05A0`, and memory end `0x00ADC950`. Pass two has 424 objects, loaded end `0x006D6756`, and memory end `0x00AF2950`. Section addresses, sizes, alignments, load permissions, all six program headers, and the 3,770-entry `t/T/w/W` symbol projection match LLD 22.1. Final heterogeneous merge sections deliberately clear incompatible `M`/`S` metadata instead of copying LLD's combined-section flags with a zero entry size.
...
- `hello`, `ls`, and `cat` each rebuild twice to deterministic 12,656-byte ELF files. Their SHA-256 values are recorded after the final metadata correction below. CupidDis accepts all three and reports RX/R/RW loads, allocated read-only `.rodata`, BSS, and RW `GNU_STACK`.
