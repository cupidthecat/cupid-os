...
Wayfinder ticket [#21](https://github.com/cupidthecat/cupid-os/issues/21) completed the linker ownership transfer begun by the CupidObj step. CupidLD now owns both `link.ld` kernel passes and the three fixed-address user-program links. No GNU or LLVM linker is invoked by root `all`, `user:all`, or `toolchain:all`; external linkers remain optional semantic oracles only. The same step made the loader and process subsystem own the lifetime of fixed-address images instead of treating their pages as independently releasable allocations.
...
- Exit, remote kill, and stack-canary termination only mark a PCB terminated. Cleanup runs under the BKL after scheduler detachment proves `on_cpu == 0xFF`; then the reaper frees the stack and releases exactly the consumed external generation. `context_switch.asm` now enables interrupts only after loading the new ESP and restoring FPU state, closing the same-CPU reaper window. Its unchanged semantic oracle is a 45-byte `.text`, SHA-256 `31c5e44537769e6d847eb442c90b5bb01e0df268017886db3613a7c3dd34094c`.

### Full-corpus and artifact evidence

- The final pass-one corpus has 423 objects, entry `0x00100000`, loaded end `0x006C05A0`, and memory end `0x00ADC950`. Pass two has 424 objects, loaded end `0x006D6756`, and memory end `0x00AF2950`. Section addresses, sizes, flags, alignments, all six program headers, and the 3,770-entry `t/T/w/W` symbol projection exactly match LLD 22.1.
- Both passes apply 39,093 relocations: 27,587 `R_386_32` and 11,506 `R_386_PC32`. 31,511 relocated words match directly; the remaining 7,582 differ only by equivalent first-occurrence merge addresses. The normalized comparison covers every loaded byte and leaves zero unexplained differences. Repeated CupidLD runs are byte-identical.
- Pass-one ELF/raw SHA-256 values are `3cf978cb929d518061f6dc9972bb4c1570917a8ece3ab57595833f93fb569675` and `1f04add841245734214cb61cab6886f15d2116149fbc13cf9d6e231f1ccd3fd3`. Final ELF/raw values are `f17df35158c709b615d38678399993c66afa3b348640a9e223afc372582b1646` and `b4befcf4a7c3d5b199c717a2d37f3e9b6773fecbd8a27a4d90983c048840e779`. CupidLD, LLD, and the production pass generate byte-identical ksyms C, SHA-256 `77d3e81bb3189a8c71a22977c8790da2640a3b732a9fb42a3b91361933256e2f`.
- `hello`, `ls`, and `cat` each rebuild twice to deterministic 12,656-byte ELF files. Their SHA-256 values are `9dc7113eef8b6b451bf02ee279cf0979d97ae686c85414f39cbe406cf9e9bd22`, `238ff49d10c36726f977e3115732931ad4e6b6fb2565cb94e4b5589165df04a4`, and `25e551d7c9a43127ef49d3a2cddbb86617d733ae3702b5651e3764c28d8fc989`. CupidDis accepts all three and reports RX/R/RW loads, `.rodata` `AMS`, BSS, and RW `GNU_STACK`.
- The checked graph now contains 672 active inputs, 480 transforms, 36 accounted unreachable source-like files, and exact coverage of 431 declared artifacts/424 linked objects. Ownership is CupidLD 5, CupidObj 181, host C compiler 280, NASM 4, host symbol reader 1, Python 8, and Make recursion 3; host linker, objcopy, and ad-hoc shell ownership are zero.
...
| Kernel loader/process contracts | PASS | 26 positive/negative cases passed under Windows Clang and WSL GCC. They cover malformed rollback, all three arenas, lease generations, create failure, immediate/remote kill, true local/remote CPU quiescence, stack-canary cleanup, and permanent-page non-release. |
...
| Full linker oracle | PASS | Two deterministic Cupid links per pass matched LLD layout and all symbols/relocations, with zero normalized or semantic unexplained differences. |
