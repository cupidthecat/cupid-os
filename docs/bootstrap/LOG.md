...
The graph represents scalars, pointers, fixed/unspecified arrays, enums, active vectors, function types as semantic markers, structs, unions, Cupid classes, and alignment-carrying wrappers. The wrapper is required for alignment attached through a typedef or type attribute: it raises that use's alignment without mutating every use of the underlying record. Atomic representation is fixed target policy: atomic `long long` and `double` raise alignment from four to eight, while other supported scalar, pointer, and enum cases retain their target size/alignment; atomic aggregate layout remains a useful explicit unsupported diagnostic. Function return/parameter legality, qualifier placement, duplicate names, anonymous-member legality, and namespaces remain parser responsibilities rather than being guessed by the layout engine.
...
The adversarial review caught concrete implementation errors before the seam was accepted. `_Bool` bit fields with widths two through eight incorrectly used the width as occupied bytes instead of the one-byte `_Bool` storage unit. An explicitly aligned bit field could remain in the preceding allocation unit. Unnamed or pack-capped union bit fields incorrectly occupied the full base type, while a zero-width field ignored its explicit alignment. Atomic `long long` and `double` incorrectly retained four-byte alignment. A record consisting only of a flexible array was accepted, successful operations retained traversal scratch, and an array of an aligned wrapper was accepted even when element size was not a multiple of its required alignment. Red regressions now pin each correction. The aligned typedef wrapper, independent function slice/reference checks, output-lifetime probes, and the syscall/process/per-CPU fixtures further prevent the old parser's simplifying assumptions from becoming this interface. The negative mode pins 24 useful diagnostics and same-job recovery rather than accepting a generic failure.
