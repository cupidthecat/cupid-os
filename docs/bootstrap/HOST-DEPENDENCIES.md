...
The five static commands share one complete checked-seed gate. The manifest binds the exact executables, source revision, target ABI, producer lineage, 19-source build plan, startup, and five link orders. The current seed contains the checked bootstrap's stage-three images from revision `03d072fefc6703a53be7bfa4948f6116d238832b`. None of the five preceding seed images matched stage two, while stage two and stage three matched all 19 C objects, startup, and five tools. The 2,553,244-byte CupidC image has SHA-256 `59d90429cdfff1f5d6f8f3b3009f588d06de78c271e2e320dfca5b5e2a58173f` and carries runtime floating truth. The same seed carries the 591-row x86 catalogue, typed CupidDis raw ranges, and CupidObj installation-source generation. ADR 0203 records the promotion.
