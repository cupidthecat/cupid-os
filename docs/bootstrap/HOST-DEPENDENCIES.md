...
| GNU `ld` / LLVM `ld.lld` | No role in the normal root or user build; retained for optional interoperability and semantic-layout comparisons | Not required for root `all`, `user:all`, `toolchain:all`, or new `bootstrap-baseline` captures | Retain only as an optional oracle/bootstrap escape hatch; CupidLD owns all five production links |
...
| CupidLD | 5 owned transforms | Two script-driven kernel links plus three fixed-address user executables; owns `R_386_32`/`R_386_PC32`, weak/strong/common/script symbols, layout/alignment, merge entries, assertions, static ELF32 serialization, and the used `link.ld` subset |
