...
a supported root. CupidObj participates in 185 transforms, including the
three installation-source generators. Python participates in 449 transforms,
...
accepted sequential baseline bytes. Hostbuild validates SOF0 or SOF1 input,
copies it without changing a byte, and rejects progressive, unsupported, or
malformed frames before checked CupidObj wraps the snapshot. This removes
...
copied file. The manifest binds a 45-input contract inventory, including the
Toolchain Makefile and both Python control modules. It separately binds the
checked seed and 41-file fixed-point source inventory. Each run derives its
...
64 registers, and fingerprint `F4420CB4`. The newest
`FSUB ST(1), ST(0)` form encodes as `DC E9`. ADR 0203 carries `FLDZ` and the
three preceding x87 forms into the trust root, while ADR 0208 carries forward
stack subtraction. This count supersedes older source-head references below.
...
Its packed i386 words preserve the exact 109,889-byte blob. The current
110,304-byte object has
SHA-256
...
The five static commands share one complete checked-seed gate. The manifest binds the exact executables, source revision, target ABI, producer lineage, 19-source build plan, startup, and five link orders. The current seed contains the checked bootstrap's stage-three images from revision `6f880cc3cf5cced72b81e0d66079aaca913d0a03`. The 2,574,032-byte CupidC image has SHA-256 `8d810739494123a3da1cba34f75f58c005e8796f2cb4e85ba57eead1578a1f4d` and carries the returns-twice boundary, runtime floating truth, and forward x87 stack subtraction. CupidASM remains 445,616 bytes, CupidDis 379,648 bytes, and CupidLD 266,672 bytes. CupidObj is now 270,700 bytes with SHA-256 `a8de7de19d1ffbec90f0603f0f796f4a03fa74b8181c62f0f395b22a52423d1d`; it carries transactional kernel-symbol source generation as well as the earlier installation and linking operations. The 5,440-byte manifest has SHA-256 `729cd702e34695cf9ee619d10446ce80838ed9a25a14efa856833b2bf37629f3`. The post-promotion reproof matches all five seed images to stage two, then matches all 19 C objects, startup, and five tools between stages two and three. ADR 0223 records the promotion.
...
| `jpegtran`, `djpeg`/`cjpeg`, or FFmpeg | No role in the normal root build. Hostbuild validates the repository's sequential SOF0 or SOF1 JPEG and copies its exact bytes before checked CupidObj wraps them | Not required by root `all`; progressive, unsupported, and malformed input fails instead of selecting a host converter | Retain only for optional asset maintenance outside the build graph |
| `mkisofs`, `genisoimage`, or `xorrisofs` | No role in the build. A checked manifest fixes the fixture tree, and the repository writer emits the tracked ECMA-119 and `RRIP_1991A` image directly | Not required by root `all`, `sync-iso`, fixture regeneration, or tests | Retain only as an optional interoperability oracle outside the build graph |
...
| CupidObj | 185 owned transforms | 172 canonical text-to-ELF wrappers, eight byte-exact binary-to-ELF wrappers, one Python-assisted JPEG wrapper, final initialized ELF-to-raw conversion, and three installation-source generators |
| CupidDis | 1 composite transform | Supplies deterministic numeric symbols through a checked private seed image; the current consumer cohort contains 4,561 text symbols and a 109,889-byte panic-backtrace blob; the host oracle remains optional |
| Python | 449 transforms | Launches checked Cupid tools and handles remaining generation, input manifests, image packaging, and verification. It remains a composite owner for the three installation-table transforms because it launches checked CupidObj, not because it constructs their source bytes |
...
for oracle use. The normal Make path calls `_symbols_from_seed`, which freezes
the pass-one kernel and runs manifest-checked CupidDis. `embed_jpeg` performs
optional image preprocessing, then calls checked CupidObj with the original
source identity and publishes the object only after live inputs pass their
drift check. `tools/mksyms.sh` and `tools/embed_jpeg_baseline.sh` are tracked
...
The hosted preprocessor contract runs 381 tracked profile executions through
the repository file adapter. This covers 238 root-kernel and Doom C inputs,
three user inputs, 105 Cupid programs, 31 ordinary strict hosted i386 roots,
two strict hosted i386 kernel-bridge roots, and two GNU runtime roots. Only
