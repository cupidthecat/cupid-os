...
| GNU `objcopy` / `llvm-objcopy` | No role in the normal build; tracked legacy/oracle helpers may still invoke it manually | Not required for root `all`, `user:all`, or `toolchain:all` | Retain only as an optional comparison/maintenance utility; CupidObj owns the production transformations |
