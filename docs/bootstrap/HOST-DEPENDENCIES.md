...
sealed descriptor arguments. Generic checked commands retain their requested
cwd. ADRs 0387 and 0389 record the repairs. The preceding `16a86f5b` pair
carried them after clean reconstruction; the current pair retains them.
...
replay exceeded the command-line limit with 431 absolute input paths and
...
entries even when it reuses the existing output. Those writes change the
root's mtime and can invalidate the artifact verifier's pinned directory.
The edge completes ISO publication before verification; it does not add ISO
content to the size policy or relax namespace checks. The audit enforces the
exact `order_only_inputs` list. Ownership remains 354 CupidBuild and 98
Python participations. ADR 0386 records the reproduced overlap and repair.
...
publication. An earlier 431-input run reproduced the tracked kernel. The normal
...
disk and ISO composite paths remain Python-coordinated. ADR
0374 records the paired seed promotion, and ADR 0375 records the handoff.

Both promoted CupidBuild seeds contain the typed Doom profile publisher. Its
...
edges and the 83 existing Doom content dependencies. It prevents
parallel output creation from invalidating directory snapshots without
weakening the drift checks or serializing unrelated roots. The audit tracks
these edges separately from content inputs; ownership remains 354 CupidBuild
and 98 Python participations.
...
`build/bootstrap` before the transaction; Windows retains its native parent
preparation. The Python profile publisher remains an optional oracle.
CupidBuild participates in 354 transforms and Python in 98. Together with the
checked CupidC runner, the promoted candidate matrices passed 36/7/42 on Linux
and 24/7/29 on native Windows. Both seeds bind revision
`9d2529a718672edcd970f24960535db6ddbde5e4` and source snapshot
`bb4f598b2f72140d82d01fbfdedc14e4c774965d08c155ca848596daf3aa054f`.
ADR 0377 records the transaction, ADR 0382 records the promotion, and ADR 0383
records direct production ownership.

The checked Windows publisher restores rejected candidates through a verified
read/traverse directory handle before reacquiring cleanup authority. The
rename-back repair retains frozen-input protection and adds no host-tool
dependency. ADR 0385 records that boundary.
...
descriptor. The child calls `fchdir` before remapping the captured streams. A
tool descriptor in slot 0, 1, or 2 is duplicated above the standard
descriptors before `fexecve` or `execveat`. The `dup2`, pipe read and write,
and wait loops retry `EINTR`; `dup2` also retries `EBUSY`. The captured stream
memfds are sealed before reading. A close-on-exec launch-status pipe preserves
an exact child exit of 125, and the i386 startup supplies
...
The normal graph runs all 186 direct root CupidObj calls through the
...
use the same runner. Python retains the disk and ISO composite
CupidObj paths for independent validation and controlled publication. The Windows CupidBuild CLI
suite completed 66 tests in 65.934 seconds with three expected skips. The
host-runner Python module completed eight tests in 0.962 seconds with four POSIX
skips. The dedicated Make contract passed, as did all six CupidASM source tests
in 3.771 seconds, strict Windows and freestanding i386 adapter compilation, and
timeout-and-seed-drift precedence.

The JPEG path now uses its seeded implementation in production. Typed
...
production transform. The later kernel-flatten handoff brought the
totals to 195 CupidBuild and 257 Python. That checkpoint's behavior matrices were
...
The ADR 0374 pair froze 59 inputs. Its kernel-flatten-capable snapshot
had SHA-256
...
kernel-symbol pair, and ADR 0374 records that earlier pair.
...
guarded normal publications: the ISR and context-switch objects, the
bootloader and SMP-trampoline raw images, the JPEG object, and the generated
kernel-symbol source. The commands build and
certify private outputs on Linux and Windows without Python inside their
transactions. The checked v2 manifests contain six tools. The later object,
link, JPEG, and kernel-symbol handoffs raised CupidBuild participation to 194
and reduced Python to 258 transforms. The profile and ISO pattern handoffs
brought those totals to 197 and 255. The current kernel compile handoff
changes them to 354 and 98.
CupidBuild checks the current host
...
and timeout paths. It freezes all six images and validates their static ELF32
...
trampoline while keeping each raw range map private. Both normal recipes now
invoke the promoted seed directly with the complete six-image production
closure, removing Python and Hostbuild from those two transforms. ADR 0355
records the capability, ADR 0356 records its paired carriage, and ADR 0357
records direct production ownership.
...
This changed no checked seed, production owner, or host-tool dependency. A
Windows rerun completed all 19/7/24 behavior groups, but that checkpoint still
needed a clean same-commit two-host pass before paired publication. ADR 0382
records the completed repaired-seed promotion.
...
The self-link produces CupidBuild beside the other five seed tools and the
...
stage pairs. Publication verification recreates the checked plan before it
recaptures the 59-file live source inventory.

The historical ADR 0374 pair below predates the `16a86f5b` pair in ADR 0382
and the current `9d2529a7` pair in ADR 0393. It binds revision
...
evidence are recorded with the promotion checkpoint.
...
The latest pre-promotion active-source audit records three supported build roots:
...
The final source-head checkpoint records 748 inputs and 452 transforms. An earlier
...
`make bootstrap-audit` and `make check-bootstrap-audit` both passed at that
...
final ELFs, CupidBuild applies it to the ISR and context-switch objects, and
...
kernel-symbol source, final kernel flattening, 157 kernel compilations, Doom profile publication,
186 ordinary CupidObj recipes, and both normal kernel links. Python participates in 98
...
transform remains. One Python runner still owns the checked paths that have
not moved to CupidBuild, plus WSL staging when Windows must run the Linux
bootstrap seed. It also owns the live post-run seed check for checked
production CupidC, user links, CupidDis inspection, and the two remaining composite
CupidObj paths. Each wrapper supplies the
six-tool capture
...
checked six-tool seed. Make passes discovered output paths through
...
during that path. The ADR 0359 Windows proof passed while WSL
was unavailable, matching 23 C objects, three assembly objects, and all six
tools between stages three and four. At that checkpoint, the Windows and Linux
behavior inventories were 14/6/19 and 25/6/32, respectively. ADR 0359 records removal of
...
complete Linux policy manifest, all sixteen observations, and the complete
Windows manifest with its six PE files. `verify-artifact-sizes` is a direct
...
the raw Windows manifest, and six regular-file size and digest observations.
...
Windows manifest and all six PE files before success. An incomplete or
...
The historical ADR 0374 3,382-byte artifact-size policy had SHA-256
`cfbf82ebd02697a668c404c8e9bd19e577b8e809b87c3736e063e4f30f0d5fe2`. Its
sixteen rows covered `boot/boot.bin`, both kernel ELFs, the flat kernel, six
Linux seed images, and six Windows seed images. The OS rows expected 2,560,
9,634,124, 9,765,196, and 9,536,524 bytes for the boot image, pass-one ELF,
final ELF, and flat kernel, respectively. That checkpoint's verification
accepted all sixteen rows, totaling 38,402,832 bytes. The contract accepts
the complete kernel-symbol or active seed-parent pair and rejects mixed
digest/revision lineage for the Linux seed and both Windows parent links.
The bootstrap log records the pre-correction `16a86f5b` Linux measurements
and passing 16-artifact check; final-manual parallel replays now pass on
both hosts, and the private four-CPU runtime frontiers with E1000 and
RTL8139 pass.
...
9,504,760 bytes against the
provisional 9,504,480-byte raw-kernel row and failed closed. After that row was
corrected, CTXT lineage corrections added 896 bytes. A second gate rejected
9,505,656 bytes against the 9,504,760-byte row and failed closed. A final CTXT
...
That checkpoint's private four-vCPU `max` and E1000 smoke brought every CPU online
...
All three passed. At the end of that checkpoint, the clean normal image was restored to SHA-256
`112a9764bc9c99382d06dabcbcf2cb6e28498d0076ccc1aec787f159b46a8bc3`.

The pre-promotion normal build passed strict inspection of all 431 production
inputs and the private four-vCPU E1000 frontier smoke. Python remains responsible for
...
The historical ADR 0360 schema-v3 `CUPMAN4` publication passed. The Cupid author
...
artifacts and a 29,271-byte manifest with SHA-256
...
that host-resolution gap. That checkpoint's audit confirmed the same 75-input
publication closure and recorded no Python-only transform.
...
The following block records the preceding five-tool v1 Windows checkpoint.
At that checkpoint, the five proven native images lived in a strict checked
execution seed. Its manifest bound the exact PE32 imports, artifacts, source
snapshot, parent Linux seed, and clean stage-four provenance. Windows selected
that cohort for output-bearing CupidC, CupidASM, CupidDis, CupidLD, and CupidObj
calls. The runner executed private PE copies directly and rechecked the whole
live cohort after each command. Linux kept the static checked bootstrap seed.
The native fixed-point driver derived its plan from a separately verified Linux
manifest and preserved both trust roles. The first full run rejected source
...
behavior matrices on the same 50-input snapshot. That native Windows candidate
proof passed cleanly. The 2,118-byte execution manifest had SHA-256
`751e1d7787a4be08e4e86814bbb7473979fe2eb8a3292baed0241967f772eaef`.
The PE images were CupidASM at 444,928 bytes with SHA-256
...
selection. ADRs 0281, 0292, 0312, 0318, and 0323 record preceding promotions.
ADR 0336 records this five-tool v1 checkpoint. ADR 0353 records the paired v2
contract, and ADR 0367 records the preceding six-tool promotion, and ADR 0370 records the preceding promotion.
...
range map. CupidBuild requires the map to match the fixed AP startup policy
and keeps it pinned while CupidDis applies known-decode, local-target, and
source-edge checks. Only the binary is published. CupidBuild owns the
transaction, output lock, drift checks, and atomic replacement. ADR 0271
records the first inspection boundary, ADR 0305 records local-target adoption,
ADR 0308 records the map handoff, and ADR 0357 records the host-dependency
removal.
...
strict inspection. The promoted CupidBuild seed coordinates the private
bootloader and SMP-trampoline candidates with the production manifest and full
checked-seed closure. It also owns the private ISR and context-switch object
transactions. Each operation publishes only after every live boundary still
...
and ADR 0336 records the later v1 reproofs.
...
seed. The typed CupidBuild recipe freezes the seed and source, verifies
the exact 4,096-byte pattern and data-only map, runs checked CupidDis, and
controls atomic publication. It does not author the candidate.
...
The `assemble-iso-pattern` handoff removes Python from this recipe's command
and prerequisite closure. Both repaired seeds carry the command. Hostbuild
retains its compatibility command and the separate ISO
image renderer and publisher. ADR 0384 records the source boundary, and
ADR 0386 records the handoff.
...
other roles. The promoted CupidBuild runner now launches these three CupidObj
generators directly. Together with the ordinary wrappers, this leaves Python
in 262 transforms. ADRs 0201, 0203, and 0204
...
canonical CupidDis text in the normal build. Promoted CupidBuild freezes the
pass-one kernel and seed, preserves CupidDis's exact text for CupidObj, and
renders the same source independently as a parity oracle. A failed tool,
missing output, byte mismatch, lock contention, or changed live input leaves
the existing destination intact. CupidDis owns symbol inspection, CupidObj
owns source generation, CupidBuild owns the transaction and publication, and
checked-seed CupidC owns compilation. Hostbuild remains an optional oracle.
ADR 0222 records the command, ADR 0223 records seed carriage, ADR 0224 records
the first recipe transfer, and ADR 0371 records the direct transaction.
...
accepted sequential baseline bytes. At that checkpoint, Hostbuild froze the source and ran
checked CupidObj `wrap-jpeg`, which accepts sequential SOF0 or SOF1 input and
rejects progressive, unsupported, or malformed frames. Python then checked the
accepted snapshot independently and required unchanged bytes. This removed
...
That Python coordinator also rechecked the live manifest and source before
atomic publication. The current Make edge instead runs promoted CupidBuild
`embed-jpeg`, including its independent native check and guarded publication.
...
checked seed and 59-file candidate source inventory. The earlier 16a cohort
published 22 artifacts with those 76/59 inventories and passed stable-copy
`CUPMAN2` verification. Fresh publication after the contract-consumer repair
in `b25a3374` and the refreshed object expectations agrees on all 62 stage
pairs and passes native manifest verification on Linux and Windows.
The full executable suite and user build pass.
The older 75/58 publication is historical. Each run
...
records the later v1 promotion.
...
Its packed i386 words preserve the exact 130,506-byte blob. The current
130,920-byte object has SHA-256
...
checked CupidObj serializes the blob. Promoted CupidBuild freezes the six-tool
seed, renders the expected bytes independently, and rejects malformed output,
an empty text-symbol set, i386 address overflow, missing output, parity
failure, or live input drift before atomic publication. Hostbuild remains an
optional oracle. This retires the generated root's GCC or Clang dependency.
ADR 0116 records the language boundary, ADR 0123 records the compiler
transfer, ADR 0224 records the first generator transfer, and ADR 0371 records
the direct CupidBuild handoff.
...
The preceding v1 five static commands shared one complete checked-seed gate. The manifest
binds the exact executables, source revision, target ABI, producer lineage,
19-source build plan, startup, and five link orders. That Linux seed contained
the stage-four images from revision
...
That v1 promoted-seed reproof reported all five initial comparisons false
because the new startup function types remained in static ELF symbol tables
and flowed into every linked tool; its stages three and four still matched
exactly. Its frozen 50-input snapshot had SHA-256
`46c5335c80d822dd5085ee22077486ea647e5396482d42454847c87e4222aa67`.
The checked build plan had SHA-256
...
| `jpegtran`, `djpeg`/`cjpeg`, or FFmpeg | No role in the normal root build. The promoted CupidBuild transaction asks checked CupidObj to validate and wrap the repository's sequential SOF0 or SOF1 JPEG, then applies its independent native parser | Not required by root `all`; progressive, unsupported, and malformed input fails instead of selecting a host converter | Retain only for optional asset maintenance outside the build graph |
...
| CupidBuild | 354 participating transforms | Directly coordinates 157 guarded kernel compilations, two guarded assembly objects, three guarded raw images, the typed JPEG object, the generated kernel-symbol source, kernel flattening, and the Doom profile manifest. It also launches CupidObj for 175 text wrappers, eight binary wrappers, and three installation-source generators, and launches CupidLD for both normal kernel links through the promoted host seed. The guarded commands validate each private artifact, recheck the publication boundary, and replace the output atomically. |
...
| CupidObj | 192 participating transforms | 175 canonical text-to-ELF wrappers, eight byte-exact binary-to-ELF wrappers, one checked `wrap-jpeg` transform inside CupidBuild's typed transaction, final initialized ELF-to-raw conversion, three installation-source generators, one kernel-symbol source generator, one production disk-image template, one production ISO fixture, and one guarded Doom profile manifest. |
...
| Checked-seed CupidObj profile path | Included in the 192 CupidObj transforms | `profile-manifest` authors the canonical Doom profile JSON from a frozen `CUPROF1` snapshot inside the promoted CupidBuild transaction. CupidBuild checks an independent native render, rechecks its frozen closure and publication boundaries, and preserves unchanged timestamps under ADR 0383. |
| CupidDis | 10 participating transforms | Supplies the 5,161 selected text-symbol entries in the 130,506-byte panic-backtrace blob, validates the complete 431-input code cohort in the transactional `kernel.bin` transform, checks source-resolved edges in the bootloader and SMP trampoline maps, inspects the exact data-only ISO pattern map, covers every executable byte and function anchor in the private ISR and context-switch objects, and inspects all three user ELFs before publication. The checked Linux and Windows seeds validate executable relocation ownership, local targets, and function anchors for both guarded assembly objects. The host oracle remains optional. |
| Python | 98 transforms | Coordinates Doom, generated installation and user compilation, complete image publication, and verification outside the native CupidBuild paths. Every transform has a Cupid participant, so no transform is Python-only. The user ABI, artifact-size, and Toolchain manifest gates combine Cupid-built contracts with independent Python oracles. The disk and ISO composite CupidObj paths keep Python for their parity, filesystem, and publication rules. |
...
CupidBuild selects it before publishing either object, and both fixed-point
...
That checkpoint retained the 5/22/21 Linux fixed-point matrix and recorded strict validation
...
exact text for checked CupidObj. Hostbuild's `embed_jpeg` path remains as
test and oracle coverage for the preceding publisher; normal Make now calls
typed `cupidbuild embed-jpeg` directly. `tools/mksyms.sh` and
`tools/embed_jpeg_baseline.sh` are tracked
...
carried both capabilities; ADR 0318 carried them in that checkpoint's Linux
and Windows seeds. ADR 0353 records the paired v2 contract, while ADR 0367
records the active six-tool carriage. Host C
remains only
...
not retire Python by itself. The subsequent raw handoff gives CupidBuild the
two assembly-object publications plus the bootloader and trampoline paths.
The fixed-point coordinators, source freezing, independent comparisons, and
remaining publication guards stay host-owned until later ownership steps
supply their own evidence. The manifest's non-producer role prevents circular
fixed-point lineage and does not restrict this normal-build ownership.

The ADR 0356 promotion checkpoint had these exact image identities:
...
| Windows | `cupidobj.exe` | 375,808 | `b6f6a5b66f8e2bcb4b779a16428d7b77a956113c5ca301344537b35839611572` |

The root, user, and Toolchain Makefiles capture the selected six platform images
...
filesystem safety, and the remaining Hostbuild transactions. CupidBuild now
owns the guarded bootloader, SMP trampoline, ISR, and context-switch
publications, the typed JPEG and kernel-symbol generators, final kernel
flattening, Doom profile publication, and 186 ordinary CupidObj recipes. It
also launches both normal kernel links, while GNU Make still declares the
normal graph. The generated active-build audit records 354 CupidBuild
transforms and 98 Python transforms across all three roots. The bootstrap
log separates each generation's candidate proofs, promoted-seed reproofs,
and artifact checks. ADR 0357 records the direct publication transfer,
ADR 0362 records the direct CupidObj handoff, and ADR 0383 records the latest
profile handoff.

### Promoted kernel-symbol coordinator

The promoted CupidBuild seeds coordinate the pass-one ELF to generated KSYM
source transaction without shell redirection. The normal Make edge invokes the
platform seed directly with the manifest and complete six-image closure.
CupidBuild captures CupidDis output privately, runs CupidObj against those
exact bytes, checks the result with an independent native renderer, and owns
locking, rollback, and guarded publication. Hostbuild remains an optional
