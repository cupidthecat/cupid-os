...
the other six transforms. The host C compiler still owns 142 transforms,
including 90 root objects. Python owns 170 transforms, including the 155
...
The strict frontier must compile all 148 checked-in sources twice. Every
...
and the normal-image gate remain part of the proof. The full 148-root
frontier passes twice against a 436-file frozen snapshot. The two object sets
are byte-identical and total 3,621,852 bytes. The combined 148-root graph
...
handoff, ADR 0135 records the Nuked OPL3 transfer, and ADR 0139 records the
JPEG and glyph-raster transfer.
...
Compiler head also evaluates static binary32 and binary64 arithmetic with
...
slices. The current 148-source checked-in cohort, complete `.cc` naming, and
...
semantics in native recipes. Seven strict checked-in roots remain host-owned.
...
Its packed i386 words preserve the exact 105,242-byte blob. Two compiles
produce the same 105,656-byte object with
SHA-256
...
width and evaluate its address once. The integrated compiler head also
...
bits, so it adds no host floating or math-library dependency. The current
checked seed predates the evaluator, and the Doom cohort remains host-owned.
...
The five static commands now share one complete checked-seed gate. The manifest binds the exact executables, source revision, target ABI, producer lineage, 19-source build plan, startup, and five link orders. The current seed contains the checked bootstrap's stage-three images at revision `7e7029637ef22a4f18c382ffb225fd6a2ea84b85`. CupidC changed from the preceding seed, while CupidASM, CupidDis, CupidLD, and CupidObj remain byte-identical. The 2,109,488-byte CupidC image has SHA-256 `39a5783a5ba07a4891b887ea36a5686098dc9ca128b29419aea1e0c2cd8ee86e`.
...
| Python 3 | Generates embedded-source/symbol tables; creates, stages, and cleans images; builds fixtures; transforms JPEG data; drives QEMU network tests with standard-library sockets; parses and correlates Ethernet PCAP captures; verifies the external-program syscall ABI; launches checked-seed CupidC for 149 normal production objects, and launches native CupidC for the Windows user cohort | Required | May remain for orchestration, verification, test control, and image packaging; code/object/link behavior stays behind Cupid tools |
| WSL on Windows | Runs the checked static i386 Linux CupidC seed for 149 normal-build production objects, three generated installation tables, and the staged Toolchain bootstrap | Required for those paths on Windows; the native Windows user build no longer uses it, and native Linux runs the seed directly | Remove from the remaining Windows build when a checked native CupidC or an equivalent Cupid-owned execution path is available |
...
| --- | ---: | --- |
| Host C compiler | 142 | 90 i386 root objects, 33 native hosted core/CupidC-preprocessing/declaration/type-layout/IR/object/ELF32/x86/CupidDis/CupidASM/CupidObj/CupidLD/kernel-bridge objects, and 19 native contract/CLI executables; these builds remain temporary bootstrap evidence even though preprocessing, declaration, type/layout, scalar and structure-value code, fixed direct and indirect cdecl calls, file and block-static object emission, assembly, inspection, object-transformation, and link semantics have shared Cupid-owned implementations |
| CupidC | 155 owned transforms | The 148-source checked-in normal cohort, generated kernel symbols, three generated installation tables, and three example external programs; Python verifies and launches the checked seed for Linux and non-user Windows paths, while Windows user builds run a private native driver snapshot; each object is validated before publication |
...
| CupidDis | 1 composite transform | Supplies deterministic numeric symbols to `_symbols_from_nm`; the current consumer cohort contains 4,384 text symbols and a 105,242-byte panic-backtrace blob; the host oracle remains optional |
| Python | 170 transforms | The 155 CupidC launches plus fourteen generation, inspection, link, and orchestration transforms and one external-program syscall ABI verification; Windows uses native CupidC for three user launches, and symbol generation still uses Python after CupidDis inspection |
...
promotion compiles unchanged `i_video.c`. Compiler head emits all 80 objects,
but all 83 Doom and port roots still use host recipes. The checked seed
predates this complete frontier. A five-tool promotion, object comparison, and
runtime proof must pass before ownership moves. This work therefore changes
neither the 90 host-built root objects nor the 142 host C transforms.
...
evidence, not host-compiler retirement: the host compiler still compiles 90
...
The production-integration item above refers to the 142 remaining host-C
transforms. Checked-seed CupidC owns the 148-source checked-in normal cohort,
