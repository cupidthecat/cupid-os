...
4. Asks each Make root for its declared final artifacts. The root cohort contains the ordered `KERNEL_OBJS` plus boot, trampoline, pass-1 ELF, generated symbols, final ELF, raw kernel, and freshly formatted disk-image boundaries; the user cohort contains the three CupidLD executables; the hosted cohort contains every declared contract/CLI executable (seventeen in the current graph, including the hosted CupidC preprocessing, type/layout, declaration, and static-object contracts). `make check-bootstrap-audit` independently proves that every linked OS object is present in the root manifest. The checked `1e079d1` evidence below predates all four CupidC contracts and therefore contains thirteen hosted artifacts.
