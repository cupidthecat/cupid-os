...
CupidObj participates in 188 transforms, including three installation-source
generators, the kernel-symbol source generator, the normal disk-image
template, and the repository ISO fixture. The root graph has 437 Cupid-owned
transforms. Only the Doom input manifest remains Python-only. ADR 0190 records
the root handoff, ADR 0196 records the contract handoff, ADR 0238 records the
disk-image transfer, and ADR 0241 records the ISO transfer.
...
| Normal-build cutover | Make invokes CupidC, CupidASM, CupidLD, CupidObj, and CupidDis without GCC/Clang/NASM/LLVM/binutils | In progress: the 438-transform root image graph assigns 242 transforms to CupidC, five to CupidASM, 188 to CupidObj, two to CupidLD, and one to CupidDis, with no host C or recursive Make transform. These assignments cover 437 Cupid-owned root transforms. Only the Doom input manifest remains Python-only. Across all three supported graphs, CupidC owns 245 transforms and no transform invokes a host C compiler; Python participates in all 449 transforms as orchestration, remaining artifact production, or manifest-backed verification. The three installation-source transforms, kernel-symbol source transform, pristine disk-image template, and deterministic ISO fixture belong to checked CupidObj. Python remains the launcher and independent parity oracle. It also owns FAT reuse, file staging, native-path safety, drift checks, locking, and atomic image publication. ADR 0204, ADR 0224, ADR 0238, and ADR 0241 record those transfers. The poisoned-host build passes. A 607.7-second Linux kernel build and a 341.6-second Windows root build produced the same 430 frozen kernel artifacts. The 8,579,824-byte pass-one ELF has SHA-256 `2350b1a4f32c674d386ca50d4be4f6be8308082291f2852cfecfde4ba151b4df`; the 8,690,416-byte final ELF has SHA-256 `80b23fbd198f4aa9ec030ac69526b2d612c49e6d6328cfa01af9a0ef6217d8bb`; and the 8,490,228-byte raw kernel has SHA-256 `53770a93658e757d25f5aeab9d3e434d4a3be2a1dc3fbe4b19869e5bf9820a06`. The fresh 209,715,200-byte normal image has SHA-256 `e815d2ef67f114a26181f0e2cbde85f892cdadd487f8d9cbee9715e720800b3e`. A private `/bin/ls.cc` JIT boot from it passed in 49.8 seconds. Native hosted binaries still need GCC or Clang when explicitly requested as development oracles. Windows runs checked i386 programs through WSL until a native checked seed exists. Checked CupidASM authors the spanning ISO lane, and checked CupidObj authors the enclosing image before Python parity and guarded publication. ADR 0227 records the lane transfer, while ADR 0241 records the image transfer. |

The ADR 0242 source-head fixed point adds CupidObj `profile-manifest` without
changing the promoted seed. Host code generators were poisoned while all 19 C
objects, startup, and five tool images matched between stage two and stage
three. Both stages passed five help cases, fifteen successful operations, and
thirteen failures, including SHA-256 padding and repeated-block vectors,
unsafe paths, case collisions, and output preservation. The normal Doom
manifest remains Python-owned until the five-tool seed carries this command
and the guarded publisher transfers its bytes. ADR 0242 records the source
capability.

The source-current ISO ownership build completed in 502.232 seconds. Its
