...
| --- | --- | --- | --- |
| `boot/boot.asm` | NASM flat binary | CupidASM flat binary | Host-owned; the shared model covers its audited 16/32-bit instruction forms, but CupidASM still needs directives, expressions, layout, flat output, source-level byte parity, and boot proof |
| `kernel/cpu/isr.asm` | NASM ELF32 object | CupidASM ELF32 `ET_REL` | Host-owned; the shared reader accepts its real 377-byte aligned `.text` and five PC-relative relocations, and the shared model matches its 169 decoded instructions, but CupidASM still needs parsing and object-emission parity |
| `kernel/core/context_switch.asm` | NASM ELF32 object | CupidASM ELF32 `ET_REL` | Host-owned; shared object/instruction support matches its 20 decoded instructions, but CupidASM still needs parsing, ABI-sensitive object parity, and boot/runtime smoke |
| `kernel/smp/smp_trampoline.S` | NASM flat binary despite `.S` suffix | CupidASM flat binary | Host-owned; the shared model covers its mixed 16/32-bit instructions, but CupidASM still needs source parsing, layout/flat output parity, and SMP smoke |
...
| Shared `toolchain/x86.*` instruction module | Host C compiler builds one implementation into the hosted contract and freestanding kernel | CupidC builds the same module; CupidC, CupidASM, and CupidDis share its typed semantic seam and private form catalogue | Instruction semantics are Cupid-owned and active-surface-tested; no frontend or source cohort transfers until the existing tools call the module |
...
| `kernel/lang/as*.c` | GCC/Clang, linked into kernel | CupidC builds host and in-OS CupidASM variants | Host-owned; shared runtime/object/instruction seams now exist, but the frontend has not migrated onto them |
| `kernel/lang/dis.c` | GCC/Clang, linked into kernel | CupidC builds host and in-OS CupidDis variants | Host-owned; shared object/instruction seams now exist, but the frontend and object-inspector behavior have not migrated |
...
| --- | --- | --- |
| Baseline | Clean, reproducible oracle build and recorded artifact/tool hashes on Windows and Linux | Historical Windows PASS at `1b5901c`: two clean 430-artifact builds matched and covered all 423 linked objects; the 431/424 shared-x86 graph requires its post-commit recapture; Linux capture pending |
| Capability audit | Every active source and generated input mapped to required C, ASM, ABI, object, linker, and inspector features | Complete for root `all`, `user:all`, and `toolchain:all`: 655 active inputs, 248 feature IDs, 452 transforms, 36 accounted unreachable source-like files, and a checked 431-artifact/424-link-object drift and coverage gate |
| Assembly migration | All four host-assembled OS sources produced by CupidASM with equivalent bytes/behavior | Not started: the shared instruction seam and active-family byte corpus are established, but no NASM-owned artifact has transferred |
