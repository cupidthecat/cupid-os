...
candidate proof passed with all five initial comparisons true and the 20/5/21
failure/help/success inventory. The current Windows candidate proof passed
with all five initial comparisons true and the 8/5/7 inventory. Both bind
revision `ad7305341003feaa7e630ab7fd45be0a214c4da7` and source snapshot
...
`106980d97475d36b7835395a5bbfb43eb1e71484cea631d80dfe47be1acc2ac3`, a
9,493,728-byte `kernel/kernel.elf` with SHA-256
`b8e4a34844190b22faf5840a06d32ef961b6835c3af028cc78e34352ffc6bf6d`,
and a 9,271,380-byte `kernel/kernel.bin` with SHA-256
...
artifact ownership counts. The promoted Linux seed carries the 5/21/20 matrix.
...
preceding relocatable local-target seed. ADR 0318 records the current seed.
...
0312 records the preceding relocatable handoff. ADR 0318 records the current
linked-image seed.
...
local-target carriage, and ADR 0312 records a preceding stage-four promotion, and ADR 0318 records the current one.
ADR 0253 records
...
| Normal-build cutover | Make invokes CupidC, CupidASM, CupidLD, CupidObj, and CupidDis without GCC, Clang, NASM, LLVM, or binutils | In progress: all 443 root transforms have a Cupid participant. Across all three supported graphs, CupidC participates in 250 transforms, CupidObj in 192, CupidASM in nine, CupidLD in nine, CupidDis in six, and Python in all 452. Four transforms run Cupid-built semantic contracts. No transform is Python-only, invokes a host C compiler, or recurses through Make. Windows selects the native checked execution seed for output-bearing tools, the user ABI, artifact-size, and `CUPMAN2` verifier contracts; Linux keeps the static seed. The `CUPMAN4` author keeps the converged Linux producer path but runs as a validated native PE on Windows. It owns all 58 stage-pair decisions, while Python retains coordination, independent comparison, safety, drift, locking, and publication work. The raw boot paths select local-target and strict-decode checks before publication. Kernel AOT links with in-kernel CupidLD. The source-current schema-v3 `CUPMAN4` publication and final `CUPMAN2` verification pass in 3,952.17 seconds. The integrated fully poisoned build passes in 874.531 seconds, and the strong four-vCPU e1000 frontier passes in 883.513 seconds with the required feature-14 PASS marker followed by the feature's clean JIT completion. |

The current fixed points bind revision
`ad7305341003feaa7e630ab7fd45be0a214c4da7` and snapshot
`73b3fa6964292a7f0b753df3535058dd6399f5e6d8e277a082ac70ce65c79e43`.
Both promoted-seed proofs start with all five images equal. Their behavior
inventories are 20/5/21 on Linux and 8/5/7 on Windows. The normal kernel
transaction keeps its broad 431-input scan and adds linked local-target
validation over both frozen kernel ELFs before flattening.
