...
The source-head candidate fixed point and publication now pass. The v1
verifier and checked runner still accept only the five manifest tools. A separate plan
adds CupidBuild's three `.cc` sources, builds all six tools through Linux and
native Windows stages, compares CupidBuild between the final generations, and
includes it in both behavior gates. Linux freezes 58 source inputs and
produces 22 C objects plus startup. The Toolchain publisher produces 22
artifacts and 62 stage pairs, including all six bootstrap tools. Promotion and
the first production recipe transfer remain open.
...
both pass. The promoted seeds retain their five-tool inventories.
...
Source-head hosted CupidDis also accepts the deterministic static i386 PE32
...
five checked Windows seed images and an import-free CupidLD fixture pass next
...
unsupported. The five seed reports match an independent Python reconstruction
of every section and import record. This adds two headers to the
then-current Toolchain inventories to 75 publication inputs and 55 bootstrap
files without changing the 19 compiled C objects. The
promoted tools still reflect the earlier 50-file snapshot and do not carry PE
input support. ADR 0338 records the source and carriage boundary.
...
Windows output-bearing recipes select it directly, while Linux fixed-point and
the static Linux Toolchain contract paths keep the Linux bootstrap seed. The
Windows user ABI, artifact-size, and Toolchain manifest verifier gates build
and run temporary PE contracts from the execution seed. The manifest author
keeps its Linux stage-four producers but runs as a native PE on Windows.
Artifact-size policy and
both manifest modes still bind the Linux manifest. Native Windows
reconstruction pairs that seed with the
...
pins the raw policy, the Linux policy manifest, fourteen file observations,
and the exact Windows seed directory containing its manifest and five PE
...
manifest, and five regular-file size and digest observations. The C contract
validates the Windows target, provenance, Linux parent link, exact tool
...
The normal build completed its 431-input code-anchor inspection, and the image
...
and ADR 0336 records the current seed pair.
...
artifact ownership counts. The promoted Linux seed carries the 5/22/21 matrix.
...
inventory to 75 publication inputs and 58 candidate files. The latest complete
publication records that larger closure and 22 artifacts. The source graph
contains 747 active language inputs, including 301
...
Repository startup and runtime code now take five complete CupidC-emitted
...
interfaces. Fixed-point and Linux-contract work runs CupidC, CupidASM,
CupidDis, CupidLD, and CupidObj on i386 Linux or through WSL. The Linux seed
...
19-source stage-two Linux union and all five Linux tools below that root. The
...
behavior for all five commands. They also produce matching native objects and
PE images for all five tools. Windows runs help plus useful success and failure
...
and the final verifier accepts all 22 artifacts. This publishes the six-tool
candidate without promoting a six-tool checked seed.
...
records failure, help, and success counts of 23/6/29 for Linux and 12/6/16 for
Windows.
...
compound assignment in either lvalue direction. ADR 0312 first carried the ADR
0293 and ADR 0296 capabilities; ADR 0318 carries them in the current checked
seeds.
...
| Toolchain self-hosting | Checked seeds rebuild host tools; consecutive post-transition generations are byte-identical on Windows and Linux | The promoted 50-input seeds remain five-tool trust units. Source head derives a 58-input candidate that builds 22 Linux C objects plus startup and all six tools. The Linux stage-three and stage-four candidates are byte-identical and pass the 23/6/29 behavior matrix. Native Windows uses checked Linux CupidC for stage-two C objects and the checked PE seed for assembly, inspection, and linking; its candidate plan adds the publication runtime, three startup objects, and CupidBuild and passes 12/6/16 behavior. The preceding generation's CupidDis certifies every generated C object and startup object before linking. Both final-stage CupidDis images strictly certify every corresponding candidate image and reject an entry-corrupted CupidBuild copy. Python still coordinates both fixed points. A six-tool manifest, promotion, and recipe transfer remain open. ADR 0336 records the seed pair, ADR 0341 records the Windows bridge, ADR 0345 records the candidate plan, ADR 0346 records image certification, and ADR 0347 records object certification. |
...
`a17c9465911da41d59b7ada71733d36c39faa5ea` and snapshot
`46c5335c80d822dd5085ee22077486ea647e5396482d42454847c87e4222aa67`.
The source-head candidate behavior inventories are 23/6/29 on Linux and
12/6/16 on Windows. The normal kernel
