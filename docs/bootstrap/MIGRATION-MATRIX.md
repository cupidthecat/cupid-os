...
Checked-seed CupidDis now has a raw-only local control-flow policy. It validates
...
Linux and Windows seed fixtures each prove one local target. The policy excludes far, indirect, and ELF
targets. Both checked seeds carry the option, and the bootloader and SMP
transactions select it before atomic publication. ADR 0300 records the source
rule, and ADR 0305 records carriage and adoption.
