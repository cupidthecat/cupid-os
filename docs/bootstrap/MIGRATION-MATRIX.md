...
Make transform, 449 Python participants, 245 CupidC participants, one
...
output has Cupid-built contract and Python participants. The two Python-only
supplemental outputs aggregate the Toolchain cohort and record its manifest. No recursive Make
...
| Capability audit | Every active source and generated input mapped to required C, ASM, ABI, object, linker, and inspector features | Complete for root `all`, `user:all`, and `toolchain:all`: 728 active inputs, 255 feature IDs, 449 transforms, 25 accounted unreachable source-like files, and a checked drift and coverage gate. The graph contains 29 assembly files, 294 headers, 405 Cupid C files, and no ordinary C translation unit. The checked graph uses canonical Windows conditionals and the C locale on every host. No supported transform invokes a host C compiler or recursive Make. |
