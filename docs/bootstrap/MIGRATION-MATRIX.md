...
inventory. Seed carriage is not a production ownership claim, so the ISO image
remains one of the two Python-only root outputs until the guarded cutover. ADR
