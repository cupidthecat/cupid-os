...
source-current Toolchain inventories, bringing them to 72 publication inputs
and 52 bootstrap files without changing the 19 compiled C objects. The
...
promotions remain historical in ADRs 0280 through 0312. The current Linux
candidate proof passed with only the preceding CupidDis unequal and the 21/5/22
failure/help/success inventory. The current Windows candidate proof passed
with only the preceding CupidDis unequal and the 9/5/8 inventory. Both bind
revision `b3f0910f84ba182d0882fc67b5983b49e9627482` and source snapshot
...
9,421,144-byte `kernel/kernel.elf.pass1` with SHA-256
...
preserve earlier promotions, and ADR 0323 records the current seed pair.
...
preceding relocatable local-target seed. ADR 0318 records the preceding linked-image seed, and ADR 0323 records the current code-anchor seed.
...
headers. The C-family inventory covers 711 files and 2,494 include operands,
including 2,217 quoted operands. These source-current counts supersede the
...
local-target carriage, and ADR 0312 records a preceding stage-four promotion, and ADR 0318 records the preceding linked-image seed, and ADR 0323 records the current code-anchor seed.
ADR 0253 records
...
| `kernel/smp/smp_trampoline.S` | CupidASM flat binary with strict CupidDis inspection | CupidASM and CupidDis production gate | Production-owned: hosted CupidASM emits an exact 4,096-byte private candidate with SHA-256 `b738ebb68f28b9b07e330761f4e9a7898f0424ab0a3835cd6079ae7d4a189e90`. It also writes the private canonical map that CupidDis consumes. Hostbuild requires that map to match code16 `[0x000, 0x01f)`, data `[0x01f, 0x210)`, code32 `[0x210, 0x254)`, and data `[0x254, 0x1000)`. The shared checked raw-image transaction pins both candidates through strict decode and local-target inspection, then publishes only the binary. The caller retains the exact mixed-mode policy. Missing, empty, malformed, different, or changed maps preserve the prior output. The optional NASM oracle remains byte-identical, and four CPUs reach online state in the SMP smoke. ADR 0308 records the handoff. |
