...
from a supported root. The three-root audit has 450 transforms, no recursive
Make transform, 450 Python participants, 245 CupidC participants, one
...
CupidObj participates in 191 transforms, including three installation-source
...
441 transforms: 440 artifact transforms with a Cupid tool owner plus the
Python-only size verifier, which emits no OS artifact. CupidDis participates
in two root transforms. The `kernel.bin`
transform runs strict validation and flat extraction against one frozen
cohort, with all 429 code inputs represented.
...
the exact `.idata` layout. Normal recipes do not use that command, so all
ownership counts above remain unchanged. The promoted seed carries the
5/18/16 matrix. ADR 0247 records the format boundary, ADR 0248 records imports
and loader execution, ADR 0258 records the preceding seed, and ADR 0265
records current carriage.
...
still participates in all 450 transforms because it launches the checked
...
passes private AOT compilation for all 106 runnable top-level programs. The
...
Hosted CupidC emits deterministic i386 ELF32 objects for all fifteen regular files in the CupidC, CupidASM, CupidDis, and user-ABI contract cohort, plus the separate hosted runtime probe. Those sources, the 19-source static Linux tool union, four strict Windows tool drivers, the direct Windows runtime contract, and `kernel/lang/as_elf.cc` use 33 ordinary strict Linux roots, four strict Windows roots, one freestanding Windows root, a two-source strict kernel bridge, and three GNU runtime roots. The third GNU root selects the Windows side of the shared runtime. The retired 64-bit profiles have no active roots. The normal build compiles each regular Linux contract twice from the checked seed, compares all seventeen new objects, links all fifteen programs and the Linux runtime probe through CupidLD, and verifies all sixteen ELF32 executables. Publication accepts only a dedicated `cupidc-contracts` directory inside the source tree. The target is checked before work and again before promotion, and an existing destination must already verify as a complete cohort. Arbitrary directories, source trees, files, and symbolic links remain untouched. The initial, private, and newly discovered contract inventories must match exactly, which catches additions, removals, and restored edits that changed a copied input. Every run derives the cohort from its executable, requires a named manifest artifact, and verifies every artifact hash, the current 62-input contract set, the checked seed manifest, and the 47-file fixed-point source inventory before the behavior matrix starts. Those 62 inputs include the small Windows probe, the native Windows tool runtime, startup, direct runtime contract, `direct.h`, the user syscall ABI contract and its six declarations, the Toolchain Makefile, the publisher, and the independent Python ABI oracle. One captured seed-manifest byte sequence supplies its digest, decoded JSON, schema validation, and checked build plan. The same gate covers complete CupidLD and CupidObj command closures. Native GCC or Clang builds remain available only as explicit development oracles. This supersedes the older nine-file, eleven-file, fourteen-file, host-built, and 64-bit profile descriptions in the historical notes and long-form rows below.

Repository startup and runtime code now take five complete CupidC-emitted command closures across CupidASM and CupidLD. The runtime supplies the checked heap, file, memory, string, `errno`, working-directory, and diagnostic interfaces. CupidC, CupidASM, CupidDis, CupidLD, and CupidObj run on i386 Linux or through WSL. Their checked seed binds the static images to the target ABI, source revision, producer lineage, and complete build plan. The harness freezes the verified seed and copies 47 current inputs, including `link.ld`, the small Windows probe, the native Windows tool runtime and startup, the direct contract, and `direct.h`, into a private compiler root. The seed producer trio builds the 19-source stage-two Linux union and all five Linux tools below that root, then the stage-two trio repeats that work for stage three. The private and live closures are rehashed at every stage and behavior boundary. Every C object, Linux startup object, and Linux tool image matches across the two stages, and both stages agree on positive and failure behavior for all five commands. Both stages also produce matching native CupidASM, CupidC, CupidDis, and CupidObj objects and PE images. Windows runs help plus useful success and failure cases for all four tools. The direct runtime contract checks allocation, named-file write and append, current-directory errors, and quote and backslash parsing. The two stages, behavior evidence, and report appear together only after the full gate passes. ADR 0086 records the sibling commands, ADR 0088 records the compiler driver, ADR 0089 records the compiler fixed point, ADR 0090 records the five-tool fixed point, ADR 0092 records the checked seed, ADR 0142 records the frozen source closure, and ADR 0268 records the native four-tool boundary.
...
compiler, and Python participates in 449. The external-program syscall ABI
...
and atomic or long-double updates remain open. ADR 0250 records runtime unsigned
four-byte output,
...
The promoted seed also passed the current user ownership frontier with exit 0
...
| 106 `bin/*.cc` roots and 22 browser `.cc` fragments | CupidObj embeds source; in-OS CupidC compiles on demand | CupidObj embeds source; CupidC remains the language owner and can also be host-run | Production embedding has transferred to CupidObj; CupidC already owns runtime compilation, while host-runnable/source self-hosting remains |
| Three `user/examples/*.cc` programs plus `user/cupid.h` | Cupid-built syscall ABI gate, CupidC to `ET_REL`, then CupidLD fixed-text link; Linux runs the checked seed directly and Windows runs it through WSL; root staging remains deliberate | CupidC/CupidLD build and a deliberate image-staging path | Compilation, link, and ABI-rule ownership have transferred. The precompile operation runs a staged CupidC contract that snapshots and rereads the six declarations while pinning version 5, 103 table fields, 101 providers, exported scalar types and constants, and both VFS record layouts. Python compares its report with an independent oracle and rechecks the fixed-point publication inputs. Closed wrappers freeze the sources, header, tool image, and build controls, validate relocatable objects and loader-compatible executables, and publish atomically. The 23-input default frontier repeats all six program artifacts. The optional 46-input native Windows frontier also requires every result to match the checked seed. Poisoned-path tests reject conventional host code generators on the normal path. Separate private-image guest boots execute hello, ls, and cat from the staged image. The promoted seed proves PE imports and one narrow freestanding Windows bridge, but the user drivers still need the complete runtime, native Windows seed carriage, and deliberate production adoption. ADR 0264 records the ABI transfer. |
...
| Linked kernel ELF to raw kernel binary | One checked-seed CupidDis and CupidObj hostbuild transaction | Hostbuild validates and flattens one frozen cohort, then publishes the raw kernel atomically | Production-owned: hostbuild freezes the selected seed manifest and five artifacts, the 429-entry input manifest and cohort, and the existing `kernel.bin` boundary. Checked CupidDis validates the private cohort. Checked CupidObj writes physical-address-ordered file-backed `PT_LOAD` bytes with zero-filled gaps and BSS exclusion from the frozen final ELF. Hostbuild rechecks live trust inputs and the output before parent-relative atomic publication. Every failure preserves the prior raw kernel. The complete transaction passed with exit 0 in 187.054 seconds and published an 8,946,332-byte raw kernel with SHA-256 `4f5f2591d01bcc4007773844e9bfb8112a16dd17fbd178014cc2056fefaab67d`. The earlier poisoned-host `make -j2` passed in 1,057.969 seconds when validation and flattening were separate. Definitive four-vCPU E1000 and RTL8139 boot frontiers passed with exits 0 in 794.034 and 758.667 seconds. Both passed SMP, frontier, framebuffer, AC97, and PC speaker checks without changing the source image. |
...
The linked-kernel row's current poisoned-host rebuild passed in 1,018.548
seconds after the OS documentation was frozen. Its identities supersede the
pre-freeze outputs cited in that row. The 2,560-byte `boot.bin` has SHA-256
...
The completed dual-NIC boot frontiers above remain pre-freeze runtime
evidence. The final four-vCPU E1000 and RTL8139 runs passed from the current
image with the partitioned USB fixture, `--smp 4`, `--cpu max`, SMP and
frontier runtime verification, a private image, and a 300-second phase
timeout. E1000 exited 0 in 725.058 seconds with 103,673 changed framebuffer
pixels, 29,608,822 AC97 frames at peak 25,600, and 76,784 PC speaker frames at
peak 30,710. RTL8139 exited 0 in 725.406 seconds with 106,151 changed pixels,
29,601,879 AC97 frames at peak 25,600, and 76,719 PC speaker frames at peak
31,501. Both used a 640 by 480 framebuffer, and the image hash remained
unchanged.
...
that policy and flat extraction to one frozen cohort of all 427 audited root
object outputs plus the pass-one and final kernel ELFs. The 9,028-byte LF-only manifest lists all 429
...
froze and rehashed the manifest and inputs and passed in 185.526 seconds with
empty streams and exit 0. The current transaction also freezes the selected
seed manifest and five artifacts plus the existing `kernel.bin` boundary.
Hostbuild runs checked CupidDis and checked CupidObj against the private
cohort, rechecks live trust inputs and output, and publishes through
parent-relative atomic replacement. Every failure preserves the prior raw kernel. The transaction
...
the 5/18/16 fixed-point matrix. `make bootstrap-audit` passed in 64.780
seconds.
...
Hexadecimal floating literals, binary32 and binary64 subnormal literals,
hexadecimal or subnormal long-double literals, decimal ratios beyond the
bounded parser, runtime mixed integer and floating arithmetic or conditionals,
atomic and long-double updates, SIMD, over-aligned
...
| Capability audit | Every active source and generated input mapped to required C, ASM, ABI, object, linker, and inspector features | Complete for root `all`, `user:all`, and `toolchain:all`: 732 active inputs, 255 feature IDs, 450 transforms, 25 accounted unreachable source-like files, and a checked drift and coverage gate. Root `all` has 441 transforms. The graph contains 30 assembly files, 295 headers, 407 Cupid C files, and no ordinary C translation unit. The checked graph uses canonical Windows conditionals and the C locale on every host. No supported transform invokes a host C compiler or recursive Make. |
...
| C migration | Every reachable kernel, tool, application, Doom, and vendored C cohort compiles and passes behavior gates with CupidC | In progress: all 238 checked-in normal roots plus the generated kernel-symbol translation compile through the checked seed. Three generated installation tables and three example programs add CupidC ownership outside that cohort. The 15 hosted Toolchain contracts also compile, link, and run through the checked i386 seed. Linux runs the seed directly; Windows uses WSL for these i386 programs. Native hosted tools and contract runners remain optional development oracles. All Doom production objects compile, and the no-WAD, explicit missing-IWAD recovery, shell-survival, and dual-NIC frontier checks pass. Full IWAD gameplay, input, audio, and save behavior remain to be checked. |
| Toolchain self-hosting | Checked seeds rebuild host tools; stage 2 and stage 3 outputs are byte-identical on Windows and Linux | A checked static i386 Linux seed rebuilds one 19-source union, fresh startup, and all five Linux tools from a private captured source root. The stage-two producer trio repeats the work below the same root. The current 47-file closure also contains the small Windows probe, native tool runtime and startup, direct runtime contract, and `direct.h`. The harness rehashes the private and live closures after each stage and after behavior checks, then publishes one complete evidence bundle. All 19 C object pairs, startup objects, and CupidC, CupidASM, CupidDis, CupidLD, and CupidObj Linux images match. Source head also builds matching native CupidASM, CupidC, CupidDis, and CupidObj objects and PE images through both stages. Windows runs help plus useful success and failure cases for all four tools and the direct runtime contract. The current proof retains the 5/18/16 matrix, passed in 871.1 seconds, and produced a 32,681-byte report with SHA-256 `d3608ab66f6781780ba3fe68eb3c5814248d1903d65f50651c8950ca46dda1e4`. Linux runs the seed directly, while Windows still uses WSL for the seed bootstrap and normal user build. ADR 0265 records the promoted Linux images, and ADR 0268 records the native four-tool boundary. CupidLD's native publisher plus checked native seed carriage remain open. |
| Normal-build cutover | Make invokes CupidC, CupidASM, CupidLD, CupidObj, and CupidDis without GCC/Clang/NASM/LLVM/binutils | In progress: root `all` has 441 transforms. Its 440 artifact transforms have a Cupid tool owner, with 242 CupidC, five CupidASM, 191 CupidObj, two CupidLD, and two CupidDis participations. The remaining Python-only size verifier emits no OS artifact and blocks `cupidos.img` publication on failure. Across all three supported graphs, CupidC participates in 245 transforms, CupidObj in 191, CupidASM in five, CupidLD in five, CupidDis in two, and Python in all 450. No transform invokes a host C compiler or recursive Make. The user compiler and Toolchain contract publisher create their output directories, so two supplemental verification or orchestration outputs remain Python-only. The three installation-source transforms, kernel-symbol source transform, pristine disk-image template, deterministic ISO fixture, and Doom profile manifest belong to checked CupidObj. Python retains FAT reuse, file staging, native-path safety, drift checks, locking, and atomic publication. ADR 0204, ADR 0224, ADR 0238, ADR 0241, ADR 0244, and ADR 0245 record those transfers and the directory boundary. Earlier cross-host artifact and image results remain historical checkpoints. The final poisoned-host `make -j2 all` passed in 1,022.190 seconds after the exact-size gate accepted all nine artifacts. Its 209,715,200-byte image has SHA-256 `326844ca58c1f864a6b9a2480dfaeb5ed71ec3df22cdb46da17a6bb356e7e726`, and the final four-vCPU E1000 and RTL8139 frontiers pass from that image. Native hosted binaries still need GCC or Clang when explicitly requested as development oracles. Windows runs checked i386 programs through WSL until a native checked seed exists. Checked CupidASM authors the spanning ISO lane, and checked CupidObj authors the enclosing image and Doom profile manifest before Python parity and guarded publication. ADR 0227 records the lane transfer, ADR 0241 records the image transfer, ADR 0244 records the profile-manifest transfer, and ADR 0245 records publisher-owned directories. |
