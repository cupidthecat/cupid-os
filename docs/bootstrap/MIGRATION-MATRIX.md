...
| Capability audit | Every active source and generated input mapped to required C, ASM, ABI, object, linker, and inspector features | Complete for root `all`, `user:all`, and `toolchain:all`: 735 active inputs, 255 feature IDs, 450 transforms, 25 accounted unreachable source-like files, and a checked drift and coverage gate. Root `all` has 441 transforms. The graph contains 31 assembly files, 296 headers, 408 Cupid C files, and no ordinary C translation unit. The checked graph uses canonical Windows conditionals and the C locale on every host. No supported transform invokes a host C compiler or recursive Make. |
...
| Toolchain self-hosting | Checked seeds rebuild host tools; stage 2 and stage 3 outputs are byte-identical on Windows and Linux | A checked static i386 Linux seed rebuilds one 19-source union, fresh startup, and all five Linux tools from a private captured source root. The stage-two producer trio repeats the work below the same root. The current 50-file closure also contains the small Windows probe, native tool runtime and startup, CupidLD publication runtime and bridge, direct runtime contract, and hosted Windows declarations. The harness rehashes the private and live closures after each stage and after behavior checks, then publishes one complete evidence bundle. All 19 C object pairs, startup objects, and CupidC, CupidASM, CupidDis, CupidLD, and CupidObj Linux images match. Source head also builds matching native objects and PE images for all five tools through both stages. Windows runs help plus useful success and failure cases for each tool and the direct runtime contract. CupidLD additionally checks exact output, collision handling, failure diagnostics, and cleanup. The current proof retains the 5/18/16 matrix, passed in 902.792 seconds, and produced a 38,162-byte report with SHA-256 `d90cf63e19ed1b4af560e4c15660d0583a1591bccdaa75157432204a82079efd`. Linux runs the seed directly, while Windows still uses WSL for the seed bootstrap and normal user build. ADR 0265 records the promoted Linux images, ADR 0268 records the shared Windows runtime, and ADR 0269 records native CupidLD publication. Checked native seed carriage remains open. |
