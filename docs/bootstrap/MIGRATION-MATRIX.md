...
Checked-seed CupidObj now performs the same checks through transactional
`wrap-jpeg` before applying the ordinary binary wrapper. The production
recipe still uses the Python validation publisher until its separate ownership
