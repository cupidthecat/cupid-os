...
Make transform, 448 Python participants, 245 CupidC participants, and no host
...
440 root outputs has a Cupid tool owner, so no root output is Python-only.
...
compiler, and Python participates in 448. The three Python-only supplemental
...
| Capability audit | Every active source and generated input mapped to required C, ASM, ABI, object, linker, and inspector features | Complete for root `all`, `user:all`, and `toolchain:all`: 727 active inputs, 255 feature IDs, 449 transforms, 25 accounted unreachable source-like files, and a checked drift and coverage gate. The graph contains 29 assembly files, 294 headers, 404 Cupid C files, and no ordinary C translation unit. The checked graph uses canonical Windows conditionals and the C locale on every host. No supported transform invokes a host C compiler or recursive Make. |
