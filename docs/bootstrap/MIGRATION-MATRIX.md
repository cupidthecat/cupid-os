...
same invalid-option failure. With the typed JPEG and kernel-symbol
transactions included, the current behavior totals are 27/6/34 on Linux and
...
the complete source-head matrices are 27/6/34 on Linux and 16/6/21 on native
...
The source closure remains at 59 inputs. The current kernel-symbol-capable
...
latest reconstructions from revision `9d10c223fc7aa22901e6f4ae81ce800ff1b62ad6`
...
records the preceding pair, ADR 0368 records the JPEG handoff, and ADR 0370
...
kernel operation writes matching v1 completion records only after both public
replacements. A later command can finish cleanup from a valid record or restore
retained backups when no record exists. Unlike the hosted CLI, the VFS path has
no pending record or absence tombstone before mutation, and its recovery
removes a readable target before restoring a backup. This adds an
...
| In-OS CupidASM artifact API | Shared CupidASM through the checked kernel adapter | CupidASM owns the same artifact forms in the host and kernel | The public adapter accepts raw `BIN` output with its canonical map, unlinked ELF32 relocatables, and linked executables through `-f bin`, `-f elf32`, and `-f exec`. A failed command preserves the previously published artifact pair, and the same process can recover on its next request. The rollback boundary covers one running command; crash-atomic publication and a concurrent writer lock remain open. |
...
`9d10c223fc7aa22901e6f4ae81ce800ff1b62ad6` and snapshot
`bac22f6a59871326ec40a58ab143eea1675b689251c76950d43d860cb2539fcd`.
Their behavior inventories are 27/6/34 on Linux and 16/6/21 on Windows. The
normal kernel
