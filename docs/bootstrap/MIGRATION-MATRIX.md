...
the existing unordered-result normalization. Hexadecimal floating literals,
`long double` literals, nonzero or floating static long-double initializers,
integer conversions involving `long double`, truth, runtime
conversion to unsigned four-byte
...
the x87 stack balanced after every result. Runtime floating truth, hexadecimal
floating literals, `long double` literals, nonzero or floating static
long-double initializers, integer conversions involving `long double`,
runtime conversion to unsigned four-byte integers or `_Bool`, runtime mixed
wide and floating arithmetic or conditionals, floating increment and
decrement, SIMD, floating atomic access, over-aligned emission, production
integration, and self-hosting remain open.
...
| Capability audit | Every active source and generated input mapped to required C, ASM, ABI, object, linker, and inspector features | Complete for root `all`, `user:all`, and `toolchain:all`: 717 active inputs, 253 feature IDs, 449 transforms, 25 accounted unreachable source-like files, and a checked drift and coverage gate. The graph contains 27 assembly files, 289 headers, 401 Cupid C files, and no ordinary C translation unit. The checked graph uses canonical Windows conditionals and the C locale on every host. No supported transform invokes a host C compiler or recursive Make. |
