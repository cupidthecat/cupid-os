...
production seed closure. The refresh candidates found CupidBuild alone
different from the preceding seed image, while all six stage-three and
stage-four tools matched. Broader Python removal remains open.
...
including the six bootstrap images. ADR 0353 records the seed promotion.
...
25/6/32 for Linux and 14/6/19 for Windows. Generation and checked comparison
both pass for that audit. The refreshed v2 seeds carry the two raw CupidBuild
commands described in ADR 0355. Their normal bootloader and SMP recipes remain
with Hostbuild until the separate ownership handoff recorded after this seed
refresh. ADR 0356 records carriage.
...
tools carry that support over the current 58-input closure. ADR 0338 records
...
0336 records the preceding seed. ADR 0353 records the active paired v2 seeds.
...
The harness freezes the verified Linux seed and copies 58 current inputs into
...
checked seed. The active paired v2 seeds retain it under ADR 0353.
...
paired seed reports record failure, help, and success counts of 25/6/32 for
Linux and 14/6/19 for Windows. The source-current audit and deterministic check
mode both pass.
...
seed, and the active paired v2 seeds retain both under ADR 0353.
...
Their behavior inventories are 25/6/32 on Linux and 14/6/19 on Windows. The
normal kernel
