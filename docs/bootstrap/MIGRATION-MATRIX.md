...
recovery tests distinguish verified backup preservation from native Linux's
...
raw kernel. That failure is recorded separately from the new paired proofs.

Checked `cupidbuild assemble-iso-pattern` publishes the exact ISO spanning
...
the existing timestamp for equal bytes. The normal `big.bin` edge now selects
CupidBuild directly with the source, Makefile, and complete seed closure.
Both repaired seeds carry the command. ADR 0384 records the source boundary,
and ADR 0386 records the recipe handoff. ISO image
publication remains a separate Hostbuild transaction.

The promoted CupidBuild seeds publish the closed Doom profile manifest through
...
requires both parents to exist. Make rejects existing links at either component
before `mkdir -p build/bootstrap`; the transaction does not
claim those directories for rollback.

The profile precedes all normal output writers in the scanned source roots.
Make applies an order-only dependency to matching bootstrap artifacts and
the three generated installation sources. This leaves unrelated work parallel.
The 171 order-only consumers do not rebuild for a profile timestamp change
alone; the 83 Doom consumers retain ordinary content dependencies. The audit
separates scheduling edges from content inputs, preserves their reachability,
and rejects a missing barrier for any of the 254 current writers.
...
candidate instead of nesting a second publisher. The common startup supplies
the shared file-information wrapper, and temporary Windows behavior manifests
...
read/traverse directory handle, then reacquires cleanup authority. Frozen-input
...
promotion, and ADR 0383 records the production handoff.
...
and owns rollback and publication. Both active seed cohorts carry the command. The
normal `%.jpg.o` and `%.jpeg.o` edges now invoke it directly, moving one
transform from Python to CupidBuild.

`cupidbuild generate-ksyms` freezes the pass-one ELF and complete seed, captures
CupidDis output privately, compares CupidObj output with an independent native
renderer, and owns guarded publication. Both active seed cohorts carry it, and
the complete source-head matrices are 28/6/35 on Linux and 17/6/22 on native
Windows. The normal kernel-symbol edge now invokes the promoted platform
CupidBuild seed directly with the complete six-image closure.

The first paired refresh failed closed while compiling that public interface:
the hosted Linux include profile did not provide `<stddef.h>` for its `size_t`
declarations. Cupid now owns that standard header for the static i386
bootstrap. It provides `ptrdiff_t`, `wchar_t`, `max_align_t`, and `offsetof`
without importing a host libc, and strict CupidC provides the target-layout
`__builtin_offsetof` operation behind the macro. Focused checked-seed and
strict frontend tests first failed on the two missing capabilities. Both now
pass.

The ADR 0374 source closure had 59 inputs. Its kernel-flatten-capable
snapshot had SHA-256
`0b591a0bef928186641b3aa1fb98c1e145e6c4905c8b6cb87c34a1ace4bc87d2`.
That pair bound the 59-input closure. CupidBuild accepts a
promoted v2 `source_input_count` of exactly 58 or 59 and rejects 57 and 60,
retaining a bounded path from the preceding generation. The first 59-input
promotion attempt failed closed during the normal build with `fixed-point
provenance differs`; no invalid seed entered either checked directory. The
ADR 0374 reconstructions from revision `0232cb57aad5d6bdfd7bd77499762514b2f0ebfd`
converged. After publication, separate self-consumption reproofs passed on both
hosts with all six initial images equal to stage two. The Linux and native
...
wrappers, and three installation-source generators. The five remaining
composite CupidObj publications keep their Python verification and publication
layers.
ADR 0358 records the original runner, ADR 0360 records its extension and
fixed-point gate, ADR 0361 records the paired promotion, and ADR 0362 records
the recipe handoff. The Windows CupidBuild CLI suite completed 67
...
contract and ran `/bin/ls.cc`. The final documentation-bearing image passed
the same private four-vCPU E1000 gate and reached normal `/bin/ls.cc` JIT
completion.
...
At the kernel-symbol checkpoint, CupidBuild was part of both checked six-tool
seeds and owned six guarded normal publications: two assembly objects, two raw
images, the JPEG object, and the generated kernel-symbol source. Its
first command implements the guarded CupidASM object transaction on Linux and
Windows. The command validates the active v2 manifest before execution,
applies the same relocatable code-anchor rule as the Python publisher, rejects
unlisted executable peers, and checks directory membership after either tool
returns. It also validates every frozen image as static ELF32 or strict PE32
before launch. The recheck still runs after a failure or timeout. The normal
ISR and context-switch recipes invoke that transaction directly.
...
both commands across consecutive generations. The refreshed seeds carry the
commands, and the normal bootloader and SMP-trampoline recipes now invoke the
promoted CupidBuild seed directly. Both rules preserve the complete six-image
production seed closure. The fresh promotion candidates found CupidC and
CupidBuild different from the preceding seed images, while all six
stage-three and stage-four tools matched. Broader Python removal remains open.

The paired v2 fixed points build all six tools through Linux and native Windows
stages. Linux freezes 59 source inputs and produces 22 C objects plus startup.
Windows produces 23 C objects and three assembly objects. Both compare all six
tools between their final generations and include them in the behavior gates.
The Toolchain publisher produces 22 artifacts and decides all 62 stage pairs,
including the six bootstrap images. ADR 0353 records the paired v2 contract,
and ADR 0367 records the preceding pair, and ADR 0370 records the preceding pair.

The self-link contract gives CupidBuild its own output beside the other five
seed tools and the runtime probe. CupidBuild compiles through the hosted profile
and links against the static i386 runtime. That runtime provides standard
`memchr`, with direct behavior coverage. This proves the complete source and
link closure; it does not give CupidBuild a normal recipe.

At the earlier kernel-compiler handoff (ADR 0393), the recorded state was:

Root `all` runs CupidASM, CupidObj, CupidLD, and CupidDis from a
manifest-bound six-tool seed selected for the host. Linux uses the static
...
then uses the converged stage-four Linux tools to build the Toolchain manifest
author. Linux runs a static ELF, while Windows runs a validated native PE. The
separate verifier uses the host-selected checked seed and runs as a native PE
on Windows.
...
443 transforms, and every one has a Cupid participant. The size verifier emits
no OS artifact; checked CupidC, CupidASM, and CupidLD build its private policy
...
artifact-size contract transfer, ADR 0302 records Toolchain manifest
verification, ADR 0304 records authoring, and ADR 0305 records local-target
seed carriage and adoption. ADR 0307 records raw stage-pair evidence.

Checked-seed CupidDis carries the raw local control-flow policy. It validates
...
trampoline contributes four, as locked by the active-source tests. Separate
Linux and Windows seed fixtures each prove one local target. Far and indirect
targets stay outside the policy. Both checked seeds carry the raw form, and
...
Checked CupidASM retains the resolved destination for raw calls and jumps
beside the range map. Its v2 rows distinguish local, external, and unprovable
transfers. Checked CupidDis can require those rows and compare
...
checked seeds carry v2, and both production transactions require source-edge
validation. This moves no transform or owner. ADR 0340 records the source
boundary, and ADR 0336 records carriage and adoption.

The hosted raw image and map use one recoverable publication. Native Windows
fixed-point CupidASM links the publication startup and runtime closure with the
DeleteFileA, FlushFileBuffers, GetFullPathNameA, and MoveFileExA imports. A
fresh reconstruction therefore cannot drop the recovery wrapper while keeping
the source that calls it. The wrapper writes linked v2 pending records before
either public replacement, advances them to v3 after both replacements, and
uses one matching v3 record as the commit witness. A v2 record remains pending
beside a legacy v1 peer, and exact recovery reaches the same decision in either
marker order.
...
The relocatable path does not treat linked `ET_EXEC` as an `ET_REL` input;
linked images enter the separate checked policy below. Both checked seeds
carry the relocatable form, and production CupidASM object transactions select
it with strict decode and relocation ownership before publication. ADR 0309
records the source boundary, and ADR 0312 records carriage and adoption.

Checked CupidDis also applies the policy to static i386 ELF32 linked
images. It scans nonoverlapping file-backed executable load regions twice and
allows a direct target to cross regions when it lands on an instruction start.
The report separates targets outside loaded memory, in loaded memory without
file-backed executable code, and inside an instruction. Far and indirect
transfers remain outside the count. A `PT_DYNAMIC` or `PT_INTERP` header
rejects the image as outside the static certification domain. Typed and CLI
negative cases cover both forms. The normal kernel transaction applies the
...
adoption. The generated audit reports failure, help, and success counts of
27/6/34 for Linux and 16/6/21 for Windows. Generation and checked comparison
both pass for that audit. At the ADR 0356 checkpoint, the refreshed v2 seeds
carried the two raw CupidBuild commands described in ADR 0355, while the
normal bootloader and SMP recipes still used Hostbuild. ADR 0357 later
transferred those recipes to CupidBuild.
...
changing which tool owns a transform. The promoted seeds carry the policy, and
the normal kernel publisher combines it with linked local-target validation.
The ownership totals remain unchanged. Minimal DWARF source information
remains a separate inspector gap. ADR 0320 records the source boundary, and
ADR 0323 records carriage and adoption.

Checked-seed hosted CupidDis accepts the deterministic static i386 PE32
profile emitted by CupidLD. It reports the bounded PE and COFF fields,
canonical sections, and named imports, then sends each executable section
through the shared x86 decoder. Strict mode requires the entry point and
constant direct relative targets to land on decoded instruction starts. All
six checked Windows seed images and an import-free CupidLD fixture pass next
to the independent Python validator. Malformed headers, overlapping or
out-of-file sections, invalid imports, unknown opcodes, bad anchors and local
targets, rollback, failed-output preservation, and same-process recovery have
focused coverage. Dynamic PE, base relocations, ordinal imports, PE symbols,
loaded spans above CupidLD's 2 GiB RVA limit, and general layouts remain
unsupported. The six seed reports match an independent Python reconstruction
of every section and import record. At the ADR 0338 checkpoint, the two PE32
headers raised the Toolchain inventories to 75 publication inputs and 55
bootstrap files without changing the 19 compiled C objects. The active v2
tools carry that support over the current 59-input closure. ADR 0338 records
the source boundary, and ADR 0353 records its checked-seed carriage.

At the earlier kernel-compiler handoff (ADR 0393), the recorded state was:

The same code-anchor policy now covers static i386 relocatable objects.
CupidASM's explicit `global name:function` form types 31 ISR entries, two
context-switch entries, and 21 fixed-point startup entries without changing
their section bytes, bindings, values, or relocations. Checked CupidDis
...
The SMP trampoline adds strict CupidDis participation to its existing CupidASM
ownership. One CupidBuild transaction freezes the selected seed and
source, assembles a private 4 KiB candidate and source-derived map, checks that
map against the fixed AP startup policy, and publishes atomically only after
strict decode and local-target inspection succeed. The map stays private. ADR
0271 records the first inspection transfer, and ADR 0308 records the map
handoff.

The ISR and context-switch objects receive the same protection at their own
publication boundary. CupidBuild freezes each source and the complete
six-tool seed, validates the private i386 relocatable candidate, requires a
...
recoverable operation so a command failure preserves the previous pair.
Matching linked v2 records are present before either public target moves.
Backups preserve old files, and validated tombstones record targets that were
absent. After both replacements, either linked v3 record commits the pair.
Recovery restores pending old/old, absent/absent, and mixed state, or finishes
cleanup from either surviving witness. It restores a backup directly over the
target without unlinking the readable file first. Fixed suffix checks bind
every private path to its normalized absolute target. This adds an
in-OS authoring route but does not transfer a normal build transform or remove
the checked host transaction. ADR 0337 records the artifact boundary, and ADR
0379 records linked kernel recovery.
...
Those reports began from uncommitted source and remain preliminary. Later
promotions remain historical in ADRs 0280 through 0323. The promoted Linux
five-tool proof passed with the preceding CupidASM and CupidDis unequal and the 21/5/22
failure/help/success inventory. The promoted Windows five-tool proof passed
with the same two preceding tools unequal and the 9/5/8 inventory. Both bind
revision `a17c9465911da41d59b7ada71733d36c39faa5ea` and source snapshot
...
reported one failure and four errors. Small fixtures omitted Windows
`publication_start.asm` and `publication_runtime.cc`, the fixed-point report
expected the preceding four-byte CupidASM result, and one PE temporary-file
read failed once. Both focused PE validator cases passed on immediate replay.
After the fixture and oracle corrections, six focused freeze and PE tests
passed in 0.736 seconds. The isolated complete fixed point passed in 1,187.863
seconds, with a 1,188.356-second wrapper. That suite passed all 86 tests in
2,444.917 seconds. After the relocatable-object cases were added, the historical
ADR 0312 checked-seed module passed all 89 tests in 3,145.502 seconds. ADR 0312
records that promotion and the full red and green chronology.
...
The Windows profile now covers all six driver mains, with compile-and-link
parity and an audit guard.

The five preceding stage-four PE32 images formed the v1 checked Windows
execution seed. Its 2,118-byte manifest has SHA-256
`751e1d7787a4be08e4e86814bbb7473979fe2eb8a3292baed0241967f772eaef`.
The active 2,852-byte v2 Windows manifest has SHA-256
`bf6147cf2e8249372869a24e5b8477ffb785d9a48eef80209366cfbaff19c7db`
and binds the 6,602-byte Linux manifest whose SHA-256 is
`770f979407f930deba0c9ba887bcd14f2350a785b1c0df6b31ddc2659c46eaae`.
Windows output-bearing recipes select the paired v2 execution seed directly.
The paired v2 Linux seed serves fixed-point and static Toolchain contract
work. The Windows user ABI, artifact-size, and Toolchain manifest verifier
gates build and run temporary PE contracts from the execution seed. The
manifest author keeps its Linux stage-four producers but runs as a native PE
on Windows.
Artifact-size policy and
both manifest modes bind the Linux v2 manifest. Native Windows reconstruction
pairs the execution seed with that exact plan manifest. Each PE reserves and
commits a one MiB stack. Its heap
...
pins the raw policy, the Linux policy manifest, sixteen file observations,
and the exact Windows seed directory containing its manifest and six PE
tools. Checked CupidC compiles the strict C11 contract, CupidASM supplies
startup, and CupidLD links a static ELF on Linux or a native PE on Windows.
A `CUPSIZE2` request carries the Linux manifest digest, the raw Windows
manifest, and six regular-file size and digest observations. The C contract
validates the Windows target, provenance, exact Linux plan-manifest link, tool
inventory, and observed bytes beside the policy and Linux manifest. On
Windows, Python verifies and
materializes the execution seed from the captured PE bytes, then rereads the
cohort and pinned repository view before success. It also requires an
independent oracle to match the contract report.
At the fourteen-path checkpoint, the focused semantic-contract,
checked-runner, and independent-policy modules contained 22, 16, and 13 tests,
for 51 total. They passed with four platform-specific skips. The active
artifact contract now covers all sixteen exact policy paths. The checkpoint
kernel outputs were a
9,596,956-byte `kernel/kernel.elf.pass1` with SHA-256
...
That normal build completed its 431-input code-anchor inspection, and the image
passed the private four-vCPU E1000 frontier smoke. Failure still prevents image
publication and preserves the existing image. ADR 0267
...
preserve earlier promotions, ADR 0323 records the preceding code-anchor pair,
ADR 0336 records the preceding seed pair, and ADR 0353 expands the active
policy and checked pair to sixteen paths and six tools per platform.
...
artifact ownership counts. The preceding Linux v1 seed carried the 5/22/21
matrix; the paired v2 Linux fixed point records 24/6/31.
...
carriage, ADR 0274 records the stack commitment, and ADR 0312 records the
preceding relocatable local-target seed. ADR 0318 records the preceding
linked-image seed, ADR 0323 records the preceding code-anchor seed, and ADR
0336 records the preceding seed. ADR 0353 records the paired v2 contract, and
ADR 0367 records the preceding pair, and ADR 0370 records the preceding pair.
...
participates in 262 transforms after CupidBuild took over these generators and
the ordinary wrappers. ADRs 0201, 0203, and 0204 record the operation, seed
promotion, and production transfer. ADR 0362 records the direct runner
handoff.
...
The normal Make image recipe passes the checked six-tool seed manifest to
...
the accepted sequential baseline bytes. At that checkpoint, Hostbuild froze the source and ran
checked CupidObj `wrap-jpeg`, which accepts sequential SOF0 or SOF1 input and
rejects progressive, unsupported, or malformed frames. Python checked only the
accepted private snapshot and required unchanged bytes. The normal path no
...
That Python coordinator rechecked the manifest and source before atomic
publication. Promoted CupidBuild `embed-jpeg` now owns the Make edge and its
independent native check. ADR 0231
...
new vector. Prefix returns the stored value. Const qualification is retained
through typedef aliases. Const direct vectors and fixed-array leaves remain
readable. Plain and arithmetic compound assignment, plus prefix and postfix
`++` and `--`, are rejected before a store.
...
function pointers remain rejected. A syntactic block-local
...
unary, cast, arithmetic, character, and `sizeof` zeros. Runtime
zeros remain unproved. Null conditional arms are neutral for explicit erasure;
every non-null object arm must be cast. Failed functions, methods, and complete
sources restore emission, patches, control state, touched prototypes and
definitions, kernel bindings, and the reused `void(void)` `__start` thunk.
This changes private JIT and AOT behavior without moving a build owner. ADR
0216 records the first fixed-array boundary, ADR 0257 records
multidimensional row descent, ADR 0294 records whole-vector updates, ADR 0299
...
raw kernel. The artifact contract group ran 46 tests in 4.160 seconds with four
expected Windows skips. After those three policy rows were updated, the repeated
fully poisoned build passed in 874.531 seconds. It checked all fourteen
...
The integrated strong private e1000 frontier passed in 883.513 seconds. Its
161,418-byte log has SHA-256
`bc30f5083b96a36362bec5975c0a88437c4f23515de329328bb03d8f6c3e9326`.
The 640x480 framebuffer changed 89,630 pixels. AC97 and the PC speaker produced
...
and permits null clearing. A compatible defined function designator writes its
...
block-static raw callback scalars. Fixed-size automatic raw callback arrays
use cleared four-byte frame slots and retain their signature through brace
initialization, indexed stores, copies, and calls. Unsized automatic arrays,
parameter arrays, record or class field arrays, and multidimensional raw
callback arrays remain unsupported. ADR 0351 records the automatic form.
...
The next paragraph preserves the settled CUPMAN2 checkpoint. Its 68-input
wording is historical; the source-current closure follows it.

Hosted CupidC emits deterministic i386 ELF32 objects for all fifteen regular files in the CupidC, CupidASM, CupidDis, and user-ABI contract cohort, plus the separate hosted runtime probe. Those sources, the 22-source static Linux six-tool union, nine strict Windows roots, the direct Windows runtime contract, and `kernel/lang/as_elf.cc` use 38 ordinary strict Linux roots, nine strict Windows roots, one freestanding Windows root, a two-source strict kernel bridge, and three GNU runtime roots. The third GNU root selects the Windows side of the shared runtime. The retired 64-bit profiles have no active roots. The normal build compiles each regular Linux contract twice from the checked seed, compares all seventeen new objects, links all fifteen programs and the Linux runtime probe through CupidLD, and verifies all sixteen ELF32 executables. Publication accepts only a dedicated `cupidc-contracts` directory inside the source tree. The target is checked before work and again before promotion, and an existing destination must already verify as a complete cohort. Arbitrary directories, source trees, files, and symbolic links remain untouched. The initial, private, and newly discovered contract inventories must match exactly, which catches additions, removals, and restored edits that changed a copied input. Every run derives the cohort from its executable, requires a named manifest artifact, and verifies every artifact hash, the current 76-input contract set, the checked seed manifest, and its 59-file source inventory before the behavior matrix starts. Those 76 inputs include the small Windows probe, the native Windows tool runtime and startup, CupidLD publication runtime and bridge, direct runtime contract, `direct.h`, `windows.h`, hosted `stddef.h`, both PE32 reader headers, the CupidBuild declarations and Windows startup, the user syscall ABI contract and its six declarations, the Toolchain Makefile, the publisher, and the independent Python ABI oracle. The catalogue's nested include adds `toolchain/x86.cc`. One captured seed-manifest byte sequence supplies its digest, decoded JSON, schema validation, and checked build plan. The same gate covers complete CupidLD and CupidObj command closures. Native GCC or Clang builds remain available only as explicit development oracles. This supersedes the older nine-file, eleven-file, fourteen-file, host-built, and 64-bit profile descriptions in the historical notes and long-form rows below.
...
artifact-size and Toolchain manifest contract sources, raising that closure
from 68 to 70. CupidBuild and the PE32 reader bring the source-current
inventory to 76 publication inputs and 59 bootstrap source files. The earlier
75-input, 58-file publication predates hosted `stddef.h` and remains historical.
The 16a cohort published all 22 artifacts with the 76/59 inventories and passed
stable-copy `CUPMAN2` verification. Fresh publication after `b25a3374` and
the refreshed object expectations agrees on all 62 stage pairs and passes
native manifest verification on Linux and Windows. The full executable suite and user build pass. The source
graph contains 748 active language inputs, including 302
headers. The C-family inventory covers 712 files and 2,503 include operands:
2,220 quoted and 283 angle forms. These source-current counts supersede the
65-input, 68-input, and 736-input totals in the broad historical rows below.

The source-current Windows fixed point uses the checked PE execution seed for
every stage-two producer. Its native compiler uses 64 KiB arena blocks, which
matches Windows allocation granularity and accommodates the enlarged compiler
source. The checked Linux seed supplies the reviewed plan and paired
provenance, but none of its executables run during this path. Native stage two
builds stage three, native stage three builds stage four, and only the final
pair is compared for publication. ADR 0341 records the former bridge; ADR 0359
records its removal.
...
Repository startup and runtime code now take six complete CupidC-emitted
command closures across CupidASM and CupidLD. The runtime supplies the checked
heap, file, memory, string, `errno`, working-directory, and diagnostic
interfaces. Linux fixed-point work runs all six tools on i386 Linux or through
WSL, and Linux contract work runs the subset it needs. Native Windows
fixed-point and output-bearing production work run checked PE copies directly.
The Linux seed binds the static images to the target ABI, source revision,
producer lineage, and complete build plan.
...
The harness freezes the verified Linux seed and copies 59 current inputs into
a private compiler root. These include `link.ld`, the small Windows probe,
native tool runtime and startup, CupidLD publication runtime and bridge, the
direct contract, and hosted declarations. The seed producer trio builds the
22-source stage-two Linux union and all six Linux tools below that root. The
stage-two trio builds stage three, then the stage-three trio builds stage four.
The driver rehashes the private and live source closures and revalidates the
live seed at every generation boundary, after behavior, and before
publication.

Every stage-three C object, Linux startup object, and Linux tool image matches
its stage-four counterpart. Both compared stages agree on positive and failure
behavior for all six commands. They also produce matching native objects and
PE images for all six tools. Windows runs help plus useful success and failure
cases for each tool. CupidLD also checks exact output, collision handling,
failure diagnostics, and candidate cleanup. The direct runtime contract checks
allocation, named-file write and append, current-directory errors, and quote
and backslash parsing. Stages two through four, behavior evidence, and the
report appear together only after the full gate passes.

ADR 0086 records the sibling commands, ADR 0088 records the compiler driver,
ADR 0089 records the compiler fixed point, ADR 0090 records the five-tool fixed
point, ADR 0092 records the checked seed, ADR 0142 records the frozen source
closure, ADR 0268 records the shared Windows runtime, ADR 0269 records CupidLD
publication, ADR 0272 records production PE execution, and ADR 0279 records the
convergence rule. ADRs 0280 and 0292 preserve preceding Linux seeds, and ADR
0312 records the preceding relocatable handoff. ADR 0318 records the preceding
linked-image seed, ADR 0323 records the preceding code-anchor seed, and ADR
0336 records the preceding seed. ADR 0353 records the paired v2 contract, and
ADR 0367 records the preceding pair, and ADR 0370 records the preceding pair.
...
22-source fixed-point plan because it tests the completed runtime rather than
...
generated source. The 22-source i386 Linux fixed point uses the same names,
...
Toolchain manifest author removes the last Python-only transform. No recursive
Make transform remains. The root
...
Source-head decimal `float` and `double` tokens use a 1536-bit integer
workspace and round once to binary32 or binary64. C99 hexadecimal literals use
a bounded target-only parser and round once to binary32, binary64, or x87
extended width. The public frontend, Linear IR, and ELF32 contracts cover
halfway parity, normal and subnormal boundaries, maximum finite values,
infinity, signed underflow zero, suffix selection, and extreme exponents. The
accepted decimal token boundary is 95 characters. Long-double decimal ratios
beyond the bounded parser, other floating-to-wide conversions, atomic
floating compound assignment, and atomic or long-double increment and
decrement remain open. ADR 0349 records hexadecimal parsing. ADR 0250 records
runtime unsigned
four-byte output,
...
ADR 0312 first carried that capability, and ADR 0318 retained it in a later
checked seed. ADR 0353 records the paired v2 contract, and the active seeds
retain it under ADR 0367.
...
`float`, `double`, or `long double` width, convert back to the left type, and
evaluate the destination once. ADR 0296 records that source-head boundary.
...
clobber. The checked-seed C11 standalone-header sweep passes 161 of 164 inputs.
`scheduler.h`, `simd_intrin.h`, and the macro-driven exact-decimal test fixture
remain exact C11-profile failures.
...
source stores its 130,506 logical bytes as little-endian `unsigned int` words
and adds two zero bytes to complete the last word. The checked wrapper owns
...
C99 hexadecimal long-double literals round exactly to the x87 extended
representation. Decimal long-double subnormals and decimal ratios beyond the
...
probe began using GNU variadic builtins. The current gate has 38 ordinary
strict Linux roots, nine strict Windows roots, one freestanding Windows root,
...
local-target carriage, ADR 0312 records a preceding stage-four promotion, ADR
0318 records the preceding linked-image seed, ADR 0323 records the preceding
code-anchor seed, ADR 0336 records the preceding seed, and ADR 0353 records the
active paired v2 seeds.
ADR 0253 records
...
The historical ADR 0360 Toolchain publication contained 22 artifacts, 75
publication inputs, and 58 bootstrap inputs. Its 29,271-byte schema-v3 manifest has SHA-256
`5fab9706abe6d938e9aa4a355ebbae293fee5404475d3d20d2591d6a9e464011`,
and its final verifier accepted all 22 artifacts. This publication established
the six-tool cohort that ADR 0353 later promoted into the paired checked seeds.

| Source or artifact cohort | Owner/path when recorded | Fixed-point owner/path | Status and next proof |
| --- | --- | --- | --- |
| `boot/boot.asm` | CupidASM flat binary | CupidBuild coordinates CupidASM plus CupidDis guarded flat binary | Production-owned: CupidASM emits the exact 2,560 bytes, SHA-256 `46cc9778da2b5cc5e8f04d7cc4b07243c3e07d466626ad84fb813dc6fef3a0d3`, byte-identical to the optional NASM oracle. It reads the kernel through LBA 20479 and installs the stack top at `0x01100000`. The source-derived v2 map records nine local relative, two local far, and one external far control edge. The normal Make rule invokes the promoted CupidBuild seed with the source, Makefile, production manifest, and all six seed images. CupidBuild freezes that trust unit, asks CupidASM for private image and map candidates, requires strict decode, local targets, and source-resolved edges from CupidDis, detects drift, and publishes atomically. A failure preserves the prior image and leaves no public map. ADR 0283 records the cutover, ADR 0340 records the v2 edge form, ADR 0336 records its production adoption, and ADR 0357 records direct publication ownership. |
| `kernel/cpu/isr.asm` | CupidASM ELF32 `ET_REL` | CupidBuild coordinates CupidASM plus CupidDis guarded ELF32 `ET_REL` | Production-owned: common entry preserves the live per-CPU `GS`, defers generic rescheduling through handler completion/EOI, and discards the source CPU's saved selector after a migration-safe suspension point. CupidASM emits a 1,892-byte object whose 417-byte `.text` has SHA-256 `bcf582569c26029d5143ec42f6de24388596c412bca2b4a672608800fe2606e3`, with 41 symbols and eleven `R_386_PC32` relocations matching the optional NASM oracle exactly. Thirty-one public entry labels carry `STT_FUNC` without changing those bytes or relocations. The Make recipe invokes the promoted CupidBuild seed directly. CupidBuild validates a private candidate and requires strict decode, relocation ownership, local targets, and code anchors from CupidDis before atomic publication. ADR 0286 records the guard, ADR 0336 records typed-anchor carriage, and ADR 0354 records direct publication ownership. |
| `kernel/core/context_switch.asm` | CupidASM ELF32 `ET_REL` | CupidBuild coordinates CupidASM plus CupidDis guarded ELF32 `ET_REL` | Production-owned: after moving BKL release and interrupt re-enable behind the new-stack handoff, CupidASM emits a 696-byte object whose 73-byte `.text` has SHA-256 `25b78f4c2cbf3dfadc6dc87a9731a097bfd9df0675534d8449c24d890114fbfa`, with three globals, one undefined handoff symbol, and one `R_386_PC32` relocation matching NASM semantics. The two defined entry labels carry `STT_FUNC` without changing those bytes or relocations. The Make recipe invokes the promoted CupidBuild seed directly. CupidBuild validates a private candidate and requires strict decode, relocation ownership, local targets, and code anchors from CupidDis before atomic publication. Scheduler handoff, nested deferral, and four-CPU runtime proofs pass. ADR 0286 records the guard, ADR 0336 records typed-anchor carriage, and ADR 0354 records direct publication ownership. |
| `kernel/smp/smp_trampoline.S` | CupidASM flat binary with strict CupidDis inspection | CupidBuild coordinates CupidASM plus CupidDis guarded flat binary | Production-owned: CupidASM emits an exact 4,096-byte private candidate with SHA-256 `b738ebb68f28b9b07e330761f4e9a7898f0424ab0a3835cd6079ae7d4a189e90`. It also writes the private canonical v2 map that CupidDis consumes. The normal Make rule invokes the promoted CupidBuild seed with the source, Makefile, production manifest, and all six seed images. CupidBuild requires the map to match code16 `[0x000, 0x01f)`, data `[0x01f, 0x210)`, code32 `[0x210, 0x254)`, and data `[0x254, 0x1000)`, plus four local relative, one local far, and one indirect unprovable control edge. The guarded transaction pins both candidates through strict decode, local-target, and source-edge inspection, then publishes only the binary. Missing, empty, malformed, different, changed, or source-edge-free maps preserve the prior output. The optional NASM oracle remains byte-identical, and four CPUs reach online state in the SMP smoke. ADR 0308 records the handoff, ADR 0340 records the v2 edge form, ADR 0336 records its production adoption, and ADR 0357 records direct publication ownership. |
...
| Toolchain publication manifest verification and authoring | Python captures each pair from a descriptor-pinned regular file. Verification runs a checked host seed; authoring runs the converged stage-four Linux tools. | Checked CupidC compiles the strict C11 verifier and author, checked CupidASM supplies startup, and checked CupidLD links them. Python retains filesystem capture, launch, independent oracle, staging, rollback, and drift checks. | `CUPMAN4` now receives 62 fixed-point pairs: 17 contract objects, 16 contract executables, 22 bootstrap C objects, one startup object, and six tool images. The author decides every equality before Python repeats the same comparisons. The historical ADR 0360 publication contained 22 artifacts, 75 publication inputs, and 58 bootstrap inputs. Its 29,271-byte manifest has SHA-256 `5fab9706abe6d938e9aa4a355ebbae293fee5404475d3d20d2591d6a9e464011`, and its final verifier accepted all 22 artifacts. The current schema binds 76 publication inputs and 59 bootstrap inputs; the fresh rebuild after `b25a3374` and the refreshed object expectations agrees on all 62 stage pairs and passes native manifest verification on Linux and Windows. The full executable suite and user build pass. `CUPMAN2` remains host-selected and runs as a native PE on Windows. ADRs 0302, 0304, 0307, 0311, and 0322 record the existing author and verifier boundary. ADR 0345 records the six-tool expansion. |
...
| In-OS CupidASM artifact API | Shared CupidASM through the checked kernel adapter | CupidASM owns the same artifact forms in the host and kernel | The public adapter accepts raw `BIN` output with its canonical map, unlinked ELF32 relocatables, and linked executables through `-f bin`, `-f elf32`, and `-f exec`. Paired output uses linked v2 pending records, absence tombstones, and v3 commit witnesses. Recovery handles old/old, absent/absent, and mixed prior state, accepts either surviving witness, and keeps legacy v1 records readable. A failed backup restore preserves the readable target. Exact suffix checks bind each private path to its normalized absolute target. Durable two-file publication and a concurrent-writer lock remain open. ADR 0379 records the recovery transfer. |
...
| Kernel symbol extraction | Promoted CupidBuild `generate-ksyms` drives checked-seed CupidDis `-n` and CupidObj `ksyms-source`, then checked-seed CupidC compiles the generated source | CupidBuild transaction, CupidDis inspection, CupidObj generation, and CupidC compilation | Production inspection, generation, validation, and publication ownership have transferred. CupidBuild freezes the pass-one ELF and six-tool seed, CupidDis emits canonical text, CupidObj serializes the packed blob, and CupidBuild repeats that work with an independent native renderer before atomic publication. Hostbuild remains an optional parity oracle; Python is not on the normal edge. Every shared symbol keeps the same address between passes. ADR 0224 records the first generation handoff, and ADR 0371 records direct CupidBuild publication. |
| Source, documentation, font, image, and other binary wrapping | CupidObj inside CupidBuild's typed JPEG transaction | CupidObj/shared object library | Production-owned. The 175 source, manual, demo, and vocabulary transforms canonicalize CRLF to LF without changing lone carriage returns. Eight direct binary wrappers stay byte-exact. Names, section policy, symbols, empty inputs, deterministic repeats, failure rollback, and final-form SMP wrapping are contracted. The JPEG path runs inside the promoted CupidBuild transaction. It freezes the repository bytes and six-tool seed, runs checked CupidObj `wrap-jpeg` under the original source identity, verifies the exact payload and identity symbols, applies an independent native parser, rechecks the publication boundary, and replaces the object atomically. Progressive, unsupported, or malformed marker streams fail without replacing the old object. Host image converters are not part of the root path. The 800,860-byte JPEG object has SHA-256 `74ab86d88302c90385bb0b858632b0d6c4ac983d6be28c976dd1a3a348204b3e` on both Windows and Linux. ADR 0235 records the original CupidObj transfer, and ADR 0368 records the direct CupidBuild handoff. |
| JPEG publication coordination | Promoted CupidBuild runs the guarded CupidObj `wrap-jpeg` publication | Promoted CupidBuild owns the guarded JPEG publication | The production handoff is complete. `cupidbuild embed-jpeg` freezes the asset and six-tool v2 seed, preserves the original symbol identity, requires the exact `.data` payload and three identity symbols, runs an independent native JPEG parser, and owns locking, drift checks, rollback, and atomic publication. Both active cohorts carry the command, and normal Make invokes it directly for `.jpg` and `.jpeg` outputs. ADR 0364 records the source boundary, ADR 0367 records preceding seed carriage, and ADR 0370 records active carriage, and ADR 0368 records the production handoff. |
| Linked kernel ELF to raw kernel binary | Promoted CupidBuild coordinates CupidDis and CupidObj | CupidBuild validates and flattens one frozen cohort, then publishes the raw kernel atomically | Production-owned: CupidBuild freezes the selected seed manifest and six artifacts, the 431-entry input manifest and cohort, and the existing `kernel.bin` boundary. Checked CupidDis validates the private cohort. Checked CupidObj writes physical-address-ordered file-backed `PT_LOAD` bytes with zero-filled gaps and BSS exclusion from the frozen final ELF. CupidBuild checks an independent native ELF render and rechecks live trust inputs and the output before parent-relative atomic publication. ADR 0375 records the handoff; Hostbuild remains an optional oracle. Every failure preserves the prior raw kernel. At the first reviewed transaction checkpoint, the operation passed with exit 0 in 187.054 seconds and published an 8,946,332-byte raw kernel with SHA-256 `4f5f2591d01bcc4007773844e9bfb8112a16dd17fbd178014cc2056fefaab67d`. The earlier poisoned-host `make -j2` passed in 1,057.969 seconds when validation and flattening were separate. Definitive four-vCPU E1000 and RTL8139 boot frontiers passed with exits 0 in 794.034 and 758.667 seconds. Both passed SMP, frontier, framebuffer, AC97, and PC speaker checks without changing the source image. Later rows record the current kernel identities. |
...
The guarded 2026-08-14 linked-kernel checkpoint includes in-kernel CupidLD
and the guarded normal boot edge. A poisoned-host normal build passed in 674.693
...
evidence images retained the checkpoint image hash above.
...
manifest author operations accounted for four Cupid-built contracts. That
checkpoint's paired seed reports recorded failure, help, and success counts
of 27/6/34 for Linux and 16/6/21 for Windows. Its audit and deterministic check
mode both passed. The current ownership and seed records appear above.
...
1536-bit converter recorded by ADR 0293. C99 hexadecimal `float`, `double`, and
`long double` constants use bounded target-only integer arithmetic under ADR
0349. Decimal long-double subnormals, long-double ratios beyond the
bounded parser, other floating-to-wide conversions, atomic floating compound
assignment, atomic and long-double increment or decrement, SIMD, over-aligned
...
compound assignment in either lvalue direction. ADR 0312 first carried the
capabilities from ADRs 0293 and 0296. ADR 0318 retained them in a later checked
seed. ADR 0353 records the paired v2 contract, and the active seeds retain both
under ADR 0367.
...
production CupidC, and checked user CupidLD plus CupidDis. Each production
wrapper keeps its source and output transaction but delegates private seed
execution and the live six-image recheck to `run_seed_tool`. ADR 0246 records this
...
The checked-seed readers retain their dual-schema boundary. V1 remains
available for parent compatibility, while the active v2 manifests name
CupidBuild as a sixth, non-producing artifact and retain exact source order,
link order, producer roles, target layout, artifact bytes, and execution
profiles. The
Linux contract binds the six-tool plan. The Windows contract binds both that
plan and its native translation.

V2 lineage points to the preceding v1 manifests and source revision. Promoted
revision and source-snapshot fields are lower-case hexadecimal identities. The
Windows record carries the exact Linux v2 manifest digest, which the host pairs
...
This section records the pre-handoff checkpoint. The normal Make edge now
uses promoted CupidBuild `generate-ksyms`, as recorded in ADR 0371.

| Path | Previous coordinator | Source-head coordinator | Production state |
| --- | --- | --- | --- |
| `kernel/kernel.elf.pass1` to `kernel/cpu/ksyms_data.cc` | Host Python freezes inputs, runs CupidDis and CupidObj, checks an independent Python rendering, and publishes | Typed CupidBuild captures CupidDis output, runs CupidObj, checks an independent native rendering, and owns guarded rollback | Carried by both promoted seeds; awaiting the Make handoff |
