...
source-current Toolchain inventories, bringing them to 75 publication inputs
and 55 bootstrap files without changing the 19 compiled C objects. The
...
promotions remain historical in ADRs 0280 through 0323. The current Linux
candidate proof passed with the preceding CupidASM and CupidDis unequal and the 21/5/22
failure/help/success inventory. The current Windows candidate proof passed
...
inventory to 75 publication inputs and 55 bootstrap files. The latest complete
publication remains the 70-input,
50-bootstrap-file checkpoint until a new fixed-point run publishes the larger
closure. The source graph contains 747 active language inputs, including 301
...
| Toolchain self-hosting | Checked seeds rebuild host tools; consecutive post-transition generations are byte-identical on Windows and Linux | The promoted 50-input seeds rebuild 19 Linux C objects plus startup and 20 Windows C objects plus two assembly objects, along with all five tools. Their behavior matrices remain 5/22/21 and 5/8/9. Source head has grown to 55 bootstrap inputs. Its Windows stage two uses checked Linux CupidC for C objects and the checked Windows tools for assembly, inspection, and linking; native stages three and four remain the byte-equality boundary. Python coordinates both fixed points, and WSL carries the Linux paths plus this one-generation Windows C bridge. ADR 0336 records the seed pair, and ADR 0341 records the source-growth bridge. |
...
`46c5335c80d822dd5085ee22077486ea647e5396482d42454847c87e4222aa67`.
Their candidate behavior inventories are 21/5/22 on Linux and 9/5/8 on
Windows. The normal kernel
