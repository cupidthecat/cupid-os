...
the custom-Linux syscall spelling, and `c967ddee` reached native Windows stage
two before exposing the missing common file-information wrapper. With that
wrapper repaired, clean `ae32be64` Linux passed every fixed-point and behavior
gate. Windows matched every stage-three and stage-four artifact, then stopped
because its copied behavior manifest still named the promoted plan while
carrying current tools. Temporary Windows behavior manifests now bind to exact
source-current plan
`98e09aab876a9fa37ec07c38a0a57a014549a14c0ab10c740b3f80ede9d65669`.
No checked seed changed and no pair was promoted, so a complete clean run still
precedes seed publication. A source-current Windows rerun with plan-matched
temporary manifests completes all 19/7/24 behavior groups.
...
The source closure remains at 59 inputs. The current kernel-flatten-capable
snapshot has SHA-256
`0b591a0bef928186641b3aa1fb98c1e145e6c4905c8b6cb87c34a1ace4bc87d2`.
The active paired seeds bind the 59-input closure. CupidBuild accepts a
...
latest reconstructions from revision `0232cb57aad5d6bdfd7bd77499762514b2f0ebfd`
...
records the kernel-symbol pair, and ADR 0374 records the active pair.
...
CupidBuild is now part of both checked six-tool seeds and owns six guarded
normal publications: two assembly objects, two raw images, the JPEG object,
and the generated kernel-symbol source. Its
...
and ADR 0367 records the preceding pair, and ADR 0370 records the active pair.
...
inventory to 76 publication inputs and 59 bootstrap source files. The latest
complete publication predates hosted `stddef.h` and records the preceding
75-input, 58-file closure with 22 artifacts. The source
...
The latest Toolchain publication contains 22 artifacts, 75 publication inputs,
and 58 bootstrap inputs. Its 29,271-byte schema-v3 manifest has SHA-256
`5fab9706abe6d938e9aa4a355ebbae293fee5404475d3d20d2591d6a9e464011`,
and the final verifier accepts all 22 artifacts. This publication established
...
manifest author operations account for four Cupid-built contracts. The active
paired seed reports record failure, help, and success counts of 27/6/34 for
Linux and 16/6/21 for Windows. The source-current audit and deterministic check
mode both pass.
