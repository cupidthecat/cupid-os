...
in the kernel-symbol, SMP-trampoline, bootloader, and `kernel.bin` root
transforms. The `kernel.bin` transform runs strict validation and flat
extraction against one frozen cohort, with all 431 code inputs represented.
...
passed a clean proof in 1,383.775 seconds, promoted the stage-four cohort, and
passed a 1,411.998-second reproof with all five initial seed comparisons true.
Native Windows then passed its clean proof in 1,152.7 seconds and promoted the
stage-four cohort. The previous seed comparison was false for CupidASM, CupidC,
and CupidDis and true for CupidLD and CupidObj. Its 1,130.9-second reproof
matched all five initial seed images. ADR 0278 records the driver, ADR 0279
records the added generation, ADR 0280 records the Linux promotion, and ADR
0281 records the Windows promotion.
...
policy, and ADR 0281 records the clean promotion.
...
`double`, and atomic or long-double updates remain open. ADR 0250 records
runtime unsigned
four-byte output,
...
This work does not move production ownership. The shared frontend, Linear IR, and deterministic i386 emitter move `float` and `double` values through objects, assignment, calls, variadic reads, and returns. Explicit casts and assignment conversion work in both directions. Unary plus and minus and binary addition, subtraction, multiplication, and division accept matching or mixed widths. Matching floating conditional arms keep their width, and mixed `float` and `double` arms use `double`. Source head also accepts runtime conditional arms that mix either floating width with a represented signed or unsigned integer no wider than four bytes. The selected integer arm receives the usual arithmetic conversion. The four arithmetic compound assignments compute at the common width, convert back to the left width, and evaluate their lvalue once. Every changed x87 result is stored immediately at its C width. A `float` rounds into a fresh four-byte slot, while a `double` receives a fresh private eight-byte snapshot. The unchanged `libm_tanh_impl` body pins nested `double` arithmetic, and the following `float` helper slice pins width conversion. The current proof emits a 45-function object with 7,144 text bytes, fingerprint `7186D278`, 46 symbols, and 97 relocations. Its decoder-driven model checks width-specific loads and stores, IEEE rounding cases, matching and mixed expressions, selected-arm conversion, call alignment, and frame state. Eight new execution cases cover signed `int`, signed `short`, unsigned `int`, and unsigned `char` in both branch directions. The model does not execute native x87 code. The current checked seed predates this source-head extension, and no active OS source needs the new expression shape, so no normal object, transform, dependency, or source suffix moves.
...
ADR 0265 records the preceding seed, and ADR 0280 records the current
...
| `toolchain/cupidasm.*`, `kernel/lang/as.cc`, and `kernel/lang/as_elf.*` | Checked-seed CupidC builds the shared frontend, the `as_elf.cc` kernel bridge, and the `as.cc` kernel adapter; GCC/Clang builds the hosted driver | CupidC builds the same shared frontend and adapters | Assembly semantics live only in `toolchain/cupidasm.*`; the legacy kernel lexer/parser was deleted. Source head rejects a duplicate raw `ORG` or a switch to another raw source section with stable diagnostics before output. The checked execution seeds need their next fixed-point promotion to carry that behavior. `as.cc` owns the 631 runtime definitions, fixed placement, and VFS/JIT policy. `as_elf` remains a checked buffer-only temporary `ET_EXEC` bridge. CupidC emits the complete static hosted command closure, CupidASM supplies startup, and CupidLD links it. Linux and WSL runs match native raw and ELF32 output plus invalid-source behavior. The normal hosted executable is built by staged CupidC and CupidLD; the host build remains an optional native oracle. |
...
hexadecimal or subnormal long-double literals, decimal ratios beyond the
bounded parser, operations that mix an eight-byte integer with `float` or
`double`, atomic and long-double updates, SIMD, over-aligned
...
arithmetic. ADR 0287 records source-head conditional conversion for signed and
unsigned integers no wider than four bytes. ADR 0288 records runtime integer
and long-double usual conversions.
...
| Toolchain self-hosting | Checked seeds rebuild host tools; consecutive post-transition generations are byte-identical on Windows and Linux | A checked static i386 Linux seed rebuilds one 19-source union, fresh startup, and all five Linux tools from a private captured source root. The promoted PE images remain the checked Windows execution seed. Source head now reconstructs stages two through four on Linux and native Windows. Stages two and three carry the transition from an older code generator; stage three builds stage four, and the driver compares that final pair. Source-stable Windows and Linux runs exposed the old comparison at `cupidobj_main` after 821.9 and 883.3 seconds. Both stopped without publication. Later uncapped proofs passed: Windows matched 20 C objects, two assembly objects, five tools, and 5/5/5 behavior cases in 20 minutes 43 seconds; Linux matched 19 C objects, startup, five tools, and 5/18/16 behavior cases in 24 minutes 22 seconds. Both bind the same 50-input snapshot, SHA-256 `d8481a39e0d1c7f42779a8c9f5fc5de10d7e5b9bc4df63ce6afe9ddd9c9716da`. Linux later passed a clean 1,383.775-second proof, promoted stage four, and passed a 1,411.998-second reproof from that seed. Native Windows later passed a clean 1,152.7-second proof and a 1,130.9-second promoted-seed reproof, both across 20 C objects, two assembly objects, five tools, and the 5/5/5 behavior matrix. Linux contract programs on Windows still use WSL. ADR 0278 records the native driver, ADR 0279 records the convergence rule, ADR 0280 records the clean Linux promotion, and ADR 0281 records the clean Windows promotion. |
| Normal-build cutover | Make invokes CupidC, CupidASM, CupidLD, CupidObj, and CupidDis without GCC/Clang/NASM/LLVM/binutils | In progress: root `all` has 443 transforms. Its 442 artifact transforms have a Cupid tool owner, with 243 CupidC, five CupidASM, 192 CupidObj, two CupidLD, and four CupidDis participations. The remaining Python-only size verifier emits no OS artifact and blocks `cupidos.img` publication on failure. Across all three supported graphs, CupidC participates in 246 transforms, CupidObj in 192, CupidASM in five, CupidLD in five, CupidDis in four, and Python in all 452. No transform invokes a host C compiler or recursive Make. Windows selects the native checked execution seed for output-bearing tools; Linux keeps the static seed. The SMP trampoline and bootloader are assembled and strictly inspected before publication. Kernel AOT links with in-kernel CupidLD. Linux contract paths on Windows still need WSL, and Python retains coordination, safety, parity, drift, locking, and publication work. |
