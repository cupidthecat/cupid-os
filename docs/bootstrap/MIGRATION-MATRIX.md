...
clobber. The checked-seed C11 standalone-header sweep passes 160 of 162 inputs,
...
as open. Compiler head folds all four operators into exact initializer data
without runtime IR. The checked seed remains unchanged.
...
| Capability audit | Every active source and generated input mapped to required C, ASM, ABI, object, linker, and inspector features | Complete for root `all`, `user:all`, and `toolchain:all`: 724 active inputs, 255 feature IDs, 447 transforms, 25 accounted unreachable source-like files, and a checked drift and coverage gate. The graph contains 29 assembly files, 293 headers, 402 Cupid C files, and no ordinary C translation unit. The checked graph uses canonical Windows conditionals and the C locale on every host. No supported transform invokes a host C compiler or recursive Make. |
