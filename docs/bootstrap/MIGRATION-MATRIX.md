...
from a supported root. The three-root audit has 449 transforms, no recursive
Make transform, 449 Python participants, 245 CupidC participants, and no host
C transform. `toolchain:all` now uses the checked seed and both rebuilt
compiler stages to compile and link all fourteen `.cc` Toolchain contracts.
CupidObj participates in 185 transforms, including three installation-source
generators. ADR 0190 records the root handoff, and ADR 0196 records the
contract handoff.
...
still participates in all 449 transforms because it launches the checked
...
increment parser share one typed helper for locals, parameters, and globals.
Postfix updates preserve the old payload in XMM2 until the new XMM0 value is
stored. An exact-byte oracle checks both widths and directions, while the
guest feature checks expression results, statement updates, globals, loop
increments, signed zero, and NaN. ADR 0194 records the boundary. Build
...
contributing to a tool image. The normal Toolchain cohort adds all fourteen
renamed contract programs. Stage-two and stage-three CupidC compile them at
the checked i386 ABI, CupidLD links them with the matching stage objects, and
all sixteen new objects and fifteen executables must match across the two
stages. The verifier rediscovers the live input membership and checks its
...
Checked-seed CupidC owns 155 checked-in sources in the strict non-Doom root
cohort. The generated kernel-symbol translation brings that strict total to
156 transforms, all using `.cc`. Another 83 checked-in Doom roots bring the
complete normal total to 239 transforms: 238 checked-in sources plus the
...
graphs, CupidC owns 245 transforms, no supported transform invokes a host C
compiler, and Python participates in 449. One Python transform
...
cross locals, assignment, unary plus and minus, addition, subtraction,
...
accept zero-valued integer constant expressions and occupy twelve BSS bytes
apiece. All six comparisons accept matching long-double values and mixed
`float` or `double` inputs. The emitter uses a balanced `FUCOMIP` sequence and
the existing unordered-result normalization. Runtime truth and `_Bool`
conversion cover `float`, `double`, and automatic `long double`. Hexadecimal
...
than `_Bool`, runtime conversion to unsigned four-byte integers, mixed wide
and floating runtime arithmetic or conditionals, and floating increment and
decrement remain open.
Matching or
...
The aggregate proof fixes two 24-byte arrays and two 28-byte records in 104
BSS bytes. Its 415-byte access function has fingerprint `BF01CC71`, eight
absolute relocations, and six symbols. The hosted i386 runtime checks their
initial zero state and then moves 1.5 through file and block members.
`sizeof(float) - 4` is the positive zero integer constant-expression case;
`1.0L` remains a useful negative.
...
statement. The shared path emits 155 strict non-Doom objects, all 83 Doom
objects, and the generated kernel-symbol object. That is 239 normal
transforms from 238 checked-in `.cc` sources plus the generated source,
...
ADRs 0196 and 0199 also supersede the `long double` gaps in the frontend, IR, and
emitter rows below. Automatic non-atomic values now support floating-width
...
object; explicit leaves accept zero-valued integer constant expressions and
occupy twelve BSS bytes apiece. Long-double literals, nonzero or floating
static long-double initializers, and integer conversions involving
`long double` remain open.

The preprocessing row below retains the profile split from before the runtime
probe began using GNU variadic builtins. The current gate has 31 ordinary
strict roots, two strict bridge roots, and two GNU runtime roots.
...
The checked seed and source head have 591 x86 forms, 244 canonical mnemonics,
64 registers, and fingerprint `DBE77533`. This supersedes older counts below.
ADR 0203 records seed carriage for `FLDZ` and the other three post-seed forms.
...
| Generated C tables (installation tables, CSS tables, and kernel symbol data) | Checked-seed CupidObj generates the three `.cc` installation tables, Python generates the kernel-symbol source and other host-owned tables, and checked-seed CupidC compiles the installation tables plus `kernel/cpu/ksyms_data.cc` | CupidObj generates installation tables; CupidC compiles them; Python may orchestrate | Installation-table generation and compilation are production-owned by Cupid tooling. Each Make recipe uses `$(CUPIDOBJ) install-source` with the matching ordinal inventory and complete checked-seed inputs. `tools/hostbuild.py` remains the byte-parity oracle but is not a prerequisite or recipe owner. The checked seed and that oracle reject complete wrapped-symbol collisions while preserving one exact docs and home BMP alias. The symbol generator still packs 109,889 logical bytes into little-endian `unsigned int` words and adds three zero bytes to finish the last word. Its checked wrapper freezes the generated source and header closure, validates the i386 relocatable object, and publishes atomically. CSS tables and other generated C remain separately tracked. ADRs 0116 and 0123 record the earlier compiler transfers; ADRs 0201, 0203, and 0204 record installation-source capability, first seed carriage, and production ownership. ADRs 0205 and 0206 record the checked request corrections. |
...
local load and store, unary plus and minus, addition, subtraction,
...
`long double`. Hexadecimal floating literals, `long double` literals, nonzero
or floating static long-double initializers, integer conversions involving
`long double` other than `_Bool`, runtime conversion to unsigned four-byte
integers, runtime mixed wide and floating arithmetic or conditionals,
floating increment and decrement, SIMD, floating atomic access, over-aligned
emission, production integration, and self-hosting remain open.
...
| Capability audit | Every active source and generated input mapped to required C, ASM, ABI, object, linker, and inspector features | Complete for root `all`, `user:all`, and `toolchain:all`: 717 active inputs, 254 feature IDs, 449 transforms, 25 accounted unreachable source-like files, and a checked drift and coverage gate. The graph contains 27 assembly files, 289 headers, 401 Cupid C files, and no ordinary C translation unit. The checked graph uses canonical Windows conditionals and the C locale on every host. No supported transform invokes a host C compiler or recursive Make. |
...
| C migration | Every reachable kernel, tool, application, Doom, and vendored C cohort compiles and passes behavior gates with CupidC | In progress: all 238 checked-in normal roots plus the generated kernel-symbol translation compile through the checked seed. Three generated installation tables and three example programs add CupidC ownership outside that cohort. The 14 hosted Toolchain contracts also compile, link, and run through the checked i386 seed. Linux runs the seed directly; Windows uses WSL for these i386 programs. Native hosted tools and contract runners remain optional development oracles. All Doom production objects compile, and the no-WAD, explicit missing-IWAD recovery, shell-survival, and dual-NIC frontier checks pass. Full IWAD gameplay, input, audio, and save behavior remain to be checked. |
...
| Normal-build cutover | Make invokes CupidC, CupidASM, CupidLD, CupidObj, and CupidDis without GCC/Clang/NASM/LLVM/binutils | In progress: the 438-transform root image graph assigns 242 transforms to CupidC, four to CupidASM, 185 to CupidObj, two to CupidLD, and one to CupidDis, with no host C or recursive Make transform. Across all three supported graphs, CupidC owns 245 transforms and no transform invokes a host C compiler; Python participates in all 449 transforms as orchestration, remaining artifact production, or manifest-backed verification. The three installation-source transforms now belong to checked CupidObj, with Python retained as the checked-seed launcher. ADR 0204 records that transfer. The poisoned-host build passes. A 607.7-second Linux kernel build and a 341.6-second Windows root build produced the same 430 frozen kernel artifacts. The 8,579,824-byte pass-one ELF has SHA-256 `2350b1a4f32c674d386ca50d4be4f6be8308082291f2852cfecfde4ba151b4df`; the 8,690,416-byte final ELF has SHA-256 `80b23fbd198f4aa9ec030ac69526b2d612c49e6d6328cfa01af9a0ef6217d8bb`; and the 8,490,228-byte raw kernel has SHA-256 `53770a93658e757d25f5aeab9d3e434d4a3be2a1dc3fbe4b19869e5bf9820a06`. The fresh 209,715,200-byte normal image has SHA-256 `e815d2ef67f114a26181f0e2cbde85f892cdadd487f8d9cbee9715e720800b3e`. A private `/bin/ls.cc` JIT boot from it passed in 49.8 seconds. Native hosted binaries still need GCC or Clang when explicitly requested as development oracles. Windows runs checked i386 programs through WSL until a native checked seed exists. The tracked ISO fixture is authored directly by repository Python and needs no external ISO command. |
