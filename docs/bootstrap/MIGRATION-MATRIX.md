...
from a supported root. The three-root audit has 447 transforms, no recursive
Make transform, 447 Python participants, 245 CupidC participants, and no host
...
CupidObj participates in 189 transforms, including three installation-source
generators, the kernel-symbol source generator, the normal disk-image
template, repository ISO fixture, and Doom profile manifest. Every one of the
438 root outputs has a Cupid tool owner, so no root output is Python-only.
...
output directories, and ADR 0246 records the shared invocation boundary.

Source-head CupidLD can also serialize one deterministic fixed-layout i386
PE32 console image and its canonical imports. Source-head CupidASM and CupidC
...
the exact `.idata` layout. The checked Linux seed and normal recipes do not
carry or use that command, so all ownership counts above remain unchanged. The promoted
seed keeps its 5/15/13 proof; source head uses a 5/17/15 matrix. ADR 0247
records the format boundary, and ADR 0248 records imports and loader execution.

The same source-head CLI publishes ELF and PE images through an adjacent
...
checked-seed carriage and production ownership. It requires a caller-controlled
...
still participates in all 447 transforms because it launches the checked
...
compiler, and Python participates in 447. The three Python-only supplemental
outputs verify the external-program syscall ABI, aggregate the Toolchain
cohort, and record the Toolchain contract manifest. No recursive Make
...
accept zero-valued integer constant expressions or bounded decimal `L`
literals with parentheses and unary signs. The emitter writes exact x87
payloads with zero padding and selects `.bss`, `.data`, or `.rodata`. All six comparisons accept matching long-double values and mixed
...
conversions between `long double` and signed or unsigned integers cover 8, 16,
32, and 64 bits. Integer output saves and restores the x87 control word around
a truncate-mode `FISTP`. Unsigned 64-bit corrections select 64-bit x87
precision while preserving the caller's rounding mode, then restore every
saved control bit before the final store. Hexadecimal
floating literals, binary32 and binary64 subnormal literals, hexadecimal or
subnormal long-double literals, decimal ratios beyond the bounded parser,
static long-double computation, floating-width conversion, and integer
conversion, nonzero integer static initializers, mixed
integer and floating runtime arithmetic or conditionals, and floating increment and
decrement remain open. ADR 0250 records runtime unsigned four-byte output,
ADR 0251 records static long-double data, and ADR 0253 records runtime
conversions between `long double` and integers.
...
the three target words in every payload. `sizeof(float) - 4` is the positive
zero integer constant-expression case; a nonzero integer initializer remains
a useful negative.
...
clobber. The checked-seed C11 standalone-header sweep passes 157 of 159 inputs,
...
source stores its 114,421 logical bytes as little-endian `unsigned int` words
...
object. Explicit leaves accept zero-valued integer constant expressions or
bounded decimal `L` literals with parentheses and unary signs. Exact payloads
and padding reach `.bss`, `.data`, or `.rodata`. Hexadecimal or subnormal long-double literals,
decimal ratios beyond the bounded parser, static long-double computation,
floating-width conversion, integer conversion, and nonzero integer
static initializers remain
open. Runtime conversion between `long double` and every signed or unsigned
i386 integer width uses the shared `FILD` and `FISTP` path.
The compiler-head GNU assembly boundary also accepts exact `fldcw %0` with
...
checked seed still owns only the older 32-bit `ldmxcsr %0` state input.
...
strict Linux roots, one freestanding Windows root, two strict bridge roots,
and two GNU runtime roots.
...
x87 integer forms. ADR 0253 records their first CupidC runtime use.
...
implicit zero, an integer constant expression equal to zero, or a bounded
...
width are supported. Hexadecimal floating literals, binary32 and binary64 subnormal
literals, hexadecimal or subnormal long-double literals, decimal ratios
beyond the bounded parser, static long-double computation, floating-width
conversion, and integer conversion, nonzero integer static initializers,
runtime mixed integer and
floating arithmetic or conditionals,
...
719-input audit and its reproducibility check pass with 449 transforms, 255
features, 25 accounted unreachable files, and source digest
`cfb0e1dcd276154a4db5c2747ed092581874a54cd4c9fb379f204e3c10f8253e`.
The source-current rebuild reused the image in 616.648 seconds and preserved
...
| Capability audit | Every active source and generated input mapped to required C, ASM, ABI, object, linker, and inspector features | Complete for root `all`, `user:all`, and `toolchain:all`: 721 active inputs, 255 feature IDs, 447 transforms, 25 accounted unreachable source-like files, and a checked drift and coverage gate. The graph contains 29 assembly files, 290 headers, 402 Cupid C files, and no ordinary C translation unit. The checked graph uses canonical Windows conditionals and the C locale on every host. No supported transform invokes a host C compiler or recursive Make. |
...
| Toolchain self-hosting | Checked seeds rebuild host tools; stage 2 and stage 3 outputs are byte-identical on Windows and Linux | A checked static i386 Linux seed rebuilds one 19-source union, fresh startup, and all five tools from a private captured source root. The stage-two producer trio repeats the work below the same root. The 43-file closure includes the Windows startup and runtime probe. The harness rehashes the private and live closures after each stage and after behavior checks, then publishes one complete evidence bundle. All 19 C object pairs, startup objects, and CupidC, CupidASM, CupidDis, CupidLD, and CupidObj images match. The promoted seed reproof remains five help paths, fifteen successful operations, and thirteen failures. Source head also produces matching CupidASM and CupidC objects and an imported PE image through both rebuilt stages. Windows validates and runs the image with exact output and exit checks, giving a 5/17/15 source-head matrix. The imported path is not promoted. Linux runs the seed directly, and Windows still uses WSL for the seed bootstrap and normal user build. Native versions of all five tool drivers, the complete Windows runtime, and checked native seed carriage remain open. |
| Normal-build cutover | Make invokes CupidC, CupidASM, CupidLD, CupidObj, and CupidDis without GCC/Clang/NASM/LLVM/binutils | In progress: the 438-transform root image graph assigns 242 transforms to CupidC, five to CupidASM, 189 to CupidObj, two to CupidLD, and one to CupidDis, with no host C or recursive Make transform. Every root output has a Cupid tool owner, so no root output is Python-only. Across all three supported graphs, CupidC owns 245 transforms and no transform invokes a host C compiler. Python participates in all 447 transforms as the checked-tool launcher and host-side safety, parity, and publication layer. The user compiler and Toolchain contract publisher create their output directories, so only three supplemental verification or orchestration outputs remain Python-only. The three installation-source transforms, kernel-symbol source transform, pristine disk-image template, deterministic ISO fixture, and Doom profile manifest belong to checked CupidObj. Python retains FAT reuse, file staging, native-path safety, drift checks, locking, and atomic publication. ADR 0204, ADR 0224, ADR 0238, ADR 0241, ADR 0244, and ADR 0245 record those transfers and the directory boundary. The poisoned-host build passes. A 607.7-second Linux kernel build and a 341.6-second Windows root build produced the same 430 frozen kernel artifacts. The 8,579,824-byte pass-one ELF has SHA-256 `2350b1a4f32c674d386ca50d4be4f6be8308082291f2852cfecfde4ba151b4df`; the 8,690,416-byte final ELF has SHA-256 `80b23fbd198f4aa9ec030ac69526b2d612c49e6d6328cfa01af9a0ef6217d8bb`; and the 8,490,228-byte raw kernel has SHA-256 `53770a93658e757d25f5aeab9d3e434d4a3be2a1dc3fbe4b19869e5bf9820a06`. The fresh 209,715,200-byte normal image has SHA-256 `e815d2ef67f114a26181f0e2cbde85f892cdadd487f8d9cbee9715e720800b3e`. A private `/bin/ls.cc` JIT boot from it passed in 49.8 seconds. Native hosted binaries still need GCC or Clang when explicitly requested as development oracles. Windows runs checked i386 programs through WSL until a native checked seed exists. Checked CupidASM authors the spanning ISO lane, and checked CupidObj authors the enclosing image and Doom profile manifest before Python parity and guarded publication. ADR 0227 records the lane transfer, ADR 0241 records the image transfer, ADR 0244 records the profile-manifest transfer, and ADR 0245 records publisher-owned directories. |
