...
CupidObj participates in 187 transforms, including three installation-source
generators, the kernel-symbol source generator, and the normal disk-image
template. The root graph has 436 Cupid-owned transforms. The ISO image and
Doom input manifest are its two Python-only outputs. ADR 0190 records the root
handoff, ADR 0196 records the contract handoff, and ADR 0238 records the
disk-image transfer.

Source-head CupidObj also has a bounded `iso-fixture` operation. It matches the
tracked ECMA-119 and `RRIP_1991A` bytes from a manifest and typed input
inventory. This is not a production ownership claim: the current checked seed
does not carry the operation, so the ISO image remains one of the two
Python-only root outputs until promotion and cutover.
...
| Disk, FAT, and ISO fixture construction and staging | Checked-seed CupidObj authors the pristine FAT16 template; Python hostbuild checks parity and manages mutable image state; source-head CupidObj can author the ISO fixture | CupidObj owns both deterministic templates; Python orchestrates FAT reuse, tree safety, parity, drift checks, staging, and guarded publication | Production disk-image ownership is shared. The normal Make recipe passes the checked seed manifest to `tools/hostbuild.py image`, which freezes the bootloader, kernel, stage inputs, seed, and live output. Checked `cupidobj disk-template` authors the MBR, boot and kernel reserve, FAT16 boot sector, two pristine FATs, and empty root directory through the byte before cluster 2. Python builds the same template independently and requires exact byte parity. It preserves a valid existing FAT filesystem or uses the complete template for a fresh image, stages the frozen files, extends the candidate, rechecks every frozen input plus the seed and output, and publishes under a cross-process lock with atomic replacement. Checked CupidASM assembles the 4,096-byte spanning ISO file. Python verifies it and still authors the production ECMA-119 and `RRIP_1991A` image without an external ISO command. Source-head `cupidobj iso-fixture` reproduces that complete image, but seed carriage and production transfer remain open. ADR 0239 records the new source capability. |
