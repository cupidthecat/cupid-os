...
The separate hosted i386 runtime contract now uses `.cc` and is classified as
CupidC-owned. It still sits outside the 19-source fixed-point plan because it
tests the completed runtime rather than contributing to a tool image. The
static audit therefore has 20 `.cc` roots, while seed membership and link
orders stay unchanged. ADR 0195 records this naming and ownership boundary.
...
| Capability audit | Every active source and generated input mapped to required C, ASM, ABI, object, linker, and inspector features | Complete for root `all`, `user:all`, and `toolchain:all`: 716 active inputs, 252 feature IDs, 500 transforms, 25 accounted unreachable source-like files, and a checked drift and coverage gate. The graph contains 27 assembly files, 14 C translation units, 288 headers, and 387 Cupid C files. The checked graph uses canonical Windows conditionals and the C locale on every host. The root image has no host C or recursive Make transform. |
