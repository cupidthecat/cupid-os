...
CupidBuild is now part of both checked six-tool seeds as a non-producer. Its
...
before launch. The recheck still runs after a failure or timeout. Production
ownership has not moved: the normal object recipes continue to use the Python
publisher, so the first recipe transfer and broader Python removal remain open.
...
four Cupid-built contract participants, and no host C transform. `toolchain:all`
...
| `kernel/cpu/isr.asm` | CupidASM ELF32 `ET_REL` | CupidASM plus CupidDis guarded ELF32 `ET_REL` | Production-owned: common entry preserves the live per-CPU `GS`, defers generic rescheduling through handler completion/EOI, and discards the source CPU's saved selector after a migration-safe suspension point. CupidASM emits a 1,892-byte object whose 417-byte `.text` has SHA-256 `bcf582569c26029d5143ec42f6de24388596c412bca2b4a672608800fe2606e3`, with 41 symbols and eleven `R_386_PC32` relocations matching the optional NASM oracle exactly. Thirty-one public entry labels now carry `STT_FUNC` without changing those bytes or relocations. Hostbuild validates a private candidate and requires strict decode, relocation ownership, local targets, and code anchors from CupidDis before atomic publication. ADR 0286 records the guard, and ADR 0336 records the typed-anchor adoption. |
| `kernel/core/context_switch.asm` | CupidASM ELF32 `ET_REL` | CupidASM plus CupidDis guarded ELF32 `ET_REL` | Production-owned: after moving BKL release and interrupt re-enable behind the new-stack handoff, CupidASM emits a 696-byte object whose 73-byte `.text` has SHA-256 `25b78f4c2cbf3dfadc6dc87a9731a097bfd9df0675534d8449c24d890114fbfa`, with three globals, one undefined handoff symbol, and one `R_386_PC32` relocation matching NASM semantics. The two defined entry labels carry `STT_FUNC` without changing those bytes or relocations. Hostbuild validates a private candidate and requires strict decode, relocation ownership, local targets, and code anchors from CupidDis before atomic publication. Scheduler handoff, nested deferral, and four-CPU runtime proofs pass. ADR 0286 records the guard, and ADR 0336 records the typed-anchor adoption. |
