...
Make transform, 452 Python participants, 248 CupidC participants, two
...
Windows user ABI and artifact-size gates build and run temporary PE contracts
from the execution seed. Artifact-size policy still binds the Linux manifest
as provenance. Native Windows reconstruction pairs that seed with the
...
source-head contract closure from 65 to 67 inputs. They also raise the current
audit to 738 active language inputs, including 297 headers. The C-family
inventory covers 703 files and
2,460 include operands, including 2,203 quoted operands. Those source-head
...
graphs, CupidC participates in 248 transforms, no supported transform invokes
...
Python participants. The root size verifier has CupidC, CupidASM, CupidLD, the
Cupid-built contract, and Python participants. It emits no OS artifact.
...
248, CupidObj 192, CupidASM seven, CupidLD seven, and CupidDis six. The native
Windows user ABI and artifact-size verifications account for the two
Cupid-built contract participants. It retains
  the 5/18/17 fixed-point matrix. `make bootstrap-audit` passed in 68.8
seconds.
...
| Capability audit | Every active source and generated input mapped to required C, ASM, ABI, object, linker, and inspector features | Complete for root `all`, `user:all`, and `toolchain:all`: 738 active inputs, 255 feature IDs, 452 transforms, 25 accounted unreachable source-like files, and a checked drift and coverage gate. Root `all` has 443 transforms. The graph contains 31 assembly files, 297 headers, 410 Cupid C files, and no ordinary C translation unit. The checked graph uses canonical Windows conditionals and the C locale on every host. No supported transform invokes a host C compiler or recursive Make. |
...
| C migration | Every reachable kernel, tool, application, Doom, and vendored C cohort compiles and passes behavior gates with CupidC | In progress: all 239 checked-in normal roots plus the generated kernel-symbol translation compile through the checked seed. Three generated installation tables and three example programs add CupidC ownership outside that cohort. The 15 hosted Toolchain contracts also compile, link, and run through the checked i386 seed. Linux runs that seed directly; Windows uses WSL for the complete Linux cohort and checked native PE tools for production. The narrower user ABI contract runs as a checked PE without WSL. Only host-built native drivers and contract runners remain optional development oracles. All Doom production objects compile, and the no-WAD, explicit missing-IWAD recovery, shell-survival, and dual-NIC frontier checks pass. Full IWAD gameplay, input, audio, and save behavior remain to be checked. |
...
| Normal-build cutover | Make invokes CupidC, CupidASM, CupidLD, CupidObj, and CupidDis without GCC/Clang/NASM/LLVM/binutils | In progress: all 443 root transforms have a Cupid participant. The 442 artifact transforms retain 243 CupidC, five CupidASM, 192 CupidObj, two CupidLD, and six CupidDis participations. The non-artifact size gate adds one CupidC, CupidASM, and CupidLD participation through its private contract. Across all three supported graphs, CupidC participates in 248 transforms, CupidObj in 192, CupidASM in seven, CupidLD in seven, CupidDis in six, and Python in all 452. No transform invokes a host C compiler or recursive Make. Windows selects the native checked execution seed for output-bearing tools, the user ABI gate, and the artifact-size gate; Linux keeps the static seed. The two raw boot paths and both production assembly objects receive strict inspection before publication. Kernel AOT links with in-kernel CupidLD. The full Linux Toolchain contract paths on Windows still need WSL, and Python retains coordination, safety, parity, drift, locking, and publication work. |
