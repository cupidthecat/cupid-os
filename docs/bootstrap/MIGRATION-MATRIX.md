...
| Shared `toolchain/cupidc_pp.*` preprocessing module | CupidC owns the shared freestanding module and every active hosted contract | CupidC builds one shared host/kernel preprocessor and both C/Cupid parsers consume its typed tape | Translation phases, dual presumed and physical locations, macros, C11 conditionals, reproducible predefined macros, `#line`, includes, forced and cache-aware traversal, pack state, typed Cupid `#exe`, diagnostics, and rollback are contracted. The checked audit covers 2,437 direct include operands, 2,196 quoted and 241 angle, with zero macro operands across 695 C-family inputs. Its generated manifest drives 385 tracked profile executions plus four generated roots. The target set has 32 ordinary strict Linux roots, one freestanding Windows root, a two-source strict kernel bridge, and two GNU runtime roots. Only the bridge searches `/kernel/lang`; every request uses checked declarations and a four-byte pointer fact. The audit pins the closure sources, object-contract input, self-host link recipe, Windows probe inputs, and user ABI contract inputs. Twenty-two browser fragments retain their owner, two delivered headers remain non-roots, and no hosted root is deferred. Named GNU variadics, numeric line-marker semantics, broader hosted-runtime/header integration, and deeper kernel integration remain |
...
| Capability audit | Every active source and generated input mapped to required C, ASM, ABI, object, linker, and inspector features | Complete for root `all`, `user:all`, and `toolchain:all`: 728 active inputs, 255 feature IDs, 450 transforms, 25 accounted unreachable source-like files, and a checked drift and coverage gate. Root `all` has 441 transforms. The graph contains 29 assembly files, 294 headers, 405 Cupid C files, and no ordinary C translation unit. The checked graph uses canonical Windows conditionals and the C locale on every host. No supported transform invokes a host C compiler or recursive Make. |
...
| Toolchain self-hosting | Checked seeds rebuild host tools; stage 2 and stage 3 outputs are byte-identical on Windows and Linux | A checked static i386 Linux seed rebuilds one 19-source union, fresh startup, and all five tools from a private captured source root. The stage-two producer trio repeats the work below the same root. The 43-file closure includes the Windows startup and runtime probe. The harness rehashes the private and live closures after each stage and after behavior checks, then publishes one complete evidence bundle. All 19 C object pairs, startup objects, and CupidC, CupidASM, CupidDis, CupidLD, and CupidObj images match. The promotion proof covers five help paths, eighteen successful operations, and sixteen failures. The promoted seed also produces matching CupidASM and CupidC objects and an imported PE image through both rebuilt stages. Windows validates and runs the image with exact output and exit checks, giving a 5/18/16 checked-seed matrix. Linux runs the seed directly, and Windows still uses WSL for the seed bootstrap and normal user build. The proof passed in 763.5 seconds. Its 17,035-byte report has SHA-256 `810704f6701b4b4627062981e1e969332d4aa5f409d2cdce3d4fcba150518f84`. The closure has SHA-256 `56e0943f82737a7013994f1a2b78fcbd5b5c762d0f5036aac5a48bfbb3dcbe32`. An independent poisoned-host reproof passed in 766.9 seconds. All five seed images match stage two, and stage two matches stage three across the complete 19/1/5 artifact set and behavior matrix. Its 17,032-byte report has SHA-256 `736872f31d853fe5b2b67c25e7ec42a1893655074a1c653112def6d66fdeac87`. ADR 0265 records the promoted images and proof. Native versions of all five tool drivers, the complete Windows runtime, and checked native seed carriage remain open. |
