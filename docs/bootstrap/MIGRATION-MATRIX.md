...
PE32 console image. The checked Linux seed and normal recipes do not carry or
use that command, so all ownership counts above remain unchanged. The promoted
seed keeps its 5/15/13 proof; source head adds one successful PE link and one
rejected-layout case for a 5/16/14 matrix. ADR 0247 records this source-only
format boundary.
...
| Capability audit | Every active source and generated input mapped to required C, ASM, ABI, object, linker, and inspector features | Complete for root `all`, `user:all`, and `toolchain:all`: 719 active inputs, 255 feature IDs, 447 transforms, 25 accounted unreachable source-like files, and a checked drift and coverage gate. The graph contains 28 assembly files, 290 headers, 401 Cupid C files, and no ordinary C translation unit. The checked graph uses canonical Windows conditionals and the C locale on every host. No supported transform invokes a host C compiler or recursive Make. |
...
| Toolchain self-hosting | Checked seeds rebuild host tools; stage 2 and stage 3 outputs are byte-identical on Windows and Linux | A checked static i386 Linux seed rebuilds one 19-source union, fresh startup, and all five tools from a private captured source root. The stage-two producer trio repeats the work below the same root. The harness rehashes the private and live closures after each stage and after behavior checks, then publishes one complete evidence bundle. All 19 C object pairs, startup objects, and CupidC, CupidASM, CupidDis, CupidLD, and CupidObj images match. The promoted seed reproof remains five help paths, fifteen successful operations, and thirteen failures, including CupidObj kernel-symbol source generation, JPEG validation, disk-template construction, ISO fixture authoring, profile-manifest authoring, and malformed-input rollback. Source head includes a deterministic fixed-layout PE32 link and a rejected-layout case. Its two rebuilt stages match across a 5/16/14 matrix, while only CupidLD differs from the promoted seed. The PE image is validated but not executed or promoted. Linux runs the seed directly. Windows uses WSL for the seed bootstrap and the normal user build. Native hosted CupidC and CupidLD remain an optional oracle, depend on Clang, and do not establish a Windows fixed point. |
