...
Make transform, 452 Python participants, 249 CupidC participants, three
Cupid-built contract participants, and no host C transform. `toolchain:all`
uses both rebuilt compiler stages for the fifteen published `.cc` contracts,
then uses the host-selected checked seed to build and run the sixteenth
Toolchain contract as the publication verifier.
...
artifact-size contract transfer, and ADR 0302 records the Toolchain manifest
...
the complete Toolchain contract cohort keep the Linux bootstrap seed. The
Windows user ABI, artifact-size, and Toolchain manifest gates build and run
temporary PE contracts from the execution seed. Artifact-size policy and the
manifest verifier still bind the Linux manifest as provenance. Native Windows
...
the current promotion. ADR 0302 records Toolchain manifest verification.

`verify-artifact-sizes` is a direct prerequisite of `cupidos.img`. Host Python
pins the raw policy, Linux bootstrap manifest, and nine file observations.
Checked CupidC compiles a strict C11 contract, CupidASM supplies startup, and
CupidLD links a static ELF on Linux or a native PE on Windows. The contract
validates the policy, manifest, and observations and emits a canonical report.
Python must reach the same result independently and recheck the live files.
Failure prevents image publication and preserves the existing image. ADR 0267
records the policy, and ADR 0297 records the contract transfer.
...
The exact-decimal fixture and fingerprint-bound x86 catalogue corpus raise the
source-head publication closure from 65 to 67 inputs. The catalogue's nested
`x86.cc` include raises it from 67 to 68. The manifest verifier
adds one active strict-C11 contract source. The current audit therefore records
739 active language inputs, including 297 headers. The C-family inventory
covers 704 files and 2,461 include operands, including 2,204 quoted operands.
Those source-head counts supersede the older 65-input and 736-input totals in
the broad rows below. The earlier supported publication keeps its historical
65-input evidence.
...
graphs, CupidC participates in 249 transforms, no supported transform invokes
...
The two verification contracts outside the user ABI path emit no OS artifact.
One Python-only supplemental output produces the Toolchain manifest. No
recursive Make transform remains. The root
...
| Toolchain publication manifest verification | Python captures a no-follow snapshot and runs a checked host seed; the decision used to remain entirely in Python | Checked CupidC compiles the strict-C11 contract, checked CupidASM supplies startup, and checked CupidLD links it; Python retains filesystem capture, launch, independent oracle, and drift checks | Production verification owns the exact 21-artifact publication, 68 publication inputs, 50 bootstrap inputs, Linux publication seed, and host-selected execution seed through `CUPMAN2`. The contract checks the schema and independently captured observations, then rereads its request before success. Make and the audit lock 126 direct inputs on Windows. Only actual build inputs receive compiler or assembler ownership. Object-comparison values remain publisher evidence because the publication does not carry the compared objects. One Python-only Toolchain manifest-production transform remains. ADR 0302 records the boundary. |
...
249, CupidObj 192, CupidASM eight, CupidLD eight, and CupidDis six. The native
Windows user ABI, artifact-size, and Toolchain manifest verifications account
for the three Cupid-built contract participants. It retains the 5/18/17
fixed-point matrix. `make bootstrap-audit` passed in 63.0 seconds, and
...
| C migration | Every reachable kernel, tool, application, Doom, and vendored C cohort compiles and passes behavior gates with CupidC | In progress: all 239 checked-in normal roots plus the generated kernel-symbol translation compile through the checked seed. Three generated installation tables and three example programs add CupidC ownership outside that cohort. The 15 hosted Toolchain contracts also compile, link, and run through the checked i386 seed. Linux runs that seed directly; Windows uses WSL for the complete Linux cohort and checked native PE tools for production. The user ABI, artifact-size, and Toolchain manifest contracts run as checked native PE files without WSL. Only host-built native drivers and contract runners remain optional development oracles. All Doom production objects compile, and the no-WAD, explicit missing-IWAD recovery, shell-survival, and dual-NIC frontier checks pass. Full IWAD gameplay, input, audio, and save behavior remain to be checked. |
| Toolchain self-hosting | Checked seeds rebuild host tools; consecutive post-transition generations are byte-identical on Windows and Linux | The static i386 Linux seed rebuilds the 19-source union, fresh startup, and all five Linux tools from a private captured root. The PE execution seed and verified Linux plan rebuild 20 C objects, two assembly objects, and all five native Windows tools. Both bind the same 50-input snapshot, SHA-256 `e76d36ed4edc7679e91ac237135fe476dff6e69946bbffca56077afbf19a47f9`. Linux passed a clean 1,294.3-second candidate proof and a 1,473.9-second promoted-seed reproof with all initial images equal and 5/18/17 behavior. Windows passed a clean 1,253.4-second candidate proof and a 1,061.3-second reproof with all initial images equal and 5/5/6 behavior. Both matrices reject unmatched executable relocations. The complete Linux Toolchain contract programs on Windows still use WSL, and Python still coordinates both fixed points. ADRs 0278 and 0279 record the driver and convergence rule; ADR 0292 records the current promotion. |
| Normal-build cutover | Make invokes CupidC, CupidASM, CupidLD, CupidObj, and CupidDis without GCC/Clang/NASM/LLVM/binutils | In progress: all 443 root transforms have a Cupid participant. The 442 artifact transforms retain 243 CupidC, five CupidASM, 192 CupidObj, two CupidLD, and six CupidDis participations. The non-artifact size and Toolchain manifest gates each add one CupidC, CupidASM, and CupidLD participation through a private contract. Across all three supported graphs, CupidC participates in 249 transforms, CupidObj in 192, CupidASM in eight, CupidLD in eight, CupidDis in six, and Python in all 452. Three transforms run Cupid-built semantic contracts. No transform invokes a host C compiler or recursive Make. Windows selects the native checked execution seed for output-bearing tools and private contracts; Linux keeps the static seed. The two raw boot paths and both production assembly objects receive strict inspection before publication. Kernel AOT links with in-kernel CupidLD. The full published Linux Toolchain cohort on Windows still needs WSL, and Python retains coordination, safety, parity, drift, locking, and publication work. |
