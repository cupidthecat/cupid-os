...
Source head and the checked seed have 592 x86 forms, 244 canonical mnemonics,
64 registers, and fingerprint `F4420CB4`. The newest row encodes canonical
`FSUB ST(1), ST(0)` as `DC E9`. This supersedes older counts below. ADR 0203
records seed carriage for `FLDZ` and the three preceding x87 forms, while ADR
0208 records forward-subtraction carriage.
