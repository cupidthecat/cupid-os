...
1,090.390 seconds with all five initial comparisons true and 5/5/6 behavior.
Both bind revision `ed6a91ba954881475ac5ab73d5168d292a584c90` and source
snapshot `a15970287b5f6d6ef5f4e0092d1b460e6b2af2624db4640d2ba5c435e43c1817`.
...
seconds, with a 1,188.356-second wrapper. The final module passed all 86 tests
in 2,444.917 seconds, with a 2,445.438-second wrapper. ADR 0305 records the
current promotion and full red and green chronology.
...
artifact ownership counts. The promoted Linux seed carries the 5/19/18 matrix.
...
carriage, ADR 0274 records the stack commitment, and ADR 0305 records the
current seed.
...
| Toolchain publication manifest verification and authoring | Python captures a no-follow snapshot. Verification runs a checked host seed; authoring runs the converged stage-four Linux tools. | Checked CupidC compiles the strict C11 verifier and author, checked CupidASM supplies startup, and checked CupidLD links them. Python retains filesystem capture, launch, independent oracle, staging, and drift checks. | Verify mode consumes `CUPMAN2` and runs as a native PE on Windows. Author mode is always a static Linux ELF; Windows uses WSL. Its `CUPMAN3` request contains independent facts for the exact 21-artifact publication, 70 publication inputs, 50 bootstrap inputs, 17 object comparisons, Linux publication seed, build plan, and generation pair. Schema `cupid.toolchain-contracts.v3` stores each publication input and object comparison as an exact `sha256` and `size` record. `CUPMAN2` rejects live input observation size drift, and Python checks the report with an independent oracle. The `CUPMAN3` author requires each object-comparison size to be nonzero and binds its record into canonical output. Those records remain producer evidence because the author does not independently read the object bytes. The author rejects lane-specific missing, extra, duplicate, unsafe, malformed, and inconsistent facts before emitting canonical JSON. The direct module passes 29 tests in 39.068 seconds, the publisher 59 in 3.518 seconds, and the pinned verifier runner 24 in 27.752 seconds with three Windows skips. No Python-only transform remains. The schema v3 publication passed in 4,273.533 seconds and wrote a 27,069-byte manifest with SHA-256 `69c5b8e62c1e61a8f1a2823d18edff794ae03239be71c881ddd8a190f1377c91`. The native Windows verifier printed `Cupid Toolchain manifest: ok (21 artifacts)`. ADR 0302 records verification, and ADR 0304 records authoring. |
...
| Normal-build cutover | Make invokes CupidC, CupidASM, CupidLD, CupidObj, and CupidDis without GCC, Clang, NASM, LLVM, or binutils | In progress: all 443 root transforms have a Cupid participant. Across all three supported graphs, CupidC participates in 250 transforms, CupidObj in 192, CupidASM in nine, CupidLD in nine, CupidDis in six, and Python in all 452. Four transforms run Cupid-built semantic contracts. No transform is Python-only, invokes a host C compiler, or recurses through Make. Windows selects the native checked execution seed for output-bearing tools, the user ABI, artifact-size, and `CUPMAN2` verifier contracts; Linux keeps the static seed. The `CUPMAN3` author stays on the converged Linux path and uses WSL on Windows. The raw boot paths select local-target and strict-decode checks before publication. Kernel AOT links with in-kernel CupidLD. Python retains coordination, safety, parity, drift, locking, and publication work. The schema v3 Toolchain publication and final post-CTXT audit pass. The 684.260-second OS build and 64.601-second smoke remain preceding checkpoint history. The definitive fully poisoned build passes in 708.912 seconds, and the strong four-vCPU e1000 frontier passes in 801.490 seconds with the required feature-14 and JIT markers once and in order. |
