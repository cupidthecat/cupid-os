...
increment and decrement also remain open. ADR 0250 records runtime unsigned
four-byte output,
...
clobber. The checked-seed C11 standalone-header sweep passes 158 of 160 inputs,
...
long-double computation, and floating-width conversion remain open. Runtime
...
represented target layout.
Hexadecimal floating literals, binary32 and binary64 subnormal literals,
hexadecimal or subnormal long-double literals, decimal ratios beyond the
bounded parser, static long-double arithmetic, comparison, truth and logical
operators, conditional selection, floating-width conversion,
runtime mixed integer and floating arithmetic or conditionals,
...
conversion. ADR 0254 records static initializer conversion.
...
| Capability audit | Every active source and generated input mapped to required C, ASM, ABI, object, linker, and inspector features | Complete for root `all`, `user:all`, and `toolchain:all`: 722 active inputs, 255 feature IDs, 447 transforms, 25 accounted unreachable source-like files, and a checked drift and coverage gate. The graph contains 29 assembly files, 291 headers, 402 Cupid C files, and no ordinary C translation unit. The checked graph uses canonical Windows conditionals and the C locale on every host. No supported transform invokes a host C compiler or recursive Make. |
