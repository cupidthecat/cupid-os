...
CupidObj participates in 186 transforms, including three installation-source
generators and the kernel-symbol source generator. ADR 0190 records the root
handoff, and ADR 0196 records the contract handoff.
...
Source head parses all 29 declarations in unchanged `simd_intrin.h` under the
...
The checked seed has 592 x86 forms, 244 canonical mnemonics, 64 registers, and
fingerprint `F4420CB4`. Source head has 596 forms, 245 mnemonics, 64 registers,
and fingerprint `DA15E97F`. Its four new forms cover canonical SHRD. The latest
checked row encodes `FSUB ST(1), ST(0)` as `DC E9`. This supersedes older
counts below. ADR 0203 records seed carriage for `FLDZ` and the three preceding
x87 forms, ADR 0208 records forward-subtraction carriage, and ADR 0226 records
SHRD at source head.
...
| Disk, FAT, and ISO fixture construction and staging | Python `tools/hostbuild.py` | Python orchestration using Cupid-produced inputs | Accepted non-code-producing host dependency. FAT image work stays cross-platform. Checked CupidASM assembles the 4,096-byte spanning fixture from `test_iso/big_pattern.asm`; Python freezes the source and seed, checks exact parity, and publishes atomically. ISO authoring is deterministic and repository-owned: a checked manifest fixes the fixture membership without recursive Make traversal, the writer emits ECMA-119 with fixed `RRIP_1991A` metadata, and it rechecks the manifest, tree, and output before atomic publication. No external ISO author participates. |
...
| Capability audit | Every active source and generated input mapped to required C, ASM, ABI, object, linker, and inspector features | Complete for root `all`, `user:all`, and `toolchain:all`: 719 active inputs, 255 feature IDs, 449 transforms, 25 accounted unreachable source-like files, and a checked drift and coverage gate. The graph contains 28 assembly files, 290 headers, 401 Cupid C files, and no ordinary C translation unit. The checked graph uses canonical Windows conditionals and the C locale on every host. No supported transform invokes a host C compiler or recursive Make. |
