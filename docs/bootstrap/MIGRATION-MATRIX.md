...
| Disk, FAT, and ISO fixture construction and staging | Python `tools/hostbuild.py`; checked-seed CupidObj can emit the pristine disk prefix | Python orchestration using Cupid-produced inputs | Production remains Python-owned. Checked-seed `cupidobj disk-template` writes the MBR, boot and kernel reserve, FAT16 boot sector, two pristine FATs, and empty root directory through the byte before cluster 2. It matches the compact and active Python layouts and handles a formerly nonterminating FAT-size cycle. The image publisher still needs to preserve an existing FAT filesystem, stage files, detect drift, and publish safely. Checked CupidASM already assembles the 4,096-byte spanning ISO fixture; Python verifies it and authors the deterministic ECMA-119 and `RRIP_1991A` image without an external ISO command. ADR 0236 records the disk-template capability, and ADR 0237 records seed carriage. |
