...
A stage boundary where one generation of CupidC, CupidASM, and CupidLD builds complete Linux images for CupidC, CupidASM, CupidDis, CupidLD, and CupidObj. The checked seed builds stage two, stage two builds stage three, and stage three builds stage four. The gate compares all 19 C objects, the independently assembled Linux startup objects, and all five linked images between stages three and four. Both compared stages execute the five tools through real success and failure cases. The same convergence relationship applies to the native Windows runtime, startup, publication objects, tool images, and behavior checks. The Linux driver revalidates its live seed manifest and artifacts at every generation boundary and before publication. The Windows driver checks its PE execution seed and Linux plan seed independently at the same boundaries. Linux behavior evidence inventories the stage-three and stage-four Windows `cupiddis_main` objects, while the audit binds their Windows-profile compile and replacement path. Uncapped preliminary proofs passed after the stack-probe codegen change. Windows matched 20 C objects, two assembly objects, and five tools with a 5/5/5 behavior matrix. Linux matched 19 C objects, startup, and five tools with a 5/18/16 matrix. Both used the same 50-input snapshot, SHA-256 `d8481a39e0d1c7f42779a8c9f5fc5de10d7e5b9bc4df63ce6afe9ddd9c9716da`. Named clean-commit reproof and seed promotion remain pending. ADR 0279 records the added generation, comparison rule, and preliminary evidence.
...
The later promoted-seed reproof includes the Windows tool runtime, startup,
...
It preserved the Linux 19/1/5 fixed point and 5/18/16 behavior matrix, then
built matching native images for all five tools from stages two and three.
Windows ran help plus useful success and failure cases for every tool. CupidLD's
case also checked exclusive candidate selection, byte-exact output, replacement of
...
The production path freezes the selected seed manifest and five artifacts, the
...
atomic publication. Any failure preserves the prior raw kernel. At the reviewed
transaction checkpoint, the complete operation passed with exit 0 in 187.054
seconds and published an
...
and the raw kernel is 9,096,008 bytes with SHA-256
...
Its five-sector boot image is unchanged. A private QEMU boot ran `ls` and
reached JIT completion in 46.9 seconds.
...
as PID 4, and observed a normal exit. The complete AOT smoke passed in 79.661
seconds.
...
Linux plan seed. Toolchain contracts, the user syscall ABI contract, and the
artifact-size policy continue to use the checked Linux bootstrap seed. ADR
0272 records the seed boundary, and ADRs 0278 and 0279 record native
...
native stages two through four. It treats stages two and three as transition
generations, compares every object and tool between stages three and four,
runs native behavior checks on the compared stages, rehashes the live closure,
...
**Host toolchain**:
External compilers, assemblers, linkers, and binary utilities confined to explicit development and oracle paths. They do not produce normal Cupid OS or Toolchain artifacts. Host Python still coordinates the checked build. Windows runs output-bearing production tools from the native checked execution seed. Source head can reconstruct native Windows stages two through four from that execution seed and the Linux plan seed, while Linux-contract paths still use the static i386 Linux seed through WSL. Preliminary native and Linux convergence proofs pass on one frozen uncommitted snapshot. Named clean-commit reproof, seed promotion, and a Python-free fixed point remain open at this boundary.
