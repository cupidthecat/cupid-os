...
bootloader, SMP-trampoline, and ISO-pattern recipes invoke the typed
transactions directly, so CupidBuild owns five guarded assembly publications.
Fixed-point carriage
or source capability alone still does not establish production ownership.

The promoted seeds also provide a typed kernel-flatten transaction for the exact
...
and the pinned `/proc` launch repair. Both clean fixed-point proofs passed.
...
the Linux image with E1000 and RTL8139 pass; the separate IWAD-backed Doom
gameplay gate remains open. The earlier `962e476b` Windows replay exceeded
the command-line limit with 431 absolute paths and preserved the old raw
kernel. ADRs 0387, 0388, and 0389 record the launch and recovery boundaries.
The Windows image with preserved FAT contents separately passes an SMP and
`ls` smoke.
...
The normal JPEG recipe now invokes this transaction directly from the promoted
CupidBuild seed. Make binds the complete production trust unit, and Python no
longer participates in that output.
_Avoid_: command wrapper, unchecked tool launch, production ownership from source presence

The promoted CupidBuild seeds also carry the typed kernel-symbol transaction. It freezes
the pass-one kernel ELF and complete promoted seed, captures private CupidDis
`-n` output without a shell, and gives those exact rows to CupidObj
`ksyms-source`. A separate bounded parser and renderer reconstruct the KSYM
blob and generated `.cc` spelling, so CupidObj output cannot publish unless the
two implementations agree byte for byte. The transaction rechecks its source,
seed, captured rows, candidate, lock, destination, and pinned parent before
replacement. ADR 0369 records the source capability, ADR 0370 records paired
seed carriage, and ADR 0371 records direct production ownership. Normal
kernel-symbol publication now enters the promoted CupidBuild transaction from
Make with the complete six-image seed closure.

The checked `compile-kernel` transaction owns all 157 kernel-profile Make
recipes. It captures each explicit source/header closure into `CUPSRC1`, runs
checked CupidC under the fixed profile, and validates the emitted ELF32 object
before publication. Missing bundle entries cannot read live files. Failed
compilation preserves the previous object; equal validated bytes retain the
existing timestamp after full input and publication checks. The largest
closure contains 90 files. ADRs 0390 through 0393 record the format, paired
proof, complete capture policy, and Make handoff. Checked `compile-doom` now
owns all 83 Doom compilations through closed source/header bundles and filtered
...
On Windows, the runner pins and rechecks the working-directory identity, pins its private root and files by handle, retains a tool handle without write or delete sharing through `CreateProcessA`, and forwards output in binary mode to preserve exact bytes. Cleanup deletes a file that changed in place when its identity still matches, but preserves a replacement with a different identity. The promoted fixed-point matrices run checked CupidObj through consecutive CupidBuild generations and compare a real relocatable result plus an invalid-option diagnostic. They also check CupidC help, a byte-equal relocatable compile, and a failing compile that preserves both outputs. Direct Make recipes bind the promoted production seed, Makefile, and all six images in their prerequisite closure.

The promoted string surface also includes standard `strrchr`. CupidBuild
...
prove that progressive JPEG and malformed ELF inputs preserve the previous
destinations. Their current inventories are 41/7/47 on Linux and 29/7/34 on native
Windows. Both active seed cohorts carry the kernel-symbol and kernel-flatten
commands.
...
member against Cupid's target layout. The active pair has 59 frozen inputs with
snapshot SHA-256
`2346284239faf9f5571223587f2e2b2dfdd5197cffd41b97a12e234386810695`.
Source CupidBuild accepts the preceding 58-input v2 manifests and the active
59-input v2 manifests. Counts below 58 or above 59 fail provenance validation.
This narrow window retains compatibility with the preceding pair while
admitting the current pair. The first promotion attempt exposed the old
58-only check and failed before it could publish a normal-build artifact.
Fresh candidates from the repaired source later converged, and ADR 0367
records their paired promotion.
...
The 156 checked-in normal-build translation units owned by checked-seed CupidC, plus the generated kernel symbol-table translation unit. All 157 sources use `.cc`. The five shared Toolchain roots are also part of the 22-source i386 Linux fixed-point plan; their native GCC and Clang rules select C explicitly with `-x c`. The symbol build enters promoted CupidBuild with the pass-one kernel and six-tool v2 seed. CupidBuild freezes those inputs, captures canonical CupidDis text privately, asks CupidObj for the packed source, checks it against an independent native renderer, rejects malformed symbols, mismatch, lock contention, or live drift, and publishes only a complete regular file. Checked CupidBuild captures that source and its two-header closure in a closed bundle before validating and publishing the data-only object. The same transaction captures the complete closures for all 157 kernel roots and uses their logical paths throughout compilation. It freezes the six-tool seed, runs checked CupidC, and verifies the live inputs and publication boundaries before accepting the object. Drift prevents publication even when compilation succeeds. Equal validated output retains its timestamp. ADR 0393 records the paired seed promotion and direct Make handoff. ADR 0124 records the first 111-root naming transfer, ADR 0126 completes the fixed-point naming boundary, ADR 0129 transfers the in-kernel CupidC lexer, ADR 0135 transfers Nuked OPL3, ADR 0139 transfers JPEG and glyph rasterization, ADR 0167 transfers the FPU and SMP roots, ADR 0176 transfers libm, ADR 0180 transfers the kernel entry and SIMD roots, ADR 0181 transfers the final strict host root, ADR 0224 transfers kernel-symbol source generation to CupidObj, ADR 0246 records the shared invocation boundary, and ADR 0276 adds the checked CupidLD kernel source. Poisoned-host rebuilds and exact recursive header closures cover every recipe. The most recent complete two-pass frontier predates that addition: it covers 155 sources in a 445-file snapshot with SHA-256 `99d03de14f544f6a76d21ed147e62018873f1e2e8dfa2f4459830b69314432c2`, and both 155-object passes are byte-identical at 3,749,796 bytes each. The current 156-source production build passed. The broader two-pass frontier targets 156 sources and 312 checked compilations; its latest rerun exceeded 2,340 seconds without a compiler diagnostic. That timeout is not a complete frontier pass. Input discovery skips hidden paths under the active include roots, which keeps private compiler staging headers out of the repository snapshot during concurrent builds. Final directory promotion retries only short permission-style locks; a persistent lock or any other filesystem error fails without publishing a partial frontier. A data-only relocatable object is valid without `.text` when its sections and symbols pass the remaining ELF checks. The combined graph includes the ISO runtime fixture as an image input. Strong four-vCPU checks cover both supported NICs, all three FPU milestones, the promoted SMP, libm, and string paths, RDRAND, all 62 crypto checks, USB storage, desktop and terminal startup, audio output, TrueType glyph use, an exact baseline JPEG decode, and in-OS CupidC execution. The strict checked-in kernel and driver cohort has no host-compiled root.
...
The 83 `.cc` Doom and Cupid platform translation units built by checked-seed CupidC in the normal image. Three sources use the exact `DOOM_COMPAT_I386` profile; the sound adapter and 79 Doom-tree sources use `DOOM_TREE_I386`. Checked `cupidbuild compile-doom` captures the selected source and all discovered profile headers into a closed bundle; missing entries cannot fall back to live files. Its filtered discovery permits unrelated object writes while rejecting source/header membership and directory-identity changes. The current profiles expose 304 `.h` and `.inc` inputs through 20 include roots. It recursively scans visible `.c` and `.cc` files beneath `kernel/doom` before and after compilation. An always-checked manifest fixes both source memberships and every input hash without changing its timestamp when the content is unchanged. A legacy `.c` file, an unlisted `.cc` file, a missing root, added or removed headers, byte drift, symbolic links, and NTFS junctions fail before object publication. The active dglibc source is 67,155 bytes and produces a 93,332-byte object with SHA-256 `e2496b01c93a7858a0c035b53aea0ad834d95d2be3f7ae49574d1759ebec34d6`. The 72,950-byte closed profile manifest has SHA-256 `eeb25fe8855563247c29bdd08a4fcc880ac023d37df810969251ba63177223f8`. The normal Make target passes the checked seed manifest to CupidBuild. Its typed publisher captures one stable profile inventory, derives the bounded `CUPROF1` snapshot, and runs checked CupidObj from the exact frozen seed. An independent native renderer must produce identical JSON. CupidBuild rechecks the seed, live inputs, candidate, output directory, and existing output under an adjacent owner lock, then preserves identical bytes and their timestamp or publishes atomically. CupidObj authors the bytes; CupidBuild owns discovery, native-path checks, freezing, parity, drift detection, locking, and publication. Python retains an optional parity oracle. Asset-free runtime checks cover active nonlocal exit, repeated quit and error cleanup, production config helpers with test-only files, native rename and copy boundaries, block-cache failure handling, RamFS limits, FAT collision, read, handle, busy-replacement, and 8.3 behavior, HomeFS ownership, depth, and batched publication, no-WAD recovery, shell survival, and the full stateful four-CPU frontier on e1000 and RTL8139. Gameplay remains a separate IWAD-backed boundary. ADR 0184 records ownership, ADR 0211 the storage bridge, ADR 0214 the shell-session lifecycle, ADR 0242 the CupidObj format boundary, ADR 0243 its seed carriage, ADR 0244 the normal publisher, and ADR 0396 the checked Doom compiler handoff.
...
The profile publisher is now carried by both checked seeds from revision
`142a9737f618ab8500308576a1c222501d639e5f` and owns the normal Make recipe.
Make prepares the POSIX `build/bootstrap` parent before invoking the typed
command; Windows retains parent creation inside the transaction. The exact
production seed closure and operation are locked by the build-graph audit.
ADRs 0382 and 0383 record promotion and production ownership.

The profile check finishes before all 254 normal writers beneath `kernel/`,
`drivers/`, and `toolchain/`. The 83 Doom consumers retain ordinary content
dependencies; the other 171 consumers, including generated installation
sources, use order-only dependencies. Unrelated roots remain parallel.
The audit preserves scheduling reachability through `order_only_inputs`
without treating those edges as content consumed by a transform.
...
preceding promoted Windows plan was
...
stage-four object and tool before a later behavior fixture exposed a stale
plan identity: the copied current tools were paired with a temporary manifest
that still named the promoted plan. Native Windows behavior copies now bind
only their temporary manifest to the plan executed by the copied cohort. The
fixture repair left the checked seed, its manifest, and its tool bytes
unchanged. Those runs promoted no pair. A Windows rerun with corrected
temporary manifests completed all 19/7/24 behavior groups. Initial candidates
from `34b597aa` then converged, but a later post-install drift test exposed a
Windows rollback sharing violation. The `962e476b` proof passed after that
repair, but full Windows replay exposed the separate command-line limit.
The clean `16a86f5b` proofs carried the subsequent launch repairs and supplied
the preceding checked images recorded by ADR 0382.

The Windows repair restores rejected candidates through a verified
read/traverse directory handle, then reacquires cleanup authority. This avoids
the rename-back sharing violation without relaxing frozen-input protection.
ADR 0385 records the source contract now carried by the checked seed.
...
The ramfs program table, homefs document table, and CupidASM demo table. Checked-seed CupidObj generates all three `.cc` sources from Make's ordinal inventories, and checked-seed CupidC compiles them under the fixed kernel profile. Each generation recipe depends on the checked runner, seed manifest, and all six seed images. Checked CupidBuild freezes each generated source and the complete five-header union, runs checked CupidC, validates each relocatable object, and publishes through the guarded compiler transaction. Drift detected by the runner's post-run check rejects the object. `tools/hostbuild.py` remains the parity oracle but no longer owns these production sources. ADR 0204 records the generation transfer, and ADR 0246 records the shared invocation boundary.
_Avoid_: every generated C file, Python-free generation, kernel source cohort

**Production checked-seed tool cohort**:
The following paragraph records the current six-tool production boundary.
The root image transforms are owned by CupidASM, CupidObj, CupidLD, CupidDis, and CupidBuild. The normal graph verifies and freezes the manifest-bound six-tool seed before each command, then checks the live trust unit again after the command. Checked production kernel, generated-install, and user CupidC calls plus checked user CupidLD links use that same invocation contract. CupidBuild supplies the frozen bundles for all 157 kernel-profile, 83 Doom, and three generated-install compilations; the user wrapper supplies its separate frozen cohort. Checked execution rejects manifest or image drift detected after the private tool returns. It contains six CupidASM transforms, 192 CupidObj transforms, three CupidLD links, seven CupidDis participations, and 440 CupidBuild participations. The extra assembly and link operations build the artifact-size contract, whose policy logic is compiled by CupidC. CupidDis supplies kernel-symbol text, participates with CupidObj in the transactional `kernel.bin` publication, strictly checks the mixed-mode SMP trampoline and bootloader, and guards the ISR and context-switch objects before publication. The two objects use direct CupidBuild transactions. The runner freezes the source and seed, validates a private i386 relocatable with executable bytes, requires complete CupidDis coverage of those bytes, rechecks the live boundaries, and publishes atomically. The final kernel gate remains an independent whole-kernel check. The SMP and bootloader paths share one checked raw-image transaction. It owns output locking, source and seed freezing, drift checks, private candidates, publication-boundary checks, and atomic replacement. Each caller retains its image and map policy. The normal boot rule enters that transaction through CupidBuild with the production manifest and complete seed closure, so standalone tool overrides cannot bypass the checked boundary. Root `all` has 443 transforms, and every transform has a Cupid participant. The artifact-size verifier builds and runs a private CupidC contract while Python pins the files, launches the checked tools, and checks an independent oracle. The Toolchain `all` verifier applies the same division to the publication, exact input inventory, bootstrap closure, and Linux publication seed. CupidASM assembles the ISO spanning fixture from checked-in source. CupidBuild verifies its exact byte lane and data-only map, runs checked CupidDis, and controls publication. The CupidObj total includes the three installation-source generators, kernel-symbol source generator, pristine disk-template author, repository ISO author, and guarded Doom profile-manifest author. Across all three roots, CupidC participates in 250 transforms, CupidObj in 192, CupidASM in nine, CupidLD in nine, CupidDis in ten, CupidBuild in 440, and Python in 12. Four transforms run Cupid-built semantic contracts. The 157 kernel-profile, 83 Doom, and three generated-install compilations, 186 ordinary CupidObj recipes, typed JPEG, kernel-symbol, kernel-flatten, Doom profile, and ISO pattern transactions, and both normal kernel links use CupidBuild without Python; Python remains on the disk and ISO composite CupidObj paths and the broader orchestration, safety, parity, and publication work. Windows runs the checked native PE execution seed directly; Linux runs the checked static ELF seed. Contract publication remains rooted in the Linux bootstrap seed. Both fixed-point drivers rebuild and compare the six-tool cohort. The checked user compiler and Toolchain contract publisher create the output directories needed for their artifacts, so empty-directory setup is not a separate graph transform. The compiler pins every POSIX component with no-follow descriptors and every Windows component with parent-relative directory handles until its final resolved-output check. Make applies `$(sort ...)` to every wildcard-discovered output list, so generators and links receive the same order under Windows and Linux host locales. The repository stores its runtime JPEG as a sequential SOF0 frame. The typed CupidBuild publisher freezes the JPEG and complete seed, asks checked CupidObj `wrap-jpeg` to validate and wrap the private snapshot under the original source identity, verifies the exact relocatable payload and identity symbols, and applies an independent native JPEG parser before atomic publication. Source, seed, candidate, destination, lock, or parent drift preserves the previous object. FFmpeg, `jpegtran`, `djpeg`, and `cjpeg` do not participate. The first cross-host comparison matched 426 of 430 kernel artifacts and traced all four remaining differences to the old host JPEG conversion. After the replacement, Linux and Windows root builds produced all 430 frozen kernel artifacts byte for byte. A fresh normal image then passed a private `/bin/ls.cc` JIT boot. ADR 0190 records the root tool handoff, ADR 0204 records installation-source ownership, ADR 0224 records kernel-symbol source ownership, ADR 0227 records the fixture transfer, ADR 0235 records the JPEG acceptance transfer, ADR 0238 records disk-template production ownership, ADR 0241 records ISO production ownership, ADR 0244 records profile-manifest production ownership, ADR 0245 records publisher-owned output directories, ADR 0246 records the shared invocation boundary, ADR 0271 records trampoline inspection, ADR 0272 records native Windows execution-seed adoption, ADR 0277 records the raw-map transaction, ADR 0280 records the clean Linux stage-four promotion, ADR 0281 records the clean Windows stage-four promotion, ADR 0282 records stage-four contract publication, ADR 0283 records the normal bootloader cutover, ADR 0286 records guarded assembly-object publication, ADR 0295 records the native Windows ABI gate, ADR 0297 records the artifact-size contract, ADR 0302 records Toolchain manifest verification, ADR 0362 records the direct CupidObj handoff, ADR 0363 records the kernel-link handoff, ADR 0368 records the JPEG handoff, ADR 0371 records the kernel-symbol handoff, and ADR 0375 records final kernel flattening with the exact 431-entry manifest and six-image seed closure.

The current production cohort freezes a six-image v2 seed on either host. The
earlier Toolchain publication checkpoint contained 22 artifacts, kept
75 publication inputs, and bound its historical 58-file bootstrap closure.
The active seed pair binds the 59-file closure. The normal ISR and
context-switch object recipes now invoke the promoted CupidBuild seed directly.
The build audit derives CupidBuild ownership from each graph's evaluated
`CUPIDOBJ` command rather than the variable name alone.
The remaining source-head and fixed-point transactions retain their existing
coordinators.

The following five-tool proof is historical. Its clean Linux candidate passed
the 21/5/22 failure/help/success inventory, and its clean Windows candidate
passed 9/5/8; CupidASM and CupidDis changed in each
...
CupidASM objects. Promoted CupidBuild applies both linked checks to the two
kernel ELFs before flattening and publishes the final raw image transactionally.
Hostbuild retains the same operation as an optional oracle. The
...
under root `all`. CupidC participates in 250 transforms, CupidObj in 192,
CupidBuild in 440, CupidASM and CupidLD in nine each, and CupidDis in ten. Four
transforms run Cupid-built semantic contracts. Its assembly ownership contract
covers all 32 active assembly sources, including five Toolchain startup
sources, with no ownerless input. Python participates in 12 for coordination
and safety, but no transform is Python-only. The Toolchain publisher now gives a checked strict
...
bootstrap C objects, one startup object, and six tool images. The author
...
tests, the publisher passes 65, and the
...
Windows author execution. An earlier complete schema v3 `CUPMAN4`
...
host import boundary. At that checkpoint, `make bootstrap-audit` and
`make check-bootstrap-audit` both passed. Its six-tool fixed-point
inventory recorded failure, help, and success counts of 28/6/35 for Linux and
17/6/22 for Windows. The CupidBuild behavior group covers its guarded object,
bootloader, and SMP-trampoline operations, the checked CupidObj runner, plus
typed JPEG, kernel-symbol, and kernel-flatten transactions, plus object and raw rollback. The
matrix also applies strict CupidDis inspection to every final candidate image.
Those historical promotion candidates found only CupidC and CupidBuild changed from
the preceding seed on each host. Every final object and all six tool images
matched between stages three and four.
...
normal linked-kernel pass selects that rule inside the promoted
`flatten-kernel` transaction before CupidObj extraction. The rule consumes the
exact 431-entry manifest and six-image production seed closure, rejects
standalone tool and Python overrides, and preserves the prior raw kernel on
failure. ADR 0375 records the production boundary.
...
**Guarded ISO pattern publisher**:
CupidBuild's typed transaction for the 4,096-byte sector-spanning input, with exact byte and map validation and timestamp-preserving publication. Source support, checked-seed carriage, and normal-recipe ownership are separate gates.
_Avoid_: ISO image author, production ownership from source support alone
...
All three tools share one frozen six-image v2 seed capture. A failed inspection
or output on either stream preserves the earlier executable. ADR 0326 records
this gate.
The following paragraph preserves an earlier five-tool PE checkpoint.
...
records the last five-tool execution seed. ADR 0353 records the paired v2
contract, and ADR 0367 records the preceding pair, and ADR 0370 records the preceding pair; the bootstrap log records the active pair.
...
The repository-owned startup and service layer for Cupid-built native Windows
tools. CupidASM obtains the command line, aligns the i386 stack, and exposes
cdecl bridges for imported stdcall APIs. CupidC parses quoted arguments into
one allocation and supplies `VirtualAlloc` heap storage, standard streams,
file reads and writes, seeking, `getcwd`, `_fullpath`, and `errno` mapping.
CupidLD authors every IAT slot, commits the bounded one MiB tool stack in full,
and adds exclusive creation, durable flush, atomic replacement, and candidate
deletion. Windows runs help plus useful success and failure cases for all six
tools, along with direct runtime and compiler contracts. The promoted PEs form
the checked Windows execution seed and own output-bearing production work on
Windows. Its 2,852-byte v2 manifest has SHA-256
`2d2cb287d90dd942b95629472e72f74013d8fcc4da64187fe87c0bcd0973cccd`.
It binds revision `142a9737f618ab8500308576a1c222501d639e5f`, source snapshot
`2346284239faf9f5571223587f2e2b2dfdd5197cffd41b97a12e234386810695`,
and exact Linux v2 plan manifest
`7eeb40dcb6a66fbd6f3e5cc1798695d5b2895c8e1f693451684a9864f1733b52`.
ADR 0268 records the shared runtime, ADR 0269 records the CupidLD publication
boundary, ADR 0272 records execution-seed adoption, ADR 0274 records the stack
commitment, and the bootstrap log records the active paired promotion.
...
0322 records native Windows author execution. ADR 0345 records the six-tool
candidate expansion.
...
A stage boundary where one generation of CupidC, CupidASM, and CupidLD builds
complete Linux images for CupidC, CupidASM, CupidDis, CupidLD, CupidObj, and
CupidBuild. The checked seed builds stage two, stage two builds stage three,
and stage three builds stage four. Linux compares 22 C objects, the startup
object, and all six images. Native Windows compares 23 C objects, three
assembly objects, and the same six tools. Both stages execute every tool
through useful success and failure cases. The drivers revalidate their live
seed manifests and artifacts at every generation boundary and before
publication; Windows also binds its execution seed to the exact Linux plan
manifest. The promoted pair binds revision
`142a9737f618ab8500308576a1c222501d639e5f` and the 59-input snapshot
`2346284239faf9f5571223587f2e2b2dfdd5197cffd41b97a12e234386810695`.
The active reports record 46/7/54 failure, help, and success groups on Linux and
34/7/41 on Windows. Production replay, artifact checks, and private runtime
validation remain separate evidence in the bootstrap log. Both matrices reject unmatched executable relocations,
validate unrelocated local targets while excluding relocated operands, require
every static ELF code anchor to begin at a decoded instruction, validate
source-resolved raw control edges, and exercise the checked CupidObj runner
plus typed JPEG, kernel-symbol, kernel-flatten, profile, ISO-pattern, and
closed kernel, Doom, and generated-install compiler transactions through consecutive CupidBuild generations. ADR 0279 records the convergence rule,
ADR 0353 records the paired six-tool contract, ADR 0360 records the runner
behavior gate, ADR 0374 records earlier carriage, and the bootstrap log records the active pair.
...
The active fixed points now start from six-tool v2 manifests. The preceding
generation's CupidDis certifies every C object before linking, not only the
startup object.
The native Windows CupidBuild startup also types all fourteen exported entries
as functions. Before promotion, fresh Linux and Windows candidate proofs passed
for these source bytes from one 58-input snapshot. Those runs began from the
five-tool parents and did not prove v2 self-consumption. ADR 0347 records the
stronger object boundary.

The preceding five-tool promoted fixed points bind snapshot
...
uses it to build the checked six-tool v2 Windows execution seed. The Windows
...
bin, docs, and demos formats. Checked CupidBuild coordinates their compilation
through checked CupidC, with a closed source and header bundle.
...
the entry is the only PE code anchor. The active v2 seeds carry this inspection
path, and the fixed-point checks reject an entry-corrupted CupidBuild image.
...
The manifest-bound set of static CupidC, CupidASM, CupidDis, CupidLD,
CupidObj, and CupidBuild executables under `bootstrap/seeds/i386-linux/`.
...
active v2 seed is generation four from revision `142a9737f618ab8500308576a1c222501d639e5f`,
with 59-input source snapshot `2346284239faf9f5571223587f2e2b2dfdd5197cffd41b97a12e234386810695`
and manifest SHA-256 `7eeb40dcb6a66fbd6f3e5cc1798695d5b2895c8e1f693451684a9864f1733b52`.
Its parent is `9d2529a7`; ADR 0396 records the paired promotion and Doom handoff.

The preceding `9d2529a7` v2 seed was generation four from revision
`9d2529a718672edcd970f24960535db6ddbde5e4`. Its build plan has SHA-256
`52dd857bcb74e079e7e2eec45eaa90a0a0838ad2f4e817bebc35c9904efbecbd`,
and its 59-input source snapshot has SHA-256
`bb4f598b2f72140d82d01fbfdedc14e4c774965d08c155ca848596daf3aa054f`.
CupidASM is 505,148 bytes with SHA-256
`a461fd72b32a8da326a1aed044d7b7617df1fd40d6c074fe4c0c7218434ff945`.
CupidC is 2,700,264 bytes with SHA-256
`79398ca08c80e50479f0b6574246f7a5074a4106db594a17e9dda630ddfb9ec8`.
CupidDis is 538,580 bytes with SHA-256
`ec6f381883f2f1230b7df20fb7fbc5d629ac895cc26dd72b091bdf4c40ebd2ef`.
CupidLD is 312,952 bytes with SHA-256
`ec1981e97b0a11bf3538df34e85d071aa81ea01596913cfb1364013f35a507c0`.
CupidObj is 397,068 bytes with SHA-256
`00064758399b61ea6425d7a4e685ef4b41bb7e566d39b550e30abe9a5cf78de8`.
CupidBuild is 622,388 bytes with SHA-256
`e228dcb9dd1799e43602945dee028661502b58dc669ad75c0a08bf6829a19174`.
The 6,602-byte manifest has SHA-256
`30adaac167ee6cdde136ce5d957e6b4ca0b0a2bdc23ff376436634a6a0db027e`.
It keeps the preceding `16a86f5b` v2 manifest and revision as parent lineage. CupidBuild
...
then passed the 36/7/42 failure, help, and success inventory. It began from the
preceding v2 cohort. CupidC carries the closed source-bundle reader, and
CupidBuild carries the transaction for all 157 kernel-profile compilations.
ADR 0353 records the v2 contract, ADR 0367 and ADR 0370 record preceding
promotions. ADR 0393 records that promotion, and ADR 0383 records the
normal Doom profile handoff.
...
bound revision `a17c9465911da41d59b7ada71733d36c39faa5ea`, and named the
50-input snapshot
`46c5335c80d822dd5085ee22077486ea647e5396482d42454847c87e4222aa67`.
Its promoted-seed reproof matched all five initial images. Stages three and
four matched nineteen C objects, startup, and five tools, then passed the
5/22/21 behavior matrix. ADR 0336 records that checkpoint.
...
At that checkpoint, the production path froze the selected seed manifest and six
...
The manifest-bound set of native PE32 CupidC, CupidASM, CupidDis, CupidLD,
CupidObj, and CupidBuild executables under `bootstrap/seeds/i386-windows/`.
The active v2 manifest binds their hashes, sizes, target ABI, exact imports, producing
revision, 59-input source snapshot, parent lineage, native plan, and the exact
Linux v2 plan-manifest bytes. The current pair binds revision `142a9737f618ab8500308576a1c222501d639e5f`,
source snapshot `2346284239faf9f5571223587f2e2b2dfdd5197cffd41b97a12e234386810695`,
and manifest SHA-256 `2d2cb287d90dd942b95629472e72f74013d8fcc4da64187fe87c0bcd0973cccd`.
Its Linux plan manifest is `7eeb40dcb6a66fbd6f3e5cc1798695d5b2895c8e1f693451684a9864f1733b52`;
both parent roles name `83d00ce7`. The bootstrap log records the paired promotion.

The preceding `9d2529a7` pair retained the following historical inventory.
Its 2,852-byte encoding has SHA-256
...
`52dd857bcb74e079e7e2eec45eaa90a0a0838ad2f4e817bebc35c9904efbecbd`.
CupidASM is 484,352 bytes with SHA-256
`17ab4874dab63bf614f9b0d94d9d64017ec5f70ddd87c59ade71646c0a1ab83c`.
CupidC is 2,627,584 bytes with SHA-256
`3fcbc74e11275a23ff5566e977223e36437f5516825729b6b8a27d0de3d995ad`.
CupidDis is 517,120 bytes with SHA-256
`87a6ba895bd52a1da6e287a7b5d585b5ca54a82dd9d43347aa441b7b9d8ed034`.
CupidLD is 296,960 bytes with SHA-256
`206b33ae6a044143a45300b175c9443559bbb748af5f6293a4505f860535196b`.
CupidObj is 377,856 bytes with SHA-256
`f2a56f4177af4b15c7dde23f6408876729c3a795b01d28ffb60d16d5d0fbd83b`.
CupidBuild is 634,368 bytes with SHA-256
`b5979e3ccccc51b7eab6dcdd8a17ab798f2f48f19fc44276347cd46fc18debcb`.
...
from the separately verified Linux plan seed. Static Linux Toolchain contract
paths continue to use the Linux bootstrap lineage. The `CUPMAN4` author keeps
that producer lineage but runs as a native PE on Windows. The `CUPMAN2`
...
`9d2529a718672edcd970f24960535db6ddbde5e4`, source snapshot
`bb4f598b2f72140d82d01fbfdedc14e4c774965d08c155ca848596daf3aa054f`,
and Linux manifest
`30adaac167ee6cdde136ce5d957e6b4ca0b0a2bdc23ff376436634a6a0db027e`.
...
and revision `16a86f5b1693e017c36c6d902df9946c5d674b17` as transition
lineage rather than active identities.
...
reconstruction and convergence, ADR 0353 records the paired v2 contract, and
ADR 0367 and ADR 0370 record preceding paired promotions, and ADR 0374
records the preceding promotion. ADR 0393 records that pair.
...
One tool call launched from a private capture of a manifest-bound six-tool
seed. The shared runner executes the selected image, then verifies the live
manifest and all six images before returning success. Linux ELF images run
...
The canonical checked-in size contract for sixteen deterministic Cupid-owned
outputs: four OS outputs, six Linux seed images, and six Windows seed images.
...
sixteen observations, and the complete Windows seed directory: its manifest
plus six PE tools. A `CUPSIZE2` request carries the Linux manifest digest, the
raw Windows manifest, and regular-file size and digest observations for all
six PE tools. The C contract validates the Windows target, provenance, exact
Linux v2 plan-manifest link, inventory, and observed bytes beside the policy
and Linux manifest. On Windows, the
...
checked seed. The focused modules contain 58 tests, with four platform-specific
skips. The final spec-review CTXT correction triggered the promotion's third
fail-closed exact-size check. It measured a 9,505,572-byte raw kernel against
the 9,505,656-byte policy row and stopped before image publication. After that
row was updated, the 3,382-byte policy passed all sixteen exact paths, totaling
38,120,452 bytes. Its SHA-256 is
`7f9c1f49d1543112bf6984def1e4ecba6df4fb7a55d2481a85deb7370cf4bfc2`.
The earlier fourteen-artifact checkpoint ran 46 tests in 4.160 seconds, with
four expected Windows skips. It reached the exact-size gate with changed
...
artifacts.
At the preceding source-head checkpoint, the kernel outputs included a
9,601,052-byte `kernel/kernel.elf.pass1` with SHA-256
`c694ba70e37e711c6490db2f17fb1869300c26703017cb1bd8adb3b51cce1c60`, a
9,732,124-byte `kernel/kernel.elf` with SHA-256
`bbdb974acc12bf57dd95bff4f734ad9ad75dd8d11dfec3267a22df455666f459`,
and a 9,504,760-byte `kernel/kernel.bin` with SHA-256
`346063e08f2fef5550e2a40b8edb1ea7cbe2f242a3084f1137a9552ed7baf84a`.
The 209,715,200-byte `cupidos.img` published at that checkpoint had SHA-256
`aa9bc411d48625837b511b32444019f0aa555a48fc5aaa2400c9259dd8607333`.
That normal build linked both ELFs and completed strict inspection of all 431
...
The historical ADR 0374 build passed after the exact-size check rejected the updated
...
whole-image CupidDis inspection and disk-image staging. That checkpoint's artifacts
were a 9,634,124-byte `kernel/kernel.elf.pass1` with SHA-256
...
Its 3,382-byte exact-size policy covered 38,402,832 bytes and had
SHA-256
`cfbf82ebd02697a668c404c8e9bd19e577b8e809b87c3736e063e4f30f0d5fe2`.
That checkpoint's 209,715,200-byte `cupidos.img` had SHA-256
`602934d88c77336659a7aa4a50e38ac7ae740fd76114cc2dfa334936b915c713`.
The bootstrap log records the pre-correction `16a86f5b` Linux measurements and passing
16-artifact check. Final-manual parallel replays now pass on both hosts.
The Linux image passes the full four-CPU E1000 and RTL8139 frontiers; the
Windows image separately passes SMP verification and a CupidC-built `ls` smoke.
...
normal `/bin/ls.cc` JIT completion. Its 35,175-byte log has SHA-256
`631d2104af9cacd28bac9c2fe2121a10cbaf5a3e9b24242598bba3f9fa5abc30`.
Neither smoke changed the image.
The promoted image also passed a fresh private four-vCPU E1000 run with the
`max` CPU model and strong SMP runtime check. `/bin/ls.cc` compiled to 911
...
The earlier publisher-recovery image passed the same private gate. All four
...
The promoted seed manifests bind the 59-input snapshot used by every built
stage and behavior check.
_Avoid_: live source root, source hash alone, public staging directory

**Promoted six-tool bootstrap cohort**:
The active v2 seed generation was built from the preceding six-tool v2 pair
and reviewed source-derived plans. Linux builds 22 C objects, one startup object,
and six tools. Native Windows adds its publication runtime and three startup
objects. Final-stage comparisons and behavior checks cover all six tools, and
...
images, and both generations reject an entry-corrupted CupidBuild copy. At the
historical ADR 0374 checkpoint, candidate behavior inventories were 28/6/35
on Linux and 17/6/22 on Windows for failure, help, and success cases. Both
plans froze the same 59 inputs at
revision `0232cb57aad5d6bdfd7bd77499762514b2f0ebfd` and snapshot
`0b591a0bef928186641b3aa1fb98c1e145e6c4905c8b6cb87c34a1ace4bc87d2`.
That Toolchain publication agreed on all 62 stage pairs and published
22 artifacts. Its refreshed v2 manifests bound that candidate as the checked seed without
...
ADR 0353 records the v2 contract, and ADR 0370 records an earlier promotion.
ADR 0382 records the preceding `16a86f5b` pair, its 59-input snapshot
`54b411b6ed05725101f5859106facb43639f2d02cb2b0ea2f6334e21375bda55`,
and clean 33/7/38 Linux and 21/7/25 Windows proofs. That Windows
self-consumption proof also passed with all six seed tools matching stages
two through four. The Linux Toolchain publication reproduced all six checked
seed images, agreed on 62 stage pairs, and published 22 artifacts. Final
manifest verification rejected concurrent root-directory drift, then passed
unchanged in a byte-verified stable copy. A later preprocessor test-inventory
correction and three refreshed object expectations now pass fresh publication.
All 62 stage pairs agree, all six tool images reproduce the checked seed, and
all 22 artifacts pass native manifest verification on Linux and Windows.
The full executable suite and user build pass.
Those results apply to the preceding pair. ADR 0396 records the current
`83d00ce7` promotion; its OS adoption checks are separate.
...
The ADR 0359 proof matched 23 C objects, three assembly objects, and all
six tools between stages three and four, then passed the 14/6/19 behavior
...
**Host toolchain**:
External compilers, assemblers, linkers, and binary utilities confined to explicit development and oracle paths. They do not produce normal Cupid OS or Toolchain artifacts. Host Python coordinates six remaining compilations, three user links, two image publications, three verification operations, one Toolchain publication, and bootstrap proofs. CupidBuild directly owns the ISR and context-switch objects, the bootloader, SMP-trampoline, and ISO-pattern raw images, the JPEG object, the generated kernel-symbol source, kernel flattening, the Doom profile manifest, all 157 kernel-profile and 83 Doom compilations, 186 ordinary CupidObj calls, and both normal kernel links. Windows runs output-bearing production tools, every native fixed-point generation, the user syscall ABI contract, the artifact-size contract, and the Toolchain manifest verifier from the native six-tool execution seed. The manifest verifier also checks the paired Linux publication seed. Linux fixed-point reconstruction and the remaining published Linux Toolchain contract cohort use WSL on Windows. Both platforms have a clean six-tool candidate proof and paired v2 seed promotion. Further Python-free coordination remains open. ADR 0341 records the historical compiler bridge, ADR 0353 records the paired v2 contract, ADR 0367 records the preceding promotion, and ADR 0370 records the preceding promotion, ADR 0396 records the active pair, ADR 0354 records the first normal CupidBuild recipe transfer, ADR 0357 records the raw publication transfer, ADR 0359 records the all-native Windows fixed point, ADR 0362 records the direct CupidObj handoff, ADR 0363 records the kernel-link handoff, ADR 0368 records the JPEG handoff, and ADR 0371 records the kernel-symbol handoff.
...
stops the build. Seed capture, the live six-tool recheck, and atomic output
...
A checked manifest contract that CupidBuild can validate without embedding its
own future digest. The active v2 Linux and Windows contracts name six tools,
including CupidBuild as a non-producer. They retain the preceding accepted manifest
and revision as parent lineage, bind the exact six-tool plans, and carry the
lower-case hexadecimal promoted revision and source snapshot. The Windows
