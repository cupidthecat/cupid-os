...
**Variadic call site**:
A call through a prototyped function type whose final parameter is an ellipsis. Linear IR keeps the named parameter count in the function type. Each call instruction owns its actual argument count and a source-ordered slice of the actual post-conversion types. Hosted i386 emission transports represented four-byte integer and pointer values plus signed or unsigned eight-byte integer ellipsis arguments. A wide argument occupies adjacent low and high four-byte words in the sixteen-byte-aligned outgoing area.
...
**Variadic cursor**:
The target `char *` value used by hosted CupidC to traverse unnamed i386 cdecl arguments. `va_start` points it just past the final named argument. A supported non-atomic four-byte integer or pointer `va_arg` advances it by four, while a signed or unsigned eight-byte integer or represented wide-enum read advances it by eight and returns an instruction-owned snapshot. `va_copy` copies the cursor, and `va_end` consumes its evaluated address without changing stored state.
