...
current target-profile gate contains 42 strict C11 roots and three GNU-enabled
...
runtime contract, `kernel/lang/as_elf.cc`, and all fifteen Toolchain contract
translation units.
...
The guarded recipe built a fresh 209,715,200-byte image with SHA-256 `8ad90a91103bf48d1e8d1e20b1b3dee48122ed1e4059b3f94cce7d750c262f16`. A private four-CPU `/bin/ls.cc` JIT boot passed from that image in 61.9 seconds. The source-current rebuild then preserved the existing FAT data and produced image SHA-256 `d1bfab4aed1f2116768ceed3e301fb14ffe2a36418eb4d4ebdf1108097cb2b05`; its private four-CPU JIT boot passed in 66.8 seconds.
...
The static i386 contract that checks the external-program table before CupidC compiles the tracked user programs. Stage-two and stage-three CupidC compile it from `toolchain/tests/user_syscall_abi_contract.cc`; CupidLD links it, and the checked Toolchain publisher requires matching objects and executables. It snapshots and rereads the six ABI declarations, checks version 5, 103 fields, 412 table bytes, public scalar types and constants, both VFS record layouts, and all 101 providers, then emits the reviewed fingerprints. Python freezes the verified publication and one shared six-file input snapshot, launches the private artifact, compares its report with the independent oracle, and rechecks the live publication and inputs. ADR 0264 records the transfer.
_Avoid_: Python-only ABI checker, user-program runtime, kernel syscall implementation

**Hosted i386 ABI profile**:
The deterministic hosted C request used to compile an i386 tool closure. Linux requests search `/toolchain` for quoted and angle includes and the checked i386 Linux declaration set for angle includes only, define `__SIZEOF_POINTER__` as four, and leave `_WIN32` undefined. Only `kernel/lang/as_elf.cc` and `toolchain/tests/cupidasm_kernel_elf_contract.cc` also search `/kernel/lang`; all 33 `HOSTED_I386_LINUX` roots are kept outside that bridge. `HOSTED_I386_WINDOWS` defines `_WIN32=1` for `ctool_host.cc`, the CupidASM, CupidC, CupidLD, and CupidObj driver roots, and the CupidLD publication runtime while keeping the same four-byte pointer fact and declaration set. The headerless Windows probe uses the separate `FREESTANDING_I386` profile. The Windows tool runtime selects the shared hosted implementation through `CUPID_RUNTIME_WINDOWS`. The CupidC command represents these roots with `-I` and `--include-angle` in caller order. Repeatable `-include` options represent preprocessing inputs that run in order before the primary source. Tool sources use strict C11. The Linux runtime, Windows runtime wrapper, and Linux behavior probe enable CupidC's GNU variadic built-ins.
...
A stage boundary where one generation of CupidC, CupidASM, and CupidLD builds complete stage-two Linux images for CupidC, CupidASM, CupidDis, CupidLD, and CupidObj, then the stage-two producer trio repeats that build for stage three. The gate compares all 19 C objects, the independently assembled Linux startup objects, and all five linked images across the two stages. Each stage also executes the five tools through real success and failure cases. The same fixed-point relationship is checked from the repository seed on Linux or through WSL. Source head adds matching Windows runtime, startup, and publication objects plus native images and behavior checks for all five tools. The resulting PE cohort is carried as a checked execution seed for Windows production. A native Windows producer fixed point remains outside this companion proof.
_Avoid_: fresh-checkout bootstrap, native Windows producer fixed point, Python-free coordination
...
Compiler head and the checked seed distinguish both aligned GNU spellings of the exponent range subtraction. Legacy `fsub %st, %st(1)` retains GNU's `DC E1` reverse-subtract meaning. Corrected `fsubr %st, %st(1)` emits canonical `FSUB ST(1), ST(0)` as `DC E9`, which computes `x - round(x)`. The checked seed and source head have 604 forms, 249 canonical mnemonics, and fingerprint `55A8970F`. The catalogue includes signed x87 `FILD` and `FISTP` memory operands at 16, 32, and 64 bits, plus canonical `SETP` and `SETNP` byte predicates. The four SHRD rows remain canonical at 16 and 32 bits with immediate or fixed CL counts. Active `libm.cc` uses the corrected subtraction spelling at all seven range-reduction sites. ADRs 0207 through 0209 record the exponent diagnosis, seed carriage, and runtime-tested source correction. ADR 0226 records SHRD, ADR 0228 records its seed carriage, ADR 0252 records the x87 integer forms, ADR 0258 records the preceding promoted seed, ADR 0259 records the parity predicates, and ADR 0265 records their current seed carriage.
...
`cupid.raw-map.v1` file. The first range starts at offset zero, and later
starts increase within the source. CupidDis decodes code ranges and renders
data ranges as `db` rows without entering the x86 decoder. The assembler, not
...
comes from revision `95f5bb6cfd0468bb8852c670ada849cb5bde79a7`. CupidC is
2,666,240 bytes with SHA-256
...
ordinary `float` and `double` updates, indexed typed CupidDis inspection, and
the 604-row shared x86 catalogue with `SETP` and `SETNP`. Its fixed point
covers nineteen C objects, startup, five tools, and the 5/18/16 behavior
matrix. The frozen 43-input digest is
`56e0943f82737a7013994f1a2b78fcbd5b5c762d0f5036aac5a48bfbb3dcbe32`.
The 17,035-byte report has SHA-256
`810704f6701b4b4627062981e1e969332d4aa5f409d2cdce3d4fcba150518f84`.
The proof passed in 763.5 seconds, and the checked 128 KiB strict-decode
throughput case completed within its 30-second limit. An independent
poisoned-host reproof passed in 766.9 seconds. All five seed images match stage
...
The current source-head reproof includes the Windows tool runtime, startup,
publication bridges, platform declarations, and direct runtime contract in a
50-input closure with SHA-256
...
builds matching native images for all five tools from both stages. Windows
runs help plus useful success and failure cases for every tool. CupidLD's case
also checks exclusive candidate selection, byte-exact output, replacement of
an existing destination, and cleanup after a forced replacement failure. The
proof passed in 801.9 seconds. The public `--cupid` driver changes CupidC, and
the PE stack policy changes CupidLD, so those two Linux seed images differ from
stage two. Stage two and stage three still match for every tool. The checked
Windows seed carries the matching native images. The 38,164-byte report has
SHA-256
...
flattening.
Its 9,028-byte graph-ordered input manifest has SHA-256
`48bdef348f6575881b9808631173e7265abc9ea89dfb84d48de72b3d2304749e`.
That separate gate passed in 185.526 seconds with exit 0 and empty streams.
The current path freezes the selected seed manifest and five artifacts, the
 431-entry manifest and cohort, and the existing `kernel.bin` boundary in one
hostbuild transaction. Checked CupidDis validates the private cohort, then
...
atomic publication. Any failure preserves the prior raw kernel. The complete
transaction passed with exit 0 in 187.054 seconds and published an
...
maintenance.
On 2026-08-13, the source-current poisoned-host `make -j2 all` build completed
...
supersede the earlier output identities below. `boot/boot.bin` is 2,560 bytes
with SHA-256
...
source snapshot, parent Linux seed, and paired-stage provenance. Windows runs
...
plan and cannot start the fixed-point command. Fixed-point reconstruction,
Toolchain contracts, the user syscall ABI contract, and the artifact-size
policy continue to use the checked Linux bootstrap seed. ADR 0272 records this
boundary.
...
The canonical checked-in size contract for nine deterministic Cupid-owned
outputs. `verify-artifact-sizes` receives `$(BOOTSTRAP_SEED_MANIFEST)`, derives
the five seed paths and declared sizes from that selected manifest, and
requires the policy to agree. It also checks the five-sector boot image, both
kernel ELFs, and the raw kernel. The verifier is a direct prerequisite of
`cupidos.img`, so a failure prevents image publication and preserves the
existing image. The normal build rejects missing, unknown, duplicated, linked,
nonregular, or differently sized members. An intentional size change updates
the implementation and policy together. This is a regression sentinel, not a
host-compiler comparison or linker capacity limit.
...
bridge, the direct runtime contract, and the hosted Windows declarations. Both
stages and their behavior checks consume that root. The
...
seed and the checked Linux build-plan seed, derives the PE plan, builds two
native generations, compares every object and tool, runs native behavior
checks, rehashes the live closure, and publishes one evidence bundle. The two
manifest roles remain separate.
...
**Host toolchain**:
External compilers, assemblers, linkers, and binary utilities confined to explicit development and oracle paths. They do not produce normal Cupid OS or Toolchain artifacts. Host Python still coordinates the checked build. Windows runs output-bearing production tools from the native checked execution seed. Source head can reconstruct two native Windows generations from that execution seed and the Linux plan seed, while Linux-contract paths still use the static i386 Linux seed through WSL. A source-stable native proof, seed promotion, and a Python-free fixed point remain open at this boundary.
