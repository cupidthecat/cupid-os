...
**Wide integer value**:
An eight-byte integer carried through hosted Linear IR as one logical value. The i386 emitter stores its bytes in a private frame snapshot and returns the low word in EAX and the high word in EDX. The current boundary covers constants, matching conditionals, fixed call results, object access through file, block-static, automatic, pointer, member, and index paths, initialization, plain assignment, declared parameters, named direct or indirect call arguments, discard, return, left and signed or unsigned right shifts, AND, OR, XOR, and conversion to or from represented integer widths. It also covers the standard `signed long long` to `unsigned long long` usual arithmetic conversion and, in GNU mode, promotion of a wide enum to its compatible wide integer type. Shift counts remain represented four-byte integers. Values without a declared parameter type, conditions, other arithmetic, and mutation remain outside the boundary.
