...
The 144 checked-in normal-build translation units owned by checked-seed CupidC, plus the generated kernel symbol-table translation unit. The checked-in cohort keeps the established 40 kernel and driver sources, 71 later kernel and driver sources, five shared toolchain implementations, and 28 source-driven roots renamed to `.cc` when production ownership moved. The strict frontier compiles all 144 checked-in sources twice to 3,514,456 matching i386 ELF32 bytes before atomic publication. Its 432-input snapshot has SHA-256 `7670679039ca8f2b9b7816a68cb9b391d8a2e65f6b03a7a043d35005b75283bf`. The two-pass link separately freezes and compiles `kernel/cpu/ksyms_data.cc`; its packed i386 words preserve the exact symbol blob with fewer initializer nodes. Poisoned-host rebuilds and exact recursive header closures cover every recipe. A data-only relocatable object is valid without `.text` when its sections and symbols pass the remaining ELF checks. The four-vCPU GUI gate reaches SMP, the 62 crypto checks, the desktop, terminal, network, and CupidC execution. The shifted external arena also loads and reaps the same fixed-address program twice, releasing its first lease before reuse.
