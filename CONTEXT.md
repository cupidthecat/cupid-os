...
A GNU-mode CupidC statement whose immutable frontend record owns a decoded template and a packed operand slice. Extended statements accept one to four typed register outputs, single-digit matching inputs, and the CSPRNG's RDTSC, CPUID, RDRAND, SETC, and tied NOP forms. Function-body basic statements and extended statements with an empty output list own no operands and are implicitly volatile. Their current exact templates use PAUSE, NOP, STI, HLT, CLI, CLD, SFENCE, and FNINIT. Linear IR evaluates output addresses before input values. The emitter preserves EBX for operand-bearing statements and avoids both a frame slot and EBX traffic for operand-free statements. A disposable hybrid image has booted with four operand-free objects from compiler head. The checked seed owns the production CSPRNG path, so normal-build ownership still waits for the next seed transition.
