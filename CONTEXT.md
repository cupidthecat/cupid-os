...
A GNU-mode CupidC statement whose immutable frontend record owns a decoded template and a packed operand slice. Extended statements accept one to four typed integer register outputs, single-digit matching inputs, and the CSPRNG's RDTSC, CPUID, RDRAND, SETC, and tied NOP forms. The port-I/O subset adds width-preserving `a` and `d` inputs, 8-bit, 16-bit, or 32-bit `=a` outputs, and modifiable `+c`, `+S`, and `+D` operands. It accepts one `memory` clobber where the source requests it. One `=r` output may instead be a modifiable four-byte object or `void` pointer for the exact `mov %%gs:0, %0` per-CPU load. A separate output-only subset snapshots a 32-bit general register, ESP, EBP, `4(%ebp)`, or EFLAGS into one four-byte object; the EFLAGS form may finish with `cli`. Compiler head also accepts an independent four-byte integer or data-pointer `r` input and an independent four-byte integer `c` input for exact CR0, CR2, CR3, and CR4 moves and RDMSR. Function-body basic statements and extended statements with an empty output list own no operands and are implicitly volatile. Linear IR evaluates output addresses before input values, once each and in source order. The emitter preserves EBX and restores ESI or EDI after repeated string I/O. The checked seed carries the port helpers and snapshot subset, but it predates the privileged-register input forms. Normal-build ownership remains a separate boundary.
