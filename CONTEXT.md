...
A GNU-mode CupidC translation-unit effect that owns one immutable basic assembly template outside every function. The frontend keeps these effects in a table separate from statement assembly, and Linear IR preserves their source order without pretending that they are function-body instructions. The checked seed emits only the twelve exact x87/SSE floating wrapper definitions at the start of `kernel/cpu/libm.c`. Those wrappers are prologue-free `STT_FUNC` symbols encoded through Cupid's shared x86 model. Arbitrary GAS text, operands, labels, directives, data definitions, and a host-assembler escape remain outside this term.
